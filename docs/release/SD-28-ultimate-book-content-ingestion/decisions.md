...
**Status:** New. Naming a pattern this codebase has independently rediscovered and independently fixed nine times, so the next agent recognizes the shape on sight rather than re-diagnosing it from scratch.
...
**Nine instances found in this codebase, oldest fix first:**
