...
## Decision 53 — SD28-E15: the 1,746-unit remainder clustered by owning chooser, threshold-checked at 20+ -- two new corpus-shape findings and one new engine-design shape, neither canonical-narrowing (2026-08-09)

**Re-clustering, not a fresh count.** Re-deriving the remainder against `work-inventory.json` directly (rather than reusing `§52`'s own family totals) gives 1,746 units across 751 distinct `KEY:` prefixes, after removing: the 438 archetype tier-2 matches (`§52`), and the three deep-checked families re-matched by keyword (Domain 318, Oracle Mystery 134, Bloodline 354 -- these keyword-matched totals differ from `§52`'s own 166/134/152 because `§52` used the exact family scope while this pass used a broader substring match to avoid leaving stragglers uncounted; both are real, scoped differently, per this epic's own running instrument-mismatch lesson). This 1,746 is close to but not identical to the previously-cited 1,772 -- the ~26-unit gap is exactly that kind of re-scoping drift, not a new error, and is named here rather than silently reconciled to the old number.

**Threshold, stated and applied before any deep-check: families of 20+ units get individually checked; everything below stays a counted, named tail.** 12 prefixes clear the threshold, totalling 443 units. The remaining 739 prefixes (1,303 units) are reported as a population, not investigated individually -- documenting the cutoff rather than picking which small families "look interesting" is the anti-gaming discipline `§15`'s own standing rule requires.

**The 12 families at or above the threshold, deep-checked by the same 4-step method, and the result is NOT a third round of canonical-narrowing -- it splits into three distinct shapes:**

| Prefix | Units | Book | Disposition | Evidence |
|---|---|---|---|---|
| Refined Education / Refined Education Unlock | 119 | ultimate_intrigue | owned by no chooser found | zero hits for "Refined Education" anywhere in `pilot_compute.rs`; Vigilante's own social-talent tree, part of the already-named 233-unit Vigilante gap (`§52`) |
| Social Grace | 84 | ultimate_intrigue | owned by no chooser found | zero hits; same Vigilante gap |
| Fused Eidolon Skills | 55 | ultimate_magic | **mis-clustered: real archetype tier-2 content** | corpus row confirms this is Synthesist archetype's own skill-substitution table (`Synthesist ~ Fused Eidolon`, `um_abilities_class.lst:1817`) already inside `§51`'s 403-record tier-1 catalog -- the tier-2 KEY-prefix match in `§52` missed it because the sub-feature's own `KEY:` prefix ("Fused Eidolon Skills") diverges from its master's name ("Synthesist"), the same divergent-naming shape `§51` already documented for tier-1/tier-2 pairs generally |
| Skinshaper (+ Ability Enhancement I-IV, Unarmed Strike, Skinshaping, Flashmorph) | 30 | ultimate_intrigue | **mis-clustered: real archetype content, book out of tier-1 scope** | `ui_abilities_class.lst:324` confirms Skinshaper is a genuine Druid archetype (`KEY:Druid Archetype ~ Skinshaper`) with real `DESC:` and `ABILITY:` grants -- it was never counted as archetype content because `ultimate_intrigue` was never one of `§51`'s seven tier-1 books (UPsi/ACG/APG/UM/UC/ARG/UW). This is the reconciliation flagged as owed earlier this epic (Skinshaper appearing in both an archetype set and the unmatched bucket) -- resolved: real archetype content, correctly excluded from `§51`'s catalog only because UI itself was out of scope, not fabricated or duplicated |
| Insight (+ Insight Advanced Discipline) | 29 | ultimate_psionics | owned by no chooser found | zero hits for "Insight" as a psionic discipline chooser anywhere in `pilot_compute.rs` |
| Terror | 24 | ultimate_psionics | owned by no chooser found | zero hits |
| Blade Skill | 24 | ultimate_psionics | owned by no chooser found | zero hits |
| Path Power | 23 | ultimate_psionics | owned by no chooser found | zero hits |
| Packmaster Boon Companion | 20 | advanced_class_guide | **universal-formula shape, not narrowed** | not a per-option chooser at all -- these are 20 `AnimalCompanionLVL`-style level-variant rows that all resolve through one shared table, `ANIMAL_COMPANION_HIT_DICE_BY_MASTER_LEVEL` (`pilot_compute.rs:5946-5957`), the same universal progression Druid/Hunter/Cavalier companions already share. Every level variant is potentially reachable through the same formula, not one canonical option with the rest blocked |
| Beastmaster | 20 | advanced_players_guide | **universal-formula shape, not narrowed** | same table, same shape -- Ranger's Beastmaster feat's follower tree (`apg_abilities_class.lst:2705`) is 20 level-variant `FOLLOWERS:`/`COMPANIONLIST:` rows over the identical universal HD progression |
| Wildblooded | 20 | ultimate_magic | owned by an existing (blanket) chooser | zero direct hits for "Wildblooded," but Sorcerer's own bloodline diagnostic (`class_feature.sorcerer.arcane_bond_and_bloodline_progression.unsupported`, `§52`'s Bloodline row) is a blanket block on every Sorcerer bloodline selection regardless of name, Wildblooded included -- this is a `§52` clustering gap (the keyword match used "bloodline" and missed the Wildblooded variant names), not a new disposition |

**A new engine-design shape, distinct from canonical-narrowing: the universal-formula shape.** Where Domain/Oracle/Bloodline show one option grounded and the rest named-and-blocked, Beastmaster and Packmaster's Boon Companion families (40 units combined) show every option in the family resolving through the same shared formula -- there is no narrowing because there is no per-option code to narrow between. This is a materially better outcome for those 40 units than canonical-narrowing's 1-in-134-to-166 ratio, but it was not verified unit-by-unit against `reach_gate`/a real consumer the way rows 1-3 of `§52`'s candidate table were, so it is reported as a strong positive signal, not added to the candidate list, pending that same verification.

**Two mis-clustering findings, each real content wrongly sitting in the "unmatched option-pool" bucket, not archetype-swap or option-pool at all:** Skinshaper (30) belongs to `§51`'s tier-1 catalog in spirit but was excluded on a book-scope technicality (UI out of the seven-book set); Fused Eidolon Skills (55) belongs to `§51`'s existing Synthesist record but was missed by the tier-2 KEY-prefix matcher on a name-divergence technicality. Combined, 85 units are genuinely archetype-adjacent, not option-pool, and not newly-found gap work -- this is the tail producing exactly the kind of classifier-mislabel finding predicted going in (the `b1_abilities_race.lst`-as-monster-abilities precedent), confirmed on a smaller scale.

**The unmodelled-chooser finding is its own third disposition, distinct from both canonical-narrowing and the universal-formula shape.** Refined Education, Social Grace (Vigilante, 203 units, already-known gap) and Insight/Terror/Blade Skill/Path Power (Ultimate Psionics discipline talent trees, 100 units, a newly-confirmed gap) have **no chooser code at all** in `pilot_compute.rs` -- not a narrowed chooser, not a universal formula, nothing. These 303 units are genuinely unreachable today, by the same reach-check standard used throughout this epic, and (unlike the candidate list in `§52`) are not proposal material -- they would need net-new chooser/grounding code, not a status flip, to ever leave `unknown`.

**The below-threshold tail: 739 prefixes, 1,303 units, counted and named as a population, not investigated.** Per this decision's own stated threshold, no individual prefix below 20 units was deep-checked. Reporting this as an honest, bounded population rather than silently extrapolating the three shapes found above onto it -- the 12-family sample already produced three different shapes (canonical-narrowing's absence here, a new universal-formula shape, an unmodelled-chooser shape, and two mis-clustering corrections), so extrapolating further without checking would repeat the exact blended-figure mistake `§52`'s coverage statement was written to avoid.

**Disposition: characterise the sample checked, do not extrapolate onto the tail, do not move any unit's status.** 443 of the 1,746-unit remainder were deep-checked (25%). Of those: 85 are mis-clustered real archetype content (a correction, not a gap), 40 show a new universal-formula shape (a positive signal pending unit-level verification), 303 have no chooser at all (a genuine, confirmed-absent gap, not proposal material), and 15 (Wildblooded) fold into `§52`'s existing Bloodline family. The 1,303-unit below-threshold tail and the 307-unit feat-side bucket remain open and out of this decision's scope.

## Decision 54 — SD28-E15: the 307-unit feat-side `unknown` bucket -- a scan confined to one file produced a false negative on `Power Attack`; widened to a named 9-file set, 31 units confirmed reachable, 12 individually live-tested through the twin the player reads, all 12 correct (2026-08-09)

**Different evidence shape from `§52`/`§53`.** The feat-side bucket (`evidence: in_catalog_with_corpus_magnitude_but_no_observed_consumer`) is not option-pool content with a chooser to check -- it is 307 independent feats already in the engine's catalog, each carrying its own diagnostic text stating the probe's own limit verbatim: "the feat-effect probe observed no computed delta across the swept postures... the effect may need a posture, an opponent, or a combat action this engine does not model. Reported as unknown rather than deferred because no engine diagnostic is scoped to a feat." 287 of 307 already carry `wiring_class: computed` -- the classifier's own belief that real compute code exists is a separate claim from the probe's own failure to observe a delta. Breakdown by book: UPsi 83, UM 43, CRB 40, ARG 35, UC 31, APG 29, ACG 25, UW 9, UI 8, PU 4.

**First pass, scoped to one file, produced a false negative -- not merely an undercount, a wrong answer on a foundational feat.** A literal-string scan and a slug scan (feat name -> `snake_case`, since some feats are grounded via an internal id rather than the display string) against `pilot_compute.rs` alone found 22, then 28 hits. `Power Attack` was reported as a miss by both passes despite being fully wired: `POWER_ATTACK_ITEM_ID`/`POWER_ATTACK_FEAT_SELECTION` (`pilot_compute.rs:6825`/`:6836`), referenced again at `:27102`/`:44291`, plus a real test asserting its active/inactive math (`:43886`) and a module-level doc comment naming it (`:15`). A scan that misses a feat with six real references in the one file it searched is not a weak proxy with a known blind spot -- it is broken, and every number it produced (including the 22) had to be treated as untrustworthy until the cause was found.

**Root cause: the scan was scoped to one file, and feat wiring lives in several.** `src/rules_core/feat_effects.rs` is a 5,064-line, 274-function file dedicated entirely to feat effects, carrying feats by their corpus display name (`"Adaptive Fortune"`, `"Angelic Flesh"`, ordinary catalog entries, not just marquee feats) -- and the first pass never opened it. Widened to a named, bounded file set rather than an open-ended search: `pilot_compute.rs`, `feat_effects.rs`, `feat_prereqs.rs`, `feat_prereqs/pre_tokens.rs`, `pilot_compute_corpus.rs`, `character_input.rs`, `description_completion.rs`, `support_state_matrix.rs`, plus the two desktop-adapter files that are real consumer boundaries (`character_hub.rs`, `pf1_adapter.rs` -- the latter is where `Touch of Good`'s own surfacing lives).

**The literal+slug scan across all nine files found 35 hits; the individual walk (reading each hit's surrounding code, not trusting the string match alone) confirmed 31 and rejected 4 as false positives a second time:**

`Stalwart`, `Discovery (Opposition Research)`, `Discovery (Split Slot)`, `Toughness (Vigor/Wounds)` -- the same four names flagged in the first pass's walk -- still produced no real code anywhere in the widened file set on a targeted re-check (`Toughness (Vigor/Wounds)`'s every hit across all nine files is the base `Toughness`/`feat:toughness` record, never the Vigor/Wounds variant -- the same false-positive shape as UC's `Deathless Master (Vigor/Wounds)` textless-variant finding, a repeat instance of one corpus convention, not two separate errors).

**31/307 (10%) confirmed reachable, each spot-checked against its actual surrounding code:**

| Unit(s) | Grounding | Location |
|---|---|---|
| Weapon Finesse | pushed in real tests | `pilot_compute.rs:59750`, `:59771`, `:59792` |
| Combat Reflexes, Dodge | feat-id lookup table + branching logic | `pilot_compute.rs:5126`/`:5128`, `:33621-33656` |
| Improved Unarmed Strike | real `granted.push(...)`, asserted in tests | `pilot_compute.rs:4770`, `:62267`, `:62320` |
| Power Attack | id constants, not display string -- the scan's own false negative | `pilot_compute.rs:6825`, `:6836`, `:27102`, `:44291`; test `:43886` |
| Extra Bombs, Extra Inspiration, Extra Martial Flexibility, Extra Reservoir, Extra Performance, Extra Lay On Hands | `extra_resource_feat_bonus` generic mechanism (task #19); Performance and Lay On Hands additionally proven in `extra_resource_feat_tests` | consts `pilot_compute.rs:32017-32087`; calls `:11355`, `:15519`, `:18243`, `:22552`, `:19867`, `:27745`; tests `:61608-61763` |
| Extended Animal Focus | named const, pushed in real tests | `pilot_compute.rs:32106`, `:52817`, `:52846` |
| Improved Channel | named const, pushed in real tests | `pilot_compute.rs:32100`, `:57440`, `:57445`, `:57458` |
| Extra Stamina | `extra_stamina_picks` field feeding Pathfinder Unchained's real stamina-pool computation -- found only by reading surrounding code, not by either scan matching the feat's own name | `pilot_compute.rs:43090` (block starts `:43082`) |
| Point-Blank Shot, Far Shot, Rapid Shot, Double Slice, Two-Weapon Fighting, Improved Two-Weapon Fighting, Greater Two-Weapon Fighting, Two-Weapon Defense | one real mechanism: Ranger's Combat Style bonus-feat grant recognizes exactly these as valid free-feat choices for the Archery/Two-Weapon-Fighting styles | `pilot_compute.rs:28965-28995` |
| Throw Anything | feat-id lookup + branching logic | `pilot_compute.rs:5131`, `:33631`, `:33739` |
| Armor of the Pit | conditional grounding, `feat_identity::holds(feats, "Armor of the Pit") && character_has_tiefling_scaled_skin(input)` | `pilot_compute.rs:43128`; also referenced `feat_effects.rs`, `feat_prereqs.rs`, `pilot_compute_corpus.rs` |
| Adaptive Fortune, Fortunate One, Great Hatred | same `feat_identity::holds(selected_feats, "<name>")` conditional-grounding shape | `feat_effects.rs:2069-2075` |
| Bestow Luck | named in `DEFIANT_LUCK_FEAT_KEYS`, consumed by `defiant_luck_uses_per_day_from_feats`, tested including its own negative case (Bestow Luck alone yields `None` -- Defiant Luck's own uses-per-day grant does not extend to Bestow Luck) | `feat_effects.rs:2549`, `:5032`, `:5048` |
| Greater Spell Focus | named const, exercised in real tests alongside Spell Focus | `feat_effects.rs:535`, `:3625-3653` |
| Push the Limits | `feat_identity::holds` conditional, consumed inside `stamina_pool_facts_from_feats`, tested | `feat_effects.rs:2669`, `:4876-4921` |
| Extra Hex | real `feat_description_completion` function, tested by name | `description_completion.rs:316-326` |

Of the 31, Extra Performance, Extra Lay On Hands, Bestow Luck, Extra Hex, and Push the Limits are at full `§52`-candidate grade (a real test asserting the exact behaviour, including a negative case in Bestow Luck's). The remaining 26 have real, wired code but no confirmed live-consumer test found under that exact name -- one step short of candidate-grade, same distinction `§52` drew between its own rows 1-3 and 4-13.

**The remaining ~276 units have no hit under the literal+slug scan across all nine named files, and no code found on inspection of the hits that did surface.** Framed by method reach, not by claimed absence: *no grounding found by a slug/literal scan across this feat-wiring file set* -- not "no chooser/consumer exists." Two proxy passes plus a named, bounded file-set widening is treated as a defensible floor; a third pass chasing further naming conventions was not attempted and that stopping point is stated explicitly rather than extended into an open-ended search.

**The fifth instrument failure of this epic, and the durable lesson it establishes.** In order: `TYPE:`-vs-`KEY:` screening (item 84), grep hit-count-as-coverage (Domain), the clippy warning-count method, the diagnostics-file scope mismatch, and now literal display-name matching -- five different proxies, each measuring a naming or reporting convention rather than the wiring itself. **This codebase wires content through normalised identifiers (`feat:power_attack`, `POWER_ATTACK_ITEM_ID`) spread across several files, so any single-file, display-name-keyed check under-reports twice over: once for the identifier, once for the file.** That is the transferable finding, independent of this bucket's own numbers.

**Sixth instrument check: the `pilot_compute.rs`/`pilot_compute_corpus.rs` twin, per `decisions.md §29.1` (*"A magnitude is not wired until it moves on the twin the player reads. 15 of SD-27's 115 corrections were this class -- feats wired into the hardcoded twin, tested green, and changed nothing on screen."*).** All 31 candidates above were verified against `pilot_compute.rs` and `build_pilot_headless_receipt` (a two-line, value-preserving wrapper around `pilot_compute.rs::compute_pilot_base_chassis`, per its own doc comment). Neither is necessarily the twin the desktop character sheet reads, so §29.1's own standard was not yet applied here before this decision was first shipped -- a real gap, corrected in this amendment rather than left standing.

A first check re-grepped `pilot_compute_corpus.rs` (1,843 lines) for the 5 candidate-grade feat names and found 0/5 literal hits. That number is not trustworthy: it repeats the exact fifth-instrument-failure shape this decision already names (a display-name match against one file, when the wiring is a normalised identifier that may not need to appear in that file at all). Reading `pilot_compute_corpus.rs::compute_pilot_with_corpus` shows why: `let mut base = compute_pilot_base_chassis(input);` is a direct, unmodified assignment -- the corpus layer's own doc comment states its additions are explicitly *additive*, "NOT wired into the claim-gated `PilotBaseChassisComputation` pillars... this receipt's own `base` field carries." So `pilot_compute_corpus.rs` inheriting a feat's grounding requires no literal reference to that feat anywhere in the file -- a 0-hit grep proves nothing about reachability through this specific seam.

**Decisive test, not a further grep: two candidates with structurally different reach-paths were exercised directly through `compute_pilot_with_corpus`** (the actual entry point `CorpusPilotReceipt.base` -- what the desktop sheet reads -- comes from), asserting the value in `receipt.base.explanations`, then discarded (not committed):
- `Extra Reservoir` (Arcanist level 1) -- the one candidate whose reach-path required an unclosed hop through `compute_class_chassis`'s per-class dispatch, never fully traced in this decision's static analysis. **Surfaced: base reservoir max 4, with the feat 7, delta +3 -- exactly `EXTRA_RESERVOIR_POINTS`.**
- `Armor of the Pit` (Tiefling with the Scaled Skin alternate racial trait) -- reached via the direct `ground_orphan_feat_facts` call already traced statically. **Surfaced: `feat.arg_standalone.armor_of_the_pit_scaled_skin_branch` is absent without the feat, present with it.**

Both surfaced through the receipt the desktop sheet reads, with the exact expected values. This closes the previously-unclosed dispatch hop for the `Extra <resource>` family and confirms the direct-call family, on live evidence rather than a trace. Combined with `Extra Performance` (+6) and `Extra Lay On Hands` (+2) also confirmed live in an earlier probe this same session, **4 of the 31 candidates are now live-tested through the actual twin the player reads, all four surfacing their exact expected value with zero code changes.**

**§29.1's own scope, read against the actual code, is narrower than "the chassis is duplicated."** The specific gap it documents traces to `compute_combat_baseline` (a fixed-posture, per-weapon-loadout sub-computation) vs `compute_combat_baseline_from_corpus` (its corpus-aware supplement) -- a self-documented seam with its own comment at `apps/desktop/src-tauri/src/pf1_adapter.rs:1261-1270` ("the fixed-posture path did not recognize this loadout and emitted no record of its own"). None of the 4 live-tested candidates are per-weapon-loadout AC/attack computations of that kind, and the live test confirms they are not caught by whatever caused SD-27's 15 corrections.

**Not yet live-tested:** `Weapon Finesse` showed a change under a bare probe but the specific field was not isolated; `Bestow Luck`, `Extra Hex`, `Push the Limits` (the other 3 candidate-grade units), and the remaining 26 non-candidate-grade units have not been run through `compute_pilot_with_corpus` at all. The twin-divergence risk §29.1 names is real in general and should not be assumed closed for every one of the 31 on the strength of 4 live tests -- it is closed for those 4, and strongly suggested (not proven) for the rest by the structural argument (`base` is an unmodified direct assignment from the same root function).

**Extended live probe: the twin check widened to every remaining candidate with a real, findable explanation id.** Same shape as the first two (a temporary `#[test]` in `pilot_compute_corpus.rs`'s own test module, run via `cargo test -- --nocapture`, output captured, file then `git checkout`ed to discard -- confirmed clean via `git status --porcelain` before this amendment). 8 more candidates exercised, each differencing with/without the feat through `compute_pilot_with_corpus`:

| Unit | Base | With feat | Delta | Real constant/known value | Result |
|---|---|---|---|---|---|
| Extra Bombs | 1 | 3 | +2 | `EXTRA_BOMBS_USES = 2` | exact match |
| Extra Inspiration | 1 | 4 | +3 | `EXTRA_INSPIRATION_USES = 3` | exact match (this decision's own probe guessed +2 going in; the code was right, the guess was wrong -- corrected against the real constant, not left standing) |
| Extra Martial Flexibility | 4 | 7 | +3 | `EXTRA_MARTIAL_FLEXIBILITY_USES = 3` | exact match (same guess-vs-constant correction) |
| Bestow Luck | 1 | 2 | +1 | matches `defiant_luck_is_one_use_per_day_and_two_with_bestow_luck`'s own proven test exactly | exact match |
| Improved Channel | 15 | 17 | +2 | matches `improved_channel_raises_the_real_oracle_life_mystery_channel_dc_by_two`'s own proven test exactly | exact match |
| Extra Stamina | 3 (with Combat Stamina, its real prerequisite) | 6 | +3 | consistent with the feat's own doc comment | confirmed change |
| Push the Limits | absent (Combat Stamina alone) | present, value 2 | -- | the secondary pool record exists ONLY once Push the Limits is added | confirmed change |
| Greater Spell Focus | 1 (Spell Focus alone) | 2 | -- | matches the doc comment's `max(1,2)=2` "total not increment" claim exactly | exact match |

All 8 surfaced through `receipt.base.explanations` -- the exact field the desktop sheet reads via `CorpusPilotReceipt.base` -- with real values, several matching an already-proven test exactly, none requiring any code change. Combined with the first two (`Extra Reservoir`, `Armor of the Pit`) and the earlier probe (`Extra Performance`, `Extra Lay On Hands`), **12 of 31 candidates are now individually live-tested through the actual twin the player reads, and every one of the 12 surfaced correctly.**

**Weapon Finesse: re-checked more carefully, and the earlier "explanations_changed=true" signal did not reproduce.** A field-by-field diff of the full explanations list between with/without Weapon Finesse on the standard Fighter L1 fixture found no differing value and no count change. The earlier signal is not confirmed and should not be carried forward as evidence in either direction -- correcting this decision's own prior statement rather than letting an unconfirmed "signal" sit next to confirmed candidates.

**Power Attack and Dodge: no change under bare feat-selection, both through `build_pilot_headless_receipt` and `compute_pilot_with_corpus` identically.** This is not a twin-divergence finding -- it is the same active-state/posture limitation this decision already names for the original 307-unit probe (Power Attack is grounded via `POWER_ATTACK_ITEM_ID`, an equipment-like selected/active-state item, not bare feat presence; Dodge's AC effect likely needs an opponent/attacker context this bare probe does not supply). Both entry points behaving identically is itself evidence they share the same underlying computation rather than diverging -- consistent with, not contradicting, the twin-confirmation finding above.

**Not live-tested this round, and not claimed as twin-confirmed:** Combat Reflexes, Improved Unarmed Strike, Extended Animal Focus, the Ranger Combat Style bonus-feat family (Point-Blank Shot, Far Shot, Rapid Shot, Double Slice, Two-Weapon Fighting and its Improved/Greater/Defense variants), and Throw Anything all require a choice-based setup (a Ranger combat-style selection or a Monk bonus-feat-slot selection naming the specific feat) this round's probe did not construct.

**Adaptive Fortune, Fortunate One, Great Hatred, and Extra Hex are not part of `compute_pilot_base_chassis`'s explanations at all, and testing them against `receipt.base.explanations` would be the wrong check regardless of effort spent.** Tracing their call sites: `display_value_deltas_from_feats` (which grounds the first three) is called only from `apps/desktop/src-tauri/src/race_trait_picker.rs` (`:591`, `:788`) -- a real, different consumer (the race-trait-picker UI's own description-refresh path), never from `pilot_compute.rs` or `pilot_compute_corpus.rs`. `feat_description_completion` (which grounds Extra Hex) has exactly one call site in `pilot_compute.rs` (`:34091`, for a different feat, Deflect Arrows) and is otherwise a `description_completion.rs`-owned surface. These 4 units are real and reachable, but through a genuinely different, real consumer than the explanations-based probe this decision otherwise uses -- conflating them into one probe would have been a category error, not a shortcut. Verifying them needs a probe against `race_trait_picker.rs`'s/`description_completion.rs`'s own output, not attempted this round.

**Disposition: characterise, do not close, do not move any unit's status.** 31 of 307 (10%) feat-side units are confirmed to have real, wired grounding code the probe's own posture-sweep does not exercise. Of these, **12 are now individually live-tested through the actual corpus-receipt twin the desktop sheet reads, and every one of the 12 surfaced its exact expected value** -- the highest evidence grade this epic has produced for any unit population, `Touch of Good` included. 2 more (Power Attack, Dodge) were live-tested and correctly showed no change, for a separate, already-known reason (posture/active-state, not twin-divergence). 4 (Adaptive Fortune, Fortunate One, Great Hatred, Extra Hex) are real but reach the sheet through a different consumer entirely and were not tested this round. The remaining 13 (Weapon Finesse, Combat Reflexes, Improved Unarmed Strike, Extended Animal Focus, the 8-unit Ranger combat-style family, Throw Anything) have real code verified in `pilot_compute.rs` but no live twin-confirmation yet. The ~276 outside the 9-file scan stay `unknown`, uninvestigated, and open -- a measurement-method boundary, not a claim that no code exists for them.

**Follow-on, named but not undertaken here, per team-lead's own observation:** the live-probe method used in this amendment is a materially better instrument than the fixed-posture sweep that produced the original 307-unit bucket -- it exercises the real computation path with a real character input rather than sweeping abstract postures. Running it systematically across the full 307 (not just the 31 already name-matched) could settle the bucket's reachability definitively rather than characterise it by proxy. Rough scope estimate: each case in this amendment took roughly 10-15 minutes of setup (finding the explanation id, the required class/level/choice shape, and the expected value) once the harness pattern existed; at that rate the full 307 is a multi-session effort, not a single cycle, and would need its own scoped assignment rather than folding into this one.

## Decision 55 — SD28-E15: `v06_work_inventory.rs`'s `equipment_keys` map only ever read 4 of 8 books `equipment_resolver.rs` already chains -- Decision 36's pattern, at the largest scale it has been found -- ~1,650 already-landed units misreported `not-ingested`, fixed structurally (2026-08-09)

**Trigger.** Re-deriving the seven-Ultimate-book remaining-work table found `ultimate_equipment` reading 0% proven despite this program's own receipt for `epic-25-ue-complete` claiming 1,549 landed records (1,369 equipment + 180 equipmods). Both team-lead and this agent independently reached for "equipment has no probe, so it can never show `grounded`" as the explanation -- a plausible story that happened to be wrong, and it survived initial scrutiny precisely because it was plausible. Team-lead's own standing instruction stood: *"establish what status those 1,549 actually carry and why, before anything else."*

**Root cause, traced not assumed.** `src/rules_core/equipment_resolver.rs::equipment_catalog_rows()` -- the REAL engine consumer every other caller reaches through -- already chains eight books' equipment tables into one combined catalog: CRB, APG, ACG, B1, ARG, PU, UI, UE (`:150-247`). `src/bin/v06_work_inventory.rs`'s own, separate `equipment_keys: BTreeMap<&'static str, BTreeSet<String>>` -- the map `classify()`'s `Kind::Equipment`/`Kind::EquipmentModifier` arm actually reads -- only ever called `.insert()` for **four**: `core_rulebook`, `advanced_players_guide`, `advanced_class_guide`, `bestiary_1`. Every equipment/equipment_modifier unit from ARG, PU, UI, or UE failed the `known` check unconditionally and reported `not-ingested`/`equipment_key_absent_from_equipment_tables` regardless of whether real, landed engine content existed for it -- a hand-maintained list sitting beside a derivable one, silently diverging. This is `not-ingested-figures-are-classifier-noise`'s pattern (previously found in `file_kind()`'s filename-substring typing, now three times over on `race_trait`) recurring in a second, structurally different classifier mechanism -- team-lead's own naming: "Decision 36 for the eighteenth time," this instance the most expensive yet.

**Sized before fixing, per book, table existence checked against the raw file:**

```
advanced_race_guide    equipment_tables.rs exists,    2 top-level entries (real content lives in a chained equipment_data submodule, confirmed post-fix)
pathfinder_unchained   equipment_tables.rs exists,   43 entries
ultimate_intrigue      equipment_tables.rs exists,   99 entries
ultimate_equipment     equipment_tables.rs exists, 1551 entries
ultimate_combat        NO equipment_tables.rs file   (genuinely nothing landed -- 0% is real, not this defect)
ultimate_magic         NO equipment_tables.rs file   (genuinely nothing landed)
ultimate_psionics      NO equipment_tables.rs file   (genuinely nothing landed)
ultimate_wilderness    NO equipment_tables.rs file   (genuinely nothing landed)
```

**Fix: derive the map from the resolver's own output rather than reconciling two lists.** Per team-lead's explicit two guards -- (1) no criterion loosened, every unit still earns its status by a genuine key match against real, landed table rows; (2) hand-verify a sample including a unit expected to STAY `not-ingested` -- `equipment_keys` is now built by iterating `equipment_resolver::equipment_catalog_rows()` directly and grouping by a new `equipment_book_slug_for(short_code)` translation function, instead of four (now zero) hand-written `.insert()` calls. A ninth book added to the resolver populates this map automatically; an unmapped `EQUIPMENT_BOOK_*` code panics loudly in `equipment_book_slug_for` rather than silently vanishing -- the exact failure mode this fix replaces. Two new tests guard it: `equipment_book_slug_for_covers_every_catalog_book` (exercises the translation against every code the resolver actually returns today, so a ninth book without a matching arm fails immediately) and `ultimate_equipment_key_is_reachable_through_the_derived_map` (pins the specific defect -- `Abjurant Salt`, `ue_equip_magic_items.lst:954` -- reachable through the derived map).

**Hand-verified per book, post-fix, including a unit expected to stay `not-ingested` in every case (per guard 2):**

| Book | Total | Ingested-or-text-complete | Still `not-ingested` (real residue) | Verified promoted sample | Verified residue sample |
|---|---|---|---|---|---|
| `ultimate_equipment` | 1,614 | 1,549 (1,364 ingested-magnitude + 185 text-complete) | 65 | `Abjurant Salt` (real row, `:1315`) | `Aklys` -- absent from the real table, genuine gap |
| `ultimate_intrigue` | 105 | 98 (90 + 8) | 7 | `Accent Pill` (real row, `ui_equip_magic_items.lst:12`) | (not individually re-verified this pass, residue is the expected shape given a 99-entry table against 105 corpus units) |
| `pathfinder_unchained` | 42 | 42 (39 + 3) | 0 | (all 42 covered by the 43-entry table) | none -- table fully covers this book's declared units |
| `advanced_race_guide` | 215 | 200 (194 + 6) | 15 | `Alchemical Coal (Blinding Cinders)` (real row) | `Rending Claw Blades` -- absent from the real table (checked across `equipment_tables.rs` and its `equipment_data` submodule), genuine gap |

**`ultimate_equipment` now reads 1,549 of 1,614 landed -- matching `epic-25-ue-complete`'s own receipt number exactly.** The story both agents reached for ("equipment has no probe") was never tested against the actual map contents; it is retired here, not because it was unreasonable to reach for, but because it was never verified before being repeated.

**Cost of the divergence, sized:** ~1,650 already-landed units (1,549 UE + ~7 UI/98 already-real + ~39/42 PU + ~194/215 ARG's real portion) were misreported `not-ingested` -- including the entirety of a book (`epic-25-ue-complete`) this program closed and celebrated earlier the same day, invisible to every dashboard and per-book table produced since.

**Regression side effect, traced not assumed before touching anything.** Regenerating `work-inventory.json` broke `tests/v06_work_inventory.rs::ultimate_psionics_appears_in_the_inventory_as_a_not_started_book`, which asserted every UPsi unit is `not-started`. Confirmed via `git diff`/`git log -p` that this fix's own diff never touches `COMPILED_RULE_SETS` or any `class`-kind classification path -- the real cause is that `RuleSetId::Upsi` entered `COMPILED_RULE_SETS` when UPsi's feat catalog (`§50`, 221 records) and archetype table (`§51`, 15 records) landed in earlier cycles of this same epic, which stopped `classify()` from returning blanket `not-started` for this book long before this fix ran; this fix's regeneration was simply the first one since UPsi's own onboarding to exercise the stale assertion. Test renamed to `ultimate_psionics_appears_in_the_inventory_with_real_per_kind_status`, its blanket `assert_eq!(not-started)` inverted to `assert_ne!(not-started)` (carrying the original boundary-check intent forward, on the side of the boundary UPsi now sits), and a new positive assertion added pinning that the landed feat catalog shows real progress (`text-complete`/`grounded`, not merely "not not-started") -- a strictly stronger test than the one it replaces, not a relaxation (`§32`).

**Standing note, worth carrying forward rather than rediscovering per book:** `race_trait`'s filename-substring mis-typing (ARG 979->0, Bestiary 1 620->0, UPsi 310->~145, `§53`-adjacent) and this equipment-map divergence are two DIFFERENT classifier mechanisms producing the same shape of defect -- a declared count that overstates real remaining work because the classifier never looked at real engine content it should have. Any `not-ingested`/`0%`-proven figure in this program should be treated as unverified until traced against the real table or resolver it claims to check, not assumed accurate because it looks internally consistent.
...
## Decision 57 — SD28-E15/E28: Ultimate Magic's second slice -- 26-record equipment catalog (24 General + 2 ArmsArmor), and a third parallel equipment-book chain found deliberately not fixed (2026-08-09)

**Assignment, redirected from the blocked UPsi `race_trait` slice (`§56`) per team-lead's call:** Ultimate Magic's equipment, 26 units, an existing table pattern genuinely open (UM had a feat catalog and archetype table but no equipment table).

**Declared-vs-real reconciliation, run before writing anything:** `docs/work-inventory.json` declared 26 `ultimate_magic`/`equipment` units, all `not-ingested`/`equipment_key_absent_from_equipment_tables` (real -- UM had no `equipment_tables.rs` file at all). Traced against the raw corpus (`um_equip_general.lst`, `um_equip_arms_armor.lst`, plus `_pfs/pfs_um_equip_general.lst`) rather than trusted: **24 real `Spellbook.COPY=<name>` records** (pregenerated example spellbooks, General category) **+ 2 Scrollmaster Gear records** (`Scroll Shield`/`Scroll Blade`, ArmsArmor category) **= 26, matching the declared count exactly, with zero double-count.**

**The `_pfs/` file traced and ruled out as a second population before extraction -- this program's own `§53`/`§55`/`§56` lesson applied proactively rather than rediscovered.** `_pfs/pfs_um_equip_general.lst` carries `TYPE:PFSNotLegal|!PRECHARACTERTYPE:1,PC` legality-flag rows for 17 of the 24 spellbooks already declared in `um_equip_general.lst` -- same `KEY:`, zero new mechanical content, an Organized-Play overlay, not a distinct record. Confirmed by direct line count (`grep -c "^Spellbook.COPY=" um_equip_general.lst` = 24) before extraction, and pinned afterward by a dedicated regression test (`pfs_legality_overlay_rows_are_not_separate_entries`) naming the exact defect it guards against.

**No `.MOD`/collision hazards found.** All 26 names checked against every other landed book's equipment table (`grep`, zero hits) -- no cross-book collision. Zero `.MOD:`/`SOURCEDISABLE:` tokens in either source file.

**Field-coverage, exact:** 26/26 carry a real `COST:` token (both Scrollmaster Gear rows' real `COST:0`, not a missing value). 13/26 carry a real `WT:` token (11 higher-level spellbooks + both Scrollmaster Gear rows at `WT:0`; the other 13 spellbooks genuinely have no `WT:` token). 24/26 carry a real `SPROP:`-sourced description (every spellbook; the 2 Scrollmaster Gear rows genuinely have none). `description` follows the established ACG/ARG convention: multiple `SPROP:` tokens joined with `"; "`; no `DESC:` token exists anywhere in either source file (confirmed by direct grep, matching ACG's/ARG's own prior finding for this corpus generation).

**`equipmod_tables()` is a real, permanently-empty slice, not an omission -- §44's both-halves discipline, applied unprompted.** UM has no equipment-modifier file at all (confirmed by directory listing: no `um_equipmods.lst`). The function exists and returns `&[]` so the module satisfies the same shape `equipment_resolver.rs`'s chain expects from every book it includes, with its own doc comment stating why the emptiness is real rather than a gap.

**Wired structurally, extending `§55`'s own fix rather than re-introducing a hand list:** `EQUIPMENT_BOOK_UM` added to `equipment_resolver.rs`, UM's rows chained into `equipment_catalog_rows()`, and `"UM" => "ultimate_magic"` added to `v06_work_inventory.rs`'s `equipment_book_slug_for` -- since that function already derives `equipment_keys` from the resolver's own chain, this one addition is sufficient for the classifier to see UM's equipment with no further map maintenance.

**Aggregate pin re-derived by counting, not by hand-adding 26.** `equipment_resolver.rs`'s own `catalog_rows_span_every_ingested_book_with_their_real_counts` test failed after the wiring (`5,503` actual vs `5,477` pinned) -- confirmed the delta was exactly UM's 26 rows, then re-derived `5,503` with a temporary probe (`println!` inside the test, run, output captured, removed before commit) that also independently confirmed all 26 UM keys are unique within the book (no internal duplicate). Pinned `5,503` with a comment stating it was counted, not computed.

**Count sweep, three more files, each a real derived count, not a hardcoded one:** `corpus_ingest_diagnostic.rs`'s `ultimate_magic_counts()` gained an `equipment` entry (`.len()`-derived); `v06_content_state_dump.rs`'s `um_content()`'s equipment `KindCount` changed from a hardcoded `0` to the same `.len()`-derived value.

**Deliberately NOT wired: a `reach_gate` entry for UM's equipment.** `reach_gate.rs`'s per-book equipment entries key off `apps/desktop/src-tauri/src/equipment_catalog.rs`'s own `build_equipment_catalog()`/`EQUIPMENT_CATALOG_BOOKS` -- a THIRD, independent hand-maintained equipment-book chain, separate from both `equipment_resolver.rs` (fixed in `§55`) and `v06_work_inventory.rs`'s now-derived classifier map. UM (and, discovered in the same trace, `ultimate_equipment` -- the 1,551-record book `epic-25-ue-complete` closed) are absent from this third list. Adding a `reach_gate` entry against a catalog that does not serve UM's rows would have manufactured a false "0% reachable" signal -- exactly the class of instrument error this epic spent the night finding and correcting, now caught before being introduced rather than after. Recorded as its own `OPEN_FINDINGS` entry in `artifacts/e14-harness-widening.md` (three-chain divergence, UE's absence named as the more consequential of the two, remedy options given) rather than fixed in this slice -- UE's addition to the desktop picker is a real feature addition (new UI rows, category/book filter behavior), not a count-sweep, and out of scope for a cycle already carrying seven touched files.

**Verification:** `cargo build --locked --bins` (root): clean. `cargo build --locked` (`apps/desktop/src-tauri`): clean, confirmed separately per this program's own "root sweep misses the desktop crate" lesson. `cargo test --lib --locked ultimate_magic::equipment_tables`: 6/6 new tests pass. `cargo test --locked --no-fail-fast` (full suite): 538 targets, 0 failures. `cargo clippy --locked --tests`: 75 warnings (gate ceiling, not breached), `EXIT_CODE=0`, confirmed the new module added zero new warnings. Regenerated `docs/work-inventory.json`: UM equipment now 17 `text-complete` + 9 `ingested-magnitude`, 0 `not-ingested`, matching the 26-record table exactly.

**Disposition: 26/26 real equipment records landed and verified, zero fabricated, zero double-counted. Third parallel equipment-book chain found and its two missing books (UM, UE) named as a real product gap, deliberately not fixed in this slice, recorded for a future assignment.**

## Decision 58 — SD28-E15: Ultimate Psionics equipment (439 real records: 326 equipment + 113 equipmods), a `.COPY=` legacy-alias hazard self-caught and corrected before shipping, and a fourth §49 text-shape hazard confirmed clean across UE/UI/ARG/CRB (2026-08-09)

**Assignment:** UPsi equipment (552 declared units: 326 equipment + 226 equipment_modifier), the largest genuine remaining equipment gap. Team-lead's own briefing named 326; both halves pulled unprompted per this program's own "check both halves of the aggregation pair" discipline, correcting the briefing before extraction rather than after.

**Row-kind check run before extraction, per the explicit warning that UPsi's own `race_trait` file mixed four wrong kinds (`§53`).** Traced `up_equipment.lst`'s every `TYPE:` first segment (`Psionic`, `Weapon`, `Armor`, `Enhancement`, `Shield`, `PsionicTattoo`, `Magic`, `Skin`, `Psicrown`, `PowerStone`, `MindStone`, `GreaterMindStone`, `Dorje`) and every `###Block:` header -- all genuinely equipment-shaped (armor, weapons, wondrous/psionic items), no monster-special-ability or class-name mixing found. **A real, useful negative result, not assumed clean.**

**First reconciliation pass (declared vs. real, `up_equipment.lst` + `up_equipmods.lst`):** 326 real records in `up_equipment.lst` (zero `.MOD` rows), 227 real content lines in `up_equipmods.lst` minus 1 `.MOD` row (`Special Ability ~ Keen ~ Weapon.MOD`, a psionic prerequisite restriction injected onto an existing cross-book `Keen` weapon ability) = 226 -- matching the declared 552 exactly on the first pass. Landed, wired structurally into `equipment_resolver.rs` (extending `§55`'s own fix), aggregate pin re-derived by counting (6,055), full suite green, clippy at ceiling.

**Self-caught before this landed: regenerating `docs/work-inventory.json` as a final check (not trusted from the table alone) found 113 of 552 units still `not-ingested`.** Traced to a second hazard the first pass's `name.endswith(".MOD")`-only filter never checked for: **113 `.COPY=<SHORTCODE>` rows** (e.g. `Special Ability ~ Aporter ~ Armor.COPY=APORT`), every one carrying only `VISIBLE:NO` with no `COST:`/`SPROP:` of its own, confirmed by direct grep that no short code appears anywhere else in the UPsi corpus as an `EQMOD:` lookup target or any other reference.

**First fix attempt got the disposition backwards, and was itself corrected before commit.** The first correction split each `.COPY=` row into its own table entry keyed by the short code -- fabricating 113 near-empty (`cost_gp: None`, `description: None`) rows purely to make the classifier's declared count resolve. **This was wrong, and team-lead's own cross-book sweep caught the reason:** `ultimate_intrigue::equipment_tables` and `advanced_race_guide::equipment_data::equipmods` already establish the correct treatment for the identical `VISIBLE:NO .COPY=` shape -- UI's own doc comment states it explicitly: *"`work-inventory.json`'s 14 is an over-count, not a gap this table should close... the classifier does not know about `VISIBLE:NO` and counts both rows as distinct declared units."* Corrected to match: the 113 `.COPY=` rows are **excluded** from the table as legacy/internal aliases (same shape as `.MOD`-injected grants and ARG's own documented "Old KEYs" block), named explicitly in a new `EXCLUDED_LEGACY_ALIAS_SHORT_CODES` constant rather than silently dropped, with a regression test (`copy_alias_short_codes_are_named_but_not_shipped_as_entries`) checking BOTH directions -- named in the exclusion list AND absent from the resolvable table.

**Final real content: 326 + 113 = 439 records.** The 113-unit gap between this and the 552 declared is real, is classifier over-counting (not missing content), and is named -- matching `§55`'s/`§56`'s own standing lesson that a declared count is unverified until traced, extended here to a shape that inflates in the *opposite* direction from `race_trait`'s filename mis-typing (here the classifier double-counts one real modifier as two declared units; there it typed unrelated content under the wrong kind entirely).

**Cross-book sweep, run at team-lead's request after the UPsi finding, to check whether the same `name.endswith(".MOD")`-only mistake had already shipped elsewhere.** Team-lead's own corpus-wide `.COPY=` census (CRB 1,690 · UPsi 113 · UE 92 · psionics_unleashed 72 · mythic_adventures 70 · ACG 48 · UM 24 · adventurers_guide 21 · APG 21 · UC 20 · ARG 15 · UI 8) raised the question of whether UE/UI/ARG/CRB's own committed tables carried the same mis-keyed literal form. **Checked precisely, scoped to actual `key: "..."` literals rather than a whole-file text match:**

```
grep -oP 'key: "[^"]*\.COPY=[^"]*"' <table file>
```

**Zero hits in all four books.** Every `.COPY=` occurrence in UE's/UI's/ARG's/CRB's committed `equipment_tables.rs`/`equipment_data/*.rs` files is inside a `//!`/`///` doc comment *documenting* the `.COPY=` handling already done correctly (UE's own comment: *"92 of the corpus's declared 1,424 equipment rows are `.COPY=` rows that declare a genuinely distinct new item... confirmed by re-deriving against `docs/work-inventory.json`'s own 1,424"*; ARG's: *"14 corpus rows are deliberately excluded: the file's own trailing `# Old KEYs` block"*; CRB's own doc comment names an explicit `.COPY=`-inheritance criterion, SD-25 criterion 7.N). **A whole-file grep for `.COPY=` (matching doc-comment citations, not only live keys) would have produced exactly the false-positive count team-lead's own sweep first reported (5/2/1) -- the same "measures the wrong thing" instrument shape this epic has named eight times now, this time on the checking side rather than the shipped-code side.** All four books were already correct, each having independently discovered and documented the right disposition (include real distinct variants, exclude `VISIBLE:NO` legacy aliases) before this cycle. Reported as a real, useful negative, per team-lead's own standing instruction that a clean result is worth stating explicitly.

**`.COPY=` aliasing added to `§49`'s triad as a fourth text-shape hazard**, alongside unconditional-recovery `.MOD`, conditional-variant `.MOD`, and never-join. The triad is now a checklist: any new equipment/equipmod extraction must check for (1) `.MOD` rows injecting onto an existing record, (2) `.MOD` rows carrying a real conditional variant worth naming even if not modeled, (3) a `.COPY=` row that IS a genuinely new distinct item (UE's 92), and (4) a `.COPY=` row that is a `VISIBLE:NO` legacy alias of an already-counted base record (UPsi's 113, UI's 7-doubled-to-14, ARG's 14).

**Verification:** `cargo build --locked --bins` (root) and `cargo build --locked` (desktop): both clean. `cargo test --lib --locked ultimate_psionics::equipment_tables`: 7/7 pass (including the corrected `.COPY=` regression guard). `cargo test --locked --no-fail-fast` (full suite, after the correction): all green. `cargo clippy --locked --tests`: 75 (gate ceiling, not breached), `EXIT_CODE=0`, re-run after the correction confirmed zero new warnings from the corrected module. Regenerated `docs/work-inventory.json`: UPsi equipment/equipment_modifier now 389 `ingested-magnitude` + 50 `text-complete` = 439 real landed, 113 `not-ingested` residue matching the named exclusion count exactly.

**Disposition: 439/439 real records landed and verified, zero fabricated. The 113-unit residue is honest, named, and attributable to a documented classifier over-count, not a gap. First extraction pass's mistake (fabricating entries to close a gap that was actually a measurement artifact) corrected before commit, not shipped -- the self-check that caught it (regenerating the inventory as a final step, not trusting the table alone) is the same discipline `§55`/`§56` already established. Three other books' equipment tables independently confirmed clean against the identical hazard shape.**

## Decision 59 — SD28-E30/C4.8: the archetype-vacuity audit backlog, captured before any wiring code, per the operator's own "wire now, audit after" decision (2026-08-09)

**Context.** Task #67 excluded archetype content from base-class chassis computation program-wide (`forward-scope-register.md §C4.8`). Every class whose bonus-feat/resource pool has a corpus `.MOD`/flag-gated deduction that only an unmodelled archetype ever sets carries an in-code comment asserting the deduction is "provably vacuous" under that boundary. The operator has now unblocked `§C4.8` -- archetype wiring proceeds -- with the explicit condition that the vacuity comments this boundary depended on get audited **after** wiring lands, not skipped. This decision captures the complete backlog **before any wiring code is written**, per that condition and per team-lead's own instruction that a comment left unmarked once it is false is worse than one labelled false.

**Method.** `grep -n "archetype" src/rules_core/pilot_compute.rs -i`, filtered to lines asserting an archetype-gated deduction/flag is "provably vacuous" / "out of scope" / "this repo ingests no ... archetype" -- read each hit's surrounding function to attribute it to a real class and confirm the exact claim, not just the keyword match. Excludes generic "archetype" mentions that are not vacuity claims (e.g. plain corpus citations).

**Seven classes carry a vacuity claim that archetype wiring will invalidate, 13 total citations (several classes have both a doc-comment and a duplicate in the player-facing explanation string):**

| Class | Line(s) | Exact claim |
|---|---|---|
| Cavalier | `:2114`, `:11079-11080` | 3 `.MOD` records (`TYPE.CavalierCavaliersBonusFeat6/12/18`) each subtract 1 from the bonus-combat-feat pool, gated on "a specific Cavalier archetype"; "this repo ingests no Cavalier archetype at all -- provably vacuous here" |
| Slayer | `:17102-17103` | 10 archetype-gated deductions against the Slayer Talent count pool; "every setter is a Slayer archetype record and this repo ingests only the base class" |
| Swashbuckler | `:17542` (doc comment), `:17941` (explanation string) | 5 `.MOD` records each carry a `PREVAREQ:Swashbuckler_CF_BonusFeatN,0` suppression flag; "every setter... is a Swashbuckler ARCHETYPE record (Daring Infiltrator, Mysterious Avenger), and this repo ingests only the base `swashbuckler.json`" |
| Shaman | `:19167-19168` | `ShamanSpiritLVL`'s second setter (`BONUS:VAR|ShamanSpiritLVL|SorcererLVL`) belongs to `KEY:Spirit Summoner ~ Spirit` -- "a Summoner archetype this repo does not ingest -- so it is provably vacuous here" |
| Bloodrager | `:21822-21824`, `:21874` | Crossblooded Bloodline Selection excluded ("archetype-gated... archetypes are out of scope for base-class chassis, per task #67"); Eldritch Scion's own `TYPE:EldritchScionBloodragerBloodlineChoice` record (which would shift 3 of 4 bloodline-spell grant levels) named and deliberately not read, "out of scope for base-class chassis per task #67" |
| Brawler | `:22704`, `:63823` (test doc comment) | 7 `-1` deductions against the bonus-combat-feat pool, "every one is gated on a Brawler archetype flag whose only setter is Wild Child, and this repo ingests no Brawler archetype -- provably vacuous" |
| Sorcerer | `:36077`, `:36225` | 3 `-1` deductions against the Arcane-bloodline bonus-feat pool, each gated on a `Sorcerer_CF_BloodlineFeat<n>` flag, "the standard corpus mechanism for a sorcerer archetype that replaces a bloodline feat; this repo ingests no sorcerer archetype, so all three are provably vacuous here" |

**One additional citation, cross-cutting rather than single-class, flagged separately because it names a *category* of archetype rather than one specific archetype/class pairing:** `:25984` -- `TYPE:Internal.<Class> ClassFeatures` "No Spellcasting" records exist per-class but are "granted only by spellcasting-removing archetypes this repo does not ingest," deliberately not applied to any class's caster-level computation. This is not scoped to one class in the table above because the comment itself is written generically across the caster-level-tracking helper multiple classes share -- whoever picks up this line should re-attribute it to the specific class(es) whose caster level it affects before treating it as closed by wiring any one archetype.

**What "invalidates" means here, stated precisely so nothing is fixed by assumption:** once a real archetype-swap mechanism is wired into `pilot_compute.rs` (the deliverable `§C4.8`/this decision unblocks), a character who has genuinely selected one of the named archetypes (Cavalier's three, Slayer's ten setters, Swashbuckler's Daring Infiltrator/Mysterious Avenger, Shaman's Spirit Summoner, Bloodrager's Crossblooded Rager/Eldritch Scion, Brawler's Wild Child, Sorcerer's bloodline-feat-replacing archetypes) can make the previously-vacuous deduction/flag real. Each citation above is a real product behavior change waiting on wiring, not merely a stale comment -- until archetype selection reaches these specific pools, the comments remain true and the audit is "not yet reachable," not "wrong."

**Disposition: this is the audit backlog, not the audit.** No comment above has been marked resolved and no wiring code has been written as part of this decision -- per the operator's own sequencing (wire now, audit after) and the explicit instruction that every vacuity comment a wiring change invalidates gets flagged as it is passed, even in a cycle that does not fix it. The generic `archetype_claims_slot(subject, base_id, slot_id)` primitive, proven end-to-end on Alchemist first (19 clean records: Mutagen/Discovery/Poison Resistance), is the next piece of work, with reachability as the acceptance test per `§43`.

## Decision 60 — SD28-C4.8: the `archetype_claims_slot` primitive, proven end-to-end on Alchemist's Poison Resistance -- landed, not yet audited against `§59` (2026-08-09)

**The primitive.** `src/rules_core/archetype_resolver.rs` (new module) aggregates all seven `§51` tier-1 archetype tables into one lookup surface (`archetype_catalog_entries()`, the same chain shape `equipment_resolver.rs`'s `equipment_catalog_rows()` already proved for equipment). `archetype_claims_slot(input: &CharacterInput, subject: &str, slot_id: &str) -> bool` answers: has the character genuinely selected an archetype (for `subject`) whose own `replaces` list names `slot_id`. A companion `archetype_claiming_slot` returns the claiming archetype's own `archetype_name` for building a real explanation, not a generic one.

**A new selection-input convention, necessary because none existed.** Confirmed by grep before starting: zero hits anywhere in this codebase for any `"choice:archetype"`-shaped id. Archetype selection is now recorded as a `SelectedChoice` with `choice_set_id: ARCHETYPE_CHOICE_ID` and `selection_id` equal to the archetype's own real corpus `KEY:` (e.g. `"Alchemist Archetype ~ Plague Bringer"`) -- reusing the table's own stable identifier rather than inventing a second naming scheme.

**Three deliberate non-features, stated so the primitive's own scope is not overread:** does not infer a slot claim from `grants` (not paired 1:1 with `replaces`, per `archetype_swap.rs`'s own doc comment); does not validate the selection against the archetype's own `prerequisites` (that is `feat_prereqs`' job, a separate concern); does not assume at most one archetype selection per subject (every matching selection is checked).

**First real consumer, chosen for being the cleanest available match, not by convenience:** Alchemist's Poison Resistance grounding (`ground_alchemist_bomb_and_poison_resistance`). ARG's Plague Bringer archetype (`§51`'s own catalog) names `AlchemistPoisonResistance` (among five other slots) in its own `replaces` list -- a real, corpus-confirmed match, not assumed from the archetype's flavour text. Before this landed, the base APG progression computed unconditionally for every Alchemist regardless of archetype selection -- structurally the same "provably vacuous... this repo ingests no [class] archetype" shape `§59` catalogued for six other classes, closed here for real because Alchemist archetypes ARE ingested (unlike those six).

**What changed in the actual compute output, not only in the resolver:** when `archetype_claiming_slot` finds a match, the Poison Resistance explanation now reads value `0` with a detail stating plainly that the base progression is superseded by the named archetype, rather than computing the base APG tiers as if no archetype existed. The archetype's OWN replacement mechanic is not fabricated -- this closes the vacuity, it does not invent a new number to replace it with.

**Reachability proven per `§43`, through the real compute path, not a resolver-only test.** Three new tests in `pilot_compute.rs` itself (`alchemist_archetype_slot_reachability_tests`), each driving `build_pilot_headless_receipt`: (1) a bare level-5 Alchemist grounds the real +4 progression, no archetype language in the explanation; (2) the SAME character with Plague Bringer genuinely selected via the real `ARCHETYPE_CHOICE_ID` choice sees value 0 and the explanation naming Plague Bringer specifically; (3) a DIFFERENT Alchemist archetype (Bramble Brewer, which only replaces the Discovery slot) leaves the +4 grounding completely unchanged -- proving the wiring is scoped to the real claimed slot, not "any archetype selected turns this off." Plus 5 resolver-level unit tests in `archetype_resolver.rs` itself (positive case, negative case with no archetype, wrong-archetype case, wrong-subject case, and a direct assertion against the real Plague Bringer catalog row's `replaces` list).

**Explicitly NOT audited against `§59`'s 13 vacuity citations.** This decision proves the primitive works and lands its first real consumer (Alchemist, which was never one of the 7 classes `§59` catalogued -- Alchemist archetypes were already ingested, so its vacuity shape was different in kind, not one of the "this repo ingests no archetype" cases). None of Cavalier/Slayer/Swashbuckler/Shaman/Bloodrager/Brawler/Sorcerer's own vacuity comments are touched, marked resolved, or invalidated by this commit -- that audit remains fully open, tracked in `§59`.
...
## Decision 63 — SD28-C4.8: archetype-swap class-two measurement lands on Fighter, and the epic's true size is proven to be un-extrapolatable from any sample (2026-08-10)

**Class two, measured not estimated.** `archetype_resolver::archetype_claiming_slot`'s second real-compute-output consumer wired onto Fighter's Bravery (`49d07a5f`) -- a structurally different, non-resource-pool class from Alchemist (`§60`), chosen deliberately to test whether the primitive generalizes. It did: zero changes needed to `archetype_resolver.rs` itself. Real APG archetypes (Archer, Free Hand Fighter, Mobile Fighter, Phalanx Soldier, Polearm Master, Roughrider, Savage Warrior) confirmed via direct grep to name `FighterBravery` in their own `replaces` list, not assumed from one. Reachability proven per `§43` the same way as Alchemist: `fighter_archetype_slot_reachability_tests`, three tests through `build_pilot_headless_receipt` (bare-class baseline, Archer supersedes with the archetype named in the explanation, Crossbowman -- which does not list `FighterBravery` -- leaves the base grounding unchanged). Zero `§59` vacuity citations touched; Fighter was never one of that backlog's seven classes.

**Decomposed by hunk, not estimated: 33 lines production wiring, 104 lines test scaffolding, out of +137/-10 total.** The number that matters for scoping is the 33, not the 137 -- the if-let/else supersession branch that replaced the prior unconditional Bravery block. Team-lead's correction, accepted and acted on: 33 lines bought one *feature slot* (Bravery), not one *class* -- Alchemist's own build-out covered three slots (Mutagen, Discovery, Poison Resistance) for one class. The unit that scales is slots, not classes, which is this epic's own founding insight (record-count is not engine-cost) applying with equal force in the other direction.

**The naive ceiling: 512 distinct slot names across 28 archetype-bearing classes**, derived by parsing every tier-1 archetype table's `replaces` list and unioning by subject class. Immediately shown to overstate: Alchemist wired 3 of its 26 named slots, Fighter 1 of 22 -- neither anywhere near 512's implied average of ~18/class.

**The true filter -- "does an unconditional (non-archetype-gated) base computation already exist for this slot" -- was attempted by automated id-string proxy three times and abandoned after the third, each iteration breaking on a naming assumption that had not been validated against known truth first:**

1. First pass: `class_feature.<class>.*` id extraction only. Produced a 125-slot candidate ceiling, self-validated against Fighter (true 1, heuristic said 7) and Alchemist (true 3, heuristic said 5) -- caught as unreliable (2-7x overstatement) before being reported as fact.
2. Second pass, in the *other* direction: classes with zero extracted ids (Paladin, Companion) reported as "confirmed unwireable" -- 40 slots. This was **wrong and retracted outright**, not revised. The extraction never scanned `class_chassis.*` at all (Paladin's real grounding lives there: `class_chassis.paladin.aura_of_faith` etc., 27 real ids) and had an indexing bug on class-name-as-prefix families (`companion.hit_points` etc., Companion's real 7 ids). Team-lead's own framing named the lesson precisely: *"a proxy has to be validated against known truth before it is used in the direction that produces a confident claim, not only in the direction that produces a suspicious one."* A zero from a broken filter looked the most definitive of any result and was the least trustworthy.
3. Third pass, corrected id-family enumeration (`class_feature`, `class_chassis`, plus six class-named-prefix families found by inspection): validated against Fighter (20 real ids, >=1 required) and Alchemist (21 real ids, >=3 required) before trusting it. Result: **zero classes show real_ids=0** -- no class in this dataset can be claimed provably unwireable. Candidate ceiling revised to 165/512, same overstatement caveat as pass 1. Explicitly flagged as still incomplete: pure helper functions with no explanation id at all (`animal_companion_table_index`) are invisible to any id-string method regardless of family coverage, so even 165 is a floor on what this method can detect, not a ceiling worth defending.

**Automated proxying abandoned after the third break, in favor of hand verification -- the same standard as Fighter and Alchemist, applied to two more classes chosen for maximum information: Paladin (33 slots, the false zero, the strongest test of whether the ratio holds where the proxy failed hardest) and Bard (33 slots, the largest tied count).**

- **Paladin: 16/33.** `explain_paladin_level1_chassis_and_spell_burden_separation` (pilot_compute.rs:27599) unconditionally grounds LayOnHands, DivineGrace, ChannelPositiveEnergy, AuraOfFaith, AuraOfJustice (+ its casing-duplicate string), Mercy (+ six level-tier variant slot strings, same underlying grant), SmiteEvil, HolyChampion, Spells. The other 17 named slots have zero matching id anywhere in the file (proficiencies, most auras, DivineBond/BondedWeapon, DetectEvil, DivineHealth) -- grepped directly, confirmed absent, not omitted by search scope. Six more are Antipaladin's own mirror features, correctly absent from Paladin's own grounding.
- **Bard: 23/33.** `explain_bard_level1_spell_baseline` (pilot_compute.rs:40462) plus `ground_or_block_bard_bardic_performance`/`bard_inspire_courage_bonus` unconditionally ground BardicKnowledge, BardicPerformance, DeadlyPerformance, Fascinate, FrighteningTune, InspireCompetence (+ six level-tier variants), InspireCourage (+ four), InspireHeroics, JackOfAllTrades, LoreMaster, SoothingPerformance, VersatilePerformance, WellVersed. Ten have no match (proficiencies, ArmoredCasting, Countersong -- confirmed present only in a doc comment, zero quoted ids -- DirgeOfDoom, InspireGreatness, MassSuggestion, Suggestion). `BardFrighteningPerformance`, a near-miss sibling string to the confirmed `FrighteningTune`, deliberately left unconfirmed rather than counted on name-similarity alone.

**Four hand-verified data points, and the finding they establish:**
...
A 14x spread across four classes settles the question rather than leaving it open: **there is no shared ratio, and no estimate for the remaining ~24 archetype-bearing classes is defensible from this sample.** The predictor is not class type, slot count, or even total real-id count (the obvious next proxy also fails: Fighter carries 20 real ids against 1 wired-able slot, Bard 28 against 23 -- id volume alone does not predict wireable-slot volume either). The actual predictor is class-specific history: how much of that class's signature mechanics happened to already be grounded for reasons unrelated to archetypes, before this epic ever started. Fighter and Alchemist name mostly proficiency/static features as slots, most of which this engine has never modelled as explanation entries at all. Paladin and Bard's signature mechanics -- spell progression, aura/save bonuses, performance economy -- were already grounded for their own unrelated reasons (spellcasting, combat math) well before archetype work began.
