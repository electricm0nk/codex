...
## Decision 54 — SD28-E15: the 307-unit feat-side `unknown` bucket -- reachability, not naming, is the right evidence shape; 24 units confirmed reachable by real code the probe does not exercise (2026-08-09)

**Different evidence shape from `§52`/`§53`.** The feat-side bucket (`evidence: in_catalog_with_corpus_magnitude_but_no_observed_consumer`) is not option-pool content with a chooser to check -- it is 307 independent feats already in the engine's catalog, each carrying its own diagnostic text stating the probe's own limit verbatim: "the feat-effect probe observed no computed delta across the swept postures... the effect may need a posture, an opponent, or a combat action this engine does not model. Reported as unknown rather than deferred because no engine diagnostic is scoped to a feat." 287 of 307 already carry `wiring_class: computed` -- the classifier's own belief that real compute code exists is a separate claim from the probe's own failure to observe a delta, exactly the distinction this decision tests unit by unit. Breakdown by book: UPsi 83, UM 43, CRB 40, ARG 35, UC 31, APG 29, ACG 25, UW 9, UI 8, PU 4.

**Method, adapted from `§52`'s: no single chooser to anchor on, so scan for a real consumer per unit instead of per family.** Two proxy scans, both checked against real code before being trusted:
1. Literal quoted-string match against `pilot_compute.rs` (does any real code reference the feat by its exact display name) -- found 22/307.
2. Slug match (the feat's name lowercased and `snake_case`d, e.g. "Power Attack" -> "power_attack") against the same file, since some feats are grounded via an internal id constant rather than the display string -- found 28/307 combined, confirmed necessary by `Power Attack` itself: grounded via `POWER_ATTACK_ITEM_ID`/`POWER_ATTACK_FEAT_SELECTION` (`pilot_compute.rs:6825`/`:6836`), never the literal string `"Power Attack"` anywhere in the file. The literal-string scan alone would have reported this foundational feat as unreachable.

**Both proxies are floors, and the slug proxy produced false positives on loose substring matching -- caught on the individual walk, not left standing.** 4 of the 28 slug-scan hits did not survive a read of the surrounding code: `Stalwart` matched only inside an unrelated doc-comment sentence; `Discovery (Opposition Research)` and `Discovery (Split Slot)` matched nothing real on a targeted re-check (the word "discovery" alone is too common); `Toughness (Vigor/Wounds)` matched only the base `Toughness` feat's own test (`:63349`) -- a different corpus record from the Vigor/Wounds variant this bucket unit actually is. All four corrected back to unconfirmed rather than left in the candidate count.

**24/307 (8%) confirmed reachable, each spot-checked against its actual surrounding code, not the string hit alone:**

| Unit(s) | Grounding | Location |
|---|---|---|
| Weapon Finesse | pushed in real tests | `:59750`, `:59771`, `:59792` |
| Combat Reflexes, Dodge | feat-id lookup table + branching logic | `:5126`/`:5128`, `:33621-33656` |
| Improved Unarmed Strike | real `granted.push(...)`, asserted in tests | `:4770`, `:62267`, `:62320` |
| Power Attack | id constants, not display string | `:6825`, `:6836` |
| Extra Bombs, Extra Inspiration, Extra Martial Flexibility, Extra Reservoir, Extra Performance, Extra Lay On Hands | `extra_resource_feat_bonus` generic mechanism (task #19); Performance and Lay On Hands additionally proven in `extra_resource_feat_tests` | consts `:32017-32087`; calls `:11355`, `:15519`, `:18243`, `:22552`, `:19867`, `:27745`; tests `:61608-61763` |
| Extended Animal Focus | named const, pushed in real tests | `:32106`, `:52817`, `:52846` |
| Improved Channel | named const, pushed in real tests | `:32100`, `:57440`, `:57445`, `:57458` |
| Extra Stamina | `extra_stamina_picks` field feeding Pathfinder Unchained's real stamina-pool computation -- found only by reading the surrounding code, not by either scan matching the feat's own name | `:43090` (block starts `:43082`) |
| Point-Blank Shot, Far Shot, Rapid Shot, Double Slice, Two-Weapon Fighting, Improved Two-Weapon Fighting, Greater Two-Weapon Fighting, Two-Weapon Defense | one real mechanism: Ranger's Combat Style bonus-feat grant recognizes exactly these as valid free-feat choices for the Archery/Two-Weapon-Fighting styles | `:28965-28995` |
| Throw Anything | feat-id lookup + branching logic | `:5131`, `:33631`, `:33739` |
| Armor of the Pit | conditional grounding: `feat_identity::holds(feats, "Armor of the Pit") && character_has_tiefling_scaled_skin(input)` | `:43128` |

None of these 24 have yet been reach-checked against a live consumer (a real test asserting the exact corpus magnitude, or an adapter surfacing the value) the way `§52`'s Domain/Oracle/Bloodline candidates were -- Extra Performance and Extra Lay On Hands are the exceptions, already proven to that standard by `extra_resource_feat_tests`. The other 22 are "real code exists and is wired," not yet "verified against a live consumer" -- one step short of `§52`'s candidate-grade bar.

**The remaining ~283 units have no hit under either proxy scan and no code found on inspection of nearby context.** Consistent with the probe's own stated limit (posture/opponent/combat-action modeling this engine does not have). Not extrapolated as confirmed-unreachable -- both proxies are name-based and this epic has twice now found real grounding hiding behind a naming convention neither proxy anticipated (`Power Attack`'s id constants, `Extra Stamina`'s facts field) -- but no further naming-convention hunt was attempted past these two passes, and that stopping point is named explicitly rather than silently extended into an open-ended search.

**Disposition: characterise, do not close, do not move any unit's status.** 24 of 307 (8%) feat-side units are confirmed to have real, wired grounding code the probe's own posture-sweep does not exercise -- a genuine reachability gap between the engine and its own verification instrument, the same shape as `Touch of Good`'s `reach_gate` gap, now measured at feat-bucket scale rather than found on one record. 2 of the 24 are already at full candidate-grade evidence; the other 22 need the same live-consumer confirmation before being proposed for reclassification. The remaining ~283 stay `unknown`, uninvestigated past two name-based proxy passes, and open.
