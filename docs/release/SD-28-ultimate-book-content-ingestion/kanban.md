...
| 32 | `epic-32-archetype-swap` | IN-FLIGHT | Archetype-swap mechanism | tier-1 data ingestion (piece 1) COMPLETE: all 7 in-scope books landed (UPsi 15 + ACG 87 + APG 80 + UM 67 + UC 65 + ARG 59 + UW 30 = 403 records), each with real corpus reconciliation, its own `.MOD`-injection-population share stated, and the shared `TYPE:`/`ABILITY:` no-convergence finding confirmed across all seven (33/33/52/27/22/14/30% -- no single number). Two populations sized and deliberately deferred: 4,550 tier-2 sub-feature records, 1,282 `.MOD`-injected-grant rows. `pilot_compute.rs` integration (pieces 2/3) blocked on an explicit scope decision (task #67 reversal) -- see `decisions.md §51`, `forward-scope-register.md §C4.8`. Note: this card was informally called "epic-30" during scoping before the `epic-30-integrity` id collision was caught; renumbered here | `epic-14-harness` | epic-24-ultimate-intrigue | 2026-08-09T02:23:00-04:00 | SD28-E30-F7-001 |
