...
| 14 | `epic-15-unknown-sweep` | IN-FLIGHT | Classification sweep | 3,734-unit `unknown` bucket characterised (not closed): 438 (13%) archetype tier-2 content, 2,989 (87%) class-native option-pool content; canonical-narrowing pattern confirmed across 3 families (Domain 1/166, Oracle Mystery 2/134, Bloodrager Bloodline ~10/152); remainder (1,746 units/751 prefixes) clustered, 12 families ≥20 units deep-checked (443 units): 85 mis-clustered real archetype content (Skinshaper/Fused Eidolon Skills), 40 a new universal-formula shape (Beastmaster/Packmaster companion tables), 303 confirmed-absent chooser (Vigilante talents + UPsi psionic disciplines), 15 folds into Bloodline; below-20 tail (739 prefixes/1,303 units) counted not investigated; 307-unit feat-side bucket: first-pass single-file scan false-negatived on Power Attack, widened to a named 9-file feat-wiring set (feat_effects.rs etc.), 31 (10%) confirmed reachable (5 at candidate grade), 4 false positives caught twice, ~276 uninvestigated past the 9-file scan; 5th instrument-failure lesson recorded (normalised identifiers across files defeat single-file display-name scans); pilot_compute.rs/pilot_compute_corpus.rs twin (§29.1) checked via live test (not a grep), extended to 12 of 31, all 12 surface exact expected values through the twin the sheet reads; Power Attack/Dodge confirmed no-change (separate posture finding, not twin-divergence); 4 (Adaptive Fortune/Fortunate One/Great Hatred/Extra Hex) traced to a different real consumer entirely (race_trait_picker.rs/description_completion.rs); 13 twin-unverified; live-probe-across-full-307 follow-on named, not undertaken; equipment-classifier map fixed structurally (Decision 36's pattern, 4-of-8-books gap in `equipment_keys` vs `equipment_resolver.rs`'s real 8-book chain, ~1,650 already-landed UE/UI/PU/ARG units recovered, hand-verified promoted+residue samples per book, UPsi not-started test regression diagnosed to its real cause and strengthened not relaxed); race_trait 7th instrument failure found while scoping UPsi slice: classify()'s Kind::RaceTrait matches by trait name only, unscoped to book/race -- 6 of 45 program-wide `grounded` race_trait units are name-coincidence false positives (UPsi x4, Bestiary x1, ARG x1-ambiguous); no book but CRB has a real race-trait ingest path at all, redirected to UM equipment (26, existing table pattern) per team-lead; no unit's status changed; `file_kind()` row-content classification fixed at the source (deferred 3x, `§53`/`§55`/`§56`) -- new `Kind::MonsterAbility`, `internal_namespace` widened (+5,028 hits program-wide), `race_trait` 8,600->3,456 program-wide (+3,107 monster_ability, 19 excluded ClassLevelAdjustment, remainder internal_namespace), both acid tests confirmed landing not vanishing (Bestiary1 523, Core Essentials 380 monster_ability, self-corrected from an initial 1,155 overestimate), framed as a correction not progress -- see `decisions.md §61` | `epic-13-calibration` | epic-24-ultimate-intrigue | 2026-08-09T00:00:00-04:00 | SD28-E15-009 |
...
| 24 | `epic-25-ue-complete` | IN-FLIGHT | Ultimate Equipment | slice 1 (1,549-record equipment catalog, 55 real cross-book collisions excluded) landed and verified; re-derived real new content is 1,549 of 1,615 declared, not the whole book — see `decisions.md §44`. Note: `work-inventory.json` read this book as 0% proven for one cycle due to a `v06_work_inventory.rs` classifier defect (never inserted UE's real table into its own map, unrelated to this book's own correctness) — fixed structurally in `decisions.md §55`, now reads the true 1,549/1,614 | `epic-14-harness`, `epic-24-ui-complete` | epic-24-ultimate-intrigue | 2026-08-08T09:22:00-04:00 | SD28-E25-F1-001 |
...
| 27 | `epic-28-um-complete` | IN-FLIGHT | Ultimate Magic | slice 1 (144-record feat catalog, 3 auto-grant exclusions, 4 textless-but-real records kept via a new `effect` field) landed and verified; raw-syntax leak-join bug caught by two guards and fixed pre-commit, never shipped -- see `decisions.md §49`; `.MOD`/text-shape hazard triad now complete across three books; slice 2 (26-record equipment catalog: 24 General + 2 ArmsArmor, `_pfs` legality-overlay double-count ruled out before extraction, wired structurally into `equipment_resolver.rs`'s chain not a hand map) landed and verified, aggregate pin re-derived by counting (5,503); found and deliberately did NOT fix a third equipment-book chain (desktop picker) missing both UM and UE, recorded in `artifacts/e14-harness-widening.md` OPEN_FINDINGS — see `decisions.md §57` | `epic-14-harness`, `epic-24-ui-complete` | epic-24-ultimate-intrigue | 2026-08-08T17:20:00-04:00 | SD28-E28-F2-001 |
| 28 | `epic-29-upsi-complete` | IN-FLIGHT | Ultimate Psionics | slice 1 (221-record feat catalog, 1 source-disabled exclusion, 1 cross-book collision excluded) landed and verified; licence precheck complete -- Dreamscarred Press's ISOGL:YES/OGL.txt declaration checked structurally, no anomaly found; DESC:-is-complete book convention required zero textless-stub exclusions; last Ultimate book -- see `decisions.md §50`; slice 2 (439-record equipment catalog: 326 equipment + 113 equipmods) landed and verified, row-kind check clean, `.COPY=` legacy-alias hazard self-caught and corrected pre-commit (113-unit residue named not fabricated), added as §49's 4th text-shape hazard, UE/UI/ARG/CRB swept clean against the same hazard -- see `decisions.md §58` | `epic-14-harness`, `epic-24-ui-complete`, `epic-2-prelaunch` | epic-24-ultimate-intrigue | 2026-08-08T20:23:00-04:00 | SD28-E29-F2-001 |
...
| 32 | `epic-32-archetype-swap` | IN-FLIGHT | Archetype-swap mechanism | tier-1 data ingestion (piece 1) COMPLETE: all 7 in-scope books landed (UPsi 15 + ACG 87 + APG 80 + UM 67 + UC 65 + ARG 59 + UW 30 = 403 records), each with real corpus reconciliation, its own `.MOD`-injection-population share stated, and the shared `TYPE:`/`ABILITY:` no-convergence finding confirmed across all seven (33/33/52/27/22/14/30% -- no single number). Two populations sized and deliberately deferred: 4,550 tier-2 sub-feature records, 1,282 `.MOD`-injected-grant rows. `pilot_compute.rs` integration (pieces 2/3): operator unblocked task #67's boundary 2026-08-09; vacuity-comment audit backlog captured first (7 classes/13 citations, `decisions.md §59`); generic `archetype_claims_slot` primitive landed and proven end-to-end on Alchemist's Poison Resistance (ARG's Plague Bringer archetype, real reachability test through `build_pilot_headless_receipt`) -- see `decisions.md §60`. §59's 7-class audit backlog remains fully open, not touched by this slice. **Class-two measurement (2026-08-09/10):** Fighter's Bravery wired as a second, structurally different consumer (`49d07a5f`), zero changes to `archetype_resolver.rs` itself. Sizing question closed, not answered: four hand-verified classes (Fighter 1/22, Alchemist 3/26, Paladin 16/33, Bard 23/33) span 5%-70% wired-able ratio with no shared pattern -- the epic's true total cost cannot be estimated from any sample this size; sizing the remaining ~24 archetype-bearing classes requires the same per-class hand verification, not a formula. Seven `Reach::NotSurfaced` findings recorded (`reach_gate.rs` `OPEN_FINDINGS`): tier-1 catalog ingested and, for wired slots, engine-grounded, but zero reaches a player -- no archetype-selection surface exists in the desktop app. See `decisions.md §63` | `epic-14-harness` | epic-24-ultimate-intrigue | 2026-08-10T07:30:00-04:00 | SD28-C4.9-001 |
