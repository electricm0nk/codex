# SD-28 — Loop Instruction

> ⚠️ **OPERATING METHOD — REQUIRED FOR THIS BUNDLE**
> This file is the operational loop-instruction for SD-28. The bundle is operated via:
>
...
2. **Branch pushed:** `tranche/8` is pushed to origin (`git push -u origin tranche/8`). (Operator-pinned 2026-08-01.)
...
5. **Dreamscarred Press licensing pre-cycle verification:** the per-book trap-report run against `~/workspace/repos/pcgen/data/pathfinder/dreamscarred_press/ultimate_psionics/` confirmed open-content status per `decisions.md §17`. (Operator-pinned 2026-08-01.)
...
0. **Shape** the book. `cargo run --locked --bin v06_work_inventory`, then read
   ...
   cycle; do not assume a template. Skipped once per book, not once per cycle.
...
1b. **Re-derive.** Before accepting any figure carried in a brief, a doc, or a
   ...
   own data: `grep -c 'CATEGORY:FEAT' ~/workspace/repos/pcgen/data/pathfinder/paizo/roleplaying_game/ultimate_combat/uc_feats.lst`
...
- Stops and reports the blocker (per the repo's `AGENTS.md` hard-stop doctrine) when:
  ...
  - **A record family cannot be surfaced without work outside this bundle's epic structure** (Decision 10's open question — the equipment-catalog widening is the known instance). The cycle reports the gap; it does not add an epic and it does not ingest without a reach claim.
...
SD-28 references the following bundles:
...
- **SD-29 (planned):** Bestiary 2-3-4-5. No live cross-reference until SD-29 is launched.
- **SD-30 (planned):** Occult Adventures + companions. Class overlap (Occultist, Spiritualist, Medium, Mesmerist in Ultimate Intrigue) is canonical-to-SD-30; SD-28 references the canonical class id only.
