...
/// SD-36 F4pre measurement: the F3c4 scan (artifacts/epic-f/scripts/f3c4_pool_pass_scan.rs) plus one SEL row per
/// selection (values, linked levels, values still printed). Kept copy: artifacts/epic-f/scripts/f4pre_pool_pass_scan.rs. Appended
/// temporarily to `src/rules_core/pilot_compute/pool_groups.rs` and run with
/// `cargo test --locked -j 8 --lib -- --test-threads=8 --nocapture f4pre_pool_pass_scan`, then removed.
    ...
    fn f4pre_pool_pass_scan() {
        ...
        for (class_id, class, reg, ns, choice) in pools {
            ...
            for (group, sel_slug) in &selections {
                ...
                let (mut sel_values, mut sel_printed, mut sel_linked_levels) = (0usize, 0usize, 0usize);
                for level in 1..=20u8 {
                ...
                println!("SEL|{class} {reg}|{group}|{ns}{sel_slug}|values {sel_values}|linked levels {sel_linked_levels}|still printed {sel_printed}");
            }
            println!(
