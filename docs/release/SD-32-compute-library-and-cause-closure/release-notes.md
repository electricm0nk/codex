...
canonical: true
owner: god-emporer
status: populated at closure (2026-08-22)
date: 2026-08-22
---
...
## Deferred findings, each with a named owner

- **Epic 2's remaining blocker shapes** — T2a (8,243 units), T2b (2,472), T9 (2,651), T4 (up to
  2,763, needs re-derivation), T12 (~3,000, overlaps T2a by 1,354-2,124 — cannot close
  independently), T7 (4, fix site identified), T8 (12, blocked on an operator ruling over a
  read-only SD-30 surface). **Owner: a successor SD-N bundle** (`forward-scope-register.md` C2.5,
  `progress.md` "## Open blockers").
- **Epic 3's 18 untabled base classes and the 28 in-books-without-ruleset population.** **Owner:**
  unassigned pending Epic 3's own further measurement (`forward-scope-register.md` C1.1).
- **The class_feature kind-unenumerable finding** — AT-32-G0-002's ten-kind list omits
  `class_feature` (18,231 rows) and 26 other real content buckets (27,847 units total,
  none force-mapped). **Owner:** needs an operator ruling; proposed target a future gate-closure
  or successor bundle (`progress.md` `## DISCOVERED`, Cycle 2).
