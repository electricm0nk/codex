...
| C2.5 | Epic 2 (card 11, `epic-2-cause-closure`)'s remaining blocker shapes, filed as `returned-to-backlog` at the closure epilogue's final-acceptance scan (`kanban.md` card 11, `progress.md` "## Open blockers"): **T2a** (8,243 units, 2,360 cleanly remappable / 5,883 needing per-value re-examination, 1,354–2,124 overlapping T12 — cannot close independently of T12), **T2b** (2,472 units, race-trait compound-key matcher), **T9** (2,651 units, per-record book onboarding backlog), **T4** (up to 2,763 units, needs re-derivation — a prior wave's own 471-unit claim was found false), **T12** (see T2a overlap), **T7** (4 units, fix site identified at `class_feature_grant_consumer.rs:374`, not implemented), **T8** (12 units, additionally blocked on an operator ruling — its fix site `scripts/observer/pf1e_dashboard_producer.py` is SD-30's Epic 0 surface, read-only from SD-32 per `technical-design.md`). Full investigation and per-shape reasoning: `artifacts/gate-3-closure-invariant/epic-2-cause-closure_cycle-1_cycle_receipt.md`. | A future SD-N. T8 additionally needs an operator ruling on write-scope before any cycle can touch it; T2a and T12 need one combined cycle (they cannot be closed independently without double/under-counting the overlap). |
