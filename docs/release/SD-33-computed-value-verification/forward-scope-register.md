...
## E1.x — Branches ruled OUT of the operator's 2026-08-26 fold, not to be re-litigated

The same stale-branch sweep that found the 45 recovered Skinwalker records and the 103 recovered
Undine fixture entries (folded into SD-33 via `fold-skinwalker`/`fold-undine`, see `release-notes.md`
"Recovered work") also found three branches that were **not** folded. Each was read and ruled out
for a specific, checked reason — recorded here so a future sweep does not re-discover them as a
surprise and re-spend the diagnosis. **These are the operator's branches to delete; SD-34 must not
re-litigate them.**

| ID | Branch | Ruling | Reason |
|---|---|---|---|
| E1.1 | `worktree-wf_a45ece26-3fc-1` | **Superseded** | 1,612 grant files — the largest file count of any branch swept, and the one most likely to be mistaken for the biggest win available. Its `class` field holds feature-group names (`"Deed"`, `"Bloodrager Bloodline"`, `"Arcanist Exploit"`) where HEAD's 240 curated records hold real class names (`"Fighter"`), and it lacks `granted_via_archetype` entirely — a field `class_feature_grant_consumer.rs` treats as authoritative and defaults to `true` when absent. Folding it would not have added coverage; it would have **silently mis-marked every one of the 1,612 records** as archetype-granted. |
| E1.2 | `worktree-wf_13156488-c9b-1` | **Superseded** | Wave 20 work, superseded in approach by SD-32's own `untabled_base_class_feature_roster`, which already covers the same ground on a different, later mechanism. |
| E1.3 | `worktree-wf_c1156061-e3f-5` | **Superseded** | Wave 30 — 27 lines of notes on a bundle that is already closed. No corpus records, no code. |

**Do not re-fold any of the three.** A branch's file count is not its value — see the retrospective's
fold section for the general lesson this register entry is one instance of.
