export const meta = {
  name: 'sd-33-closure-epilogue',
  description: 'SD-33 closure epilogue — AT-33-E6-001 passed; write the retrospective, sweep, open the PR, release notes',
  whenToUse: 'After AT-33-E6-001 attempt 10 returned gate PASS with zero shortfalls. Runs workflow-instruction.md 11 steps 2-5.',
  phases: [
...
// ---------------------------------------------------------------------------
// Dispatch — AT-33-E6-001 attempt 10 returned gate PASS, shortfalls []. Run
// workflow-instruction.md §11 steps 2-5, in order. Order is load-bearing:
// retrospective and sweep land BEFORE the PR opens.
// ---------------------------------------------------------------------------

const SCAN_PASSED = {
  criterion: 'AT-33-E6-001', attempt: 10, gate: 'PASS', status: 'complete', shortfalls: [],
  sweep: { findings: 0, exit: 0, records_examined: 48634, tokens_compared: 412734 },
  sweep_green_honestly: true, illegitimate_route_found: 'none',
  hand_derivation_checked: true, sweep_still_fails_on_mismatch: true,
  hand_derivation_conclusion: 'sweep-wrong on both defects, confirmed from the pinned oracle .lst bytes at 7f818006e371188e5717fd18d74d18a420747fc6',
  fix_commit: '1bfb80d7b7 (2 sweep .rs files, 3 package docs, 1 retro jsonl; 0 data/corpus files touched)',
  epic5_consequence_followed: true, epic5_units_affected: 1, epic5_rows_moved: 0,
  epic5_note: 'scan CORRECTED the lane\'s "0 affected" membership claim: 1 of 10 records (inner_sea_gods:equipment:fugitive_finder) is in the 8,330 population, verdict unverifiable; 9 of 10 are NOROW. 0 of 8,330 rows could move. Conclusion unchanged, correction event filed.',
  preexisting_failing_suites: { count: 31, failures: 49, grew: false, of_executed: 8026, proof: '0 of 31 failing targets carry a commit since f652db7ac7' },
  lib_suite: { passed: 2837, failed: 0 }, no_run_exit: 0, targets: '543 of 543',
  desktop_tauri_suite: { passed: 548, failed: 0 },
  row_counts: { fixture: 1741, literal: 6589, combined: 8330 },
  unexamined_set_empty: true, oracle_disagreement: 0,
  verdicts: { agree: 811, unverifiable: 7519, disagree: 0, reasonless_unverifiable: 0, duplicates: 0 },
  open_blockers_empty: true, deferrals_open: 3,
  denominator_gate: 'PASS (0 violations of 60 files)',
  kanban: 'rows 1-18 all complete (18 of 18); 27 of 27 receipt paths exist',
  verify_sh_note: 'site-dashboard-check HUNG and was killed after ~12 min — publish-site-dashboard.sh --check invokes v06_work_inventory --summary with no timeout wrapper. Filed as an INCIDENT retro event, not an environmental note. Every load-bearing stage was run directly and passed.',
}

log('SD-33 CLOSURE EPILOGUE. AT-33-E6-001 attempt 10 returned gate PASS with zero shortfalls. Running §11 steps 2-5. (Wave 9 halted on an orchestrator-side bug: the failure regex matched the WORDS "blocked-escalated" inside a sentence stating rows are NOT blocked-escalated. The scan itself passed.)')

phase('Epic 6 — Closure epilogue')

const retroSweep = await agent(retrospectiveAndSweepPrompt(SCAN_PASSED), { model: 'sonnet', label: 'retrospective-and-sweep', phase: 'Epic 6 — Closure epilogue' })
...
return {
  bundle: 'SD-33', branch: BRANCH, closed: true,
  scan: SCAN_PASSED,
  epic6: { retroSweep, archPr, notes },
}
