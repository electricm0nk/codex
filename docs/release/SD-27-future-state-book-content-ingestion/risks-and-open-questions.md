...
| Q | Question | Default |
|---|---|---|
| Q1 | E3 per-book ordering for the 2 in-scope future-state books | Alphabetical by book name: advanced_race_guide (E3.1) → adventurers_guide (E3.2) |
...
| Q4 | Adventurer's Guide (AG) per-field completion ceiling — SD-25/SD-26's corpus-intake pass covered CRB/APG/ACG/Bestiary-1 but not AG | Unknown; E3.2 (AG parity baseline) should re-verify AG's real ceiling against the corpus directly (same method as SD-26 `decisions.md §11.4`'s table) before assuming any number |
...
| Q7 | Which per-book content kinds (classes, spells, equipment, feats, races, bestiary, archetypes, traits, domains) does each of the 2 in-scope future-state books (ARG, AG) actually have? | Templated; per-book inventory step in E2.x cycle 1 reads `~/workspace/repos/pcgen/data/pathfinder/paizo/roleplaying_game/<book>/` and records `content_kind_counts`. A book with no archetype LST does not get an `archetypes/` directory; the absence is honest, not a missed cycle |
