...
SD-27 ships the resolution of **2 future-state book stubs** (Advanced Race Guide, Adventurer's Guide) that SD-26 registered in Epic 4. **The 17 remaining future-state books** (Bestiary 2-6, Bonus Bestiary, Horror Adventures, Monster Codex, Mythic Adventures, Occult Adventures, Pathfinder Unchained, the 6 Tier-2 Ultimate books) **are deferred to a follow-up SD (SD-28 or later)**, operator-gated on SD-27's first two books closing cleanly. Beginner Box and Core Essentials were removed from scope per operator directive 2026-07-27 (redundant to other tomes; will not be brought in). This is the "tune, then go wide" model: 2 books prove the per-book pre-build + verification + parity cycle pattern; the next bundle (operator-pinned) goes wide.
...
### 1.2 Epic 2 — Book Pre-Build + Verification (2 future-state books: ARG + AG)
...
- **Tier-1 (2 books, serial, operator-gated):** advanced_race_guide (E2.1), adventurers_guide (E2.2). Per-book pre-build is a cycle; per-book verification is a cycle; per-book parity is a cycle. The 3 cycles per book (pre-build + verify + parity) are 6 total cycles for E2.
...
The 2 books in this bundle run sequentially (E2.1 → E2.2). The 2 books' pre-build cycles are file-disjoint (`data/corpus/advanced_race_guide/` vs. `data/corpus/adventurers_guide/`) and could run in parallel, but the operator's "tune, then go wide" model means the first book closes (or fails) before the second book dispatches. **Cycle failure on E2.1 (ARG) → operator reviews the per-cycle receipt → operator picks the next action (re-tune, defer, or proceed to E2.2).**
...
| Stage | In-scope book (CRB, APG, ACG, B1) | Future-state book (ARG, AG) |
|-------|-------------------------------------|------------------------------|
...
| **Stage 4: Parity** | Cycle 2.1'' / 2.2'' (PCGen parity baseline) | Cycle 2.1'' / 2.2'' (PCGen parity baseline) |
...
- **Criterion 2.1'** — Advanced Race Guide **Stage 3: Verify**. One cycle. Reads the pre-built cache at `data/corpus/advanced_race_guide/`, runs the dual-audit gate, hands the operator a verification report. **Much shorter than a creation cycle** because the records already exist. Gates 2.1''.
- **Criterion 2.1''** — Advanced Race Guide **Stage 4: Parity baseline**. One cycle. Runs the SD-26 PCGen pipeline against a hand-authored `pf_advanced_race_guide_human_<class>_level1_golden.pcg` fixture. Comparator runs against the pre-built cache. Gates 2.2.
- **Criterion 2.2** — Adventurer's Guide Stages 1+2: License prep + pre-build. Same shape as 2.1 but for `data/corpus/adventurers_guide/`. Operator-gated. Gates 2.2'.
- **Criterion 2.2'** — Adventurer's Guide Stage 3: Verify. Same shape as 2.1'.
- **Criterion 2.2''** — Adventurer's Guide Stage 4: Parity baseline. Same shape as 2.1''.
  ...
  - Updates the `book_stub` registry entry's `content_kind_counts` field with real numbers and `status: "Resolved"` + `resolved_at: <ISO-8601>` + `resolved_by: claude-code` + `bundle_of_record: SD-27` + `cycle_receipt: artifacts/epic_2/<book>_cache-cycle_receipt.md`.
...
Two cycles, one per book (ARG + AG). Each cycle:
...
|---|------|------|--------|--------|-------|
| 1 | advanced_race_guide | T1 | **SD-27** | pre-build + verify + parity | Per-book pre-build is a cycle (E2.1); verification is a cycle (E2.1'); parity is a cycle (E2.1''). 3 cycles for ARG. |
| 2 | adventurers_guide | T1 | **SD-27** | pre-build + verify + parity | Same shape as ARG. 3 cycles for AG. |
...
| 13 | pathfinder_unchained | T1 | SD-28+ | deferred | |
...
**17 deferred future-state books** (rows 3-19) are operator-gated on SD-27 closing cleanly. The operator picks the next batch for SD-28 after E4.5 closes. Tier-2 (Ultimate-line) books are operator-pinned for a separate bundle because their denser equipment/feat content is a different cycle-shape problem (more records per book, longer per-book cycles).

**Note (2026-07-27):** Beginner Box and Core Essentials were removed from the original 21-book plan. They are redundant to other tomes and will not be brought in. Their stubs at `data/stubs/beginner_box.json` and `data/stubs/core_essentials.json` (if they exist on disk) are out-of-scope and may be deleted by the closure epilogue if the operator authorizes a stub-cleanup pass.

## 4. File-touch partition

SD-27 cycles may write to:

- `data/corpus/<book>/` for the 2 future-state books in this bundle (advanced_race_guide, adventurers_guide) + the 4 in-scope books (CRB, APG, ACG, Bestiary 1) during the 2.0.6-2.0.9 retro-fit cycles.
- `docs/governance/wired-integration-stubs-registry.md` (serial, one cycle at a time).
- `data/stubs/<book>.json` (serial, one cycle at a time; only ARG + AG stubs are updated to `content_kind_counts: <real>` during SD-27; the 17 deferred books' stubs stay at `null`).
...
- `data/corpus/<book>/` for the 4 in-scope books — v0.6's lane.
- `docs/release/v0.6/` — v0.6's lane.
...
- Bundle-level: `wired-integration-discipline` four-check audit clean against the bundle's combined diff.
- Bundle-level: `architecture-truth-up` script clean against the bundle's combined diff.
