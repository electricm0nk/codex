...
- **Closure increment (CORRECTED 2026-07-21):** ~~On `tranche/5-1 → develop` promotion (Epic 7), tranche-base advances to 6 and build resets to 0. Resulting version after SD-23 closure: `0.6.0`.~~ **Wrong — this repeats the exact SD-22 tranche-version-bump mistake** (see `[[sd22-tranche-version-bump-correction]]`): the tranche-base digit only advances when a *new* `tranche/N` branch is cut for the next bundle. `tranche/5-1` is a dash-release *within* tranche 5, not a new tranche cut — SD-23's own closure does not create `tranche/6`. On `tranche/5-1 → develop` promotion, **only the build position advances**: `0.5.96 → 0.5.97`. Tranche-base stays `5` until an actual `tranche/6` branch is cut for a future bundle. Caught and corrected by the operator on 2026-07-21, before Criterion 30 executed (PR #327 was open, CI still running, no version file touched yet) — this error was baked into the bundle's own planning docs (this section, plus `epic-breakdown.md`'s Epic Closure sub-step 7 and Criterion 30, and `acceptance-and-verification.md` gate 15) before execution began, not introduced during a cycle.
- **Captured 2026-07-20:** `0.5.96`, from `apps/desktop/src-tauri/Cargo.toml:3` at `origin/develop` HEAD `f36c211`. Root `Cargo.toml` has no `[workspace]` section (standalone `0.1.0` package) — "develop's Cargo.toml workspace version" resolves to the desktop app's Cargo.toml, which is where SD-22 also stamped `0.5.96` at closure. Applies to Criterion 6 (progress.md capture, done) and is the base Criterion 30 increments from at closure — to `0.5.97`, per the correction above, not `0.6.0`.
