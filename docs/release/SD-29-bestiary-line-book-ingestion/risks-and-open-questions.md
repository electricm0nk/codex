...
| R-29-002 | Bestiary 1 monster-surface prerequisite blocks Epic 7 | DM Toolkit extension records gap; cycle records `decision-blocked` and moves to next ready card | `successor-forward-scope-register.md C3.1` records the toolkit extension as operator-on-request retrofit; cycles land even if Epic 7 surfaces |
...
**Owner:** operator (per-cycle at Epics 5 and 6 closure; Epic 7's gating).
