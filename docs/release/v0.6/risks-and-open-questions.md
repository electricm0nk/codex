...
2. **0.6 version bump precedent.** `buildVersionTriple.test.ts:44-47` memorializes a prior erroneous 0.6 bump that was reverted and hard-asserts `0.5.`. The bump is backend's fixed first task; until it lands, any CI run asserting the triple will fail on `tranche/6`.
