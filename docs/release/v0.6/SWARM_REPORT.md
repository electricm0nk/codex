...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | Closed task #10 (Oracle deepening) then task #19 (Category B feat-effects) same session, then self-caught and fixed a real gap in #19 after it had already shipped. **Task #19 (`042e1c32` then `980658f2`)**: Extra Rage (+6 rounds), Extra Performance (+6 rounds), Extra Ki (+2 points), Extra Lay On Hands (+2 uses) all wired. First landed the display-vs-activation-budget fix (Rage/Bardic Performance each read their per-day budget in 3 places; folded the bonus inside the shared formula so all 3 sites agree by construction — lead independently reproduced the bug this prevents by reverting the budget-check sites in an isolated worktree, both `*_widens_the_activation_budget_not_merely_the_displayed_total` tests failed as expected). **Then found its own miss**: all four feats are `STACK:YES MULT:YES` in the corpus — genuinely repeatable, each instance stacking — but the original `extra_resource_feat_bonus` tested presence via `.any()`, so a second Extra Rage granted +6 instead of +12. Root cause named honestly: the corpus record was read through a `grep -E "^(BONUS|PRE)"` filter chosen around the fields expected to matter, which silently dropped the `STACK:`/`MULT:` lines — verifying the magnitude is not the same as verifying the whole record, and the same class of field (`!PREABILITY:`) already bit once on Oracle's Cinder Dance/Lame conflict. Fixed to count occurrences and multiply (saturating). Also retroactively proved (not just asserted) the no-class-ownership-gate claim with two tests: a Fighter holding all four feats gains none of the four resource records, and neither a level-1 Monk nor a level-1 Paladin gains a resource below their own class's level gate. Lead independently verified `980658f2` in a fresh isolated worktree: 653/653 lib, both SD-24 audits 3/3, 10/10 Category B tests, clippy unchanged (23), confirmed both `STACK:YES MULT:YES` directly against the real `cr_feats.lst` record and the fix's own test by reverting to the old `.any()` logic (failed exactly as described: 6 vs required 12). Task #19 genuinely done, not just done-looking. Idle-time audit of all 16 already-shipped feats for the same STACK/MULT blind spot found one real, low-severity, currently-unreachable issue (Skill Focus, `STACK:NO MULT:YES`, duplicate-id facts on repeated same-skill picks) — lead-verified directly against the corpus and reproduced empirically, filed as task #21 for featmate's lane. **Task #1's first half landed (`73c70055`)** — level-aware spellcasting diagnostic (no longer claim-blocks below level 4, where Bloodrager genuinely has none per RAW), the false "no class-skill list" claim corrected with the real 11-skill list grounded (7th widening instance), and the levels 4-20 CAST/KNOWN table built straight off the corpus tokens. Self-corrected the lead's own "1→3" guidance to 1→2 (Bloodrage + Spells) — `Class Skills` is `CATEGORY:Internal`, same as Warpriest's identical record which Warpriest's own count already excludes; lead confirmed against the corpus. Lead independently verified: 670/670 lib, 13/13 Bloodrager tests, ACG audit 3/3, clippy unchanged. Still queued: the 110-of-183 spell list (ruled acceptable three times now — go), then Cavalier (#6), Summoner (#17), and #20 (wire featmate's new `effective_feats` leaf in) |
...
| scout | in_progress | Landed Warpriest's re-scope (greenlit, task #9, closed), Oracle's re-scope (greenlit, task #10, closed), and Bloodrager's re-scope (`bloodrager-spellcasting-scoping.md`, greenlit, task #1, now with backend) — caught a real shipped bug (spellcasting diagnostic claim-blocks levels 1-3 despite Bloodrager having no CAST/KNOWN rows there, lead-verified directly against `acg_classes.lst`'s `PRECLASS:1,Bloodrager=4` gate) plus a false class-skill claim (7th instance of the recurring widening bug) and corrected a "201 unique spells" scoping-tag error to the real, re-derived 183 (110 reachable in this repo's ingested corpus). **Cavalier scoped and greenlit** (`cavalier-remaining-features-scoping.md`, task #6, queued for backend behind Bloodrager) — the "26-entry chooser" deferral was a cross-book count, real APG total is 6; caught a real decoy-variable hazard the lead found on review and folded the correction into the doc itself. **Summoner Eidolon MVP scoped and greenlit** (`summoner-eidolon-mvp-scoping.md`, task #17, queued behind Cavalier) — found the Hunter-Wolf reuse framing doesn't hold at the function level (`wolf_companion_hit_dice`'s `debug_assert_eq!(companion_level, 1)` hard-blocks it, lead-verified), a genuine correction to the lead's own directive; canonical Quadruped form, chassis+pool MVP, every formula independently re-verified including a hand-computed 20-level pool table. **Now scoping task #5 (Brawler)** — expected thinner yield than the last three since backend already deepened it once, taken over the two design-shaped candidates (Slayer/Swashbuckler) to keep the queue moving in the established corpus-verification groove |
| featmate | in_progress | Landed Category A of the CRB feat re-sweep (`19ca4ae6`, 573 lines, feat_effects 38→68 tests): Improved Initiative, Endurance, 12 combat-maneuver feats (6 Improved + 6 Greater — corrected from the scoping doc's 11 after deliberately including Improved Grapple, currently only reachable via Monk's synthetic path), Stunning Fist, and the movement trio. Every formula checked against both the corpus BONUS token and the authoritative BENEFIT prose. Lead re-verified: 68/68 feat_effects tests, 621/621 full lib. Producer side only — consumer wiring into pilot_compute.rs is backend's lane. **Task #21 closed (`144bf89f`)** — dedup Skill Focus facts by skill name, case-insensitive (the consumer lowercases for its explanation id, so case variants of one skill would otherwise double-ground under one id too). Tests written first, all 3 confirmed RED behaviorally before the fix. Lead independently re-verified in an isolated worktree: 71/71 feat_effects, 656/656 clean lib, and reproduced the exact pre-fix duplicate by reverting the dedup logic (both the same-skill and case-variant tests failed as expected). **Task #20's leaf half landed (`bb7d0c95`)**, held in_progress since the consumer wiring is backend's — new `effective_feats(selected_feats, class_granted_feats)` unions chosen and class-granted feats, verbatim passthrough preserving repeat counts (Fleet-style stacking) with dedup only on append. Found the defect was broader than scoped: Ranger's automatic level-3 Endurance grant (`RANGER_ENDURANCE_LEVEL`) was equally invisible to every feat-presence check, not just Monk's Stunning Fist — caught while investigating, not in the original task. Lead independently verified: 7/7 new tests, 663/663 clean lib, and reproduced the exact bug a naive whole-list dedup would reintroduce (Fleet×2 collapsing to Fleet×1) by perturbing the fix. One open subtlety flagged for backend's wiring pass: Stunning Fist is corpus-encoded `ABILITY:FEAT|VIRTUAL`, not `AUTOMATIC` like Improved Unarmed Strike/Endurance — likely inconsequential here but worth a quick check before treating all three identically. **Weapon Focus id-space investigated**: `selected_feats` carries only a synthetic `feat:weapon_focus`, never the catalog key, and the target is a compound `feat:weapon_focus:weapon:<name>` selection_id inside `choice:fighter_bonus_feat` — a producer keyed only on the catalog key + a clean new choice set would ground nothing for the real shipped Fighter fixture, same miss-shape as Ranger's Endurance. Lead ruled: union both sources, dedup by weapon, don't touch the existing longsword-only gate. **Starting Spell Focus now** (clean Mechanism B build, no legacy id to reconcile) as the fast win, Weapon Focus next |
