...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Slayer's full build committed (`77910256`, 7th ACG/APG class-specific closure, first non-caster full build) — lead re-verified against the real committed HEAD: all 6 Slayer tests, coverage audit 3/3 ("seven" closures), full lib 457/457. Standing by |
...
**Progress: Fighter, Wizard, Rogue, Ranger, Paladin, Barbarian, Sorcerer, Cleric, Druid, Bard genuinely reach Computed via the compute engine (10 of 27) — all 10 now reachable through the real product UI, fully closed out (Sorcerer/Cleric/Druid's choice-picker gap closed via `9bafe303`'s canonical-default seeding, `characterHubModel.ts` updated and live-verified in `adf57cfb` with real per-class `levelOptions`: Sorcerer `[1,2]`, Cleric `[1,2,3]`, Druid `[1]`). Monk has 6 of 7 restricted-list feats closed (partial, only Deflect Arrows remains). Skald, Bloodrager, Brawler, and Hunter (all ACG) plus Cavalier, Alchemist, and Inquisitor (all APG) each have a real named ability genuinely grounded but stay Blocked on other deferred features (partial). **Arcanist (ACG, `ce73a598`) and Warpriest (ACG, `03bb35d3`) are the first two classes this segment to attempt genuine FULL builds outside CRB** — real prepared spellcasting with independently-verified own tables, plus a class-specific resource/feature (Arcane Reservoir for Arcanist; Blessings/Sacred Weapon for Warpriest), all real; Arcanist stays Blocked on Exploits alone even with a valid spellbook (a spell-seed follow-on alone won't reach `Computed` here, since `exploits_deferred` is unconditionally claim-blocking — backend attempted the seed, caught this before committing, correctly reverted); Warpriest stays Blocked on its own other-features diagnostic alone. Warpriest's closure also caught and fixed a real class-skill-bonus widening bug (Climb/Intimidate/Swim bonus was silently not applying for a class whose real corpus list includes all three). **Slayer (ACG, `77910256`) is the third full build and the first with zero spellcasting scope** — a non-caster whose four remaining sub-features (Sneak Attack dice, Trap Sense, Trapfinding, Track) are all real, independently-verified flat formulas, honestly counted as 4 distinct wired features (not folded down, since none share an underlying mechanism the way Cantrips/Orisons did for Arcanist/Warpriest) and grounded as standalone records with no total-integration, the same already-precedented idiom Barbarian's and Rogue's own Trap Sense already use; also caught a third instance of the class-skill-bonus widening bug (Climb/Intimidate/Swim). Stays Blocked on its own other-features diagnostic alone (Studied Target, opponent-dependent; Slayer Talents, a chooser-list). 10 classes total across both books have real partial engine progress. 7 classes remain fully untouched beyond BAB/save/HP dispatch. See the detailed table below for the full done/in-progress/queued breakdown by class.**
...
### ACG (10 classes) — 7 of 10 have real partial engine progress, 3 still dispatch-only
...
**ACG tally: 0 of 10 reach Computed (a real named-feature or spellcasting gap keeps every ACG class Blocked regardless of chassis work), 7 of 10 (Skald, Bloodrager, Brawler, Hunter, Arcanist, Warpriest, Slayer) have real engine progress genuinely grounded — all 7 stay Blocked on their own permanent remaining-feature gaps, Arcanist's own `exploits_deferred` diagnostic confirmed unconditionally claim-blocking (a Wizard-style spell-seed alone can't clear it, unlike Wizard's own genuinely-conditional remaining diagnostic) — 3 of 10 untouched beyond chassis dispatch.**
...
4. Remaining untouched ACG/APG classes' (Investigator/Shaman/Swashbuckler/Oracle/Summoner/Witch — 6 total) class-skill-lists/class-features/spellcasting — the single largest remaining bucket in this whole epic.
