...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | Closed tasks #10, #19, and now **#1 (Bloodrager, fully closed, both `73c70055` and `f24c6f24`)** — level-aware spellcasting diagnostic, false class-skill claim corrected (7th widening instance), levels 4-20 CAST/KNOWN table, and the full 183-entry spell list (110 corpus-reachable, self-audited two real measurement-tool bugs — a directory-listing miss that undercounted to 28, a fuzzy-match false positive that overcounted to 111 — before reporting). `named_features_wired` 1→2, backend's own correction to the lead's "1→3" guidance, confirmed right (`Class Skills` is `CATEGORY:Internal`, same exclusion Warpriest's count already established). Lead independently verified every commit this cycle: 686/686 lib (current), all Bloodrager/spell-list tests, ACG audit 3/3, clippy unchanged. **Task #20 closed (`6d0d859d`)** — `class_granted_feats`/`effective_character_feats` route Monk's 1st-level Improved Unarmed Strike + Stunning Fist and Ranger's 3rd-level Endurance into all 4 real consumer producers. **Honest correction to how this had been described**: of ~13 `feat_effects.rs` producers, only 4 have any real consumer in `pilot_compute.rs` (lead independently re-derived the same 4 by grepping every call site), so this wiring is a provable no-op on today's behavior, not a live-wrong-answer fix as earlier framed — still worth landing to close the latent trap and future-proof new producers. The other 8 orphan producers (including Weapon/Spell Focus, initiative, CMB) are real, larger, separate scope, filed as task #22. Lead verified: 705/705 lib, both audits 3/3. **Now on Cavalier (#6)**, then Summoner (#17), then Swashbuckler (#14) |
