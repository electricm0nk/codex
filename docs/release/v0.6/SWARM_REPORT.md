...
| scout | in_progress | Landed Warpriest's re-scope (greenlit, task #9, closed) and Oracle's re-scope (`oracle-remaining-features-scoping.md`, greenlit, task #10, closed). **Operator lifted the new-class-work pause 2026-07-27** — now scoping task #1 (Bloodrager's remaining spellcasting blocker): whether Bloodrager's spell list is a reuse candidate off an existing built list (`SPELLLIST:2|X|Bloodrager`-style token, the way Oracle reused Cleric's) or needs its own independently-verified per-level table the way Oracle built one mirroring Sorcerer's shape, plus any bloodline-specific complications (bonus spells, bloodline-gated caster level) |
