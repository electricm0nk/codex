...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | **#11 (Witch spellcasting) stopped before building — a genuine third caster shape found, dispatched to scout for scoping (risks item 57).** Backend went to mirror Shaman's own stage 2 and found Witch's real posture token is `SPELLSTAT:INT KNOWNSPELLS:LEVEL=0` (cantrips-only automatic knowledge, no full 0-9 ladder like Cleric/Druid/Shaman, no `SPELLBOOK:YES` like Wizard) — a genuinely third shape matching RAW's "spells stored in and prepared from the familiar," not a variant of anything already built. Correctly recognized that building it Shaman-shaped would ship a Cleric-with-INT that passes every test and is wrong about the class's own defining mechanic — stopped rather than ship it, exactly the no-stub-doctrine failure mode named plainly instead of shipped quietly. Also found a real factual error in shipped code (`spellcasting_class.rs:22-26` wrongly claims Witch shares Cleric/Druid's shape) and correctly stayed out of the docs lane rather than write the scoping doc itself. Lead independently re-verified both claims directly against `apg_classes.lst` before dispatching to scout — confirmed exactly. What's already reusable: the 326-list (task #33) and Witch's `CAST` table, byte-identical to Cleric's through all 20 levels including L11 (found while investigating Shaman's own L11 divergence). Dispatched the one-line `spellcasting_class.rs` comment correction, holding for scout's scoping doc before touching Witch spellcasting again. **Starting #11 (Witch: consume the already-shipped 326-spell list — no spellcasting posture exists yet)**, lead-confirmed the gap is real (`witch_spell_list::` has zero references anywhere in `pilot_compute.rs`). Same shape Shaman's own stage 2 just closed for itself, so the build pattern is now proven. **#12 (Shaman) fully closed 2026-07-27, all three stages** (`47bd518e` list, `27e9814c` spellcasting, `917780b4` spirits) — full detail in Shaman's own per-class row below. Headline: own 304-spell list; prepared-caster spellcasting live (a genuine single-cell L11 corpus divergence ruled by the lead, risks item 56, to keep the corpus value over the peer row); all 9 remaining spirits grounded (a real PRE-gate-on-the-grant-not-the-record near-miss caught before shipping, a genuine decoy correctly kept separate — Battle Spirit's `rounds_per_day` vs. the other eight spirits' `uses_per_day`, both sharing `3+CHA`); `named_features_wired` genuinely 2 (a documentation-only overclaim from the Familiar closure back on `8e47479a` is corrected in the per-class row too). Lead independently re-verified all three stages against the real committed HEADs: root lib 793/793 → 800/800 → **809/809**, all 31 Shaman-related tests, both SD-24 audits 3/3 throughout, clippy baseline. Idle, holding for next assignment. **Pre-2026-07-27 summary (compressed; full detail in prior commits' diffs)**: closed #1/#4/#5/#6/#8/#9/#10/#14/#17/#19/#22 and Tier-0 deepenings across Slayer/Investigator/Cavalier, all lead-verified — real findings included a genuine PCGen corpus authoring bug (Swashbuckler Deeds, item 50) and a cross-workspace verification gap (`apps/desktop/src-tauri`, a separate Cargo project the root sweep never covered). **2026-07-27, the full spell-list undercount wave (tasks #23-33, CLOSED)**: two independent parsing bugs (a `CLASSES:` substring-match bug and a bracket-prereq-throws bug) fixed across all nine spell lists — Ranger 114, Paladin 95, Bard 264, Druid 271, Sorcerer 578, Cleric 301, Alchemist 147, Witch 326, Bloodrager 200 — every number lead-independently re-verified against the real committed HEAD, root lib 774→785, no "Core Rulebook `<class>` spell list" string remains anywhere (items 53-54 have the full detail) |
