...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Investigator's full build landed (`46eb6c3d`, 10th ACG/APG class-specific closure) — Trapfinding/Trap Sense/Inspiration pool-size grounded (swapped-floors hazard correctly avoided, verified independently), plus a genuinely new architectural change: `selected_skill_class_skill_bonus_applies` split into three independent per-skill functions (Climb/Intimidate/Swim), Investigator being the roster's first 2-of-3 partial match. Caught and fixed a real regression in an existing QA-owned test before shipping (stale assertion text from the old shared-scalar phrasing). Lead re-verified against the real committed HEAD: all 6 Investigator tests, the 9-test skill-bonus regression suite (zero change across Fighter/Rogue/Warpriest/Slayer/Swashbuckler/Wizard/Arcanist), coverage audit 3/3 ("nine" closures), full lib 488/488, desktop 215/215 (unchanged), full workspace sweep in flight. Scout also found a real shipped bug in Warpriest's Sacred Weapon dice-count formula during a systematic audit — escalated, not yet fixed. Standing by |
...
### APG (6 classes) — 4 of 6 have real partial engine progress, 2 still dispatch-only
...
**APG tally: 0 of 6 reach Computed (a real named-feature or spellcasting gap keeps every APG class Blocked regardless of chassis work), 4 of 6 (Cavalier, Alchemist, Inquisitor, Oracle) have a real named ability or full build genuinely grounded, 2 of 6 untouched beyond chassis dispatch.**

### ACG (10 classes) — 1 of 10 genuinely Computed, 8 of 10 have real partial engine progress, 1 still dispatch-only
...
**ACG tally: 1 of 10 (Arcanist) genuinely reaches `Computed` — MILESTONE, first non-CRB class this segment, via the Metamagic Knowledge follow-on closure (`54d8048b`) narrowing its last claim-blocking diagnostic; engine-Computed and product-reachable (Path A closed, `772298bc`+`25c5bcae`). 8 of 10 (Skald, Bloodrager, Brawler, Hunter, Warpriest, Slayer, Swashbuckler, Investigator) have real engine progress genuinely grounded but stay Blocked on their own permanent remaining-feature gaps — 1 of 10 (Shaman) untouched beyond chassis dispatch.**
...
4. Remaining untouched ACG/APG classes' (Shaman/Summoner/Witch — 3 total; Witch and Shaman already scoped and greenlit, Summoner held as genuinely subsystem-blocked) class-skill-lists/class-features/spellcasting — the single largest remaining bucket in this whole epic.
