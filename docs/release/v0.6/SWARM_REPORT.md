...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | Closed #10, #19, #1 (Bloodrager), #6 (Cavalier, `named_features_wired` 1→6, decoy-variable hazard confirmed and guarded), #14 (Swashbuckler, 3→12 after the lead's variant-vs-independent counting ruling, plus a genuine PCGen corpus authoring bug found and fixed — `risks-and-open-questions.md` item 50), and #17 (Summoner, bounded Eidolon MVP, `named_features_wired` 0→1, APG's own zero-untouched milestone) — all lead-verified in isolated worktrees. **#22 (orphan feat_effects producers) landed `2dee5a1e`, 7 of 8** — Toughness integrated into the real Fighter HP total (proven by differencing), the other 6 (Improved Initiative/Endurance/Fleet/Nimble Moves/12-feat combat-maneuver family/Stunning Fist/Spell Focus) grounded standalone; deliberately did NOT integrate Spell Focus into the spell-save-DC totals despite them existing (keyed by level, Spell Focus keyed by school — integrating would fabricate a mapping the records don't carry). This also gives #20's `effective_feats` wiring its first real effect (a Monk's granted Stunning Fist and Ranger's granted Endurance now reach live producers). **Lead cross-checked all 13 producers against real call sites and found the 8th (`weapon_focus_facts_from_choices`) still had zero** — reopened, then closed for real (`34b16f04`): wired, and the deterministic Fighter fixture (which already carries Weapon Focus (Longsword) via the legacy compound id) now grounds a record it never had before — a genuine behavior change, not latent capability. Root cause of the miss named honestly: the original "8 orphans" inventory was taken once and reused at verification time, but `feat_effects.rs` gained producers in between (featmate actively editing the same file) — "checking the members of a list isn't checking the list is complete." Lead re-verified by re-enumerating all 14 producers programmatically: every one now has ≥1 real call site. Lib **739/739**, both audits 3/3, clippy baseline. `named_features_wired` unaffected (consumer wiring, not new grounding). **Brawler (#5) closed (`6ce9d1f1`)** — all six remaining features grounded, `named_features_wired` 3→9, both flagged hazards (ManeuverTraining's two-line stacking gate, Knockout's modifier-not-score idiom) confirmed real and correctly guarded. Lead independently verified: 747/747 lib, 16/16 tests, ACG audit 3/3, clippy baseline. **Tier 0 landed (`3f44acdd`)**: Slayer's Studied Target (new, 4→5), Investigator's Studied Combat/Strike/Studied Defense (deepening closed #8, 6→8), Cavalier's Challenge damage (deepening closed #6, folded into existing slot) — no new architecture needed for any of the three, exactly as scout's design predicted. A dedicated test proves Cavalier's two Challenge scopes (damage-vs-target, AC-penalty-except-target) can't bleed into each other. Self-caught a near-miss (almost reported a second corpus bug on `InvestigatorTalentLVL` by pattern-matching Swashbuckler's genuine one, caught by checking tree-wide instead of trusting a directory-scoped grep) and correctly labeled two perturbation tests as structurally unable to fail (PF1's own defensive bounds never bind within 1-20) rather than let them look like coverage. Lead independently verified: 754/754 lib, both audits 3/3, clippy baseline. **Slayer Talents closed (`f6dc67e9`)** — count + canonical Foil Scrutiny, `named_features_wired` 5→6, 10 archetype deductions confirmed vacuous (3rd class in a row with this shape, now a standing check). Lead re-verified: 757/757 lib, ACG audit 3/3, clippy baseline. **Familiar subsystem closed (`8e47479a`)** — Toad's `+3` wired into real max-HP, shared Witch/Shaman implementation. **Found and fixed a real cross-workspace verification gap**: `apps/desktop/src-tauri` is a separate Cargo project with no workspace linkage to root, so "full root sweep clean" never actually covered it — a `pub(crate)` visibility break in the desktop crate hid behind an all-clean root sweep until backend built it explicitly. Folded into the `swarm-status-sync` skill as a standing check. Lead re-verified both crates independently: 763/763 root lib, APG audit 3/3, desktop 215/215. **#23's first attempt (`6239ef87`) held back** — shipped 249 entries, lead diffed against a corrected parser and found 60 real ACG spells silently missing, a genuine data gap. **Hexes landed (`b74da6af`)** while that was in flight — real structural find (all 27 hexes share one save-DC variable, no narrowing needed), Flight integrated into real Swim. #11's Familiar+hexes halves genuinely done. **#23 closed for real 2026-07-27 (`0ca6fd89`)**: root-caused the whole saga — a `CLASSES:.*Witch=` substring grep only matches when Witch is last in its comma group, so 249 and even the interim 326 re-derivation were both wrong; true count is **324** (250 APG + 74 ACG), confirming scout's and the lead's original figure all along, self-reported as its own prior mistake rather than silently corrected. Same commit fixed Bloodrager's list the identical way (183→**200**, see ACG table) and found `alchemist_spell_list` short by 17 on the same bug (104 shipped, should be 121 — logged as task #24). New regression tests on both lists pin real mid-group spells the old grep shape can't find. Lead independently re-verified against the real committed HEAD: **772/772** root lib, `witch_spell_list::tests` 5/5 and `bloodrager_spell_list::tests` 4/4 (both including the new regression test), array counts re-derived directly from source scoped to the real data arrays (324/200 confirmed). **#24 closed 2026-07-27 (`5f9f23aa`)**: same bug fixed on `alchemist_spell_list` (104→**121**), plus two things found in the process worth naming — a stale-prose defect in the just-shipped Bloodrager header (the array/test/level-breakdown were already corrected in `0ca6fd89`, but the surrounding doc comment still quoted 183-era secondary figures; caught on a re-read, not a failing test, since nothing about a wrong comment fails a build) and the real scope of the bug: **every one of the 6 CRB spell lists is short too**, two by over 100% (Ranger 51→114, Paladin 45→95). Filed as #25 (a yes/no book-scope question) but the lead re-derived the true scale directly (risks item 53) and ruled it isn't a preference call — PF1 RAW never scopes a class's list by sourcebook. Lead independently re-verified #24: 773/773 root lib, both new regression tests, SD-24 APG audit 3/3, clippy baseline. Operator confirmed dispatching the full 6-list fix now (tasks #26-32) rather than holding for the reconciliation review. Backend routed to #26 (Ranger, largest relative gap after Paladin) next |
