...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | **Self-audit caught a real defect in its own prior commit (`7a89d322`)**: `66b8aae9`'s count edit replaced the only sentence naming Cavalier/Alchemist's counts, leaving a stale "every other APG class remains at 0" clause standing where it now wrongly appeared to cover them (the sentence was already stale before the edit — Alchemist/Cavalier had long since grown past 1 — but the edit made it locally worse). Found by deliberately checking a prior commit for the exact rot-shape it had been fixing elsewhere. The identical stale sentence existed in `acg/mod.rs` too, corrected in the same commit. Both modules now carry the full explicit roster (APG: Summoner 1/Witch 2/Alchemist 3/Oracle 5/Inquisitor 5/Cavalier 6; ACG: all ten, 2 through 12) rather than a summary clause, specifically because no test asserts a doc comment — the roster form is the one that resists silent rot, since a reader can diff it against the match arms directly. Lead independently re-verified against the real committed HEAD: both rosters confirmed to match the real `class_coverage` match arms exactly, root lib 813/813 (unchanged, doc-only), both SD-24 audits 3/3, clippy baseline. Idle. **Shared-mechanism sweep clean (risks item 58)**, no code changes to commit — but found a real, reusable criterion rather than just reporting nothing. Enumerated all 62 `ground_*` functions, generalized to every non-class-named helper shared across class contexts (9), triaged each. Only one shape can produce a counting asymmetry: one feature-grounding helper (not arithmetic) shared by two classes that both have a coverage row — `ground_familiar_master_benefit` is the only helper in the codebase meeting both conditions, and it's already fixed. Lead spot-checked rather than accept the clean result: confirmed no `sd24_crb_class_coverage_audit.rs` exists (Druid/Hunter's shared companion helper genuinely can't be asymmetric) and confirmed `effective_combat_feat_intelligence_score`'s exactly two production call sites (Brawler/Swashbuckler, independently crediting 9/12). Now adding a guard test for the one real instance. **Self-initiated fix while idle (`66b8aae9`)**: found and corrected a real cross-class inconsistency of its own making — Shaman's own `named_features_wired` 1→2 bump (task #12, Familiar credit) never got mirrored onto Witch, which shares the exact same `ground_familiar_master_benefit` implementation. Witch corrected 1→2, with the shared `Witch | Summoner => 1` match arm correctly split rather than bulk-bumped (Summoner has no familiar, correctly stays 1). Lead independently re-verified: root lib 813/813 (unchanged), both audits 3/3, clippy baseline, both dispatch call sites and the split match arm confirmed directly in the code. **#11 (Witch) fully closed 2026-07-27 (`7175898d`)** — spellcasting, Familiar, and hexes all genuinely built; full detail in Witch's own per-class row below. Headline: found and correctly stopped on a genuine third caster shape (Witch is cantrip-auto-known + familiar-store for levels 1-9, neither Cleric's nor Wizard's shape) rather than ship a wrong-mechanic mirror of Shaman's stage 2; scout's scoping (risks item 57) found the answer already exists twice in this codebase (Alchemist's/Investigator's own Known-backs-Prepared shape), so no new architecture was needed after all. Cantrip exemption genuinely wired into the validation gate, not just declared (confirmed by the lead after scout flagged it as the likely failure spot); the wrong-build failure mode proven with a real perturbation test. Also fixed a real factual error in shipped code (`spellcasting_class.rs`'s stale "same as Cleric/Druid" comment). Lead independently re-verified: root lib **813/813**, all 20 Witch tests, both SD-24 audits 3/3, clippy baseline. **#12 (Shaman) fully closed 2026-07-27, all three stages** (`47bd518e`/`27e9814c`/`917780b4`) — own 304-spell list, prepared-caster spellcasting (a genuine single-cell L11 corpus divergence ruled to keep the corpus value, risks item 56), all 9 remaining spirits grounded (a real PRE-gate-on-the-grant near-miss caught before shipping, a genuine rounds-vs-uses-per-day decoy kept separate). Lead independently re-verified all three stages: root lib 793→800→**809/809**, all 31 Shaman tests, both audits 3/3 throughout. Idle, holding for next assignment. **Pre-2026-07-27 summary (compressed)**: closed #1/#4/#5/#6/#8/#9/#10/#14/#17/#19/#22 and Tier-0 deepenings across Slayer/Investigator/Cavalier, all lead-verified. **2026-07-27, the full spell-list undercount wave (tasks #23-33, CLOSED)**: two independent parsing bugs fixed across all nine spell lists — Ranger 114, Paladin 95, Bard 264, Druid 271, Sorcerer 578, Cleric 301, Alchemist 147, Witch 326, Bloodrager 200 — every number lead-verified, root lib 774→785 (items 53-54 have the full detail) |
