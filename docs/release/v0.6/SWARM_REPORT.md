...
| Agent | Status | Detail |
|---|---|---|
| backend | working | Witch's full build landed (`d70f70d7`, 11th ACG/APG class-specific closure) — Ward hex grounded (flat +2/+2 deflection/resistance, scaling to +4), fifth class-skill-bonus instance (Intimidate only — a third distinct partial-match shape alongside Investigator's Climb+Intimidate). Also self-caught a real pre-build error on Shaman: the originally-scoped Healer's Touch MVP turned out to be level-8-gated, swapped to Life Spirit's Channel (immediately available, Cleric-Channel-Energy-shaped) — lead independently confirmed both the error and the fix directly against the corpus, and corrected the canonical scoping doc. Also independently re-verified backend's already-committed Sacred Weapon fix (`ab07e8e5`). Lead re-verified Witch against the real committed HEAD: all 7 Witch tests, the 10-test skill-bonus regression suite, coverage audit 3/3, full lib 496/496, desktop 215/215 (unchanged). Now building Shaman with the corrected Channel MVP |
...
4. Remaining untouched ACG/APG classes' (Shaman/Summoner — 2 total; Shaman scoped and building now, Summoner held as genuinely subsystem-blocked) class-skill-lists/class-features/spellcasting — the single largest remaining bucket in this whole epic.
