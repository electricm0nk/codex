...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Arcanist's Path A closure committed (`772298bc`) — `compose_character_input`/`apply_level_up` now seed the starter spell + Metamagic Knowledge choice, so any request reaching the backend with `class_id: "class:arcanist"` now genuinely completes as `Computed`. Lead re-verified against the real committed HEAD: both new tests, full desktop suite 214/214. **Lead then independently found Arcanist isn't in `characterHubModel.ts`'s `CLASS_OPTIONS` at all** — the real Create Character dropdown is a plain `<select>` populated only from that array, no free-text/Other option, so a real user still cannot select Arcanist in the shipped UI today, unlike Sorcerer/Cleric/Druid (already listed as `headless-only` before their own Path A). Flagged to backend as a distinct, still-open gap, not conflated with this closure. Standing by |
