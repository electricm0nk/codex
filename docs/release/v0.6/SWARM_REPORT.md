...
| Agent | Status | Detail |
|---|---|---|
| backend | working | Per operator direction (2026-07-26, "focus on completing in-progress work"), deepened Brawler rather than starting a new class (`4555a862`): Brawler's Cunning (flat INT-13 floor for combat-feat prereqs) + Brawler's Strike (level-gated DR-bypass progression, honestly inert below level 5) + a 6th class-skill-bonus instance (Brawler genuinely has all three tracked skills). `named_features_wired` 1→3, Brawler's own remaining-feature count 11→9. Lead re-verified in true isolation (stashed both this commit's own uncommitted follow-on work and featmate's concurrent feat_effects.rs, confirmed the pure commit compiles and passes standalone): full lib 506/506, coverage audit 3/3, desktop 215/215. Now wiring featmate's feat_effects skill bonuses into the real consumer (compute_selected_skill_modifiers) — combined working-tree state compiles and passes 515/515, still finishing the end-to-end proof test |
