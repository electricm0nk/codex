...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | Landed Oracle's deepening (task #10, both halves: `db64e788` curses + `c1ce97fe` revelations) — Lame/Wasting/Deaf curses (each independently formulated: land-speed penalty off the character's real race speed not a hardcoded 30, CHA/Intimidate trade, initiative penalty + opposed-Perception -4) plus 6 Tier-1 Life Mystery revelations gated behind a genuine `choice:oracle_revelation` option-B mechanism (proven load-bearing: a dedicated test shows a Mystery pick alone grounds nothing). `named_features_wired` 2→5, confirmed via the actual corpus KEY-prefix precedent (curses are top-level `Oracle ~ ...` records like Inquisitor's Stern Gaze/Track; revelations are `Life Mystery ~ ...` records that fold into the Mystery slot like Judgment's sub-types) — stays 5 even once the remaining 9 mysteries land. Two integrations (Sidestep Secret→Reflex, Nature's Whispers→AC) proven by differencing the same character with/without the revelation, plus a spoof test proving an unrelated class is untouched. Correctly left Bone/Near Death un-integrated (disease/mind-affecting/poison-only bonus, no per-category save facet in this engine — folding it into the flat save total would be a correctness regression). Caught a real illegal-combo corpus fact (Cinder Dance + Lame is `!PREABILITY`-blocked) and diagnosed rather than silently netting the two magnitudes to zero. Lead re-verified in an isolated worktree at the committed SHA: 643/643 lib, both SD-24 audits 3/3, 36 Oracle tests, full workspace sweep 478 blocks clean, clippy unchanged (23 pre-existing warnings). Task #10 marked completed. Now briefed and starting Category B feat-effects (task #19: Extra Rage/Performance/Ki/Lay On Hands, additive modifiers on existing computed totals) |
