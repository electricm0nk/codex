...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Landed two things together (`25c5bcae`): (1) fixed the Arcanist colon-convention bug with a real translation layer (`arcanist_metamagic_knowledge_feat_name`, a generic `metamagic:<slug>` → real feat name transform, not a hardcoded single-feat mapping) plus a genuine end-to-end regression test that exercises the real save path and was verified to actually catch the original bug (reverted, confirmed red, restored, confirmed green); (2) Oracle's full APG build landed as the 8th ACG/APG closure (spontaneous known-spell posture mirroring Sorcerer's shape, Life Mystery/Healing Hands, Clouded Vision curse) — stays permanently Blocked (proven via a dedicated test, not assumed), unlike Arcanist. Lead re-verified both against the real committed HEAD: full lib 474/474, desktop 215/215, both coverage audits, the real save-path regression test, all Arcanist and Oracle dispatch tests individually. Standing by, awaiting steer on the remaining 5 classes |
...
### ACG (10 classes) — 1 of 10 genuinely Computed, 6 of 10 have real partial engine progress, 3 still dispatch-only
...
**ACG tally: 1 of 10 (Arcanist) genuinely reaches `Computed` — MILESTONE, first non-CRB class this segment, via the Metamagic Knowledge follow-on closure (`54d8048b`) narrowing its last claim-blocking diagnostic; engine-Computed, not yet product-reachable (Path A gap named honestly, separate follow-on). 6 of 10 (Skald, Bloodrager, Brawler, Hunter, Warpriest, Slayer) have real engine progress genuinely grounded but stay Blocked on their own permanent remaining-feature gaps — 3 of 10 untouched beyond chassis dispatch.**
...
4. Remaining untouched ACG/APG classes' (Investigator/Shaman/Swashbuckler/Summoner/Witch — 5 total) class-skill-lists/class-features/spellcasting — the single largest remaining bucket in this whole epic.
