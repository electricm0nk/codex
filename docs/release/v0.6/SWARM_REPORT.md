...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | **#12 (Shaman) stage 1 landed 2026-07-27 (`47bd518e`)**: own 304-spell list, levels 0-9 split 17/48/46/46/41/30/22/22/18/14, 283/304 (93%) corpus-reachable — independently derived before reading the scoping doc, matches its figure exactly. **Two corrections to the inherited survey, both real**: the spells-per-day table IS corpus-derivable after all (all 20 `CAST:` rows sit in `acg_classes.lst`, removing stage 2's main expected risk), and the Familiar subsystem is **already built and already wired for Shaman** (`ground_familiar_master_benefit` dispatch + `SHAMAN_CLASS_ID` already in `bonded_familiar_species`'s allowlist, lead-confirmed directly in the code) — but a stale diagnostic at `pilot_compute.rs:14265` still calls Spirit Animal "an unbuilt Familiar subsystem," telling users something false right now; queued for a later stage. **A real near-miss named honestly**: filled the 21st not-ingested slot from memory as "Whispering Lore," caught it before shipping, the real entry is `Web Shelter` — recorded as its own fabrication instance, the same class of error this segment has spent all day flagging in parsers. The not-ingested list is a const with a both-directions-accurate test (stale entries fail loudly if ingestion later widens, rather than the gap silently shrinking); `Commune With Birds` is documented as doubly-absent (its only book record is `#`-commented, no `data/corpus/` entry either) with its own exclusion test. Lead independently re-verified against the real committed HEAD: root lib **793/793**, all 8 Shaman spell-list tests including the both-directions test, array count confirmed 304 by direct read with zero duplicates, `Commune With Birds` confirmed absent from the main list and resolving to `None`, both the Familiar-wiring and stale-diagnostic claims confirmed directly in the code. Stage 2 (spells-per-day + prepared-caster trio) next. **Pre-2026-07-27 summary (compressed; full detail in prior commits' diffs)**: closed #1/#4/#5/#6/#8/#9/#10/#14/#17/#19/#22 and Tier-0 deepenings across Slayer/Investigator/Cavalier, all lead-verified in isolated worktrees — real findings along the way included a genuine PCGen corpus authoring bug (Swashbuckler Deeds, risks item 50), a cross-workspace verification gap (`apps/desktop/src-tauri` is a separate Cargo project the root sweep never covered), and the Familiar subsystem shared with Witch/Shaman. **2026-07-27: the full spell-list undercount wave, tasks #23-33, now CLOSED.** Root cause was two independent parsing bugs discovered in sequence: (1) a `CLASSES:.*<Class>=` substring grep only matches when a class is last in its comma group — first found on Witch (249 shipped, 60 real ACG spells silently missing, `0ca6fd89`), then found to be repo-wide (all 6 CRB lists short 26-124%, risks item 53, operator-confirmed dispatch of tasks #26-32); (2) a trailing bracketed prereq (`[PREVAREQ:Heroic,1]`, the APG Hero Points optional rule) throws on `int(level)` and gets silently dropped by a swallowed exception — found by backend holding #27 rather than shipping a coincidentally-matching Paladin total, which reopened the already-closed #23 (task #33) and affected Sorcerer/Cleric too (risks item 54, ruled: bracket-gated spells belong on the list). **Final state, every number lead-independently re-verified against the real committed HEAD**: Ranger 114 (`b6ef5d6b`), Paladin 95 (`672b6a79`), Bard 264 (`926ae061`), Druid 271 (`dfafecb7`), Sorcerer 578 (`0e69695b`), Cleric 301 (`a6220a03`), Alchemist 147 (`058e36c3`), Witch 326 (`e6d08004`), Bloodrager 200 (needed no widening — zero CRB/APG records). Every CRB list's own half was byte-identical to a correct re-parse (zero substring-bug damage anywhere in CRB); only Paladin/Sorcerer/Cleric/Witch carried the bracket defect on top. Two real behavior changes to already-closed classes outside their tasks' own scope, both disclosed proactively rather than found later: Oracle silently gained Cleric's +65 spells (`SPELLLIST:2|Cleric|Oracle`, #31; all 36 Oracle tests re-confirmed green), Investigator silently gained Alchemist's +26 extracts (`SPELLLIST:1|Alchemist`, #32; all 15 Investigator tests re-confirmed green). Witch's own module header was rewritten with the full honest count history rather than quietly overwritten — a prior revision wrongly asserted 324 was "correct all along" and blamed the lead's own 326 as an overcount; the header now names that as backend's own error, not the lead's. Root lib went 774→785 across the wave; every list carries its own regression test naming a real mid-group and/or bracket-gated spell the old grep shape provably can't find; both SD-24 audits 3/3 and clippy at the 23-warning baseline held throughout. No "Core Rulebook `<class>` spell list" string remains anywhere in the codebase — lead independently swept and confirmed, not just took backend's word. Idle, holding for next assignment |
