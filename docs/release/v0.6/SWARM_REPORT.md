...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | Closed tasks #10, #19, and now **#1 (Bloodrager, fully closed, both `73c70055` and `f24c6f24`)** — level-aware spellcasting diagnostic, false class-skill claim corrected (7th widening instance), levels 4-20 CAST/KNOWN table, and the full 183-entry spell list (110 corpus-reachable, self-audited two real measurement-tool bugs — a directory-listing miss that undercounted to 28, a fuzzy-match false positive that overcounted to 111 — before reporting). `named_features_wired` 1→2, backend's own correction to the lead's "1→3" guidance, confirmed right (`Class Skills` is `CATEGORY:Internal`, same exclusion Warpriest's count already established). Lead independently verified every commit this cycle: 686/686 lib (current), all Bloodrager/spell-list tests, ACG audit 3/3, clippy unchanged. **Now on #20's `effective_feats` wiring** (real correctness gap: Ranger's auto-granted Endurance silently returns 0 today), then Cavalier (#6), Summoner (#17) |
...
| featmate | in_progress | Landed Category A of the CRB feat re-sweep (`19ca4ae6`) and task #21 (`144bf89f`, Skill Focus dedup). **Task #20's leaf half landed (`bb7d0c95`)** — new `effective_feats(selected_feats, class_granted_feats)` union, held in_progress since consumer wiring is backend's; found the defect was broader than scoped (Ranger's auto-granted Endurance, not just Monk's Stunning Fist). **Spell Focus landed (`5d90dc91`)** — feat_effects 78→90, full lib 686/686. Found a real token-vs-prose trap: Greater Spell Focus's `BONUS:DC|...|2` is a TOTAL not an increment (both feats share `TYPE=SpellFocus`, PCGen takes the highest, matching RAW's "+1 that stacks with Spell Focus" = 2) — confirmed exact against `cr_feats.lst`, second instance of this trap after Extra Channel. Deliberately grounds nothing for Greater Spell Focus without base Spell Focus (genuinely ambiguous magnitude in isolation, unlike Acrobatic Steps' unambiguous 15 ft without its own prereq) — lead ruled keep the asymmetry, ambiguity is the right axis not prerequisites. Lead independently verified all of it. **Weapon Focus id-space investigated and ruled** (union synthetic `feat:weapon_focus` + catalog key, union new choice set + legacy `choice:fighter_bonus_feat` compound, dedup by weapon) — building now |
