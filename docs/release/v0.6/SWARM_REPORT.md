...
| Agent | Status | Detail |
|---|---|---|
| backend | in_progress | **#12 (Shaman), stage 2 of 3 landed 2026-07-27 (`27e9814c`)**: prepared-caster spellcasting live, consuming stage 1's own 304-spell list. **A genuine single-cell corpus divergence, ruled by the lead (risks item 56)**: Shaman's level-11 `CAST` row (`4,4,4,3,3,2,1`) diverges from Cleric/Druid/Wizard/Witch's shared row (`4,4,4,4,3,2,1`) — one 3rd-level slot later than every full-caster peer — while levels 10 and 12 are byte-identical to all four. Lead independently re-checked all three rows directly against `acg_classes.lst`/`cr_classes.lst`, confirmed byte-for-byte. **Ruled: keep the corpus value.** Unlike Swashbuckler's Deed gate (item 50), there is no internal contradiction here — no sibling variable proves an authoring defect, the row is self-consistent and monotonic — so a single-cell peer difference alone isn't evidence of an error under this segment's own "ground exactly what the corpus says" discipline. Backend had already pinned it correctly either way, with a test asserting both the value used and the peer value explicitly not used. Also landed: the access ceiling is genuinely derived from the slot table itself (not a duplicated constant ladder, preventing drift), and the live dispatch wiring was proven RED-then-GREEN through the real pipeline rather than only unit-tested in isolation. The stale "unbuilt Familiar subsystem" diagnostic text flagged in stage 1 is now fixed. Lead independently re-verified against the real committed HEAD: root lib **800/800**, all 22 Shaman tests, both SD-24 audits 3/3, clippy baseline, the L11 test read directly and confirmed accurate, the ceiling-derivation and RED/GREEN-wiring claims both confirmed directly in the code. Stage 3 (remaining 9 spirits) next. **Stage 1** (`47bd518e`): own 304-spell list (17/48/46/46/41/30/22/22/18/14 across 0-9, 283/304 reachable), Familiar subsystem confirmed already wired for Shaman, a self-caught near-miss (`Web Shelter` vs. a misremembered "Whispering Lore") named honestly. **Pre-2026-07-27 summary (compressed; full detail in prior commits' diffs)**: closed #1/#4/#5/#6/#8/#9/#10/#14/#17/#19/#22 and Tier-0 deepenings across Slayer/Investigator/Cavalier, all lead-verified — real findings included a genuine PCGen corpus authoring bug (Swashbuckler Deeds, item 50) and a cross-workspace verification gap (`apps/desktop/src-tauri`, a separate Cargo project the root sweep never covered). **2026-07-27, the full spell-list undercount wave (tasks #23-33, CLOSED)**: two independent parsing bugs (a `CLASSES:` substring-match bug and a bracket-prereq-throws bug) fixed across all nine spell lists — Ranger 114, Paladin 95, Bard 264, Druid 271, Sorcerer 578, Cleric 301, Alchemist 147, Witch 326, Bloodrager 200 — every number lead-independently re-verified against the real committed HEAD, root lib 774→785, no "Core Rulebook `<class>` spell list" string remains anywhere (items 53-54 have the full detail) |
