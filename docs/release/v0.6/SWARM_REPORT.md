...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Closed task #10 (Oracle deepening) then task #19 (Category B feat-effects) same session. **Task #19 (`042e1c32`)**: Extra Rage (+6 rounds), Extra Performance (+6 rounds), Extra Ki (+2 points), Extra Lay On Hands (+2 uses) all wired, magnitudes double-verified against `feat_data/general.rs` and raw `cr_feats.lst` before any code was written. Found the real hazard the brief didn't anticipate: Rage's and Bardic Performance's per-day budgets are each read in 3 places (display + 2 activation-budget checks), and Bardic Performance's display was computed inline while its budget checks called the shared formula — applying the feat at display alone would have shown a widened total while still judging the character over the un-widened base, the same shape as the earlier Rage/Charmed Life divergence. Fixed by moving the bonus inside each shared formula (`barbarian_rage_rounds_per_day`/`bard_bardic_performance_rounds_per_day` now take `selected_feats`) and routing Bard's inline display through it, so all 3 sites agree by construction. Lead independently reproduced the bug this fix prevents: reverted the two budget-check call sites to blind the feat in an isolated worktree, and both `*_widens_the_activation_budget_not_merely_the_displayed_total` tests failed immediately, confirming the protection is real. Tests written first this time (backend's own commitment from last cycle, kept). Lead re-verified in a separate isolated worktree at the committed SHA: 650/650 lib, both SD-24 audits 3/3, clippy unchanged (23 pre-existing warnings). Tasks #10 and #19 both marked completed. Idle, holding for next assignment — operator input requested on priority (see lead's note below the table) |
