...
**Update (2026-07-28, lead): task #73 (Wizard Spell Focus DC regression test) and task #47 (Inquisitor spellcasting + Bane + full Judgment widening) both closed, lead-verified, pushed as `098393d0` and `8f1300e5`.** #73 turned out to be a scoping/tracking oversight, not a build: the Spell Focus/Greater Spell Focus DC grounding it asked for already existed (`5d90dc91`, predating #73's own greenlight by a day). The dispatched agent checked before building, found the real gap was test coverage only (prior coverage used a generic Fighter fixture, never an actual `class:wizard` one), and added exactly that — no production code touched. Lead independently re-verified: `cargo test --lib` 968/968, the new test standalone, both SD-24 audits 3/3, clippy clean at the 80-warning baseline, plus a direct corpus check of the `STACK:NO MULT:YES` take-highest rule the agent's stacking claim depends on. **#47 (dispatched agent, not a named teammate) built Inquisitor's own 219-spell independent spontaneous list (re-derived and cross-confirmed against risks item 67's number), the Bane pool, and all 9 Judgment types (Destruction/Healing/Piercing/Resiliency/Resistance newly grounded as standalone level-driven `BONUS:VAR` facts, no opponent-dependence, same bar as Monster Lore/Track).** `named_features_wired` 5→6 (Bane earns its own slot; Judgment stays one slot; known-spell posture adds no slot, mirroring Oracle). Lead independently re-verified pre-merge: 864/864 lib, both audits 3/3, 30/30 Inquisitor-specific tests, all 5 new Judgment formulas spot-checked directly against `apg_abilities_class.lst` (confirmed real `BONUS:VAR\|...LVL\|InquisitorLVL` terms, not decorative), and the 219 figure cross-checked against risks item 67's own independently-reconciled number and per-level breakdown (15/38/43/44/35/24/20) — exact match. **Real merge-time finding**: #47's own worktree was built before task #64 (Touch of Good domain-power generalization) landed, so its diagnostic text claimed "no domain power exists for Inquisitor" — flatly contradicting #64's already-verified, already-shipped finding that Inquisitor's Good domain genuinely grants Touch of Good. Rebasing #47 onto current `tranche/6` surfaced this as a real 4-hunk merge conflict (not a mechanical adjacent-insert one); lead hand-merged to preserve both landed facts — Touch of Good still named as grounded, #47's 5 new Judgment types and known-spell posture folded in alongside it — re-ran the full verification suite against the merged result (983/983 lib, both audits 3/3, clippy 80/0) before pushing. Both verification worktrees removed after use.

**Update (2026-07-28, ~03:40 ET): task #58 (Rogue Talents Resiliency) landed, lead-verified, `66a8f699` cherry-picked as `3213d2c0` — and it found the brief itself was wrong about something real.** The brief claimed Resiliency "lands on durability.rs's computed temp-HP total" — the agent checked rather than trusted this and found **no such total exists anywhere in this crate or its downstream desktop consumer**, lead-independently confirmed by reading `apps/desktop/src-tauri/src/character_hub.rs` directly (only `max_hp`/`current_hp` fields exist, zero `temp_hp`). Correctly refused to either fabricate a false "integrated into a total" claim or silently expand scope into building a two-crate temp-HP subsystem — grounded both classes' magnitudes as standalone flat facts instead (the same honest-gap idiom as Witch's Ward hex) and documented the correction plainly in `durability.rs`'s own module doc. Also found and correctly applied a real detail the brief didn't have: Investigator's own talent-selection cadence is 3/5/7/9..., not Rogue's 2/4/6/8..., confirmed via independent research since the brief was silent on it. Correctly bumped Investigator's `named_features_wired` 8→9 and confirmed CRB (Rogue) has no equivalent mechanism. Lead independently re-verified: 860/860 lib (in its own isolated worktree), coverage audit 3/3, spot-checked the `character_hub.rs` claim directly before crediting it.

**Update (2026-07-28, ~03:30 ET): task #66 (Wizard Abjuration School) landed, lead-verified, `2484c62f` cherry-picked as `a02b8236`.** Found and corrected a real gap the original brief didn't have: Resistance's exact formula (two `BONUS:VAR|AbjurationResistanceBonus|5` lines, flat +5 stacking to +10 at level 11+, lead-confirmed byte-exact) plus Protective Ward's and Energy Absorption's, all three verified directly against `cr_abilities_class.lst`. Correctly excluded Arcanist's half pending School Understanding as instructed. Found the existing Wizard-school-selection mechanism (`wizard_has_canonical_specialization_selections`, previously Evocation-only) and added a parallel, mutually-exclusive Abjuration check rather than modifying existing behavior. Correctly confirmed the SD-24 coverage audits don't cover Wizard (CRB) at all. 14 new tests, written RED-first. Lead independently re-verified: 849/849 lib, 14/14 in the dedicated test file, all three formulas spot-checked against the raw corpus directly.

**Update (2026-07-28, ~03:15 ET): task #54 (Rage Powers) landed, lead-verified, `477eb9ec` cherry-picked as `0290ce1d`.** The corrective-redo agent confirmed its own background runs were unaffected by the #61 pkill incident (all exit 0 with real output — the earlier repeated "waiting" pauses were genuinely long compiles, clippy alone took ~9 minutes, not a killed process). Grounds Raging Climber and Raging Swimmer for Barbarian and Skald exactly as scoped: both purely passive, gated on the corpus's own `PREVAREQ:Raging,1`, wired into the real Climb/Swim totals via a new shared helper reusing the already-shipped rage-activation checks. Correctly did NOT bump Skald's `named_features_wired` (Raging Climber/Swimmer are tagged `KEY:Rage Power ~ ...`, a different corpus class-prefix than `KEY:Skald ~ ...`, folding into the already-deferred Rage Powers slot rather than adding a new one — documented explicitly) and correctly confirmed Barbarian (CRB) has no coverage mechanism to touch at all. Named its own scope limit honestly: doesn't validate that a character actually selected these two powers among their limited rage-power slots, since no chooser-selection enforcement exists for any of the 60 rage powers yet. Self-caught two TDD baseline miscalculations before going green. Lead independently re-verified in the worktree before merging: 856/856 lib, both SD-24 audits 3/3, spot-checked the actual grounding function and commit message against the real diff.

**Update (2026-07-28, ~03:00 ET): task #56 (Arcanist Familiar Exploit) landed, lead-verified, and the wrong-base incident turns out subtler than first understood.** The #56 agent self-detected the same wrong-base problem (its worktree started at develop's HEAD, not tranche/6) and self-corrected via `git fetch && git reset --hard origin/tranche/6` before doing any real work, then broadcast the finding — meaning the earlier "5 correct, 5 wrong" snapshot only reflected each worktree's *current* HEAD at check time, not its *original* base; some of the "correct" ones may have silently self-corrected the same way #56 did. Lead independently re-verified #56's commit (`3ade5392`) in its worktree: 850/850 lib, coverage audit 3/3, `named_features_wired` confirmed at 3 — cherry-picked cleanly into `tranche/6` as `51c28a0b`. Spot-checked the other four presumed-correct worktrees (#44/#47/#50/#58) directly — all four confirmed genuinely on real `tranche/6` commits, no develop-area contamination found. Task #56 closed.

**Update (2026-07-28, ~02:50 ET): real incident caught immediately — 5 of the 10 dispatched agents' worktrees were created from the wrong base branch (risks item 86).** Checked each worktree's base commit right after the first agent (#61) reported back, since its diff against real `tranche/6` was ~50,000 lines (unusable). Found the same defect in 4 more (#49, #54, #64, #66); confirmed the other 5 (#44, #47, #50, #56, #58) were genuinely based on real `tranche/6` commits. Sent corrective messages to all 5 affected agents (including resuming the completed one) instructing a hard reset to `origin/tranche/6` and a verification step before redoing their work. The 5 correctly-based agents were left running unattended.

**Update (2026-07-28, ~02:35 ET): operator directive to run all queued work. All three named teammates (backend, featmate, scout) are currently unavailable** (backend wrapped up its session, featmate on session-limit cooldown until 9:40pm ET, scout holding by its own conclusion). Per the standing orchestrator-never-executes-directly doctrine, the lead dispatched 10 fresh isolated-worktree Agent-tool subagents (not the named teammates — new sessions, one per queued task) to work the entire backlog in parallel: tasks #44, #47, #49, #50, #54, #56, #58, #61, #64, #66. Each was briefed with the full pre-verified corpus facts, gates, and formulas already established this session (so none needs to re-derive from scratch, though each is instructed to spot-check before trusting the brief), required to follow TDD (failing test first), and instructed to commit locally without pushing. The lead will independently re-verify every commit in its own isolated worktree before merging into tranche/6, exactly as with every prior commit this session — nothing here is exempt from that discipline just because it was lead-dispatched rather than teammate-initiated. Results to follow as each agent completes.

| Agent | Status | Detail |
|---|---|---|
| backend | idle, session wrapped up | **Correctly declined task #49 on capacity grounds rather than half-start it — the right call, agreed.** Reasoning matched this session's own standing lesson exactly: a half-verified cap widening left in the shared checkout on low remaining context is the same failure shape as the earlier #39-inside-#38 incident, just with featmate's preserved work at risk instead. Left useful handoff notes before signing off: the four #36 Monk features now use the two-branch gate-absence convention post-#46 (extend that same shape to new capstones, not a bare `if level >= GATE`), #52's text correction is accurate and shouldn't be reverted, and the level-12 ceiling is referenced across `tests/sd13_monk_level5/6/7/8_progression.rs` beyond just the constant — a cap widening will likely need those updated too. Task #49 released back to pending, genuine staffing gap (both featmate and backend now unavailable), queued for a fresh session. **Handed task #49 (Monk cap widening)** — featmate hit their session limit; the full workspace sweep lead started for #52 was killed by the environment before completing (no result, not a failure) — #52's own verification stands on 849/849 lib + all four Monk suites, independently confirmed, which is sufficient; the sweep gap stays honestly open rather than claimed either way. Work preserved on branch `wip/sd49-monk-widening-featmate`, full state and re-verification instructions in the task description. Task #52 closed (`8ad1f054`), lead-verified: 849/849 lib, all four Monk integration suites green (17/18/19/20), text-only change confirmed (Stunning Fist's DC/uses-per-day are genuinely grounded and sit right above the sentence denying it, Improved Unarmed Strike genuinely stays ungrounded).** Backend honestly flagged not re-running the full workspace sweep (timed out at 10m on their end) rather than let "verification: green" imply the usual standard — lead is closing that gap with a full `cargo test --workspace --all-targets` sweep in the background now, result to follow. Session wrap-up: idle at end of usable context, backlog left clean (#44/#47/#50/#54/#56/#58). Task #54 pre-build verification done (risks item 74), task released back to pending rather than half-built on low context — the right call.** Found the `KEY:Rage Power ~ <Name>` namespace trap (a bare-key grep returns zero and looks like the records don't exist — same shape as the Witch 324-vs-326 saga, self-caught before reporting), a fourth `RagePowersLVL` setter (`FighterLVL` via `KEY:Viking ~ Viking Rage Power`, archetype-only, lead-confirmed), and an `Unchained Rage Power` duplicate in the unignested `pathfinder_unchained` book. All three written directly into #54's own description. Session summary: landed #11/#12/#23-34/#36/#39/#42/#46 plus Master Craftsman wiring and the shared-mechanism sweep/guard this session, all lead-verified. **Task #46 closed (`03bb0f8f`), lead-verified: 849/849, both audits 3/3, diff confirmed matching the commit message exactly** — all four Monk features from #36 now emit a value-0 absence record below their gate, matching Purity of Body's own convention in the same function. Found while reading Skald's DR for unrelated work, not from a failing test — a real "nothing asserts cross-feature convention consistency" gap, same family as the earlier shared-mechanism counting hazard. Test tightened to assert strictly-positive at the gate rather than merely `is_some`, closing a real gap where an absence record could have masked a future magnitude regression. On task #54 (Rage Powers: Raging Climber + Raging Swimmer, Barbarian + Skald), queued after #46.** Task #42 closed (`5ae84d2b`), lead-verified: 849/849 (+5), both SD-24 audits 3/3, `named_features_wired` confirmed at 9.** Three real corrections found by re-deriving from the corpus rather than the brief: (1) the six remaining features' real gates live on the class table's own per-level `ABILITY:` rows, not the grant lines (which carry only archetype-suppression flags) — reading grant lines alone would suggest everything's ungated; (2) Greater/Mighty Bloodrage's magnitudes already existed in `bloodrager_bloodrage_tier`, reused rather than double-built, closing a real gap where a non-raging bloodrager's receipt never named them at all; (3) Uncanny Dodge's magnitude lives on an internal "Uncanny Dodge Tracker" record, with Improved Uncanny Dodge expressed as a second +1 to the same counter rather than a distinct value. Every feature grounds both branches (value-0 below gate, per the Skald/Barbarian DR convention). Idle. **Provenance incident resolved (risks item 66): task #39 (Bloodrager DR) landed complete but got swept into `e076c8ad` (Paladin's commit) by a same-file `git add` collision — self-caught by both backend and featmate independently, lead-verified, no rewrite, tracking corrected (task #39 closed, #42 trimmed to 6 remaining features).** Also self-flagged a real minor consistency gap in own `ecfad0f8` (Monk #36) while reviewing Skald's DR for this writeup — the four Monk features emit nothing below their level gates, unlike Skald/Barbarian/Bloodrager's two-branch DR convention; filed as task #46, low priority. Resuming task #42 (Bloodrager: ground remaining 6 class features, from scout's roster survey #40).** Task #36 closed (`ecfad0f8`), lead-verified: 839/839 (+6), both SD-24 audits 3/3, Perfect Self grep confirmed absent.** All four originally-scoped Monk features grounded with corpus-exact formulas and gates (Fast Movement 10*floor(lvl/3) ft @3, Maneuver Training CMB delta @3, High Jump +lvl Acrobatics @5, Wholeness of Body heals lvl hp @7). Two real gotchas caught by reading raw records instead of the brief: Fast Movement is armor/encumbrance-conditional (grounded anyway, condition named not applied — this engine models no encumbrance state); the old High Jump deferral had conflated the flat +lvl (unconditional) with a separate ki-spent +20 (correctly still deferred). Fixed a self-contradiction the change would've shipped (Purity of Body's own text claimed High Jump "deliberately not grounded" two records above the newly-grounded High Jump record). Perturbation-proved on three axes. **Did not include Perfect Self's DR**, which risks item 62 had folded into #36's scope — confirmed via direct grep on the committed HEAD, zero hits. Split out as its own task (#41) rather than left silently short. #16-adjacent scoping done (task #34, risks item 59), investigation only, no code touched.** Enumerated all 50 General feats exhaustively: the two-skill gap is genuinely empty (all 10 already correctly triaged), but Master Craftsman (a single chosen Craft/Profession skill) has zero handling anywhere — the one real cheap widening left. Corrected #16's own "Focus-feat chooser" framing rather than answer it as asked: the chooser mechanism already exists three times (Skill/Spell/Weapon Focus), the real blocker is that no school-keyed spell-DC total or per-weapon attack total exists to integrate into — a product/architecture decision, not a feat-effects widening. Lead independently re-verified every quantitative claim directly against `feat_data/general.rs` and the compute layer (50/12/10 count, `compute_selected_skill_modifiers`'s exact Climb/Intimidate/Swim scope, zero Master Craftsman references, all three chooser functions, zero school-keyed DC ids, zero `attack.*` ids). Task #16's own description corrected to hand this off accurately. Idle. **Familiar-count consistency thread fully closed 2026-07-27 (risks item 58), all four commits lead-verified.** Self-initiated while idle: found Witch and Shaman share one Familiar implementation but only Shaman's own count credited it (`66b8aae9`, 1→2); swept the whole codebase for the same shape and found a sharp criterion — a counting asymmetry needs one feature-grounding helper shared by two classes that both have a coverage row, and exactly one helper meets that; self-audited its own fixing commit and caught a doc-rot instance it had just created (`7a89d322`, both modules now carry an explicit roster instead of a summary sentence); landed a guard test (`1101a7ad`, 814/814) that enumerates every APG/ACG class rather than the two known ones, so a future third consumer fails loudly. **Worth recording the process, not just the result**: the guard's first perturbation attempt passed when it shouldn't have, and backend correctly read that as "wrong lever tested" rather than "guard too weak" — the mechanism needs both an allowlist entry and a dispatch call to activate, and the first attempt changed only one. Lead independently re-verified every commit against its real HEAD: root lib 813→813→813→**814/814**, both SD-24 audits 3/3 throughout, clippy baseline, every roster/match-arm claim and the guard test's own enumeration logic confirmed directly in the code rather than taken on trust. Idle. **#11 (Witch) fully closed 2026-07-27 (`7175898d`)** — spellcasting, Familiar, and hexes all genuinely built; full detail in Witch's own per-class row below. Headline: found and correctly stopped on a genuine third caster shape (Witch is cantrip-auto-known + familiar-store for levels 1-9, neither Cleric's nor Wizard's shape) rather than ship a wrong-mechanic mirror of Shaman's stage 2; scout's scoping (risks item 57) found the answer already exists twice in this codebase (Alchemist's/Investigator's own Known-backs-Prepared shape), so no new architecture was needed after all. Cantrip exemption genuinely wired into the validation gate, not just declared (confirmed by the lead after scout flagged it as the likely failure spot); the wrong-build failure mode proven with a real perturbation test. Also fixed a real factual error in shipped code (`spellcasting_class.rs`'s stale "same as Cleric/Druid" comment). Lead independently re-verified: root lib **813/813**, all 20 Witch tests, both SD-24 audits 3/3, clippy baseline. **#12 (Shaman) fully closed 2026-07-27, all three stages** (`47bd518e`/`27e9814c`/`917780b4`) — own 304-spell list, prepared-caster spellcasting (a genuine single-cell L11 corpus divergence ruled to keep the corpus value, risks item 56), all 9 remaining spirits grounded (a real PRE-gate-on-the-grant near-miss caught before shipping, a genuine rounds-vs-uses-per-day decoy kept separate). Lead independently re-verified all three stages: root lib 793→800→**809/809**, all 31 Shaman tests, both audits 3/3 throughout. Idle, holding for next assignment. **Pre-2026-07-27 summary (compressed)**: closed #1/#4/#5/#6/#8/#9/#10/#14/#17/#19/#22 and Tier-0 deepenings across Slayer/Investigator/Cavalier, all lead-verified. **2026-07-27, the full spell-list undercount wave (tasks #23-33, CLOSED)**: two independent parsing bugs fixed across all nine spell lists — Ranger 114, Paladin 95, Bard 264, Druid 271, Sorcerer 578, Cleric 301, Alchemist 147, Witch 326, Bloodrager 200 — every number lead-verified, root lib 774→785 (items 53-54 have the full detail) |
...
| scout | idle | **Task #67 closed — pre-verified Domains before #64, found the archetype-contamination risk unfounded (zero archetype records in the real power namespaces, lead-confirmed) but its own #62 Domains count understated by ~60% (matched only 1 of 4 coexisting key formats, the exact trap #63's own hazard section named).** Confirmed #61 and #66 both safe on independent re-check. Named the real pattern behind three same-day misses: scrutiny aimed outward, not at its own prior output. Recommended stopping the audit here rather than manufacturing more checks — agreed. Idle by its own conclusion, holding rather than generating an eleventh doc. On task #67 (backlog pre-verification pass), its own proposal, approved.** Correctly named its own asymmetry — checked for archetype inflation when a finding looked suspicious, skipped it when the finding looked good (five classes, wanted to be true). Proposed a targeted sweep of every queued build's class-credit claims for exactly this error class rather than more open-ended surveying, given the backlog's grown to 9+ tasks and the marginal survey value has dropped post-Domains. Agreed. Task #65 closed (`arcane-schools-shared-verification-scoping.md`), lead-verified exactly — Schools confirmed genuinely shared (two base-class setters, both screened for archetype status this time before being credited).** Applied the #63 lesson proactively and self-caught its own #62 inflation (Savant archetype, Arcanist wrappers counted as base content) before the lead had to — real base figure 66/58, not 177/96. Flagged a real sequencing dependency (Arcanist's credit needs School Understanding, deferred in #55) before it could be discovered mid-build. Recommended sequencing after Domains (#64); filed as task #66, not urgently dispatched. Idle, holding for next assignment. On task #65 (Wizard Arcane Schools, shared-vs-parallel check), the third named family from #62's triage.** Task #63 closed (`cleric-domains-generalization-scoping.md`), lead-verified with one important correction (risks item 80).** Found six `DomainLVL` setter paths and claimed 5 base classes (adding Hunter, Paladin to Cleric/Druid/Inquisitor) — lead independently confirmed the two new paths are BOTH archetype-only (`Divine Hunter ~ Domain`, `Temple Champion ~ Domain Granted Power` / `Sacred Servant ~ Spells`), same exclusion already applied to Bloodrager's Primalist and Fighter's Viking Rage Power. Real answer stays 3 classes, matching the original #62 count — corrected before any generalization build could have wired unreachable archetype-only paths. Everything else confirmed exactly: the three coupling points (clean formula, two hardcoded ids), no canonical domain pick needed, shared-namespace ruling adopted. Dispatched as task #64, correctly scoped to Cleric/Druid/Inquisitor only. On task #63 (Cleric Domains first-slice scoping) — the largest finding of the whole ladder-seam thread.** Task #62 closed (`class-ladder-families-triage.md`), lead-verified — derived the family set empirically rather than checking only named candidates, surfacing Domains (292 records, 159 magnitude-bearing, genuinely shared across Cleric/Inquisitor/Druid via one namespace + a shared `DomainLVL` var) as bigger than Bloodlines on every axis and the first family since Rage Powers with real multi-class leverage. Lead independently confirmed the existing partial Cleric coverage is genuinely `class_chassis.cleric.*`-namespaced, not class-agnostic — a real generalization pass is needed to extend to Inquisitor/Druid, which is what makes the leverage real rather than assumed. Filed as its own major future workstream (task #63, first-slice scoping), same treatment Bloodlines got. Previously **on task #62 (survey for class-specific-ladder siblings), continuing its own reframed seam.** Task #60 closed (`draconic-bloodline-ladder-scoping.md`), lead-verified exactly — Dragon Resistances' two-line natural-armor stacking confirmed by independent recomputation (1/2/4 at levels 3/9/15+, not 3 from a naive single-line read — the rare case this session where the corpus is right and the shallow read is wrong), plus a genuine third nesting level (energy type branches separately from the dragon-type choice). Ruled to ground the type-agnostic magnitudes and defer the energy-type label. Dispatched as build task #61. Previously **on task #60 (Draconic Bloodline ladder scoping, dedicated workstream).** Task #59 closed (`bloodlines-canonical-narrowing-scoping.md`), lead-verified — **self-corrected two of its own prior claims on the spot.** Bloodlines is the richest family surveyed (149 magnitude records vs the next-highest 21), reversing its own post-#57 "vein exhausted" call as an ordering artifact from three descending samples. Bigger correction: Bloodlines isn't actually shared — Sorcerer's and Bloodrager's records are entirely distinct despite ten overlapping bloodline names (lead-confirmed the two `KEY:` namespaces directly), the shared-name-not-shared-thing trap landing on scout's own #51 survey classification this time. Agreed with scout's recommendation not to force this into the quick-pick pattern; filed as its own dedicated workstream (task #60) instead. Previously **on task #59 (Bloodlines scoping), the fourth and likely final shared chooser family from #51.** Task #57 closed (`rogue-talents-canonical-narrowing-scoping.md`), lead-verified — confirmed the class-legality-constraint pattern a fourth time (Investigator's own 40-record whitelist, self-corrected from a stale "14" in its own earlier #45 doc). Recommended Resiliency, confirmed present in the whitelist; flagged this as the thinnest of three families scoped and explicitly recommended NOT rushing to build it ahead of #54/#56 — agreed, filed as low-priority task #58 rather than urgently dispatched. Previously **on task #57 (Rogue Talents canonical-narrowing scoping), third-ranked from #51.** Task #55 closed (`arcanist-exploits-canonical-narrowing-scoping.md`), lead-verified byte-exact — confirmed the class-specific constraint pattern repeats a third time this session: 36 of 46 Exploits cost Arcane Reservoir points (this engine doesn't model resource spend), correctly checked rather than assumed absent. Ruled Familiar as the canonical pick over Item Crafting: reuses already-shipped class-agnostic machinery (`ground_familiar_master_benefit`, genuinely namespaced `familiar.` not `witch.`/`shaman.`) and lands on the computed max-HP total, extending the familiar thread to a third class — real cross-class reuse, not new design. Dispatched as task #56. Previously **on task #55 (Arcanist Exploits canonical-narrowing scoping), runner-up pick from #51.** Task #53 closed (`rage-powers-canonical-narrowing-scoping.md`), lead-verified byte-exact — recommended Raging Climber (`= RagePowersLVL`, no arithmetic, passive, lands on the computed Climb total), flagged a real Skald-legality constraint the brief missed (Skald cannot grant activation-gated rage powers to allies, corpus-confirmed via its own DESC naming Terrifying Howl as excluded), and precisely hedged a three-vs-two-class credit question that turned out right — Bloodrager's own `RagePowersLVL` connection is archetype-only (`Bloodrager Archetype ~ Primalist`), lead-confirmed, not base-class. Ruled to ground both Raging Climber and its exact twin Raging Swimmer rather than just one (risks item 72, deliberate deviation from strict one-pick-per-chooser since the twin is zero extra risk). Dispatched as task #54. Previously **on task #53 (Rage Powers canonical-narrowing scoping) — the real headline finding from #51.** Task #51 closed (`roster-survey-round-2.md`), lead-verified — single-class scoping essentially exhausted; found a genuine fifth id prefix (`ability_modifier.<class>.*`, another consumer-integration category, lead-confirmed real) and identified Rage Powers (Barbarian + Skald, 60 records, zero magnitudes grounded, both grant-rate tokens confirmed) as the highest-leverage remaining gap on the whole roster. Dispatched as #53. Previously **on task #51 (roster survey round 2), using the fully corrected methodology (id-undercounting + name-overcounting + spell-count discipline).** Task #48 closed (`skald-remaining-chooser-features-scoping.md`), lead-verified — **a real "shipped bug" flag fully resolved in the shipped code's favor.** Scout found Skald's Raging Song rounds/day formula disagrees with the corpus's raw token by -2/level and deliberately declined to guess which side was right. Lead resolved it without RAW recollection, using Skald's own DESC text ("counts as the bard's bardic performance") to cross-check against Bard's already-shipped sibling record: Bard's token and formula are algebraically identical via a -2 base-offset convention; applying that same convention to Skald proves the CORPUS token has the transcription defect, not the shipped code (risks item 69, same precedent class as item 50's Swashbuckler deed gate). No code fix needed, just a comment recording the ruling — folded into build task #50. Five remaining flat magnitudes confirmed real (Well-Versed, Spell Kenning, Lore Master, Versatile Performance's genuine single-arg `min()`, Rage Powers), plus a real methodology finding: four of the five collide by name with Bard/Barbarian's own features, and a bare name-grep would have wrongly shown them as already covered — the inverse of Stern Gaze's undercounting, both now folded into item 67. Dispatched as task #50. Previously **on task #48 (Skald: scope remaining chooser-shaped features), tied #3 alongside Inquisitor from #40's survey.** Task #45 closed (`inquisitor-remaining-features-scoping.md`), lead-verified — the brief's own framing was wrong; spellcasting (219 spells, own list, spontaneous) is the dominant remainder, not chooser-shaped, not thin.** Real methodology catch: Stern Gaze is already grounded but produces no explanation record (feeds the Intimidate total directly), so item 60/63's id-count survey undercounts consumer-integrated wins — worth carrying forward. **Spell count fully reconciled**: scout independently re-verified their own 219 four ways and honestly reported they could only explain 1 of the lead's 3 extra spells; lead re-ran the script with per-spell output and found the real bug — three `#`-prefixed disabled/commented-out duplicate spell records were being counted as active. Fixed: 219, identical per-level breakdown to scout's original count. Scout was right, lead's script had the bug. Follow-up sweep found this bug touches only one already-shipped list (Paladin), and it's benign there (the active record independently agrees). Task #47 updated to build from 219 directly. Bane and Sacred/Profane Judgment widening confirmed real and small. Idle, holding for next assignment. Previously **on task #45 (Inquisitor: scope remaining chooser-shaped features), its own #3 pick from #40's survey.** Task #43 closed (`hunter-spellcasting-scoping.md`), fully lead-verified including an independent script re-derivation of every headline number (union 300, overlap 85, different-level overlap 27, castable 255 — all matched exactly against the two shipped spell-list modules, not just re-checked against the doc's own arithmetic) and a byte-exact re-transcription of the full 20-level CAST/KNOWN table straight from `acg_classes.lst`, whose own comment independently confirmed the leading-0/KNOWN+1 sentinel reasoning. Lead ruled on the doc's one open question (risks item 65): adopt take-the-lower for the 27 same-spell-different-level conflicts, recorded as a deliberate modeling choice, not a corpus fact. Dispatched as build task #44. Previously **on task #430 closed, `roster-survey-next-candidate.md` landed, lead-verified. Enumerated prefixes fresh rather than reusing the recently-corrected list and found a genuine fourth class-bearing prefix (`class.<name>.*`, 12 ids, progression/burden identity records) — proving the "enumerate, don't name" rule (item 60 addendum) right within the hour it was written. Ranked Bloodrager #1 (1 real feature, thinnest on the roster, six flat corpus-gated remaining features, two of which are tier extensions of an already-shipped formula), Hunter #2 (reuse-backed spellcasting now that Ranger/Druid lists are fixed), Inquisitor/Skald #3 (thin, chooser-shaped). Lead independently re-verified every one of Bloodrager's seven formulas and gate levels byte-exact against `acg_abilities_class.lst`, including the "zero numeric tokens" claim for Indomitable Will/Tireless Bloodrage. Dispatched as task #42. Idle, holding for next assignment. **`stale-deferral-sweep.md` landed and fully lead-verified (risks item 62)** — swept `pilot_compute.rs`'s deferral language and found the DR family self-inconsistent: Barbarian/Skald/Fighter Armor Mastery grounded standalone, Paladin's Aura of Righteousness and Monk's Perfect Self deferred/absent on the exact consumer-absence reasoning item 52 already overturned. Lead independently re-verified both corpus tokens (`DR:5/Evil` Paladin, `DR:10/Chaotic` Monk) directly against `cr_abilities_class.lst` and confirmed Paladin's own shipped comment cites the reasoning verbatim. Own split recommendation adopted as-is: Paladin standalone (task #38), Monk folded into in-flight #36, Bloodrager its own future-pass task (#39), bloodline/spirit sub-entries deferred further. Also spot-checked ~7 "confirmed correctly deferred" claims (Ranger Camouflage, Fighter Armor Mastery, Skald's ally-extension, etc.) — all held up. Idle, holding for next assignment. **Landed 6 scoping docs this session (Bloodrager, Cavalier, Summoner, Brawler, Swashbuckler — all greenlit — plus the #11-15 stale-framing sweep, which found Witch/Shaman spellcasting are real reuse jobs, Slayer Talents needed no design work, and Swashbuckler Finesse reuses Brawler's Cunning idiom; only Monk's Deflect Arrows framing survived unchanged). **Landed the opponent-interaction architecture design** (`opponent-interaction-architecture-design.md`, greenlit) — the two "architecture gaps" turned out to be opposite problems. Slayer/Investigator/Cavalier's opponent-scoped bonuses (Studied Target, Studied Combat/Strike, Challenge damage) all read only the character's own level and need NO new architecture — proven by the precedent that Slayer's own Sneak Attack is already grounded standalone despite being flanking-conditional. Reversed the lead's earlier Studied-Target ruling on this evidence (risks item 52). Monk's Deflect Arrows confirmed correctly deferred — zero corpus magnitude exists, a genuinely different failure mode the policy reversal doesn't reach. **Familiar subsystem scoped (`familiar-subsystem-design.md`), greenlit** — self-corrected an own first-look prediction (Tier 2 → Tier 0): a familiar's master-benefit bonus is a flat magnitude needing nothing from the familiar's own stat block, same shape as Studied Target. Canonical Toad ruled (its `+3 max HP` lands on the most load-bearing computed total in the app). Real decoupling found: zero `PREABILITY` gates Witch's spellcasting on the Familiar anywhere in the corpus — pure flavor text, so the 324-spell list (already 100% ingested) is buildable now, split into its own task (#23). Caught and correctly dismissed a false-alarm reachability hazard (`core_essentials` directory is organizationally-separate but `SOURCELONG:Core Rulebook` — not a missing book) before it shipped. Recommended sequencing #11 (small, shared-file) ahead of #23 (large, isolated module) on collision-risk grounds rather than size — this swarm's real failure mode has been concurrent shared-file edits, not big tables. Idle, holding |
| featmate | **session limit hit** (resets 9:40pm ET) | **Mid-commit-2 on #49 when the session ended. Work preserved, not lost** — lead found the stranded worktree (a session-scoped `/tmp` path, not durable), committed featmate's own uncommitted diff as-is with zero authorship changes, and pushed to `wip/sd49-monk-widening-featmate` so any teammate can pick it up regardless of local sandbox. Contents: an early superseded Perfect Self attempt (`fe42a430`), the three formula fixes through level 20 (`4b4066a6`, featmate's own report says 847/847 clean, not yet lead-re-verified), and the in-progress commit-2 snapshot (`a3c1300f` — cap already bumped to 20, a new capstone test suite added, completeness unknown). Task #49 handed off with full instructions to re-verify fresh in an isolated worktree before trusting anything. Found and self-diagnosed a real worktree-verification hazard mid-#49 (risks item 76): a shared `CARGO_TARGET_DIR` between two trees of the same package produced a false-green test run on code missing entire functions.** Caught it because a deliberately-broken append should have failed to compile and didn't; traced to a stale fingerprint cache from sharing the target dir with the main repo for build speed. Fixed with a dedicated target dir per worktree, re-running #49's RED from a clean isolated build before claiming anything. Lead independently confirmed this session's own worktree verifications are unaffected (no `CARGO_TARGET_DIR` set anywhere in the lead's environment, confirmed by an actual throwaway-worktree build). Still on task #49 (Monk cap widening), pre-build investigation done and greenlit (risks item 70) — found three existing formulas that would ship wrong values the moment the cap moves, all three lead-independently re-verified against the raw corpus.** Slow Fall's nine `-2` `.MOD` terms confirmed provably vacuous (all nine `Monk_CF_SlowFall<N>` flags `DEFINE` to 0, archetype-only); level 20 confirmed a separate "any distance" DESC record, not the naive 100 ft extension. Flurry's corpus `FlurryAttacks` token confirmed to evaluate to 7 at level 20 — a decoy counting total attack-routine entries including BAB iteratives, not flurry-granted attacks; existing RAW-sourced 2/3/4 semantic extended instead. Unarmed damage die's `min(5, MonkLVL/4)` cap confirmed correct, bands 4-5 (2d6/2d8) are the real gap. AC Bonus's own +4 understatement at 20 confirmed pre-existing and already honestly documented, correctly left untouched rather than silently widened. Two-commit sequencing greenlit: formula fixes first (provably inert at the current cap), then the widening + all 7 features together. Building now. **Stopped #41 before writing code and found the ruling's own option (b) wasn't real — Perfect Self's DR would be provably dead code, not a Master-Craftsman-shaped unreachable-character case, since `explain_monk_level1_chassis` early-returns on `supported_monk_level`'s `1..=12` gate before any Perfect Self branch could sit.** Lead independently re-traced and confirmed exactly (function boundary, early return, range check). Ruling reversed (risks item 68): #41 closed as correctly-deferred-not-built; cap question split into its own task (#49, widen `MAX_SUPPORTED_MONK_LEVEL` to 20 + ground the 6 newly-reachable capstones), dispatched to featmate with the already-parked worktree. **Diagnosed the real cause of item 64's wrong test count (risks item 66): not a remembered number, a real provenance collision — backend's uncommitted Bloodrager DR got swept into `e076c8ad` by featmate's own same-file `git add`.** Self-corrected a process claim held all session ("staged only my file, explicit path" is not isolation when a co-resident agent has uncommitted edits in the same file); now building **task #41 in a worktree** specifically to avoid repeating it. **Ruled toward option (b) on #41's open question** (ground Perfect Self as scoped-but-currently-unreachable rather than widen `MAX_SUPPORTED_MONK_LEVEL`) — lead ratified, see below. Task #38 closed (`e076c8ad`), lead-verified — DR 5/evil grounded as its own record, clean split preserving the aura's other two ungrounded clauses, stale deferral text corrected in place, flat value pinned at both level ends, correctly checked (and correctly found N/A) whether a coverage count needed bumping since CRB classes have no `named_features_wired` surface. Full detail in Paladin's own per-class row. **Real recurring issue: this is the second commit in a row (after #37) where the claimed full-lib count is wrong** — commit message says 839/839, lead's isolated-worktree re-run says **844/844** (3 new tests, matches the commit's own stated test count, so the delta is right, just not added to the correct base). Flagged directly to featmate as a pattern, not a one-off. **Task #37 closed (`9145f436`), lead-verified** — `named_features_wired` bumped 1→6 (Eidolon + Bond Senses/Maker's Call/Merge Forms/Twin Eidolon/Summon Monster, the latter's duration/uses/spell-level folded as one slot per the Cleric-Channel-Energy convention). Real root-cause finding, not just the number: Summoner sat in the coverage audit's zero-canary SKIP list with no row in the expected-count table either, so nothing asserted the value at all — it could have drifted to any number silently. Added the missing table row so it's genuinely covered going forward; all three surfaces (match arm, doc-comment roster, audit assertion) updated together. Lead independently re-verified in an isolated worktree at `9145f436`: `sd24_apg_class_coverage_audit` 3/3, full lib 833/833. **Queued next: task #38 (Paladin Aura of Righteousness DR, risks item 62).** Master Craftsman grounded (`6178e1bc`)**, picked up immediately on the corrected #16 handoff. +2 on a chosen Craft/Profession skill, corpus re-verified rather than trusting the relay (`BONUS:SKILL|LIST|2` + `BENEFIT` text match exactly, `STACK:NO MULT:YES` matches the Skill Focus dedup contract). Reconciled its own earlier 185-feat sweep, which had deferred this to the Focus-feat slice before that chooser pattern existed — correct then, buildable now that the pattern's proven three times over. **Real caveat named, not glossed**: requires 5 ranks in the chosen skill (`PRESKILL:1,TYPE.Craft=5,TYPE.Profession=5`), but the deterministic skill posture pins ranks at 1, so no character this engine currently composes could legally hold it — grounded anyway since prereq validation is a separate concern from magnitude grounding, flagged rather than silently tallied alongside reachable feats. Correctly declined to model the second token (`MasterCraftsmanRanks`, substituting skill ranks for caster level on item creation) — all 8 `ItemCreation` feats carry zero numeric tokens, nothing to substitute into. Producer built, consumer wiring queued for backend (lead confirmed zero references in `pilot_compute.rs`). Lead independently re-verified: root lib 823/823 (+9 tests), corpus tokens confirmed directly, both named claims (ItemCreation's own state, the missing wiring) confirmed rather than assumed. Idle, asked for more work. **Earlier: producer side of #16's original scope (Category A/B, all three Focus families) completed** — Category A (33 CRB feats, `19ca4ae6`), Skill Focus (task #21, `144bf89f`), Spell Focus (`5d90dc91`), Weapon Focus (`d322cb61`, 13/13 tests + 699/699 clean lib lead-verified). Weapon Focus's headline finding: it encodes stacking the OPPOSITE way from Spell Focus — `WeaponFocusToHit`/`GreaterWeaponFocusToHit` are genuinely separate variables (lead confirmed both `BONUS:VAR|...|1|TYPE=Base` tokens on the same `Default.MOD` record), so they truly add to +2, unlike Spell Focus's shared `TYPE=SpellFocus` take-highest. Correctly applied the ambiguity ruling in the opposite direction too: Greater Weapon Focus's own +1 is unambiguous so it grounds without base Weapon Focus, unlike Greater Spell Focus. Task #20's leaf half (`effective_feats`, `bb7d0c95`) also landed, broader in scope than asked (Ranger's Endurance, not just Monk's Stunning Fist). Everything outstanding in this lane is now consumer wiring — backend's queue. Standing down on new producer work, idle |
...
| Paladin | **Computed** | `full-except-human-level-1` — same shape as Ranger | `ee3c50ce` (spell posture), UI fix `3fd04f25`, spell-list fix `672b6a79` (task #27, 45→95 — book scope plus one bracket-gated record, `Heroic Fortune`; both a book-scope-only defect and the bracket bug in one class, unlike Ranger which had neither of the earlier substring bug's shapes — see risks items 53/54). **Depth-widened 2026-07-27 (task #38, `e076c8ad`), lead-verified**: Aura of Righteousness's DR 5/evil grounded as its own `class_chassis.paladin.damage_reduction` record (reversing the stale item-52-shaped deferral flagged by risks item 62) — deliberately a clean split, not a full reversal, since the aura's own value-0 identity record stays 0 (compulsion immunity has no magnitude, the ally +4 morale bonus is scoped to other creatures this codebase models nowhere). Flat and pinned at both ends (no further DR tier past level 17, level-20 test asserts it stays 5). Status was already Computed before this landed; this widens correctness depth within an already-closed class. Lead independently re-verified in an isolated worktree at `e076c8ad`: level-17 file 14/14, level-20 file 13/13, both SD-24 audits 3/3, full lib **844/844** (commit message claimed 839/839 — a real discrepancy, corrected here after direct re-run) |
...
| Monk | **Blocked** — 6 of 7 feats closed; cap widened to level 20 and all 7 class-feature capstones grounded 2026-07-28 (task #49); only Deflect Arrows remains, and it is permanently un-groundable, not just unbuilt | `table_class_id` still not widened (base-attack/save/fast-movement stay standalone). Dodge/Catch Off-Guard/Throw Anything (provably vacuous preconditions) plus Combat Reflexes (extra-AoO capacity, `max(Dex mod,0)`), Scorpion Style (DC + duration, both real Monk-derived numbers), and Improved Grapple (+2/+2 CMB/CMD magnitude, reusing the Dwarf Stability idiom — no new pillar needed) are all closed for real, each explicitly not claiming the trigger event itself is resolved. **Deflect Arrows is the one remaining gap of any kind on this class, and a dedicated design pass (task #15, `cc807da8`) confirmed it is structurally un-groundable, not merely deprioritized: it carries zero `BONUS` tokens anywhere in the full ingested corpus, only `TYPE:Combat` and its two prereqs — the entire mechanic exists as RAW prose with no numeric magnitude to derive, so no engine investment (however sophisticated) produces a value that isn't in the source data.** **Cap widened to level 20 and all 7 capstones grounded 2026-07-28 (task #49, `efeeaeb2`+`84da0134`, lead-verified 875/875 lib + both SD-24 audits 3/3 + the new 21-test capstone suite, formulas spot-checked byte-exact against the corpus — Diamond Soul SR `10+MonkLVL`, Quivering Palm DC `10+(MonkLVL/2)+WIS`)**: Diamond Soul, Quivering Palm, Timeless Body, Tongue of the Sun and Moon, Empty Body, Abundant Step, and Perfect Self's DR (task #41's own "correctly-deferred, never actually built" verdict confirmed accurate before this closure — grep of the pre-#49 HEAD found zero Perfect Self code, so this is the DR's first real grounding, not a double-build) all now genuinely computed through level 20, not just admitted past the old level-12 gate. Three pre-existing formulas (unarmed strike die, Flurry attack count, Slow Fall reach) also extended through 13-20, proven inert at the old cap before the widening landed. **Net: Monk now has exactly one remaining gap, and it is a permanent one — worth a product decision on whether "every groundable feature closed, one feature permanently out of scope for a documented structural reason" should read differently than a generic `Blocked`, rather than staying indistinguishable from a class that's merely mid-build.** **Operator ruling 2026-07-28 (task #68, `9afd2462`): the product answer for zero-magnitude features like this one is not "wait for a bigger engine" — it's to render the feat's own text description on the character sheet and let the player self-adjudicate, exactly as the physical rulebook does.** `FeatsTab` previously showed every selected feat (not just Deflect Arrows) as its raw internal ID string with no name or description at all; now resolves each to its real catalog entry. Deflect Arrows itself is confirmed live-rendering correctly: "Combat · You can know arrows and other projectiles off course, preventing them from hitting you." This doesn't change Deflect Arrows' `Computed`/`Blocked` status (still no numeric magnitude), but it does resolve the actual player-facing problem — the feat is no longer invisible | `18920c3d` + `b1a453a1` (first 3 feats), `e45b622f` (remaining 3, adversarial-reviewed), `ecfad0f8` (4 class features, task #36), `efeeaeb2`+`84da0134` (cap widening + 7 capstones, task #49), lead re-verified against committed HEAD: 875/875 lib, both SD-24 audits 3/3 |
...
| Inquisitor | 3/4 | d8 | **Blocked** — real progress: Justice/Protection/Purity/Smiting judgments (all four reuse the same activation-gated, uses-per-day-budget-enforced mechanism; Protection `1+level/5` to Armor Class, Purity `1+level/5` to all saves, Smiting a standalone DR-bypass fact) plus Stern Gaze's Intimidate half (`max(1,level/2)`, unconditional) plus Monster Lore (+WIS on Knowledge), Cunning Initiative (+WIS on Initiative), and Track (`max(level/2,1)` on Survival, byte-identical to Slayer's own) all genuinely grounded as standalone facts. The last three were corrected in a follow-on (task #18) after a real methodology fix: this codebase's own established precedent (Bard's Bardic Knowledge, Slayer's Track, Barbarian's Damage Reduction) already tolerates a genuinely-verified standalone flat fact with zero live consumer — the original "needs a live consumer" bar this closure first applied was stricter than the codebase's own dominant practice. **Correction 2026-07-28 (task #64): the "Domain grants no power per RAW" line above was wrong and is now retracted.** Inquisitor's own Domain feature (`Inquisitor Domain ~ Good` et al.) genuinely grants domain powers — Touch of Good confirmed byte-exact against `apg_abilities_class.lst:381` — the earlier "provably out of scope" verdict was itself the stale claim, not the codebase's own diagnostic comment it was checking. Touch of Good's Good-domain power is now generalized from Cleric to Inquisitor (shared `active_touch_of_good_bonus`, gated on each class's own domain-choice seam), one Domain of many, `named_features_wired` deliberately left unchanged since this is a partial slice not a full Domain-family close. Studied-target-style opponent-dependent judgments (Destruction/Healing/Piercing/Resiliency/Resistance) still need real engine state that doesn't exist and stay deferred. Spellcasting (WIS spontaneous, no reusable spell list) stays deferred. `named_features_wired` 1→2→5, though scout's #45 found this undercounts by 1 — Stern Gaze is real but consumer-integrated (no explanation record), invisible to id-count surveys. **Remaining scope fully scoped 2026-07-27 (task #45)**: spellcasting is the dominant item (219 spells per scout, own list, spontaneous, Oracle-shaped validation — lead flags the exact count needs re-derivation, got 222 independently, see risks item 67), plus Bane pool (flat, `InquisitorLVL` rounds/day) and Sacred/Profane Judgment (widens the already-built Judgment machinery). **Task #47 CLOSED 2026-07-28 (`8f1300e5`).** Spellcasting built: a real, independently-verified 219-spell own list, spontaneous, WIS-based, levels 0-6 (real `CLASS:Inquisitor` record carries no `SPELLLIST` token, unlike Hunter/Oracle, so no existing list module could be reused). Bane pool grounded (flat `InquisitorLVL` rounds/day, no consumer needed). All 9 Judgment types now grounded — Justice/Protection/Purity/Smiting were already shipped; Destruction/Healing/Piercing/Resiliency/Resistance newly built as standalone level-driven facts (each carries a real `BONUS:VAR\|...LVL\|InquisitorLVL` formula per `apg_abilities_class.lst`, no opponent-dependence, same "no live consumer required" bar already applied to Monster Lore/Track). `named_features_wired` 5→6 (Bane earns its own slot; Judgment stays one slot per convention; known-spell posture adds no slot, mirroring Oracle). Stays Blocked on: Detect Alignment, Discern Lies, Exploit Weakness, Solo Tactics, Stalwart, Stern Gaze's Sense Motive half, domain power for any domain but Good (Good's Touch of Good stays grounded per task #64). Lead independently re-verified pre-merge (864/864 lib, both audits 3/3, 30/30 Inquisitor tests, all 5 new Judgment formulas spot-checked against the raw corpus) and re-verified again post-merge after resolving a real conflict against task #64's own landed Touch-of-Good fact (983/983 lib, both audits 3/3, clippy 80/0). Committed `22abe6e5` (Justice) + `14126998` (Protection/Purity/Smiting/Stern Gaze) + `6c8ca561` (Monster Lore/Cunning Initiative/Track) + `4c8ae5e5` (Domain/Touch of Good generalization, task #64) + `8f1300e5` (spellcasting/Bane/Judgment widening, task #47) |
...
| Bloodrager | **full** | d10 | **Blocked** — real progress: Bloodrage (Str/Con/Will/AC, self-only by RAW, mirrors Barbarian's Rage exactly) genuinely grounded. **Closed 2026-07-27 (task #1)**: fixed a real shipped honesty bug — the spellcasting diagnostic claim-blocked unconditionally at every level despite Bloodrager having no `CAST:`/`KNOWN:` rows below level 4 per RAW (`PRECLASS:1,Bloodrager=4`), now level-aware and non-blocking below 4; corrected a false "no class-skill list" diagnostic claim, grounding the real 11-skill list (7th widening instance); built the full levels 4-20 spells-per-day/known table straight off the corpus tokens; built the complete spell list off the corpus tokens. **Corrected 2026-07-27 (`0ca6fd89`)**: the original 183-entry list (110 of 183 corpus-reachable) undercounted for the same `CLASSES:.*Bloodrager=` substring-grep reason diagnosed on Witch (#23) — corrected to the true **200 entries, all corpus-reachable**, with a regression test pinning real mid-group spells the old grep shape couldn't find. `named_features_wired` 1→2 (Bloodrage + Spells; class skills excluded, `CATEGORY:Internal`, same exclusion as Warpriest's identical record). **Roster-surveyed 2026-07-27 (task #40, scout) as the thinnest class on the whole roster** — only `bloodrage_execution` counted as a real named feature; every remaining item confirmed flat with a verified corpus gate (lead independently re-checked all seven formulas byte-exact against `acg_abilities_class.lst`: Fast Movement, Uncanny Dodge, Blood Sanctuary, Improved Uncanny Dodge, Damage Reduction, Greater Bloodrage, Mighty Bloodrage). Greater/Mighty Bloodrage are tier extensions of the already-shipped base formula, not new mechanisms. **Dispatched as task #42.** **DR closed 2026-07-27 (task #39) — landed inside `e076c8ad` via a shared-checkout provenance incident, not its own commit (full writeup risks item 66).** `ground_bloodrager_damage_reduction`/`bloodrager_damage_reduction_amount` confirmed complete and correct despite the misleading commit message: gate is implicit in the integer division (`(BloodragerLVL-4)/3` first reaches 1 at level 7, no `PRE` token — a real finding worth knowing, since a token search alone would wrongly conclude it's ungated), the `.max(0)` clamp is load-bearing (the raw expression goes negative below level 4 under truncating division, a test pins this), and both branches (above/below gate) ground a record per the Skald/Barbarian convention. Lead-verified in an isolated worktree at `e076c8ad`: 844/844 lib, both audits 3/3. **Task #42 corrected from 7 to 6 remaining items** (DR removed, already done): Fast Movement, Uncanny Dodge, Blood Sanctuary, Improved Uncanny Dodge, Greater Bloodrage, Mighty Bloodrage. Still Blocked on those six (until #42 lands) plus Indomitable Will/Tireless Bloodrage (zero corpus tokens, confirmed correctly deferred) and the Bloodline chooser slot. Committed `15560e62` (Bloodrage) + `73c70055` (diagnostic+tables) + `f24c6f24` (spell list) + `0ca6fd89` (200-entry correction), lead re-verified against the real committed HEAD: 772/772 lib, `bloodrager_spell_list::tests` 4/4 including the new regression test, clippy unchanged. **Remaining 6 features closed 2026-07-27 (task #42, `5ae84d2b`)** — Fast Movement (flat +10, distinct from Monk's/its own third magnitude for the shared name), Uncanny Dodge/Improved Uncanny Dodge (0/1/2 tier on an internal counter), Blood Sanctuary, Greater/Mighty Bloodrage (reused the already-shipped tier function rather than double-building). `named_features_wired` 2→9. Stops being the roster's thinnest class. Stays Blocked on Indomitable Will/Tireless Bloodrage (zero corpus tokens) and the Bloodline chooser. Lead re-verified against committed HEAD: 849/849 lib, both SD-24 audits 3/3 |
...
| Hunter | 3/4 | d8 | **Blocked** — real progress: Animal Companion (Wolf, effective druid level = hunter level, reuses Druid's own reviewed stat-block math via 2 newly-shared functions) genuinely grounded and unconditional, plus Wild Empathy (`CHA+level`, flat unconditional check-modifier fact) and Animal Focus (Bull picked as the canonical choice, `+2/+4/+6` STR enhancement at levels 1/8/15, activation-gated with a genuinely enforced per-day minutes budget). Animal Focus was a real correction under the standalone-grounding methodology fix — a prior pass wrongly deferred it as "a chooser," when each of the 13 real options is actually a flat, self-scoped magnitude that narrows cleanly (the same Oracle-Mystery-style pick-one-canonical shape). Nature Training confirmed correctly deferred (zero numeric `BONUS` token at all, a feat-qualification flag only, not a missed win). Spellcasting deliberately deferred to its own follow-on (reuses already-built Druid+Ranger spell lists, a real reuse-backed lift, not from-scratch). `named_features_wired` 1→3. **Spellcasting scoped 2026-07-27 (task #43, `hunter-spellcasting-scoping.md`), lead-verified in full**: `SPELLLIST:2|Druid|Ranger` confirmed a union (against Skald's/Oracle's own shipped `SPELLLIST` precedents), full 20-level CAST/KNOWN table re-transcribed byte-exact from `acg_classes.lst`, Oracle-shaped spontaneous posture confirmed (`MEMORIZE:NO`), six Summon Nature's Ally auto-knowns confirmed bounded. Lead independently re-derived the union/overlap arithmetic with a standalone script against the shipped Druid/Ranger modules (300 union, 85 overlap, 27 different-level, 255 castable, 45 out-of-range — all exact). Ruled (risks item 65): the 27 same-spell-different-level conflicts resolve take-the-lower, a deliberate modeling choice not a corpus fact. **Dispatched as build task #44** — expected to close Hunter's last named spellcasting gap but not by itself flip it to Computed (Hunter Tactics/Empathic Link tiers/Master Hunter stay open). Committed `21c5b13a` (Animal Companion) + `f1c46e19` (Wild Empathy + Animal Focus), lead re-verified against the real committed HEAD: 556/556 lib, `sd24_acg_class_coverage_audit` 3/3, all 9 Hunter-specific tests individually. **Task #44 (spellcasting) and task #85 (5 more flat-magnitude features) both since closed 2026-07-28.** #44 built the Druid+Ranger-union spontaneous spell list per its own scoping doc. #85 (`8451cf49`) grounds Track, Bonus Tricks, Precise Companion, Second Animal Focus, and Master Hunter — the last two share one literal corpus token (`BONUS:ABILITYPOOL\|Hunter Animal Focus\|1`, confirmed byte-exact by the lead directly against `acg_abilities_class.lst:1184`/`:1189`) and are modeled as two increments to one shared counter rather than two independent mechanics, deliberately kept distinct from the unrelated already-shipped Animal Focus activation counter. Lead independently re-verified #85 in an isolated worktree: 988/988 lib, `sd24_acg_class_coverage_audit` 3/3, all 32 Hunter-specific tests, clippy at the 80-warning baseline. Still Blocked on Hunter Tactics/Empathic Link tiers and any remaining named features |
...
| Skald | 3/4 | d8 | **Blocked** — real progress: Inspired Rage (self-application inferred, mirrors Barbarian's Rage), real spontaneous spellcasting (known-spell posture + access ladder/per-day/save-DC, reusing Bard's spell list and pure lookup functions directly — byte-identical tables, 2-source verified), self-Damage Reduction (`0/1/2/3` at levels 9/14/19, mirroring Barbarian's own standalone DR-record shape exactly, ally-extension via Raging Song deferred since this engine models no allies), and Bardic Knowledge (`max(level/2,1)`, byte-identical to Bard's own formula) all genuinely grounded. DR and Bardic Knowledge were corrected in via task #7 after the same standalone-grounding methodology fix Inquisitor's task #18 established — both were initially mis-scoped as blocked under an over-strict "needs a live consumer" bar. **Task #54 (`0290ce1d`) grounded Raging Song's own Raging Climber + Raging Swimmer** (both flat `=RagePowersLVL`, no arithmetic to transcribe wrong, Skald-eligible per its own DESC excluding standard-action/rounds-of-rage powers). **Task #50 (`df02a5f2`+`36980d2f`) then closed Well-Versed (flat `+4` vs. bardic-performance/sonic/language-dependent saves, byte-identical to corpus `SkaldWellVersedBonus`), Lore Master (pool size `min((level-1)/6,3)`, byte-identical to corpus, use itself unmodeled — no skill-check-resolution engine exists), Spell Kenning, Versatile Performance, and the Rage Powers pool-size formula** — reconciled cleanly with #54's prior work after a real rebase conflict (both touched the same dispatch block; #54 grounds two specific powers, #50 grounds the pool-count formula, genuinely complementary not contradictory). `named_features_wired` 1→2→3→8. Still Blocked on the ally/opponent-targeting Raging Song variants and the remaining 58 named-but-unmodeled Rage Powers (Skald has none of Bard's ~20-feature prior investment). Committed `d2eb0798` (Inspired Rage) + `7e7f6fbd` (spellcasting) + `6c8ca561` (DR + Bardic Knowledge) + `0290ce1d` (task #54, Raging Climber/Swimmer) + `df02a5f2`+`36980d2f` (task #50, Well-Versed/Lore Master/Spell Kenning/Versatile Performance/Rage Powers pool-size), lead re-verified against the real committed HEAD: 881/881 lib, `sd24_acg_class_coverage_audit` 3/3, 26/26 Skald tests, two formulas spot-checked byte-exact against the corpus |
