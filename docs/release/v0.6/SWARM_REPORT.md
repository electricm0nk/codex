...
| Agent | Status | Detail |
|---|---|---|
| backend | idle | Swashbuckler's full build landed (`d267fe79`, 9th ACG/APG class-specific closure) — Panache/Charmed Life/Nimble grounded, fourth class-skill-bonus widening fix. Two honest evidentiary findings: Panache's own formula is DESC-text/archetype-cross-validated (no literal `BONUS:VAR` on the base class), Charmed Life's real grant level (2nd) confirmed via web search then independently re-confirmed by the lead directly in the corpus's own per-level grant table. Lead re-verified against the real committed HEAD: 8 Swashbuckler tests, coverage audit 3/3 ("eight" closures), full lib 482/482, desktop 215/215 (unchanged), full workspace sweep clean. Standing by |
...
### ACG (10 classes) — 1 of 10 genuinely Computed, 7 of 10 have real partial engine progress, 2 still dispatch-only
...
**ACG tally: 1 of 10 (Arcanist) genuinely reaches `Computed` — MILESTONE, first non-CRB class this segment, via the Metamagic Knowledge follow-on closure (`54d8048b`) narrowing its last claim-blocking diagnostic; engine-Computed and product-reachable (Path A closed, `772298bc`+`25c5bcae`). 7 of 10 (Skald, Bloodrager, Brawler, Hunter, Warpriest, Slayer, Swashbuckler) have real engine progress genuinely grounded but stay Blocked on their own permanent remaining-feature gaps — 2 of 10 untouched beyond chassis dispatch.**
...
4. Remaining untouched ACG/APG classes' (Investigator/Shaman/Summoner/Witch — 4 total) class-skill-lists/class-features/spellcasting — the single largest remaining bucket in this whole epic.
