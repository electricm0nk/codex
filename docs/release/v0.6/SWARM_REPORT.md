...
| Agent | Status | Detail |
|---|---|---|
| backend | working | Widened Inquisitor's Judgment to Protection/Purity/Smiting + Stern Gaze (`14126998`, task #3 from the operator-directed blocker-resolution backlog) — caught and fixed a real gap in scout's own sizing pass by applying the wiring-consumer constraint: only features/judgments landing on an already-computed total got built (Monster Lore/Cunning Initiative/Track and 5 other judgments stay deferred, no live consumer exists). Lead re-verified against real committed HEAD: 524/524 lib, `sd24_apg_class_coverage_audit` 3/3, 12/12 Inquisitor tests, full workspace sweep 477/0 failures. Holding on Skald (#7) pending scout's revised scoping pass (same consumer-existence check) before starting |
...
| scout | working | Read-only scoping/audit lane (`.md` docs only, never `.rs`). Landed Investigator ACG scoping, Shaman/Summoner/Witch re-scoping, Eidolon-subsystem sizing (self-corrected 353→104 real Evolution records), and is mid-audit re-deriving every already-landed closure's formulas from raw corpus — found the real shipped Warpriest Sacred Weapon dice-count bug (`ab07e8e5`) this way. Continuing the systematic re-derivation audit across remaining closures |
| featmate | idle | Landed `skill_bonuses_from_feats` (Athletic/Persuasive/Intimidating Prowess, `feat_effects.rs`, 9-test module) — the `feat_effects.rs` half of the feat-effects widening slice, with backend owning the cross-lane consumer wiring per the established save-bonuses lane split. Committed and lead-verified (`e08f971f`). `named CRB feats with real mechanical effects wired: 4→7 of 185`. Holding, not scoping new work, per "complete before starting new" — two candidate next targets (more two-skill General feats gated on new computed skills; a chosen-target mechanism for Focus feats) are named but not greenlit |
...
| Inquisitor | 3/4 | d8 | **Blocked** — real progress: Justice/Protection/Purity/Smiting judgments (all four reuse the same activation-gated, uses-per-day-budget-enforced mechanism; Protection `1+level/5` to Armor Class, Purity `1+level/5` to all saves, Smiting a standalone DR-bypass fact) plus Stern Gaze's Intimidate half (`max(1,level/2)`, unconditional) genuinely grounded. Widening from the original single-Justice closure caught a real gap in the sizing pass that proposed it: Monster Lore/Cunning Initiative/Track and the other 5 judgment types (Destruction/Healing/Piercing/Resiliency/Resistance) all target skill/combat totals this engine doesn't compute anywhere (Knowledge, Initiative, Survival, damage-roll, HP-tick, concentration-check, DR-received, energy-resistance) — correctly deferred, not fabricated. Domain grants no power per RAW (provably out of scope). Spellcasting (WIS spontaneous, no reusable spell list) stays deferred. `named_features_wired` 1→2. Committed `22abe6e5` (Justice) + `14126998` (Protection/Purity/Smiting/Stern Gaze), lead re-verified against the real committed HEAD: 524/524 lib, `sd24_apg_class_coverage_audit` 3/3, 12/12 Inquisitor tests, full workspace sweep 477/0 failures |
| Oracle | 3/4 | d8 | **Blocked** — real progress: full build, not a single ability, first genuine FULL build in APG. Real spontaneous known-spell posture (own independently-verified per-level `CAST`/`KNOWN` table — level 1 `0,3`/`4,2`, level 2 `0,4`/`5,2`, level 3 `0,5`/`5,3` — mirroring Sorcerer's own known-spell-cap + per-day-cap validation shape, since Oracle is spontaneous unlike Cleric/Wizard/Arcanist/Warpriest's prepared shape; reuses Cleric's own spell-list content for the shared `SPELLLIST:2\|Cleric\|Oracle` portion) + Mystery (Life Mystery's Healing Hands, flat `+4 Heal` bonus) + Curse (Clouded Vision, flat 30-ft vision cap) all genuinely grounded, each narrowed the same three-branch way Warpriest's Blessing was. Class-skill list confirmed clean (no Climb/Intimidate/Swim — no bug here, same shape as Wizard/Arcanist). **Unlike Arcanist, stays permanently Blocked** on `other_features_deferred` (Cure Wounds/Inflict Wounds spontaneous conversion, Tongues, the other 9 Mysteries, the other 4 Curses) — proven via a dedicated test asserting the Blocked status even with everything else recognized, not assumed. `named_features_wired = 2` (Mystery + Curse slots; known-spell posture/Orisons fold into the general mechanism, same reasoning as Arcanist's Cantrips/Warpriest's Orisons). Committed `25c5bcae`, lead re-verified against the real HEAD: 474/474 lib, `sd24_apg_class_coverage_audit` 3/3, all 14 Oracle-related tests individually confirmed including the permanently-Blocked proof |
...
| Investigator | 3/4 | d8 | **Blocked** — real progress: full build, not a single ability. Real prepared caster shape (same simpler shape as Wizard/Arcanist, `SPELLSTAT:INT MEMORIZE:YES SPELLBOOK:YES`) but spellcasting deliberately deferred to its own follow-on slice (reuses the unbuilt Alchemist formula list — confirmed zero such list exists anywhere, 104 real raw records exist as source). Chassis table already existed pre-closure; this closure only widened the gate. Trapfinding (`max(level/2,1)`) + Trap Sense (`level/3`, no floor) + Inspiration pool-size (`max(1,level/2+INT)`) all genuinely grounded — the swapped-floors hazard (Investigator's Trapfinding has the floor Slayer's lacks, and vice versa for Trap Sense) correctly avoided, verified independently against Investigator's own corpus record rather than copied from Slayer. Investigator is the roster's first 2-of-3 partial class-skill match (Climb+Intimidate yes, Swim no) — forced a genuine architectural change: `selected_skill_class_skill_bonus_applies` split into three independent per-skill functions, behavior-preserving for all 6 existing classes (confirmed via the full regression suite), the only correct model for a partial match. Caught and fixed a real regression in an existing QA-owned test before shipping (stale assertion text). `named_features_wired = 3`. Stays Blocked on `other_features_deferred`. Committed `46eb6c3d`, lead re-verified against the real committed HEAD: 488/488 lib, `sd24_acg_class_coverage_audit` 3/3 ("nine" closures), all 6 Investigator-specific tests, the 9-test skill-bonus regression suite, desktop 215/215 (unchanged) |
...
| Skald | 3/4 | d8 | **Blocked** — real progress: Inspired Rage (self-application inferred, mirrors Barbarian's Rage) AND real spontaneous spellcasting (known-spell posture + access ladder/per-day/save-DC, reusing Bard's spell list and pure lookup functions directly — byte-identical tables, 2-source verified) both genuinely grounded; still Blocked on 18 other unbuilt named features (Bardic Knowledge, Rage Powers, Raging Song, Spell Kenning, and the rest — Skald has none of Bard's ~20-feature prior investment, confirmed not expected to reach full Computed this cycle). First APG/ACG class with any real pillar-integration work, committed `d2eb0798` (Inspired Rage) + `7e7f6fbd` (spellcasting), lead re-verified against the real HEAD: 428/428 lib, coverage audit 3/3 |
...
| Warpriest | 3/4 | d8 | **Blocked** — real progress: full build, not a single ability. Real prepared spellbook (own independently-verified per-day table — matches Cleric's exactly at levels 1-2, genuinely diverges from level 3 on, even though spell-list content/casting shape are shared via `SPELLLIST:1|Cleric`) + Blessings (flat uses-per-day `level/2+3`, save DC `level/2+10+WIS`) + Sacred Weapon (base-damage-die formula, honestly near-zero at level 1) + Destruction Blessing's Destructive Attacks (self-application-only activation-gated minor power, mirroring Cleric's Touch of Good) all genuinely grounded. Also caught and fixed a real class-skill-bonus widening bug: `selected_skill_class_skill_bonus_applies` was missing Warpriest despite its real corpus class-skill list including Climb/Intimidate/Swim (mirror image of a historical Wizard false-positive bug). Stays Blocked on `other_features_deferred` alone (19 other Blessing types, Sacred Weapon's active enhancement mechanic, Fervor, Channel Energy, Sacred Armor, and more) — confirmed via the actual `HeadlessReceiptStatus::Blocked` test. Committed `03bb35d3`, lead re-verified against the real committed HEAD: 451/451 lib, `sd24_acg_class_coverage_audit` 3/3 ("six" closures), all 8 Warpriest-specific tests individually confirmed, skill-bonus regression tests confirmed clean |
