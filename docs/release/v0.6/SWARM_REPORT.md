...
**UPDATE (2026-07-24, ~22:45 ET) — class 1 of 24 (Ranger) landed and independently verified.** Given the scale of this decision, one adversarial review was run on the scoping plan before greenlighting continuous implementation — it found a real bug: the plan's "Ranger has zero self-block" claim was wrong in a way that risked a false-positive `Computed` status for levels 2+ while Ranger's spellcasting was genuinely uncomputed. Backend fixed all 4 findings (real spell-posture diagnostic added, Paladin confirmed unaffected, multiclass combos tested, plan/comment corrected). Lead independently verified every claim rather than trusting the report — 4296/4296 lib, 212/212 desktop, all 4 review requirements confirmed by directly running the relevant tests. Continuing through the remaining 23 classes per the operator's standing directive, no pause for further approval. The earlier split of "8 CRB classes = future epic" vs. "non-CRB classes = just add wealth data" was itself a shortcut — non-CRB classes need full chassis work too, not a wealth number standing in for it. This is now the swarm's primary active work, not backlog. See risks-and-open-questions.md item 8 for the corrected scope and framing.
