...
| Agent | Status | Detail |
|---|---|---|
| backend | working | Landed Investigator's Poison Resistance + Alchemy pre-slice (task #8, `named_features_wired` 3→5, `fb3c08b0`) — re-derived Poison Resistance directly from the raw corpus rather than trusting scout's own scoping doc, and found the doc's claimed tiers wrong (real gates are levels 2/5/8/10, not 1/2/3/20; no `+8` tier; immunity at 10 not 20). Lead independently confirmed the correction against the raw corpus before accepting it; scout's doc fixed. Lead re-verified against real committed HEAD: 557/557 lib, `sd24_acg_class_coverage_audit` 3/3, all 7 Investigator tests. Now scoping the spellcasting subsystem (the ~104-record Alchemist formula spell list, `alchemist_spell_list.rs`) before writing code |
...
| Investigator | 3/4 | d8 | **Blocked** — real progress: full build. Real prepared caster shape (same simpler shape as Wizard/Arcanist, `SPELLSTAT:INT MEMORIZE:YES SPELLBOOK:YES`), Trapfinding/Trap Sense/Inspiration pool-size, and now Poison Resistance + Alchemy (task #8 pre-slice) all genuinely grounded. Poison Resistance's real progression (re-derived directly against `InvestigatorPoisonLVL`'s own `PREVARGTEQ:InvestigatorLVL,2/5/8/10` gates, correcting an earlier secondary-source scoping error): None below level 2, `+2` (2-4), `+4` (5-7), `+6` (8-9), full immunity at 10+ — no `+8` tier exists, immunity is at level 10 not 20. Alchemy grounds a flat `+level` Craft-alchemy competence bonus (same shape as Bard's Bardic Knowledge). Spellcasting subsystem (reuses the still-unbuilt Alchemist formula list, 104 real raw records) remains the one deliberately deferred piece — in progress. `named_features_wired = 5`. Committed `46eb6c3d` (chassis) + `fb3c08b0` (Poison Resistance + Alchemy), lead re-verified against the real committed HEAD: 557/557 lib, `sd24_acg_class_coverage_audit` 3/3, all 7 Investigator-specific tests |
