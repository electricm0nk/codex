...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 22 of 27 total classes still need real work to actually reach Computed (Fighter/Wizard/Rogue/Ranger/Paladin now do) | Backend | CRB/APG/ACG | in progress | PALADIN GENUINELY REACHES COMPUTED (ee3c50ce) -- lead-verified 8/8 new tests + 307/307 lib + 212/212 desktop; real spell list (45 entries) spot-checked against raw corpus (Bless: CLASSES:Cleric,Paladin=1, exact match); diagnostic-ordering fix confirmed directly in code (validation runs before the single-class/Human gate, proactively avoiding the exact structural flaw the Ranger review found -- this time caught before shipping, not after). 29 QA-owned tests/** files (30 tests, one root cause) need updating -- backend offered to draft the diffs for QA to review/adopt, lead endorsed it. Scaling pattern flagged: QA cleanup size tracks each class's pre-existing test depth, not backend's fix size -- may become the pacing bottleneck for some remaining classes. ACG's 10-class BAB/save/HP dispatch also landed (71cd41b6), spot-verified via git show (HD values, multiclass-loophole avoidance), full test verification pending until the shared checkout stabilizes. Backend now drafting the 29-file Paladin test cleanup directly for QA to review |
