...
| Sub-task 6: frontend equipment-modifier-attachment UI | UI | CRB | in progress | design done (Gear tab "Attach Modifier" button, reuses `ItemPickerModal` scoped to Equipmods); sent backend a concrete `attach_equipment_modifier` command proposal, holding on UI code until it lands (no-stub discipline) |
