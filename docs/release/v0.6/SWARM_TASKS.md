...
| Sub-task 1: `applied_modifiers` schema field on `EquipmentSelection` (~67 construction sites, 42 QA-owned test fixtures) | Backend | CRB | in progress | 797bd436, lead-verified 262/262 lib + 204/204 desktop; 25 non-QA sites done, 42 QA sites still need the mechanical fix (expected, `cargo build --tests` fails until QA lands theirs) |
| Sub-task 2: per-weapon attack-bonus resolution via `applied_modifiers` (closes item 1's 2+-weapon gap) | Backend | CRB | queued | depends on sub-task 1's non-QA half, which is done; backend standing by on whether to start in parallel with QA's fixture fix or wait |
...
| Sub-task 4: corpus-aware `Computed` verdict, replacing the exact-posture gate (item 27) | Backend | CRB | blocked | depends on sub-task 1; operator decided full replacement, not additive -- Fighter/Wizard/Rogue posture must re-verify identically through the new path |
| Sub-task 5: command-layer wiring (create_character/level_up_character/etc.) | Backend | CRB | blocked | depends on sub-task 4; highest-risk step, needs fullest test coverage |
| Sub-task 6: frontend equipment-modifier-attachment UI | UI | CRB | blocked | depends on sub-task 1; sub-task 2 has no user-facing value without this |
