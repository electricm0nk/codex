...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 10 of 27 classes genuinely reach Computed (Fighter/Wizard/Rogue/Ranger/Paladin/Barbarian/Sorcerer/Cleric/Druid/Bard), Monk has 3 of 7 restricted-list feats closed, Skald and Bloodrager (both ACG) each have one real ability grounded but stay Blocked on deferred spellcasting, 15 classes remain fully untouched beyond chassis dispatch | Backend | CRB/APG/ACG | in progress | All 11 CRB classes now have at least some real engine work landed, 10 of them fully Computed. First 2 APG/ACG class-specific closures landed: Skald's Inspired Rage (`d2eb0798`) and Bloodrager's Bloodrage (`15560e62`), both lead-verified against committed HEAD (406/406 lib, zero workspace regressions). Real product-reachability gap found by frontend (2026-07-25): Sorcerer/Cleric/Druid's Computed posture needs a class choice (bloodline/domain/nature-bond) the real creation UI currently has no way to submit -- pending frontend's live-verify confirmation before risks-and-open-questions.md gets the writeup. Monk's remaining 4 restricted-list feats and the other 14 untouched APG/ACG classes' skill-lists/features/spellcasting are the largest remaining bucket. Detailed per-class/race table maintained in SWARM_REPORT.md |
