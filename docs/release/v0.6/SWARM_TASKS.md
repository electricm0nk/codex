...
| Sub-task 1: `applied_modifiers` schema field on `EquipmentSelection` (~67 construction sites, 42 QA-owned test fixtures) | Backend | CRB | queued | dispatched, coordinate the mechanical update with QA |
| Sub-task 2: per-weapon attack-bonus resolution via `applied_modifiers` (closes item 1's 2+-weapon gap) | Backend | CRB | blocked | depends on sub-task 1 |
