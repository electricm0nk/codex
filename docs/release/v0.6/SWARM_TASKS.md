...
| Sub-task 4: corpus-aware `Computed` verdict, replacing the exact-posture gate (item 27) | Backend | CRB | in progress | SCOPE GROWN 2026-07-24: the gated functions are hardcoded arithmetic, not corpus-resolved math -- a naive gate swap would serve fabricated AC/attack-bonus/skill numbers for any newly-accepted loadout. Backend proposed, lead approved: move `baseline_armor_class`/`baseline_melee_attack_bonus`/selected-skill-modifiers to real corpus-resolved computation first (byte-identical re-check against today's fixed posture as the correctness proof), then swap the gate. Sequencing: AC/attack-bonus pillar, then skill pillar, then command-layer gate swap |
...
| Sub-task 6: frontend equipment-modifier-attachment UI | Backend+UI | CRB | in progress | backend half done (3a05539a, `attach_equipment_modifier` command + DTO + persistence-writer bugfix; lead-verified 4284/4284 + 211/211); frontend now clear to wire the Gear tab |
