...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 10 of 27 classes genuinely reach Computed via the compute engine (only 7 of those 10 reachable through the real product UI today -- Sorcerer/Cleric/Druid need a choice picker that doesn't exist yet, confirmed live by frontend), Monk has 6 of 7 restricted-list feats closed (only Deflect Arrows remains), Skald/Bloodrager/Brawler/Hunter (all ACG) each have one real ability grounded but stay Blocked on other deferred features, 13 classes remain fully untouched beyond chassis dispatch | Backend | CRB/APG/ACG | in progress | All 11 CRB classes now have at least some real engine work landed, 10 of them fully Computed. Four APG/ACG class-specific closures landed: Skald's Inspired Rage (`d2eb0798`), Bloodrager's Bloodrage (`15560e62`), Brawler's AC Bonus (`19c792e1`, first always-on/no-gate shape), Hunter's Animal Companion (`21c5b13a`, first closure reusing an earlier closure's own code -- Druid's Wolf stat block). Monk's remaining-3-feats closure (`e45b622f`) landed after a full adversarial review revised the original plan -- Combat Reflexes/Scorpion Style/Improved Grapple all closed via groundable static numbers (no new engine needed), and the review found Improved Grapple could reuse an existing precedent (Dwarf Stability's CMD-bonus idiom) rather than needing a brand-new CMB/CMD pillar, a genuine scope reduction. All lead-verified against committed HEAD, zero workspace regressions. Product-reachability gap confirmed real and written up in risks-and-open-questions.md (frontend, `833ea89c`) -- not yet scoped/assigned as new work. Only Deflect Arrows (needs a genuinely new opponent-interaction engine) and 13 untouched ACG/APG classes remain in this bucket. Detailed per-class/race table maintained in SWARM_REPORT.md |
