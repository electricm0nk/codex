...
| Sub-task 4: corpus-aware `Computed` verdict, replacing the exact-posture gate (item 27) | Backend | CRB | in progress | pillars built (uncommitted): `compute_combat_baseline_from_corpus` + `compute_selected_skill_modifiers_from_corpus`, armor/shield-only widening; lead-verified 8/8 new tests including the byte-identical correctness proof across Fighter/Wizard/Rogue at every training breakpoint; not yet committed, not yet wired to any command (that's sub-task 5) |
