...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 22 of 27 total classes still need real work to actually reach Computed (Fighter/Wizard/Rogue/Ranger/Paladin now do) | Backend | CRB/APG/ACG | in progress | Barbarian's combat-time activation-state build in progress (schema landed, pillar integration next), one adversarial review completed and all 6 findings addressed. Sorcerer (c23d4054), Cleric (fca4e64e), and Druid (dda46d4a) all landed and lead-verified: 344/344 lib + 212/212 desktop at Druid's landing, 169-entry spell list count and 3 spot-checks exact, gate-ordering fix confirmed in code; all three correctly stay Blocked (bloodline/domain-powers/nature-bond diagnostics permanent). QA test targets: Sorcerer's 27 files landed and lead-verified 100% (76fe82da); Cleric's 28 and Druid's 17 queued next, in that order. Bard's spell-list survey done and lead-verified exact (164 spells); backend building schema for Barbarian first, Bard's own spell posture still pending. 3 previously-unknown stale ge06_* negative-control tests found and flagged (Cleric now recognized by table_class_id broke their "unsupported class" premise) |
