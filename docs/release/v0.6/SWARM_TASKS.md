...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 23 of 27 total classes still need real work to actually reach Computed (Fighter/Wizard/Rogue/Ranger now do) | Backend | CRB/APG/ACG | in progress | RANGER GENUINELY REACHES COMPUTED (b7642d97) -- lead-verified 8/8 new tests + 293/293 lib, plus a scratch-verified positive-case gap closed and flagged (no permanent test proved a valid non-empty prepared spell reaches Computed, only empty+invalid cases). Real spell-list data spot-checked against raw corpus. APG's 6-class dispatch (c511c132) still correctly Blocked, not done. Backend now on Paladin, proactively fixed the same structural flaw class before shipping this time. Frontend's characterHubModel.ts should update Ranger's support level once backend confirms the real safe level range |
