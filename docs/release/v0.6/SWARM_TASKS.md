...
| Class/multiclass breadth: 8 of 11 CRB classes have no working chassis at all (Cleric/Druid/Bard/Sorcerer/Barbarian/Monk/Paladin/Ranger) | Backend | CRB | queued | OPERATOR GO-AHEAD 2026-07-24 (risks item 8); backend to produce a scoping/sequencing plan first (not a bounded task), queued after sub-task 5 + item 7's wealth work |
| Starting wealth for the non-CRB classes in the operator's table (10 named, "12" count needs reconciling against the real d20pfsrd source first) | Backend | APG/ACG | queued | OPERATOR GO-AHEAD 2026-07-24 (risks item 7); needs class-id recognition + wealth-table entries per class, no full chassis required; queued after sub-task 5 |
