...
| Full class/race chassis breadth, CRB+APG+ACG (Bestiary has no classes) -- 22 of 27 total classes still need real work to actually reach Computed (Fighter/Wizard/Rogue/Ranger/Paladin now do) | Backend | CRB/APG/ACG | in progress | Paladin's 29-file QA cleanup committed (c1844765), lead-reverified identical results post-commit. Only remaining gap: sd24_acg_class_coverage_audit.rs needs the same update APG's got. Backend continuing the class sequence (Barbarian/Monk/Cleric/Druid/Sorcerer/Bard queue) |
