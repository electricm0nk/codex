...
**The rule this addendum most needs to survive.** A single aggregate coverage percentage may not be published alone; the `computed`-class figure must appear beside it. Measured: of `core_rulebook`'s 4,743 held units, 777 (16.4%) genuinely require bespoke wiring, 3,844 are reachable by three mechanical checks, and 122 must first be disambiguated. An aggregate that mixes them lets the cheap majority bury the expensive minority and read as near-complete while every hard record is untouched. This is the existing `Minimum truth rules for aggregated views` prohibition — no summary may imply a green state the evidence does not support — restated for a new axis.
