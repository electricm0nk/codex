...
- The operator's loop-instruction files at `~/workspace/SD-NN-...-loop-instruction.md` are the editor of record; the repo-local copies here are a one-way sync from them. Dispatch is via the `Workflow` tool invoked from a live session (per `docs/governance/loop-instruction-template.md §2`), not `/loop /batch` — the latter cannot run unattended.
