...
- **Epic 4 — Closure Epilogue**: `programs/codex/requirements/SD-21-campaign-manager-and-persistence/release-notes.md` (NEW; generated by the loop's release-notes generator) + the closure PR itself (the loop runs `gh pr create` + worktree cleanup + branch sweep). No code surface.
