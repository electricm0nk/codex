...
| `epic-3-pi-gate` | COMPLETE | PI-Screening Provenance Gate | per-class PI-blacklist sweep (SD30-E3-F1, COMPLETE) + declared-PI reader wired into class_feature ingest (SD30-E3-F2, COMPLETE, `decisions.md §39`/`§53`) + corpus-wide declared-PI backfill (SD30-E3-F3, COMPLETE — `transcribe_monster_tables.py`/`transcribe_companion_tables.py` both gained NAMEISPI-drop/DESCISPI-redact, zero live corpus exposure re-confirmed, see `progress.md` cycle `SD30-E3-F3-001`) + regression gate (SD30-E3-F4, COMPLETE — `tests/sd30_declared_product_identity_in_shipped_class_features.rs`, proven RED-then-GREEN against real shipped output, `VERIFY_EXIT=0`/16-16 PASS confirmed, `decisions.md §54`, `progress.md` cycle `SD30-E3-F4-001`) — all four F1-F4 sub-scopes confirmed on `tranche/10` by content (symbol grep, not card status); `epic-3-pi-gate`'s own gating relationship fully discharged for SD-30's scope, inherited by `SD-31-corpus-closure-grind`'s `epic-3-chassis-sweep` | sd30-e3-f4-regression | 2026-08-14T23:54:00Z | `SD30-E3-F4-001` |
...
| `epic-8-code-review` | COMPLETE (review recorded, both surviving findings verified by content and fixed-in-bundle — including a third, deeper defect the second finding's own investigation surfaced in `gen_book_cache.rs`'s generator, live in shipped `inner_sea_gods` monster data, now fixed and regenerated; `./scripts/verify.sh` `VERIFY_EXIT=0`, 16/16 stages; `v06_corpus_trap_report -- --audit` re-derived `TRAP_AUDIT_EXIT=0`; on `tranche/10` at `fc461781a`) | Bundle Code Review | full-bundle diff review vs. branch point (`decisions.md §26`) | sd30-e8-triage | 2026-08-15T02:52:00Z | `SD30-E8-F3-001` |
| `epic-9-closure` | COMPLETE (both `SD30-E9-F1-001` blockers resolved and re-confirmed by content: `epic-7-version` green at `4630fec2`, `epic-8-code-review` green at `fc461781a`; DoD-3 `v06_corpus_trap_report -- --audit` re-derived `TRAP_AUDIT_EXIT=0`; full gate green at closing tip `44497b67e`, `VERIFY_EXIT=0`, 16/16 stages, `progress.md` `SD30-E9-F2-001`; tranche promotion PR opened, not merged — operator merges personally) | Closure Epilogue | tranche promotion PR | sd30-e9-closure | 2026-08-15T03:30:00Z | `SD30-E9-F2-001` |
