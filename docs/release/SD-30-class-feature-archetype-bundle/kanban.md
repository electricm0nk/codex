...
| `epic-3-pi-gate` | IN-FLIGHT (SD30-E3-F1/F2/F3 sub-scopes COMPLETE; F4 work delivered and proven this cycle — `SD30-E3-F4-001` — gate exit code pending, see `progress.md`) | PI-Screening Provenance Gate | per-class PI-blacklist sweep (SD30-E3-F1, COMPLETE) + declared-PI reader wired into class_feature ingest (SD30-E3-F2, COMPLETE, `decisions.md §39`/`§53`) + corpus-wide declared-PI backfill (SD30-E3-F3, COMPLETE — `transcribe_monster_tables.py`/`transcribe_companion_tables.py` both gained NAMEISPI-drop/DESCISPI-redact, zero live corpus exposure re-confirmed, see `progress.md` cycle `SD30-E3-F3-001`) + regression gate (SD30-E3-F4 — `tests/sd30_declared_product_identity_in_shipped_class_features.rs`, proven RED-then-GREEN against real shipped output, `decisions.md §54`, `progress.md` cycle `SD30-E3-F4-001`) — F4's own DoD item 1 (`verify.sh` exit code) not yet obtained at cycle-return time; flip this card to COMPLETE once a resumed cycle confirms `VERIFY_EXIT=0` | sd30-e3-f4-regression | 2026-08-14T23:54:00Z | `SD30-E3-F4-001` |
