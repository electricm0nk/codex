...
| `epic-13-verdict-paths` | READY | Verdict-Path Capability, 100% mandate | real (non-placeholder) verdict paths for the ~3,547 unmeasurable units incl. 2,109 `ambiguous`; classifier work bound by `SD-30-class-feature-archetype-bundle/decisions.md §50(c)`'s accuracy-not-movement rule (`decisions.md §45`) | — | — | — |
