...
22), not a partial ingest to attempt.**
...
`OPEN-ISSUES.md` row 22, needs a domain ruling before it is safe to attempt.**
...
with the transcription. Worked example in `OPEN-ISSUES.md` row 23:
...
introduced here. **Logged as `OPEN-ISSUES.md` row 23 — the single highest-leverage lever a future
...
### 5. `monster_ability` — investigated, did not ingest unsafely (`OPEN-ISSUES.md` row 24)
...
`git status --porcelain` clean. Logged in `OPEN-ISSUES.md` row 24 as a `wiring_class`-cache-drift
...
Epic 6-F1's card, a different kind). `OPEN-ISSUES.md` row 24 names both as concrete follow-ons
...
- Investigated why only 9 of 120 candidate units promoted rather than declaring "1,173 enriched"
  ...
  defect in 101 of them (§4, `OPEN-ISSUES.md` row 23), the single highest-leverage follow-on this
  cycle located.
...
- Did NOT hand-add `MonsterAbilityRecord` entries under time pressure once the `spell` lever ran
  ...
  and logged both precisely instead (§5, `OPEN-ISSUES.md` row 24).
