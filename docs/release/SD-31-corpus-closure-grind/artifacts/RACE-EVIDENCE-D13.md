...
`core_essentials/races/dwarf/dwarf_abilities_race.lst:21` (Dwarf's base declaration,
unmodified by any citing book; corrected from `:15` -- SD31-W13-INTEGRATE-001 review found
that coordinate cites a blank line, apparently transposed from the row's own
`SOURCEPAGE:p.21` -- the quoted content below is verbatim correct at :21):
