...
1. **`epic-1-race-chassis` → `epic-6-ingest-lanes` F3/F4.** No `race` or `race_trait` ingest cycle
   ...
   **Open, batch 1 (SD31-E1-F1-001, 2026-08-15): Fetchling, Grippli, Ifrit, Oread, Sylph, Undine
   ...
   (Bestiary 2) and Skinwalker (Bestiary 5) and every other unmodelled race — a book mixing modelled
   and unmodelled races is only PARTIALLY open, per-record, not claimable whole.**
