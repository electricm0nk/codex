...
| `epic-2-verdict-paths` | READY | Verdict-Path Capability, 100 % mandate | hand-labelled ground-truth sample (gate) → classifier build/accept or close-at-F1 → **`ambiguous` dead-end closed or registered**. **SD31-E2-F1 landed 2026-08-15 (`sd31/e2-groundtruth`, merged onto `tranche/11`); repaired 2026-08-15 by `SD31-E2-F1-002-relabel` (branch `worktree-wf_49e8e5da-ca5-2`, not yet merged — awaiting integration cycle). `OPEN-ISSUES.md` row 3's `BLOCKER` is RESOLVED: all 105 canned units re-labelled from the real corpus record (103 confirm the engine's verdict with genuine evidence, 2 disagree — new Findings D/E); `0` canned strings remain. Sample widened 150→185 units (row 5: 45/31-thin cells → 48/29-thin cells — improved, not fully closed, still not enough for F2's per-cell rates on the thinnest cells). Row 4 resolved for future draws only (`scripts/sample_ground_truth_units.py` committed and used for the widening draw); the original 150-unit v1 draw stays permanently non-reproducible. New machine-checkable guard `scripts/ground_truth_evidence_guard.py` (+ self-test) catches this exact defect shape in future; found a smaller residual gap in the untouched 45 (`OPEN-ISSUES.md` row 8, not fixed — barred from re-opening those 45) and is therefore not yet wired into `verify.sh`'s default stages (row 9). Findings D/E/F (new, `OPEN-ISSUES.md` row 10) extend A/B/C. Decision 1(e) item 4's "close at F1" path is now unblocked on evidence quality; F1 itself is genuinely closable pending operator/Epic-2-owner sign-off. F2 (classifier build) still not dispatched — its own acceptance needs row 5's residual thin cells addressed first per this cycle's honest reporting.** | sd31-e2-groundtruth / sd31-w1-integrate / sd31-e2-relabel | 2026-08-15 | `SD31-E2-F1-001` / `SD31-E2-F1-002-relabel` |
