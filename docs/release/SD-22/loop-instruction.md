...
4. **Decision record** at `./decisions.md` — the 5-item decision record (§1 scope, §2 tranche/5 + codex-tranche-5, §3 deferred shape decisions, §4 Epic 9 — Closure Readiness added 2026-07-19, §5 corpus sourcing corrected 2026-07-19); tells you *why* the bundle is shaped this way.
5. **Sibling doctrines** at `../../doctrine-external/spec-domain-lifecycle.md`, `../../doctrine-external/identifier-discipline.md`, and `../SD-21/decisions.md §18` (the build-version scheme `<major>.<tranche-base>.<build>` amendment). (Corrected 2026-07-19: all five paths in this list previously pointed at `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/` and `governance/` — neither exists inside a git clone of `codex`; only `docs/release/SD-22/` and `docs/doctrine-external/` do. A cold session following the literal paths as written would have hit "file not found" on every one of them.) **Further corrected 2026-07-20**: the 2026-07-19 note above overcorrected. `governance/` genuinely doesn't exist in this repo (it's operator-workspace-only, `/home/workspace/governance/`), but this repo's own `programs/codex/requirements/` directory *does* exist and is git-tracked (see `SD-13`, `SD-16`, `SD-17`, `SD-21` subdirectories) — it's a distinct, narrower thing from the operator's out-of-repo `/home/workspace/programs/codex/requirements/` path that `docs/release/README.md`'s front matter describes as the "upstream authority root." This repo's own `programs/codex/requirements/<bundle-slug>/release-notes.md` was, at that point, believed to be the correct location for `release-notes.md`. **Corrected a third time, same day (2026-07-20)**: per a full repo-wide docs-structure consolidation, `release-notes.md` was relocated *again* — this time to `docs/release/SD-NN/release-notes.md` for good, with the regex-locked CI/schema contract itself updated (`^docs/release/[^/]+/release-notes\.md$` now, not the `programs/codex/requirements/` pattern above) across every schema, script, workflow, and app-code reference. See that consolidation commit's message and `docs/release/README.md`'s Cross-reference section for the full history.
...
| `docs/release/SD-22/release-closure-checklist.md` (relocated 2026-07-20 from a stray `docs/SD-22/` path) | NEW; Epic 8 criterion 29 cycle writes the four-step closure-process checklist using the `<major>.<tranche-base>.<build>` triple (per-position increment rules: build per-CI-build, tranche per-tranche-promotion, major per-main-publish). | One cycle at a time. |
| `docs/release/SD-22/release-notes.md` | NEW; Epic 7 criterion 25 cycle generates release notes. **Corrected twice, 2026-07-20**: this row briefly said `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/` was correct — that was itself superseded by a full docs-structure consolidation relocating `release-notes.md` to `docs/release/SD-NN/` for every bundle, CI/schema contract included. Required sections (locked by `tools/release/check_release_manifest.py`'s `REQUIRED_NOTES_SECTIONS`): Summary, User-Visible Changes, Defects Fixed, Operational Notes, Verification Evidence, Known Issues, Update Eligibility — not the epic-oriented "New content / DM toolkit / Maintenance / Versioning" shape this row originally specified. | One cycle. |
...
- `./decisions.md` — 5-item decision record (§1 scope, §2 tranche/5 + codex-tranche-5, §3 deferred shape decisions, §4 Epic 9 — Closure Readiness added 2026-07-19, §5 corpus sourcing corrected 2026-07-19 + /batch deferred).
...
- `./corpus-source-inventory.md` — per-content-unit routing (Rust module / test / cycle artifact / RuleSetId); "Content shape" prose columns are illustrative only, not authoritative (see its corrective banner).
- `./ingest.md` — the per-cycle ingest pipeline for Epic 3/4/5/6 (direct real-`.lst` transcription; no corpus-loader abstraction).
