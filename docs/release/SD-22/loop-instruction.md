...
| `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/release-notes.md` | NEW; Epic 7 criterion 25 cycle generates release notes. **Corrected 2026-07-20**: this file lives under `programs/codex/requirements/`, not `docs/release/SD-22/` — see the corrected note in this file's "Required reading" item 5 and `epic-breakdown.md` criterion 25. Required sections (locked by `tools/release/check_release_manifest.py`'s `REQUIRED_NOTES_SECTIONS`): Summary, User-Visible Changes, Defects Fixed, Operational Notes, Verification Evidence, Known Issues, Update Eligibility — not the epic-oriented "New content / DM toolkit / Maintenance / Versioning" shape this row originally specified. | One cycle. |
