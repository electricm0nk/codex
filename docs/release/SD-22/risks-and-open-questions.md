...
| A cycle can't reach the LST corpus tree (cloud sandbox with only `codex` cloned, no PCGen source added) | `src/pcgen_import` lookup at the documented path (`decisions.md §5`) finds nothing | Self-healable if the session can add `https://github.com/PCGen/pcgen` as a second git source and retry; only routes to Open Blockers if neither the local sibling repo nor a cloned public mirror resolves |
...
### Judgment-1 — Epic 1's grep-audit pattern flags the bundle's own approved `tests/sd22_*.rs` naming convention in doc-comment citations
- Surfaced by: Epic 9 cycle on `2026-07-20T00:00:00Z` during eval cycle #1 (this cycle, closure-readiness eval)
- Criterion nearest to the observation: criterion-1 (E1.1, source-code identifier audit)
- Why Epic 9 didn't self-heal: `grep -rE "sd22_|SD22_|Sd22|SD-22-[A-Z][0-9]" apps/desktop/ apps/desktop/src-tauri/ src/rules_core/` returns 24 hits, not the "zero hits" the criterion's literal wording calls for. Every single hit is a rustdoc-comment citation of the shape `` `tests/sd22_<book>_<class_or_subset>_resolves.rs`'s ``, inside `rules_tables/{apg,acg,beastiary1}/*.rs` module doc comments, naming the test file that exercises that module — using `corpus-source-inventory.md`'s own mandated `test_fixture_path` naming convention (`tests/sd22_apg_class_alchemist_resolves.rs`, etc.), which didn't exist yet when E1.1's original zero-hit baseline ran (before Epic 3 landed). Targeted re-greps for every leak category the criterion's own scope doctrine actually names (Tauri command names, TS `Sd22`/`SD22_` identifiers, `data-testid="sd22-...`, `SD-22-Ex...` audit-comment IDs, `t_[a-f0-9]{8,}`/`AV-PAY-[0-9]+` tokens) all return zero hits — the substantive identifier-discipline concern is genuinely clean. But whether the criterion's exception clause (which today names only `decisions.md`/`epic-breakdown.md` prose as exempt) should also exempt doc-comment citations of the bundle's own approved test-file names is a scope-of-record call, not a mechanical fix — renaming ~24 doc comments (or, worse, renaming the `tests/sd22_*.rs` files themselves, which would contradict `corpus-source-inventory.md`'s load-bearing routing table) is exactly the kind of "technically flagged but reads as correct behavior" case this section exists for.
- Recommended next-SD action: either (a) amend `epic-breakdown.md` criterion 1's exception clause to explicitly exempt doc-comment citations of `tests/sd<N>_*.rs`-shaped file names (mirroring the existing `decisions.md`/`epic-breakdown.md` prose exception), or (b) if the operator wants doc comments to stop citing test file names by their literal path, do that as a small, explicit follow-up identifier-cleanup slice in the next SD's Epic 1.
- Status: deferred

### Judgment-2 — Epic 5 (Bestiary 1) landed 8 of a stated "default 8-12" monster-block-subset range; is 8 sufficient for closure?
- Surfaced by: Epic 9 cycle on `2026-07-20T00:00:00Z` during eval cycle #1 (this cycle, closure-readiness eval); originally self-flagged (not self-resolved) by the subset-08 cycle itself (`artifacts/beastiary1/subset_08_cycle_receipt.md`, `progress.md` lines ~3105-3120)
- Criterion nearest to the observation: criteria 14-17 (Epic 5, Bestiary 1 content-source ingest)
- Why Epic 9 didn't self-heal: `acceptance-and-verification.md` line 101 states criterion 15's verification is "per-subset artifacts (default 8-12)". 8 subsets (41 monsters) meets the stated floor of that range but not the ceiling, and nothing in `acceptance-and-verification.md` / `epic-breakdown.md` / `loop-instruction.md` states which end of the range constitutes "done" for closure purposes. All 8 landed subsets have clean artifact evidence (Red-phase, Green-phase, Files touched, Cycle metadata all present per `corpus-source-inventory.md §6`'s contract — verified this cycle) and criterion 17 (DM-toolkit consumption) has been satisfied since subset 01, so this is not a shortfall in the artifact-evidence sense Epic 9 self-heals; it is a scope-boundary judgment call ("is the floor of a stated range good enough, or does the bundle need the ceiling") that only the operator/orchestrator can make. 10 unused, real, non-parenthetical CR-3 monster names remain on-corpus (Lion, Ogre, Pegasus, Rust Monster, Shadow, Unicorn, Violet Fungus, Wasp Swarm, Wight, Yeth Hound) if the operator elects to continue past the floor.
- Recommended next-SD action: operator/orchestrator makes an explicit closure call on Epic 5 (accept 8 subsets as closed, or direct 1-2 more subset cycles to approach the 12-subset ceiling) before or during Epic 7's final scan. If deferred past SD-22's close, the next SD's Bestiary-1-follow-on epic can pick up subset 09+ using the same 10 unused CR-3 names already enumerated.
- Status: deferred
