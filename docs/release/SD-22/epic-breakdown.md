...
25. **Generate release notes**: produces `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/release-notes.md` — this criterion's path was already correct here, but the Epic 7 cycle that executed it followed `loop-instruction.md`'s file-touch-partition table instead (which wrongly pointed at `docs/release/SD-22/release-notes.md`, corrected 2026-07-20). Required sections are locked by `tools/release/check_release_manifest.py`'s `REQUIRED_NOTES_SECTIONS`, not the epic-oriented shape originally listed here: **Summary**, **User-Visible Changes** (from Epics 3+4+5's new content, Epic 6's DM toolkit), **Defects Fixed**, **Operational Notes** (from Epic 1's maintenance pass, Epic 8's versioning), **Verification Evidence**, **Known Issues**, **Update Eligibility**.
