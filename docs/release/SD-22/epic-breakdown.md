...
- **Epic 3 — APG content-source ingest**: every per-class cycle **first parses the class's real record** from `apg_classes.lst`/`apg_abilities_class.lst`/`apg_equip_*.lst` via `src/pcgen_import/lst_parser/*` (per `decisions.md §5`, corrected 2026-07-19), then writes a class-specific test fixture in red, ingests the class in green, and persists the cycle artifact. Cross-book invariants in `corpus-source-inventory.md` §1.3 are part of the green acceptance.
- **Epic 4 — ACG content-source ingest**: same shape as Epic 3 (LST parse first, per `decisions.md §5`) with `corpus-source-inventory.md` §2 in the red-phase reading set.
- **Epic 5 — Bestiary 1 content-source ingest**: per-monster-block-subset cycle; parses `b1_races.lst`/`b1_abilities_race.lst` first (per `decisions.md §5`); test fixture has one canonical monster per subset (Goblin for subset 1, Kobold for a CR-1 band focused subset, etc.).
...
**Scope doctrine (operational rule):** under SD-19 §9 source-book subdirectory pattern, APG populates `src/rules_core/rules_tables/apg/` as sibling directories to SD-19's `rules_tables/crb/`. The APG cycle's per-cycle landing shape is one APG class's table entries (per-cycle shape mirrors SD-19's per-school loop pattern). **Each per-class cycle begins by parsing the class's real `.lst` record** (`apg_classes.lst` for chassis, `apg_abilities_class.lst` for features) via `src/pcgen_import/lst_parser/*`, per `decisions.md §5` (corrected 2026-07-19) — the `corpus-source-inventory.md` §1.1 row's Content-shape column is illustrative only, not the source of truth (see that file's corrective banner). Each class table includes structured data for that class's features, spells (if any), equipment (if any), and any class-specific race interactions, transcribed from the `.lst` record with source provenance cited in a doc comment.
...
**Scope doctrine (operational rule):** same shape as Epic 3, for the ACG content. Per-class cycle shape; populates `rules_tables/acg/` as sibling directory to `rules_tables/apg/`. Each per-class cycle begins by parsing the class's real `.lst` record (`acg_classes.lst`/`acg_abilities_class.lst`/`acg_equip*.lst`) via `src/pcgen_import/lst_parser/*`, per `decisions.md §5` (corrected 2026-07-19), with `corpus-source-inventory.md` §2.1 as routing (not content) reference.
...
**Scope doctrine (operational rule):** Bestiary 1 populates `src/rules_core/rules_tables/beastiary1/` per SD-19 §9. Per-monster-block cycle shape (one cycle per monster-block subset of the 300+ Bestiary 1 monsters). Each per-monster-block cycle begins by parsing the subset's real `.lst` records (`b1_races.lst`/`b1_abilities_race.lst`) via `src/pcgen_import/lst_parser/*`, per `decisions.md §5` (corrected 2026-07-19), with `corpus-source-inventory.md` §3.1 as routing (not content) reference.
...
25. **Generate release notes**: produces `docs/release/SD-22/release-notes.md`. This criterion's path has been corrected twice on 2026-07-20: first it pointed at `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/` (believed correct at the time, since the Epic 7 cycle that actually executed this criterion had instead followed `loop-instruction.md`'s then-wrong file-touch-partition table pointing at `docs/release/SD-22/`), then a full repo-wide docs-structure consolidation relocated `release-notes.md` — for every bundle, and the CI/schema contract itself — to `docs/release/SD-NN/` for good. Required sections are locked by `tools/release/check_release_manifest.py`'s `REQUIRED_NOTES_SECTIONS`, not the epic-oriented shape originally listed here: **Summary**, **User-Visible Changes** (from Epics 3+4+5's new content, Epic 6's DM toolkit), **Defects Fixed**, **Operational Notes** (from Epic 1's maintenance pass, Epic 8's versioning), **Verification Evidence**, **Known Issues**, **Update Eligibility**.
...
29. **Bump process established**: `docs/release/SD-22/release-closure-checklist.md` (relocated 2026-07-20 from a stray `docs/SD-22/` path) records the four-step process for future closure epilogues (mirroring SD-21's Epic 5 criterion 27). Per-position increment rules: build per-CI-build, tranche per-tranche-promotion, major per-main-publish.
...
3. Parses the cycle's real `.lst` record (content-source ingest cycles only; per `decisions.md §5`, corrected 2026-07-19), then reads the cycle's parity test fixture (for content-source ingest cycles) or the boundary-contract test (for DM-toolkit cycles).
