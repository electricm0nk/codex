...
| E7.22-26 | 7 — Closure Epilogue | closure:* | Final scan (22) clean — every criterion 1-21/27-30 `complete`, criterion 31 `complete`, all historical `## Open blockers` entries resolved. Closure PR opened `tranche/5 → develop` (23, not merged — operator/orchestrator action). Worktree/branch sweep (24): one worktree found (already on `tranche/5`, nothing to remove); five local branches found, all <30 days old, none qualified for deletion — no deletions performed. Release notes generated (25) — **corrected 2026-07-20**: originally landed at `docs/release/SD-22/release-notes.md` in the epic-oriented "New content/DM toolkit/Maintenance/Versioning" shape, following `loop-instruction.md`'s file-touch-partition table rather than `epic-breakdown.md` criterion 25's own (already-correct) path. Relocated to `programs/codex/requirements/SD-22-content-source-ingest-and-dm-toolkit/release-notes.md` (the CI/schema-contracted location, matching SD-16/SD-21's pattern) and rewritten into the 7 sections `tools/release/check_release_manifest.py` actually requires (Summary, User-Visible Changes, Defects Fixed, Operational Notes, Verification Evidence, Known Issues, Update Eligibility), as part of a repo-wide docs-structure cleanup. Version bumped `0.5.95` → `0.5.96` (26) — **corrected 2026-07-20**: this cycle originally landed a tranche-base bump to `0.6.0` in error (misreading criterion 26's "tranche promotion only" clause as applying to the bundle's own closure rather than to an actual new-tranche-branch event); the operator caught it and it was reverted to a build-only increment, with the same sibling-preservation cascade (JS build-version/build-label fixtures, CI workflow stamp) re-corrected in the same pass. See `artifacts/epic_7/closure_epilogue_cycle_receipt.md` (original cycle) and the `## Cycle log` correction entries below. | **complete** | see `## Cycle log` |
...
| E8.29 | 8 — Build Version | version:closure_checklist | `docs/release/SD-22/release-closure-checklist.md` (relocated 2026-07-20 from a stray `docs/SD-22/` path) — four-step version-bump process, mirrors SD-21's E5.27 doc | **complete** — see `artifacts/epic_8/release_closure_checklist_cycle_receipt.md` (Epic 9 self-heal this cycle: added the missing `## Cycle metadata` + `## kanban` sections, and fixed the CI workflow drift the artifact itself had flagged) | (this cycle's commit, see `## Cycle log`) |
...
### cycle-2026-07-20 (correction) | repo-wide docs-structure cleanup | doc:structure | operator-requested | complete

Separate from SD-22's own criteria, the operator asked for a repo-wide
docs-structure cleanup (docs scattered across `docs/`, `docs/release/`,
and `programs/codex/requirements/`). Landed on `tranche/5-1` (branched
from `develop` after SD-22's own PR #325 merged), not `tranche/5` — this
is post-closure repo hygiene, not a new SD-22 criterion, but touches
SD-22's own doc tree so it's logged here for continuity.

**What moved:** `docs/SD-20/boundary-contract.md`, `docs/SD-21/release-
closure-checklist.md`, and `docs/SD-22/release-closure-checklist.md`
(stray one-level-too-shallow paths) relocated into their proper
`docs/release/SD-N/` homes. `docs/release/SD-22/release-notes.md` moved
to `programs/codex/requirements/SD-22-content-source-ingest-and-dm-
toolkit/release-notes.md` (see the E7.22-26 status-matrix row's
correction note above) and rewritten into the CI-contracted 7-section
shape; `programs/codex/requirements/SD-21-campaign-manager-and-
persistence/release-notes.md` (already in the right location, pre-
existing gap) rewritten into the same shape, preserving all of its real
content/commit SHAs.

**What didn't move:** `programs/codex/requirements/SD-13-*/artifacts/`
and `SD-17-*/artifacts/` — these predate the `docs/release/` convention
entirely and are left as frozen historical record; migrating them would
touch ~470 inert doc-comment citations for no functional benefit.

**Doctrine corrected:** `loop-instruction.md`'s 2026-07-19 "corrected"
banner (which said `programs/codex/requirements/` "doesn't exist inside
a git clone of `codex`") was itself imprecise — it conflated the
operator's separate out-of-repo `/home/workspace/programs/codex/
requirements/` path with this repo's own, real, git-tracked `programs/
codex/requirements/` directory, which is exactly where a bundle's
`release-notes.md` belongs (a regex-locked CI/schema contract). Fixed
in `loop-instruction.md`'s required-reading note, its file-touch-
partition table, and `epic-breakdown.md` criterion 25.
