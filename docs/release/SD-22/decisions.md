...
- In a cloud/remote sandbox that only has `codex` checked out: **self-serve** — run `git clone --depth 1 https://github.com/PCGen/pcgen /tmp/pcgen-corpus` via Bash (the public upstream PCGen repo, which the local sibling repo's `origin` remote confirms is the canonical source) and use `/tmp/pcgen-corpus/data/pathfinder/paizo/roleplaying_game/<book>/`. Corrected 2026-07-19 (post-launch retrospective): earlier phrasing described this as a routine-config action ("add a second git source"), which only an external orchestrator can do via the RemoteTrigger API — a bare cloud session with only Bash/Read/Write/Edit/Glob/Grep cannot add a source to its own session. The self-clone approach needs no external setup at all, matching this document's own "self-sufficient, no shared state beyond on-disk files" design goal.
