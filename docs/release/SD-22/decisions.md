...
- *Per-class Epic 1+2 ordering.* APG's 9-class ingestion order (Alchemist → Cavalier → Gunslinger → Inquisitor → Magus → Oracle → Summoner → Witch + any APG printing additions) is operator-pinned at SD-22 cycle launch. ACG's class order (Alchemist → Arcanist → Bloodrager → Brawler → Hunter → Investigator → Shaman → Skald → Swashbuckler → Warpriest + any ACG printing additions) is operator-pinned at SD-22 cycle launch.
...
## 5. Corpus generation is in-bundle; `/batch` deferred (operator directives 2026-07-18)

**Decision (corpus generation — operator directive 2026-07-18):** SD-22's Epic 3/4/5 structured-data input files are **no longer operator-supplied**. Each ingest cycle **generates its own corpus input file** (`corpus/<book>_<unit>.json`, e.g. `corpus/apg_alchemist.json`) from PF1 OGL/SRD content as the **first step of the cycle**, before the RED phase's test fixture is written. This supersedes `technical-requirements.md` §5's operator-supplied posture ("Paizo content sources available"). The generation spec for each unit is the corresponding row of `corpus-source-inventory.md` — its *Content shape* column is the authoritative field list for what the generated corpus file must contain.

**Why.** No corpus files exist anywhere in the repo or the operator workspace at launch (pre-launch verification 2026-07-18), and blocking every Epic 3/4/5 cycle on operator-supplied data would make the operator the loop's bottleneck across ~20 class cycles + N monster-subset cycles. The SRD/OGL content is publicly licensed and the `corpus-source-inventory.md` gap-fill (2026-07-19) gives each cycle an in-repo authoritative spec of what to generate, so generation is mechanically checkable rather than free-form.

**Fabricated-data-risk trade-off (recorded explicitly).** SD-19's CRB tables were derived only from in-repo verified formulas; SD-22 accepts model/SRD-sourced data. Mitigations: (a) every ingest cycle carries per-cycle representative-sample spot-check tests asserting known-good values from the published rules (the `corpus-source-inventory.md` failure-mode inventory names the exact cells to check); (b) entries that look suspicious but pass their tests are **Epic 9 judgment-log material** — logged to `risks-and-open-questions.md` §"Open judgments deferred to next SD", not silently accepted; (c) divergence between SRD-generated data and the printed book is a recorded risk row in `risks-and-open-questions.md`.

**Operational consequence.** The loop-instruction's SD-22-specific hard stop ("RED test depends on a structured-data input file the operator hasn't supplied") is deleted; Epic 3/4/5 cycles generate the corpus file first, and only unresolvable source ambiguity (SRD conflict, missing SRD coverage for a unit) routes to `## Open blockers`. The pre-launch checklist item "at least one structured-data input file exists" is replaced by "corpus generation is in-bundle per this §5."
