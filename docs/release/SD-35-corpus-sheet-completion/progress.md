...
`artifacts/epic-7-closure/AT-35-E7-001_cycle2_receipt.md`, kanban row 100. Per
`acceptance-and-verification.md §3a` — *"If anything is short: STOP"* — **no retrospective, no
sweep, no PR**. Steps 6 (the full `verify.sh`), 9, 10 and 11 were **not reached**; saying that
plainly rather than implying a green build ran. Every figure below was re-derived at this HEAD;
nothing is quoted from cycle 1 or from a lane's account.

**Green at HEAD:** `completion_atlas.py --check` → `population=49438 buckets=10 unclassified=0
overlap=0`, `DONE: 49438`, A/B/C/D/M/V/U/X/Z all `0`, `done_evidence_violations=0
missing_clearing_mechanisms=0 stale_derived_at=False citation_failures=0`;
`pcgen_residue_gate.py --check --closure` → `live_files=0 live_hits=0 shipped_data_files=0
shipped_data_hits=0 shipped_scanned=11 verdict=PASS`, every pattern and all five live roots
`files=0 hits=0`; `token_coverage.py --check` → `verdict=PASS`, six sub-checks `ok=True`,
`non_done=0 refused_non_done=0`; `grep -rlE 'BONUS:|DEFINE:|PRE[A-Z]+:|%CHOICE|CL='
data/sheet_rules/ | wc -l` → `0`. **No carve-out in any closure instrument**: across
`pcgen_residue_gate.py`, `completion_atlas.py`, `token_coverage.py`, `cycle_scope_gate.py` and
`missing_engine_tables.py` the only exclusion constants are `EXCLUDED_PREFIXES: tuple[str, ...] =
()` — empty and pinned empty — and `EXCLUDED_DIR_NAMES = {node_modules, dist, target, .git}`,
build-output directory names. No book list, no live-path allow-list. **No active `## Open
blockers` entry** (B14 §17, B15 §18, B16 §19 all struck and ruled).

**Cycle 1's shortfall S5 is RESOLVED at this HEAD.** Ruling B17 and row 98's
`AT-35-E6-005-SHIPPED-DATA` landed since. Re-derived independently, not read from a receipt:
`bundle.resources` names five entries, `find resources -type f | wc -l` → **11**, and
`grep -rlE 'raw_tokens|raw_bonus_chains|BONUS:|DEFINE:|PRE[A-Z]+:|%CHOICE|%LIST|SAB:|DESC:|TYPE='
resources/ | wc -l` → **0**. The surviving token arrays sit in
`apps/desktop/src-tauri/fixtures_src/`, which is **not** in `bundle.resources` — a converter
input, kept by `decisions.md §11`. Correction `1789401507303-at-35-e7-001-edd383`.

`§3a`'s mandated independent grep now returns **33 lines**, classified mechanically by the gate's
own `cfg_test_ranges()` rather than by eye: **24 doc-comment/prose**, **7 inside `#[cfg(test)]`
regions**, **2 converter-input JSON**, and the `other` bucket came back **empty**. All are ruled
out by B14, B15 and `§11`, and **none is claimed as a shortfall** — manufacturing one against a
standing ruling is the failure `§3a`'s closing line warns about.

**Four shortfalls stand:**

- **S1 — 55 of 99 kanban rows are not `complete`** (51 `in-progress`, 2 `blocked`, 1
  `blocked-escalated`, 1 `not-started`), **including row 26, the criterion card for
  `AT-35-E6-003`**, plus rows 43–46 and 48–58 / 60–75 / 77–95. Two rows closed and two were added
  since cycle 1, so the non-`complete` population did not fall (cycle 1: 42 complete of 97; now 44
  of 99). `epic-breakdown.md § AT-35-E7-001` is explicit that *"there is no 'complete or filed
  under Open blockers'"*, and relabelling 55 rows to make this scan pass is `§5`'s forbidden move
  run in reverse.
- **S2 — `data/sheet_rules/_refused.json` is not empty:** `records=49438 converted=49296
  refused=142`, all one token type `no_corpus_record`. Every id is `DONE` in the atlas by an
  earlier route (`refused_non_done=0`), so no unit is un-rendered — but `§3a` names the **file**.
- **S3 — `token_coverage.py --check` reports `refused=142`** where `§3a` requires zero refused
  units. Same population as S2; the gate's verdict is `PASS` because its partition balances, and
  the acceptance bar is stricter than the gate.
- **S4 — 83 open deferrals** (`python3 scripts/retro.py summary --since 2026-09-07 --json` →
  `deferrals.open = 83`), 53 of them under `AT-35-E6-003`'s four actors
  (`-RULED` 17, base 16, `-SWEEP` 12 + 6 lower-cased, `-FINISH` 2). At least one defers DoD scope
  on its face — kanban row 41's 10-unit `AT-35-E5-005` residue
  (`1789064249278-at-35-e5-005-579de8`).

`SCOPE_GATE: EXEMPT (Epic 7 acceptance-scan cycle — closes zero units by design, decisions.md §2)`
· `closed=0 relabeled=0 rust_lines_changed=0 ratio=n/a builds_recorded=0 pcgen_live_files=0` ·
`incident 1789401507169-at-35-e7-001-8ebce7`, `correction 1789401507303-at-35-e7-001-edd383`,
`deferral 1789401507436-at-35-e7-001-97631a`.

**Before this scan can be re-run:** close or obtain a ruling on the 55 non-`complete` kanban rows
(row 26 first); empty `_refused.json` or amend `§3a`'s "empty" to "zero non-`DONE` refusals" by
ruling (the same ruling disposes of S3); dispose of the 83 open deferrals; then run
`AT-35-E7-001` in full including the whole `verify.sh` and steps 9–11.
...
- **Receipt rows:** `closed=0 relabeled=0 rust_lines_changed=201 ratio=n/a builds_recorded=0 pcgen_live_files=0`. `ratio=n/a` because `closed=0` by design; the 201 lines are one `vec_init_then_push` rewrite, no behaviour change.
- **PCGen residue:** `live_files=0 live_hits=0 baseline_files=260 baseline_hits=12736 verdict=PASS` at start **and** at end — unchanged.
- **Refused tokens: none.** No record was converted this cycle.
- **The four reds, each fixed at its source — no stage silenced, no skip added, no ignore list widened:**
  1. **`site-dashboard-check`** — the real producer re-ran (~19 min) and rewrote the feed (`books=30 overall_pct=95.0 items=46074`, re-derived from the committed feed by the command in the receipt's figures table). The input pin is **unchanged** before and after, which confirms rather than contradicts the diagnosis: the pin watches one input and the feed also derives from the unit ledgers. **No number was copied into an assertion.**
  2. **`figure-provenance`** — 2 violations, both prose in a command column of `AT-35-E6-004_cycle1_receipt.md`. The values were right; the commands were not runnable. Wrote `AT-35-E6-004_cycle1_b15_skipline_delta.py` (it loads the **pre-B15** gate out of git by SHA, so the "before" figure does not depend on the working tree), **ran it**, and confirmed it reproduces all eight claimed figures exactly. `violations=2 → 0`.
  3. **`desktop`** — floor 576, measured 570, **zero failures**. Attributed *before* the floor was touched, by a method independent of `verify.sh`'s count: enumerating every `#[test]`/`#[tokio::test]` fn at `6e4b1f7b4e` and at HEAD reproduces **576 and 570**, and the net −6 decomposes into **37 retired and 31 written**. Every one of the 37 guarded a function Epic 6 deleted; every population-shaped guard among them was replaced. Floor lowered to 570 with the attribution, re-derive commands, replacement mapping and eight deleting SHAs **in `verify-baselines.env`'s own log** — the only form `§8` permits.
  4. **`clippy`** — `vec_init_then_push` at `equipment_catalog.rs:889`: 21 `push` calls → one `vec![]`. **No ceiling raised, no lint allowed.** `cargo clippy --locked --tests -j 6` on the desktop crate → zero warnings.
- **Corrections against the gate report (it is testimony, and two claims did not survive re-derivation):** the four SHAs it blamed for the lost desktop tests deleted **none** of them (`…-f20460`, real deleters are `a4120e043f`, `77b10113a6`, `526173470e`, `5a3a67c2dd`, `81d8199a06`, `798bf8ebda`, `ef7d54daf1`, `592ea43501`); and its `.gitignore` escalation named a path that does not exist here (`…-b8db63` — the dir is `.claude/worktrees/`, already excluded but only by the **local-only, never-committed** `.git/info/exclude:11`).
- **Worktree sweep (`§10 step 2`), the step the isolated gate worker could not run: FOUND 8, REMOVED 6, RETAINED 2.** All 8 were dirty and **6 held gate reports and retro logs never folded into the repo** — removing them as the step says would have destroyed 5 epic gate reports and 11 retro logs permanently. Folded first (two colliding Epic 5 regate reports both preserved; two append-only logs union-merged by event id, recovering 1 lost event), then removed. Incident `…-55cb8e`, key `unfolded-gate-artifacts-die-with-the-worktree`. `.claude/worktrees/` 15G → 9.4G.
- **Full gate re-run by this cycle itself: `RESULT: PASS`, `VERIFY_EXIT=0`, 49 of 49 stages PASS, 0 FAIL**, 7,364 s (2 h 02 m 44 s), 2026-09-14T01:26:01Z → 03:28:45Z. All four formerly-red stages green: `site-dashboard-check` (feed current), `figure-provenance` (`files_checked=274 figures_examined=613 violations=0`), `desktop` (570 passed), `clippy` (root:0 desktop:0). Widest scope: `root-lib` 3390, `root-full` 8919 across 419 suites with all 365 `tests/*.rs` executed, desktop crate 570, frontend green.
- **Three stale root floors raised on this run's own measurement** (BASELINE NOTES, all growth, none a failure): `BASELINE_ROOT_LIB_TESTS` 3261→3390, `BASELINE_ROOT_FULL_TESTS` 8772→8919, `BASELINE_ROOT_TEST_BINARIES` 413→419 — the last attributed exhaustively to the six test-binary files Epic 6 added.
- **2 operator escalations, neither blocking Epic 7:** `wf_291be5c8-5f3-30` holds uncommitted **live Rust WIP** across 13 files (2 absent from the repo) and `wf_291be5c8-5f3-19` holds a substantively-modified SD-34 `completion-atlas.json`; plus moving `.claude/worktrees/` from `.git/info/exclude` into the committed `.gitignore`.
...
- **Scope gate:** `SCOPE_GATE: EXEMPT (Epic 6 closure cycle — closes zero units by design, decisions.md §2)`. Run for the record:
  ...
  ```
- **Receipt rows:** `closed=0 relabeled=0 rust_lines_changed=0 ratio=n/a builds_recorded=1 pcgen_live_files=0`.
- **Refused tokens: none.** `token_coverage.py --check` → `refused=142 refused_non_done=0`; zero units were in scope, so no `deferral` is owed.
- **The criterion's bar, met:** `python3 scripts/pcgen_residue_gate.py --check --closure` → `live_files=0 live_hits=0 verdict=PASS`, exit 0, with all fifteen patterns and all five roots — `src/rules_core`, `src/saved_character`, `src/campaign`, `src/homebrew_authoring` **and `apps/desktop`** — at `files=0 hits=0`, and `identifier_files=0 identifier_hits=0`.
- **The zero was not taken on the gate's own word.** Three checks, because this criterion's job is to catch a green gate that does not measure what it claims: (1) B14, B15 and B16 are **implemented**, not merely documented — `code_only` skips `//` lines and scans every other line whole, `cfg_test_ranges`/`_live_lines` skip the `#[cfg(test)]` **item region**, `RUNTIME_IMPORT_PATTERNS` counts `pcgen_import` and stays out of the `identifier_*` subset, `EXCLUDED_PREFIXES` is empty, 29 gate unit tests OK; (2) an **independent census** — `AT-35-E6-004_cycle1_independent_residue_census.py`, a character-level Rust scanner that understands string literals, raw strings and block comments, borrowing nothing from the gate but its roots and patterns — reads `INDEPENDENT live_files=0 live_hits=0`, `over_skip_hits_hidden_by_gate=0`, `files_with_boundary_mismatch=0`; (3) a planted one-line live read makes the **stage** fail (`FAIL pcgen-residue-gate (live_files=1 live_hits=1 verdict=FAIL)`, `RESULT: FAIL`) and removing it makes it pass.
- **And check 2 found a real defect, which is why it exists.** Before the fix the two censuses disagreed: `gate_skipped_lines=91053` vs `90433`, `files_with_boundary_mismatch=3`. `cfg_test_ranges` counted braces line by line, so `b"not-json-{garbage"` inside the first `#[cfg(test)]` module of `apps/desktop/src-tauri/src/update/transaction.rs` left it one `{` deep at that module's closing brace and the skip ran on to the next test module — **blanking 618 lines of shipping code** (1869–2486). Nothing there read PCGen, so the verdict was right **by luck**: the `validate-proxies-against-known-truth` / `AGENTS.md` rule 7 shape exactly. Fixed by brace-matching over a new `mask_non_code()`; the mirror case (a `// }` in prose ending a region early) is fixed with it. RED→GREEN pinned by two new tests, both failing first for the right reason (`BONUS: 0 != 1`, `SAB: 1 != 0`). `correction 1789337005325`. **620 lines of shipping code returned to the scanned surface and the count stayed at zero.**
- **Closure mode is now the stage's mode.** `scripts/verify.sh`'s `run_pcgen_residue_gate` reads `${PCGEN_RESIDUE_GATE_CLOSURE:-1}` (was `:-0`): `scripts/verify.sh --only pcgen-residue-gate` → `==> pcgen-residue-gate — python3 scripts/pcgen_residue_gate.py --check --closure` / `PASS (live_files=0 live_hits=0 verdict=PASS)`. The ratchet stays reachable as `PCGEN_RESIDUE_GATE_CLOSURE=0` as a diagnostic aid; `scripts/pcgen-residue-baseline.env` is untouched.
- **Oracle parity agrees with the epic's start.** Same pin `7f818006e371188e5717fd18d74d18a420747fc6`, same 29-character roster, `chassis 382 / 376 / 6` **identical to AT-35-E6-001's**; lines `146 / 145 / 1` → `156 / 154 / 2` because Epics 2–6 made 10 more lines renderable and agreement rose with them. All **7** of the start's disagreements are still present with the same `ours`/`oracle` values (`gone_since_start=0`), and the document is field-for-field identical to SWEEP cycle 13's. Artifact: `AT-35-E6-004_cycle1_sheet-parity-after.json`.
- **The tool side is intact** (`decisions.md §11` — KEPT for Starfinder): `cargo build --locked --bin sheet_rule_convert --bin gen_book_cache` → `TOOLSIDE_EXIT=0`; `python3 scripts/oracle_harness/run.py --help` → `ORACLE_HELP_EXIT=0`; `scripts/pcgen-oracle-pin.env` unchanged since `c3500e7984`. **Zero net deletions of function bodies**, proved by name rather than by diffstat: over `src/pcgen_import` + `scripts/oracle_harness` + `src/oracle_validation`, `tool_side_files start=39 head=88`, `fn/def definitions start=536 head=1652`, `names_present_at_start_and_absent_at_HEAD=0`. The one file git reports `D` (`source_content_payload.rs`) is a **split** — all five of its definitions live at `src/pcgen_import/ir_content_payload.rs`.
- **`cargo tree` — reported for what it can settle.** `codex-desktop v0.15.0` depends on `codex`, `tauri`, `serde`, `serde_json`, `sha2`, `base64`, `uuid`; no converter **crate**. But `src/pcgen_import/` is a **module** of the `codex` crate, so `cargo tree` is crate-grained and cannot settle module independence — claiming otherwise would repeat the confident-proxy error B16 was ruled over. The module-level proof is the gate (`root apps/desktop files=0 hits=0`, `pattern pcgen_import files=0 hits=0`), the independent census, and a grep finding **166** `pcgen` occurrences in the desktop crate and frontend, **all** of them `//!`/`///`/`//` provenance or `#[cfg(test)]`.
- **Verified once, at the final tree:** `NO_RUN_EXIT=0` (419 test executables), lib `LIB_EXIT=0` `3390 passed; 0 failed; 16 ignored`; `sheet_rule_convert -- --check` exit 0 over 8 kinds; `grep -rlE 'BONUS:|DEFINE:|PRE[A-Z]+:|%CHOICE|CL=' data/sheet_rules/ | wc -l` → **0**; `completion_atlas` `missing_clearing_mechanisms=0 citation_failures=0`; `token_coverage` `non_done=0 refused=142 verdict=PASS`; `shape_engine_boundary` `magnitude_bearing=26396 not_held_by_engine=0`; `missing_engine_tables` `population=0 citation_failures=0`; `denominator_gate` `files_checked=148 violations=0`; `verify.sh --only pi-sweep` PASS. `cargo test --no-fail-fast`, `corpus_literal_sweep`, `v06_work_inventory` and clippy correctly did **not** run: no Rust, no `src/`, no `data/corpus/` and no `apps/` line was written — the diff is three script files. `docs/work-inventory.json` byte-identical.
- **Movement:** closure **none** (Epic 6 closes no corpus unit); relabel **none**; reachability **none** (the live side was already at zero when the cycle opened); instrument-correction **one** — `gate_skipped_lines` 91,053 → 90,433. That is 620 lines of shipping code returned to measurement, not progress, and is never to be reported as progress.
- **Epic 6 is complete.** Next dispatch: Epic 7 / `AT-35-E7-001`, the final-acceptance scan, which re-runs `pcgen_residue_gate.py --check --closure` at HEAD — now also the `pcgen-residue-gate` stage's default mode.
