...
| Document | Status |
|---|---|
| `../../../retro/sd35-corpus-sheet-completion-retrospective.md` | **SD-35's own. Written at closure by `AT-35-E7-002` (2026-09-15) and cited from THIS file in the same cycle**, grounded in `python3 scripts/retro.py summary --since 2026-09-08 --json` (731 events) in `sd31-retrospective.md`'s shape. Every figure the `§11` step-2 bar names is reported with the command that re-derives it: **units per cycle** min `0`, median `0`, max `21,911` over 127 receipts (`closed=` on each receipt) — one cycle closed 21,911 of the 23,315 units not DONE at the cut; **lines per unit** mean `1.42`, median `1`, p95 `4`, max `201`, with 44,840 of 49,450 units printing exactly one line; **build time** `188.97 s` → `145.89 s` cold and paired (`artifacts/epic-1-tax-cut/build-time.json` `paired_rerun`), test binaries 544 → 362 with the test-entry list byte-identical at 8,723; the **`Words` share per kind** over all 70,317 shipped rules, 39,941 of 70,317 corpus-wide, ranging from 719 of 10,553 for `monster` to 459 of 459 for `deity` and 136 of 136 for `language`; and the **PCGen residue per epic**, `live_files` 260 at Epic 1 to **0** at Epic 6, with Epic 6 accounting for the whole −254 and the gate's pattern list only ever widened. It also carries the `§11` step-3 closure sweep with its found-vs-removed counts, and the operator escalation that `git worktree remove` has never been a permitted action for any agent in this program. |
| `../../../retro/sd34-book-completion-retrospective.md` | SD-34's. **Written 2026-09-08 by AT-35-E1-006** (SD-34 merged without running its epilogue, `../decisions.md §12`) and cited from THIS file and from SD-34's own `references/README.md` in that cycle. Its "changes for SD-35" table names where each row is enforced; the three not already in `../decisions.md §9` were added there as L14–L16 in the same cycle. The 17 open SD-34 rows and 29 open deferrals are dispositioned in `../artifacts/epic-1-tax-cut/sd34-open-row-map.json` (1,590 of 1,590 units mapped) and `sd34-deferral-dispositions.json` (8 resolved, 1 superseded, 20 mapped). |
