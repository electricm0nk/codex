...
- **Figures + their re-derive commands:**
  ...
  | heads whose last-wins value the partition moves: `VISIBLE` 473, `EQMOD` 65, `ALTEQMOD` 2, `COST` 2 — and no others | the 2,110 copy-base records that ship tokens | the same walk as the row above (`python3` over `data/corpus/`, reading each record's base row out of `$PCGEN_CORPUS_ROOT`), computing the stable partition of `data.raw_tokens` and diffing the last-wins `(key -> value)` map before and after |
