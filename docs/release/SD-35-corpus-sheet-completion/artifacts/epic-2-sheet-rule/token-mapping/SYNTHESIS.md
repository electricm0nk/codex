...
- `CSKILL` -- axis D. Proof: 842 instances / 836 records hold, but non-DONE units re-derive to 425 against the row's 495 (14%); the rule is fine. **Fix applied:** re-state the unit count with its join
...
- `%CHOICE / %LIST (value markers)` -- axis D. Proof: Host counts re-derived: ABILITY 202, TEMPBONUS 184 (row: 153, 20% off), BONUS 173, SPROP 100, DESC 86, COST 37, AUTO 33, ASPECT 19, TEMPLATE 5, SR 2; the binding rule itself is right and Expr::Choice exists. **Fix applied:** correct the TEMPBONUS figure; keep the rule
