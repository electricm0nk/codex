...
| 22 | `bucket-x-choice-filter` | 5 | AT-35-E5-004 | complete | Units emptied at `26bdfa8d5b` (`artifacts/epic-3-place-and-surface/AT-35-E3-002_cycle1_receipt.md`); X 168 → 0. Cycle 1 pays the second Evidence clause and clears deferral `1788922132640-at-35-e3-002-ac4da5`: the filter is built (`src/rules_core/level_up_option_filter.rs`, served on `preview_level_up` as `featOptions`/`refusedFeatOptions`), plus the converter defect it exposed (354 files, `PreStatScore_<AB>` read 0) — `artifacts/epic-5-residues/AT-35-E5-004_cycle1_receipt.md`, `a56096b861` |
