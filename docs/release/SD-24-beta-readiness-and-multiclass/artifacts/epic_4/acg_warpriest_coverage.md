...
**Bug fix (v0.6 alpha swarm, 2026-07-26, found by scout's formula-audit pass, lead independently confirmed):** `warpriest_sacred_weapon_base_dice`'s own `dice_count` term used `level/20`, the `PREBASESIZELT:M` (smaller-than-Medium) branch's own denominator from `acg_abilities_class.lst`, transcribed by mistake into this function's applicable `PREBASESIZEEQ:M` (Medium) branch -- a sibling-branch mix-up. Corrected to the real EQ:M formula, `1+min(1,level/15)`. This closure's own bounded 1-3 level scope never diverges between the two denominators (both give `dice_count=1`), so the bug was silently wrong only for level 15+ Warpriests, and only ever surfaced in the explanation's own prose text (never a separate `.value` field any prior test could have caught). New dedicated test `warpriest_sacred_weapon_dice_count_matches_the_real_eq_m_corpus_branch_at_every_level` covers the full level range including the previously-affected 15-19 band, proven RED-then-GREEN against the real bug before landing the fix.
