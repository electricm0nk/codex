...
No changes to the update/install mechanism in this combined release — eligibility follows the same Linux AppImage mechanism established in SD-16 (`docs/release/SD-16/release-notes.md`). Existing eligible installs Check/Install through the same governed channel-index flow. The release tag format is `${channel}-v${VERSION}-${SHORT_SHA}` (e.g. `alpha-v0.7.<build>-<sha>`); the manifest tag format is `${channel}/v${VERSION}-${SHORT_SHA}` (e.g. `alpha/v0.7.<build>-<sha>`), used by the channel-index URL.

---

## Appendix A — Detailed SD-27 release notes (tranche/7 content)

The full SD-27 release notes follow verbatim from
`docs/release/SD-27-future-state-book-content-ingestion/release-notes.md`.
Refer to that file for the most current version after any
post-v0.7 amendments.
