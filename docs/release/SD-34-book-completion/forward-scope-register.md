...
**Snapshot: refreshed 2026-09-07 as of commit `5f6b18f4e3`** (wave 50 wave-end gate — full
40/40 confirmed). **Wave 51 is in flight, uncommitted, at the time of this refresh** and will
move the numbers below within the hour; every figure in this register states the commit it was
re-derived against so a reader knows exactly which snapshot it reflects. Re-derive against this
exact snapshot with `git show 5f6b18f4e3:docs/work-inventory.json > /tmp/wi.json && python3 -c
"import sys; sys.path.insert(0,'scripts'); import completion_atlas as ca; import json;
units=json.load(open('/tmp/wi.json'))['units']; print(ca.partition(units)['counts'])"`, or run
`python3 scripts/completion_atlas.py --check` / `--by-book` directly against the live
`docs/work-inventory.json` for a current-moment re-check (will differ once wave 51 commits).
...
| C1.6 | **Fable-review method correction, 2026-08-31 — supersedes C1.5's figures and the hand-modeling cost model.** `fable-review.md §1.b` + `artifacts/fable-review/TOKEN-MODEL.md`: the live remainder is **22,369 of 42,472** (C1.5's 35,650/136/29 figures are from the stale pre-reconciliation snapshot; live census: 83 mechanisms over the 35-book remainder, 22 cover 90%). The remainder's **token vocabulary is 189 types, 28 covering 90% of instances**; the SD-32 engines already cover the BONUS:VAR/DEFINE shapes on **6,708 units (30.0%)**. Operator ruling 2026-08-31: hand-modeling per unit is the wrong method — **cost scales with token vocabulary, not unit count** (interpreter route ~85–350h to 66.2% of remainder vs 3,900–31,000h hand-modeling; SD-27 §24.1's interpreter ban was already overturned by SD-31 Decision 20). **SD-35 step 1: one corpus-wide pass of the existing interpreter + stack-reader over the 6,708 BONUS:VAR/DEFINE-bearing units, oracle-fixture-verified.** | SD-35's `scope-draft.md` + `workflow-instruction.md §3`. |
| C1.7 | **Wave-50 committed-snapshot refresh, 2026-09-07 — supersedes C1.6's 2026-08-31 figures with the current committed numbers.** Snapshot: commit `5f6b18f4e3` (wave 50 wave-end gate; wave 51 in flight, uncommitted, will move these again). Corpus-wide, re-derived directly against `docs/work-inventory.json` at this commit via `completion_atlas.py`'s own `_bucket_of()`, not copied forward: `population=49438 DONE=25906 A=449 B=11589 C=4180 D=2084 M=4449 V=392 U=202 X=168 Z=19` (sum 49,438, matching `completion_atlas.py --check`'s own committed output). **The 35-book SD-35 scope** (corpus minus `core_rulebook` + `ultimate_campaign`, C1.6's own split): population 42,472; live remainder (all non-DONE buckets, C1.6's own definition) is **21,725 of 42,472** — down from C1.6's 22,369 of 42,472, reflecting cumulative wave 42-50 movement outside the two vehicle books, including wave 50's own 99-unit cross-book bonus (`progress.md` WAVE 50 UPDATE) landing in 10 other books. **The two vehicle books' own remainder — the exact population wave 51 is dispatched against:** `core_rulebook` M 778 / B 392 / C 191 / D 138 (1,499 of 6,701) + `ultimate_campaign` M 36 / D 2 (38 of 265) = **1,537 actionable units**, matching wave 50's own receipt (`artifacts/bucket-d-mining/wave50_core_rulebook_ultimate_campaign_cycle_receipt.md`) exactly. Re-derive: `python3 scripts/completion_atlas.py --by-book` (per-book) or `--check` (corpus-wide), against the commit in question. | SD-35 planning; supersedes C1.6's figures, not a new deliverable. |
| C1.8 (recorded here) | **Operator ruling, 2026-09-07 — Campaign Codex is a paper character-sheet generator, not a simulation.** Most rules only need their text printed on the sheet; only values that actually feed sheet totals need to be computed. SD-35 (and any remaining SD-34 wave) must not build simulation-style compute engines or "consumer-delta" proofs for print-only rules. This extends this bundle's own already-established "text-only features are complete" precedent (`decisions.md §2a`/`§20`) directly to SD-35's backlog-classification question: much of what `fable-review.md §1`'s B-SYNTH classified as "needs computation" (e.g. engine 2, display-class record placement, 6,718 of the 22,369-unit 35-book remainder as measured 2026-08-31) may re-classify as print-only once re-read against this ruling — which would materially change the ~85–350h / "~72% of the remainder is engine-shaped" cost model in `fable-review.md §1.b`. **Now formally recorded in `decisions.md §23`** (wave 51 wave-end gate, 2026-09-07); wave 52+ must re-scope the remaining CR/UC buckets under it before dispatching, and SD-35 planning still needs the fable-review engine-shape percentages re-derived against it before committing to a cost model. **Sub-note—known one-line census fix carried into the first SD-35 batch** (recovered 2026-09-07 from wave-13 lane-2 salvage 38855f4406 before branch deletion): `core_rulebook:class_feature:monk_ki_pool` stays `engine-does-not-hold` only because `CLASS_FEATURE_ID_MAGNITUDE_SUFFIXES` in `src/bin/v06_work_inventory.rs` lacks `"size"`; the engine already emits `class_chassis.monk.ki_pool_size`. Feeds a sheet total under the 2026-09-07 ruling, so in scope. | SD-35 planning (Fable review); `decisions.md §23` is now the citation. |
