...
| C1.8 | **Operator ruling, 2026-09-07 (communicated out-of-band; NOT YET recorded in `decisions.md`) — Campaign Codex is a paper character-sheet generator, not a simulation.** Most rules only need their text printed on the sheet; only values that actually feed sheet totals need to be computed. SD-35 (and any remaining SD-34 wave) must not build simulation-style compute engines or "consumer-delta" proofs for print-only rules. This extends this bundle's own already-established "text-only features are complete" precedent (the v0.6 Path-A ruling `decisions.md` already carries) directly to SD-35's backlog-classification question: much of what `fable-review.md §1`'s B-SYNTH classified as "needs computation" (e.g. engine 2, display-class record placement, 6,718 of the 22,369-unit 35-book remainder as measured 2026-08-31) may re-classify as print-only once re-read against this ruling — which would materially change the ~85–350h / "~72% of the remainder is engine-shaped" cost model in `fable-review.md §1.b`. **Confirmed not yet written down anywhere in this repo**: `grep -rn "character-sheet generator\|print-only\|paper character" docs/release/SD-34-book-completion/ docs/governance/ docs/architecture/` returns zero hits at commit `5f6b18f4e3`. This row exists so SD-35 planning has it; it still needs formal recording in `decisions.md` (not done by this edit — wave 51 has that file open) and a re-derivation of the fable-review engine-shape percentages against it before SD-35 commits to a cost model. | SD-35 planning (Fable review); a `decisions.md` entry, whichever wave picks it up next. |
