...
|---|---|---|---|---|
| 1 Completion Atlas | E1-001..008 | no | `scripts/completion_atlas.py` (new), `scripts/tests/test_completion_atlas.py` (new), `src/bin/v06_work_inventory.rs` (the rename), `docs/work-inventory.json` (**Epic 1 is its writer**), `scripts/verify.sh` + `scripts/denominator_gate.py` (E1-006 stage + default-scope widening), `scripts/verify.sh` (E1-007 trap-audit stage), **`data/corpus/**` via the guarded `gen_book_cache` path — AT-34-E1-008 ONLY** (`decisions.md §13`), `artifacts/epic-1-atlas/` | launch gates |
