...
| 10 | `epic-6-race-trait-lane-extend` | **IN-FLIGHT** — round 1 claimed 2026-08-11 by `sd29-racetrait-r1`. This is a **loop-until-dry** lane: a round delivers as much gate-verified ingest as its turn holds, records `units_ingested` and a freshly re-derived `units_remaining`, and a successor round resumes from that receipt. Round 1's scope is `decisions.md §43.5`'s probe repair (the lane's once-paid dominant cost) plus the APG record whose ingest binary shipped but whose output was never landed. Prior status `READY` (reopened by `epic-12-reopen`, `decisions.md §42`) — the companion mis-classification fix IS landed and stays landed; the **corpus-wide ingest is outstanding**. The race-chassis ceiling (`crb::race_traits()` models exactly 7 races) is a real structural finding and SD-29 now owns confronting it — it is not a licence to close the lane. Prior receipts preserved | Race-Trait Lane — extend | corpus-wide, **26** books. **Denominators re-derived 2026-08-11 from `docs/work-inventory.json`: `race_trait` 3,447 total / 21 grounded / **3,426 remaining**. The prior "3,447 remaining" conflated total with remaining; the 21 grounded are `core_essentials` 20 + `advanced_race_guide` 1** | `epic-6-race-trait-lane-pilot` | sd29-racetrait-r1 | 2026-08-11T18:00:00Z | SD29-E6-F2-002 (round 1 of a loop-until-dry lane) |
