...
| 11 | `epic-7-companion-lane-pilot` | NOT-STARTED — settled by `epic-11-closure` 2026-08-11. Its cycle refused at Cycle-mechanics step 1c (`preflight-disk` EXIT=1, 91% used / 47G free, twice, with `reclaim.sh --apply` in between) and correctly left the row unclaimed rather than parking it `IN-FLIGHT` under an agent that did no bounded work. Receipt: `progress.md` `## Cycle SD29-E7-F1-001`. The disk condition has since cleared (80% used, 97G available at closure), so this is a ready re-dispatch for a successor bundle, not a corpus finding. Closed for SD-29; carried in `release-notes.md` §Known issues 1 | Companion Lane — pilot | mechanism-build + `inner_sea_combat` (10 units) | `epic-3-provenance` | sd29-e7-companion-pilot (refused) | 2026-08-11 | SD29-E7-F1-001 |
| 12 | `epic-7-companion-lane-extend` | NOT-STARTED — settled by `epic-11-closure` 2026-08-11. Never eligible: its `Depends-on` (card 11) never reached COMPLETE. No cycle claimed it and no work was attempted. All **1,696** corpus-wide `companion` units remain `not-ingested`/`not-started`, **0** grounded (re-derived at closure from `docs/work-inventory.json`). Closed for SD-29; carried in `release-notes.md` §Known issues 1 | Companion Lane — extend | corpus-wide, 17 books, 1,683 remaining units minus the pilot's 10 | `epic-7-companion-lane-pilot` | — | — | — |
...
| 15 | `epic-10-review` | COMPLETE | Bundle Code Review | full-bundle diff review vs. branch point (`decisions.md §27`) | `epic-4-proven-equip-mod`, `epic-4-proven-spell`, `epic-4-proven-feat-race-class`, `epic-5-monster-lane-extend`, `epic-6-race-trait-lane-extend`, `epic-7-companion-lane-extend`, `epic-9-version`, `epic-8-toolkit` (COMPLETE or `decision-blocked`) | sd29-e10-review | 2026-08-11T13:00:00Z | SD29-E10-F1-001 |
| 16 | `epic-11-closure` | COMPLETE | Closure Epilogue | tranche promotion PR | all cards above (COMPLETE or `decision-blocked`) | sd29-e11-closure | 2026-08-11T14:00:00Z | SD29-E11-F1-001 |
