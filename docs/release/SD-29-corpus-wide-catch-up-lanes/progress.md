...
## Cycle SD29-E11-F1-002 — `epic-11-closure-run2` (Closure Epilogue, RUN 2) — **NOT CLOSED**

**Actor:** `sd29-closure-r2` · **Date:** 2026-08-13 · **Branch:** `tranche/9` ·
**Tip at receipt:** `0ddfc126` · **Branch point vs `origin/develop`:** `a1295856`
(251 commits, 4,643 files, +307,600 −15,518 — `git diff --shortstat $(git merge-base HEAD origin/develop)..HEAD`)

### 0. THE RULING, first: SD-29 is **NOT CLOSED** by this cycle

**Material workable work remains in two lanes.** Per this card's own instruction —
*"If material WORKABLE work remains: DO NOT CLOSE"* — this cycle writes an honest status receipt,
leaves cards `8` (`epic-5-monster-lane-extend`) and `12` (`epic-7-companion-lane-extend`) `READY`,
leaves **PR #360 OPEN and unmerged**, does **not** refresh `docs/architecture/` to a closure state,
does **not** rewrite `release-notes.md`'s "Known issues" into a shipped-issues section, and returns
`partial`.

That is a correct result, not a failure. It is also the second time this bundle has stood at this
exact decision point, and the first time it was answered wrong.

| lane | raw remainder | **REAL workable** | disposition |
|---|---|---|---|
| `companion` | 826 | **53** | **WORKABLE — no new mechanism.** Card 12 stays `READY` (round 9) |
| `monster` + `monster_ability` | 1,515 | **10** registration-only **+ 229** mechanism-blocked | **10 WORKABLE.** Card 8 stays `READY` (round 11) |
| `race_trait` | 2,934 | **3**, all needing a race-variant chassis this card does not own | **DRY** — card 10's `DRY` ruling reproduced and confirmed |

**63 units are workable today by lanes that already exist and are already registered.** That is not
a rounding residue and it is not a successor's problem: `decisions.md §42` (operator directive, "sd-29
isn't done. let's get after it.") put exactly these two lanes back inside SD-29.

### 1. Every figure re-derived, command first

`docs/work-inventory.json` at this tip was regenerated on the merged tree by commit `3b0a414c`;
it was **not** regenerated by this cycle (a `cargo run` would have contended with the in-flight
full gate for the same `CARGO_TARGET_DIR`). Recorded as an unattended-mode default in §7.

**(a) Raw per-kind denominators, in-scope books only**
...
print('units total', len(d['units']))"
```

```
companion        total=1696  grounded= 870  remaining= 826
monster          total=1270  grounded=1239  remaining=  31
monster_ability  total=3107  grounded=1623  remaining=1484
race_trait       total=3447  grounded= 513  remaining=2934
units total 38540
```

**(b) `companion` REAL workable = 53** — the classifier's exclusion set intersected with unit
status, rather than the printed `reachable remainder` line, which does not subtract already-grounded
rows:
...
Cross-check with the instrument the lane's own rounds use, `python3
scripts/classify_companion_rows.py` (no args, corpus-wide): `1696 units / 773 excluded / 923
reachable`; `923 − 870 grounded = 53`. Two independent derivations agree, per `AGENTS.md`'s
two-implementations rule.

> **Correction to the dispatch brief.** It carried `left=52` from companion round 8. The re-derived
> figure at this tip is **53**. The brief's number is off by one and the difference is not explained
> by anything that landed since — round 8's own receipt (`decisions.md §65`) published
> `615 units / 479 excluded / 136 reachable` over five named books and ingested 84, which gives 52
> *within those five books*; the corpus-wide pass finds one further reachable-and-remaining row
> (`core_essentials`, 1). A per-book selection is not a corpus-wide ceiling, and this is the size of
> the error that produces.

**(c) `monster` + `monster_ability` REAL workable = 10, mechanism-blocked = 229**

```
python3 scripts/classify_monster_ability_rows.py | tail -6
  -> remaining monster+monster_ability units : 1515
     orphan monster_ability rows             : 1406
       of which in ZERO-monster books        : 866 across 13 books
...
`1515 / 75 / 229` **reproduce `§64.5` exactly** before being used. Per-book reachable, recomputed
from the classifier's own columns (`mon + abil − ORPHAN − PI − COPY`):

| book | reachable | REAL | why |
|---|---|---|---|
| `bestiary` | 58 | **0** | instrument over-report, `§60.2`: 54 cross-table owners + 4 `.MOD`-only overlays |
| `inner_sea_bestiary` | 7 | **0** | Product-Identity residue, `§58.1` |
| `horror_adventures` | 9 | **9** | `RuleSetId::Ha` already registered — pure ingest |
| `occult_adventures` | 1 | **1** | needs a whole new `RuleSetId` for one record |
| **total** | **75** | **10** | |

Plus **229** rows in six books that are reachable in the corpus and unreachable by any pass this
repo runs, pending `§64.1`'s `ABILITY:Internal|AUTOMATIC|` bundle hop. That is a mechanism, and it is
scanned, counted and checked in — it is not a discovery a successor still has to fund.

**(d) `race_trait` — DRY confirmed, 3 residual and all chassis-blocked**

```
python3 scripts/race_trait_ceiling.py
  -> CEILING 571 (553 Racial Trait rows + 18 Subrace rows over the 18 modelled races)
     by status {'not-ingested': 58, 'grounded': 513}
     chassis-blocked residue: 3447 - 571 = 2876
```

The 58 `not-ingested` inside the ceiling, every one already carrying a recorded finding:

| book | n | class |
|---|---|---|
| `advanced_players_guide` | 49 | ARG `KEY:` republications — republished, not new (`§39`) |
| `bestiary` | 3 | Drow Noble — needs a **race-variant chassis**, not a race-trait ingest |
| `core_essentials` | 2 | `Aasimar ~ Default` / `Tiefling ~ Default` — `PREABILITY`-gated subrace selectors |
| `horror_adventures` | 1 | `Half-Elf ~ Starchild` |
| `inner_sea_races` | 2 | `Elf ~ Sovyrian-Born` (declared PI, `§53.3`), `Human ~ Tribalistic Languages` (upstream gap, `§45.4`) |
| `monster_codex` | 1 | `Oversized Goblin` (mechanism-blocked, remedy named in its `OPEN_FINDINGS` entry) |

Card 10's `DRY` ruling stands, re-derived rather than transcribed. **`{'grounded': 513,
'not-ingested': 58}` reproduces `§64`'s correction to `§49.8` exactly**, including the one row that
moved for the declared-PI name.

### 2. Base recovery — the shared checkout was 143 commits stale AND 2 commits ahead

`git status` at cycle start:

```
Your branch and 'origin/tranche/9' have diverged,
and have 2 and 143 different commits each, respectively.
```

The two local-only commits were real, unpushed work (`fd7bd058` the magnitude-fidelity decision,
`4c6f30cb` the `site/` landing page). **This is the fourth consecutive cycle handed a wrong base**
(`§63 §0`, `§65 §0` record the second and third). Merged rather than reset, so neither side was lost:
merge commit `0ddfc126`, pushed to `origin/tranche/9` **before any other work began** — the
stranded-work failure this bundle has now hit twice does not get a third instance.

**One conflict, in `decisions.md`, and it is worth recording as a class.** Both sides had
independently minted a **Decision 46**: the local commit's magnitude-fidelity decision, and
`origin`'s monster round-2 decision. The board's own note after `§47`/`§49` and `§60`/`§62` predicted
exactly this for concurrent lanes; it happened again across a checkout boundary rather than a lane
boundary. **Resolved by keeping both and renumbering only the unpushed one to `§67`**, with a
pointer left in place at the seam — the pushed number is the one every other document already cites,
so moving it would have broken live cross-references, and deleting either would have destroyed a
decision.

### 3. Gate — `./scripts/verify.sh` FULL, exit code captured directly
...
**No on-screen artifact is produced by this cycle, and that is not a waiver.** Item 8 binds *"any
player-visible family you surface"*; this cycle surfaces none — its whole diff is
`docs/release/SD-29-corpus-wide-catch-up-lanes/*` plus the merge. The item-8 artifacts that cover
the families this bundle actually shipped are the ones each lane round produced, most recently Epic
10 run 2's two passes at `de303fd0`. `verify-on-screen.sh` was deliberately **not** run, because the
full gate was in flight and the two must not run concurrently on this box (`sd29-driver-fix`'s OOM
incident, 22 GiB and zero swap at the time).

### 5. The two failures this bundle should carry out of it, stated as data

**(a) The first closure was premature, and the mechanism was a taxonomy abuse, not an oversight.**
`73f1421f` / `ac217788` closed SD-29 on 2026-08-11 with three lanes unfinished, by routing them
outward through `DECISION-BLOCKED` — a state `kanban.md`'s legend reserves for *"a card that
genuinely needed an operator decision and got one."* The operator rescinded it the same day
(`§42`). What makes it retrospective-grade rather than an isolated error: **the closure cycle had
the same instruments this one has**, and the numbers it would have needed were derivable by the same
commands. What it lacked was the question — *is the remainder workable, or merely large?* Sixteen
lane rounds since have answered that question for three kinds, and the answer moved a "4,300-unit
backlog" to **63 workable units**. The lesson is that **a remainder is not a ceiling and a ceiling is
not a remainder**, and neither is legible without a row classifier (`§45.1`, `§46.1`).

**(b) A server crash cost round 2, and the board could not see it.** The race-trait round-2 cycle
died with its workflow and left 5 commits — including a real 72-record ingest — stranded on
`worktree-wf_9029acd8-6b0-7`; `kanban.md` and `progress.md` both read as though nothing had landed
until the orchestrator recovered and merged them (`e1f0bdd9`). **The board is not an instrument for
merged-ness.** The standing rule this bundle now runs under — commit AND push to `origin/tranche/9`
as you go, and grep `origin` for a dependency's symbols before claiming it done — exists because of
this, and this cycle followed it in §2 before doing anything else.

Both are emitted to the retrospective log rather than left in prose.

### 6. What a closure cycle will need when the lanes ARE dry

Recorded so run 3 does not re-derive the shape:

1. **Cards 8 and 12 to COMPLETE-or-DRY on their own receipts** — 63 workable units, two lanes,
   both with registered chassis. Neither needs a new decision.
2. `docs/architecture/` refresh to current-state truth (closure-template §6) — **not done here**,
   deliberately: refreshing it to describe a state the bundle has not reached is how the first
   closure's documents came to disagree with its board.
3. `release-notes.md` "Known issues" — still describes **unfinished work**, not shipped issues. It
   is left that way *because the work is still unfinished*; rewriting it now would make it wrong.
   The rewrite is a real closure task and it is named here so it is not forgotten.
4. **PR #360 updated, never replaced.** It is `OPEN`, `tranche/9` → `develop`, and this cycle left
   it untouched apart from what the pushed commits show. Do not open a second PR. Do not merge —
   the operator merges.
5. `decisions.md §66`'s standing condition on any closure statement, unsoftened: the bundle-scope
   `scripts/wired-integration-audit.sh` run is **NOT clean** (exit 1, 13 hand-classified
   `placeholder` hits, remedy named as C1.4b). Run 1 reported it clean; a run-3 closure that does
   the same repeats the error verbatim.

### 7. Unattended-mode defaults taken this cycle (recorded, not raised)

1. **Merged rather than reset** on finding the checkout diverged 2/143. A reset would have been
   faster and would have destroyed two commits of real work.
2. **Renumbered the *unpushed* `§46` to `§67`** rather than the pushed one, and left a seam note.
   The safer default is to move the number nobody has cited yet.
3. **Did not regenerate `docs/work-inventory.json`.** It was regenerated on this merged tree by
   `3b0a414c`; a `cargo run` here would have contended with the in-flight gate for one
   `CARGO_TARGET_DIR` and could have produced a plausible wrong number rather than an error
   (`AGENTS.md`, shared-target-dir).
4. **Did not run `verify-on-screen.sh`** — see §4; concurrent with a full gate is a recorded OOM.
5. **Did not touch `release-notes.md` or `docs/architecture/`** — see §6.2 and §6.3. Both edits are
   closure edits and this is not a closure.
6. **Changed no card's STATUS; annotated exactly one card's NOTE.** Cards 8 and 12 were already
   `READY` on the merged tree with accurate round pointers, and card 10 was already `DRY` — this
   cycle reproduced all three figures and had nothing to correct, which is itself worth recording:
   the board and the corpus agree at this tip. Card 16 (`epic-11-closure`) stays `READY` and its
   note now carries run 2's ruling and run 3's precondition, because a `READY` row that still reads
   as though only the rescinded run 1 had happened is the kind of stale board entry that cost this
   bundle its round-2 recovery.
