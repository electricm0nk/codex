# SD-22 release closure checklist: build version bump
...
The version is a `<major>.<tranche-base>.<build>` triple, e.g. `0.5.95`:
...
- **`<tranche-base>`** — increments per-tranche-promotion (moving to a new
  tranche integration branch, e.g. `tranche/4` → `tranche/5`). Dash releases
  off the same tranche (e.g. `tranche/4-1`) keep the same `<tranche-base>`
  value.
- **`<build>`** — increments per-CI-build (one per `GITHUB_RUN_NUMBER`; the
  ...
  branch). Never resets within a tranche; it is a single counter shared
  across every branch, not per-tranche.
...
version" step, which reads:

```
VERSION="0.4.${GITHUB_RUN_NUMBER}"
```

Bump the `<major>.<tranche-base>` prefix here too whenever it changes — this
is the publish-time stamp that overwrites `package.json` and
`tauri.conf.json` at CI publish time, so if this line is left on an old
prefix, every published/tester build will keep showing the old numbering
scheme regardless of what the repo files say. `GITHUB_RUN_NUMBER` is already
the correct monotonic, never-resets counter for the `<build>` position — do
not touch that part.
...
```
feat(sd22): bump version to <major>.<tranche-base>.<build>
```

for example:

```
feat(sd22): bump version to 0.5.95
```
