...
| Oracle-parity comparator | `oracle_validation` is still structurally incapable of *claiming* parity: `DimensionStatus` has no `Passed` variant (only `Candidate`/`Blocked`/`NotYetGrounded`), `ClaimTierFloor` has no `OracleChecked` variant (only `Computed`); the in-crate comparator and report-writer are absent. SD-25 Epic 4 landed the *runner half* as out-of-crate scaffolding — `scripts/pcgen-run-character.sh` (real headless PCGen Gradle batch-export) + `scripts/pcgen-normalize-output.py` (XML → golden-fixture shape), smoke-tested end-to-end by `tests/pcgen_runner_smoke.rs`. That pipeline deliberately asserts only structural/identity comparability, not numeric parity (the pilot golden fixture's own `codex_output_state=unresolved` says it is not yet value-comparable), so no parity claim exists anywhere yet. | `src/oracle_validation/golden_fixture.rs:117-121`; `src/oracle_validation/selected_parity_dimensions.rs:12-16`; `scripts/pcgen-run-character.sh`; `scripts/pcgen-normalize-output.py` |
