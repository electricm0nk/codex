...
> **SD-36 Epic B closure epilogue (2026-09-17, operator ruling D3):** `src/bin/v06_work_inventory.rs`
> (the classifier/gate every corpus-coverage section below cites), `src/rules_core/support_state_matrix.rs`,
> its desktop bridge, and `apps/desktop/src-tauri/src/reach_gate.rs` are **retired** — deleted outright,
> not refactored. `docs/work-inventory.json` is now a FROZEN sidecar (`docs/work-inventory.FROZEN.md`),
> no longer regenerated by any of these instruments. Every "Corpus coverage" section from here down,
> and every inline mention of `v06_work_inventory`/`reach_gate` inside them, is **historical narrative
> about a retired instrument** — marked `(retired, SD-36 D3)` at each live-voice call site — and none
> of it is re-derivable against the current tree. The one live, current-state corpus-completion figure
> is now the frozen public status snapshot: `site/status-data.json` (`overall.pct` 100.0 of 49,450
> units, `scripts/site/check_frozen_status.py` gates it never drifting) plus its per-book detail files
> under `site/status-data/`. No other row in this doc re-verified this pass.
...
* **`core_essentials` is no longer an engine book for companions or races.** `decisions.md §9`
  ...
  `ultimate_magic` 32 → 59, `apg` 4 → 17, `crb` 84 → 84. `reach_gate`'s (retired, SD-36 D3) companion claim went from
...
* **The `race` kind's doneness verdict now reads the product, not the seven-variant CRB enum.**
  ...
  `src/rules_core/race_creation.rs`, so `v06_work_inventory` (retired, SD-36 D3) calls the SAME function that builds
...
* **The `decisions.md §9` re-attribution widening in `v06_work_inventory::classify` (retired, SD-36 D3) requires the
...
* **A consumer can now NAME the record whose magnitude it read.** That last field is the pattern:
  ...
  `v06_work_inventory`'s (retired, SD-36 D3) probe credits THAT record and nothing else. It is the record-level
...
* **-116, operator ruling §16 (`decisions.md §9`/§16, `artifacts/OPERATOR-RULINGS-2026-08-19.md`):
  ...
  asserted every real run of `v06_work_inventory` (retired, SD-36 D3), not just `cargo test`) was added this cycle after
...
* **A measurement-instrument blind spot for an entire content family.** `v06_work_inventory.rs`'s (retired, SD-36 D3)
...
* **A cross-cutting instrument blind spot was found and logged, not silently trusted.** A separate
  ...
  it never enumerates. The real gate (`v06_work_inventory`'s `modelled_class_books()`, retired, SD-36 D3) is unaffected
...
own regenerations). `status: "unknown"` — a record `v06_work_inventory.rs`'s (retired, SD-36 D3)
...
`classify()` call sites (`AT-33-E4-002`, `src/bin/v06_work_inventory.rs` (retired, SD-36 D3),
