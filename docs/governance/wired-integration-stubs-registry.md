...
(Entries 0007-000n reserved for the remaining 17 `book_stub` future-state books (criteria 4.4, 4.6-4.22, minus `beginner_box` done by 4.2 and `bestiary_2` done by 4.5) plus any other operator-directed exceptions. Any accidental stub found by the per-cycle audit goes into `risks-and-open-questions.md` as a Wired Integration Cleanup candidate, not here — the registry is operator-granted only.)
