...
(Entries 0023-000n reserved for the remaining 1 `book_stub` future-state book (`ultimate_wilderness`, per `content-unit-inventory.md §2.2`'s 21-book list) under whichever of the two still-open criterion numbers — 4.12, 4.22 — the next cycle resolves it to; `content-unit-inventory.md`'s "Pre-existing count discrepancy" note flags 4.12 as a likely orphaned criterion number left over from the bundle's original 22-book miscount, so only one of {4.12, 4.22} is expected to actually register `ultimate_wilderness`, with the other closed as a no-op per operator confirmation) plus any other operator-directed exceptions. Any accidental stub found by the per-cycle audit goes into `risks-and-open-questions.md` as a Wired Integration Cleanup candidate, not here — the registry is operator-granted only.)
