...
(Entries 0016-000n reserved for the remaining 8 `book_stub` future-state books (criteria 4.12-4.22, minus `beginner_box` done by 4.2, `bestiary_2` done by 4.5, `bestiary_3` done by 4.4, `bestiary_4` done by 4.6, `bestiary_5` done by 4.7, `bestiary_6` done by 4.8, `bonus_bestiary` done by 4.9, `core_essentials` done by 4.10, `horror_adventures` done by 4.11, `monster_codex` done by 4.13, and `mythic_adventures` done by 4.14) plus any other operator-directed exceptions. Any accidental stub found by the per-cycle audit goes into `risks-and-open-questions.md` as a Wired Integration Cleanup candidate, not here — the registry is operator-granted only.)
