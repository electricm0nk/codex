...
(Entries 0008-000n reserved for the remaining 16 `book_stub` future-state books (criteria 4.6-4.22, minus `beginner_box` done by 4.2, `bestiary_2` done by 4.5, and `bestiary_3` done by 4.4) plus any other operator-directed exceptions. Any accidental stub found by the per-cycle audit goes into `risks-and-open-questions.md` as a Wired Integration Cleanup candidate, not here — the registry is operator-granted only.)
