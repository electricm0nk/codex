...
| `evidence_freshness` | `AwaitingInitialEvidence` | No Half-Elf deterministic pilot fixture is yet minted; nothing to refresh from |
...
3. **No Half-Elf `ComputationExplanation` is emitted.** There is **no** `race.half_elf.ability_bonus_target` analog (a Half-Elf deterministic pilot fixture is required before one can be honestly grounded) and no `race.half_elf.bonus_feat_grant` analog (Half-Elf gets no named bonus feat at level 1 in PF1).
...
- **Half-Elf ability-bonus to one chosen ability.** No Half-Elf deterministic pilot fixture is minted; no `choice:half_elf_ability_bonus` choice set is defined; no per-ability-modifier emission is wired up.
...
| `tests/sd13_race_half_elf_bounded_semantics.rs` | New bounded proof surface |
| `tests/sd13_support_state_matrix.rs` | Add a Half-Elf-row classifier pin |
...
1. Mint a Half-Elf deterministic pilot fixture (parallel to `pf1_human_fighter_level1_sd13_deterministic_input.txt`).
