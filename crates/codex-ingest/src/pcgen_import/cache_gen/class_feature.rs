...
use codex::rules_core::pi_screening::{self, DeclaredProductIdentity, PI_BLACKLIST_TERMS};
...
pub struct CacheRecord {
    ...
    pub license: codex::rules_core::shape_b_v1::License,
...
fn redact_desc_token_if_pi(tokens: &mut [RawToken], license: codex::rules_core::shape_b_v1::License) {
    if license != codex::rules_core::shape_b_v1::License::PiRedacted {
        return;
    }
    for t in tokens.iter_mut() {
        if t.key == "DESC" {
            t.value = codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string();
        }
    }
}
...
fn redact_concatenated_blacklist_tokens(tokens: &mut [RawToken]) -> bool {
    ...
    for t in tokens.iter_mut() {
        if t.value.is_empty() || t.value == codex::rules_core::shape_b_v1::REDACTED_PI_MARKER {
            continue;
        }
    ...
    let scrubbed = tokens
        ...
        .map(|t| {
            ...
            if blacklist_hit || identity_hit {
                any_redacted = true;
                RawToken { key: t.key.clone(), value: codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string() }
            } else {
                t.clone()
            }
        })
...
pub fn generate(
    ...
    for (book, book_units) in units_by_book {
        ...
        for unit in book_units {
            ...
            let name_is_pi = declared.name || name_license == codex::rules_core::shape_b_v1::License::PiRedacted;
            ...
            let stored_desc = match &stored_desc {
                Some(v) if v != codex::rules_core::shape_b_v1::REDACTED_PI_MARKER => {
                    if pi_screening::blacklist_term_hit_including_concatenated(v).is_some() {
                        ...
                        Some(codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string())
                    } else {
                        stored_desc
                    }
                }
            ...
            if concat_redacted && license != codex::rules_core::shape_b_v1::License::PiRedacted {
                license = codex::rules_core::shape_b_v1::License::PiRedacted;
                pi_marker = Some(codex::rules_core::shape_b_v1::PI_MARKER_REDACTED.to_string());
            }
            if concat_redacted {
...
mod tests {
    ...
    fn a_blacklisted_name_is_flagged_even_with_no_nameispi_declaration() {
        ...
        assert_eq!(license, codex::rules_core::shape_b_v1::License::PiRedacted);
        let (license2, _, _, _) = pi_screening::classify_field("name", "Death (Pharasma)");
        assert_eq!(license2, codex::rules_core::shape_b_v1::License::PiRedacted);
    }
    ...
    fn name_skip_is_the_union_of_declared_and_blacklisted() {
        fn should_skip(declared_name: bool, name: &str) -> bool {
            ...
            declared_name || name_license == codex::rules_core::shape_b_v1::License::PiRedacted
        }
    ...
    fn scrub_name_pi_tokens_redacts_identity_restatement_and_blacklist_hits_only() {
        ...
        assert_eq!(by_key["KEY"], codex::rules_core::shape_b_v1::REDACTED_PI_MARKER, "own key restatement must scrub");
    ...
    fn scrub_name_pi_tokens_does_not_over_redact_a_clean_formula_sharing_a_generic_key_segment() {
        ...
        let by_key: BTreeMap<&str, &str> = scrubbed.iter().map(|t| (t.key.as_str(), t.value.as_str())).collect();
        assert_eq!(by_key["KEY"], codex::rules_core::shape_b_v1::REDACTED_PI_MARKER, "own key restatement must still scrub");
        assert_eq!(
    ...
    fn generate_redacts_a_class_field_resolved_from_a_real_corpus_class_name_that_is_itself_pi() {
        ...
        assert_eq!(
            ...
            Some(codex::rules_core::shape_b_v1::REDACTED_PI_MARKER),
    ...
    fn generate_keeps_description_in_pi_field_when_both_name_and_desc_are_pi() {
        ...
        let json: Value = serde_json::from_str(&written).unwrap();
        assert_eq!(json["data"]["description"].as_str(), Some(codex::rules_core::shape_b_v1::REDACTED_PI_MARKER));
    ...
    fn redact_desc_token_if_pi_redacts_only_desc_when_license_says_pi_redacted() {
        ...
        redact_desc_token_if_pi(&mut tokens, codex::rules_core::shape_b_v1::License::PiRedacted);
        assert_eq!(
    ...
    fn redact_desc_token_if_pi_is_a_no_op_when_license_is_not_pi_redacted() {
        ...
        redact_desc_token_if_pi(&mut tokens, codex::rules_core::shape_b_v1::License::Ogl);
    ...
    fn redact_concatenated_blacklist_tokens_redacts_a_pascalcase_concatenated_hit() {
        ...
        assert!(any_redacted);
        assert_eq!(tokens[0].value, codex::rules_core::shape_b_v1::REDACTED_PI_MARKER, "KEY carries a word-bounded hit too");
        assert_eq!(tokens[1].value, codex::rules_core::shape_b_v1::REDACTED_PI_MARKER);
        assert_eq!(tokens[2].value, codex::rules_core::shape_b_v1::REDACTED_PI_MARKER);
    ...
    fn redact_concatenated_blacklist_tokens_skips_a_value_already_redacted() {
        let mut tokens = vec![RawToken {
            key: "DESC".to_string(),
            value: codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string(),
        }];
        // The marker string itself must never trip a second (spurious) hit.
        let any_redacted = redact_concatenated_blacklist_tokens(&mut tokens);
        assert!(!any_redacted);
        assert_eq!(tokens[0].value, codex::rules_core::shape_b_v1::REDACTED_PI_MARKER);
    }
