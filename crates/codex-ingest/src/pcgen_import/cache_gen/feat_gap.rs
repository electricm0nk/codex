...
pub fn generate(
    ...
    for spec in BOOK_SPECS {
        ...
        for (record_index, entry) in feat_gap_rows_for(spec.rule_set).iter().enumerate() {
            ...
            if declared.name || name_license == codex::rules_core::shape_b_v1::License::PiRedacted {
...
mod tests {
    ...
    fn screen_prerequisites_redacts_a_plainly_spelled_blacklist_term() {
        ...
        assert!(any_redacted);
        assert_eq!(screened, vec![codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string()]);
    }
    ...
    fn screen_prerequisites_leaves_clean_lines_untouched_and_redacts_only_the_hit() {
        ...
        assert_eq!(
            screened,
            vec!["PRETOTALAB:8".to_string(), codex::rules_core::shape_b_v1::REDACTED_PI_MARKER.to_string()]
        );
    }
