...
//! of [`codex::rules_core::composed_input::compose`] and false of the
...
///
/// SD-36 Epic A / operator ruling D1: this used to be an inherent
/// `impl ComposedInputDiagnostic` method. Now that `pcc_package_loader.rs`
/// lives in `codex-ingest` and `ComposedInputDiagnostic` stays in `codex`,
/// an inherent impl here would violate the orphan rule (`E0116`) -- a
/// crate may only add inherent methods to a type it defines. Every field
/// on `ComposedInputDiagnostic` is `pub`, so a free function building it
/// via struct literal crosses the wall the same way any other
/// `codex::rules_core` type construction does.
fn include_resolution_failure(include: &IncludeDiagnostic) -> ComposedInputDiagnostic {
    ComposedInputDiagnostic {
...
/// and then calls [`compose`](codex::rules_core::composed_input::compose) to wire it to the character input.
///
...
/// a [`SourcePackageContent`], then [`compose`](codex::rules_core::composed_input::compose)s it with the chosen
/// `CharacterInput`.
pub fn load_composed_core_rulebook(
    ...
    let resolution = match resolve_pcc_includes_from(corpus_root.as_ref(), pcc_path) {
        ...
        Err(include) => {
            diagnostics.push(include_resolution_failure(
                &include,
            ));
    ...
    for include in &resolution.diagnostics {
        diagnostics.push(include_resolution_failure(include));
    }
...
/// [`CORE_RULEBOOK_PACKAGE_ID`](codex::rules_core::composed_input::CORE_RULEBOOK_PACKAGE_ID) is the canonical choice for the
/// pre-loop slice.
...
mod tests {
    use super::*;
    use codex::rules_core::composed_input::CORE_RULEBOOK_PACKAGE_ID;
