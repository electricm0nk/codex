...
//! `codex_ingest::pcgen_import::race_trait_formula_binding::resolve_same_row_formula`,
...
use codex_ingest::pcgen_import::race_trait_formula_binding::resolve_same_row_formula;
...
use codex_ingest::pcgen_import::race_trait_tokens::ADOPTED_RACE_SELECTOR_CHOOSE_PREFIX;
use codex::rules_core::race_resolver::{ADOPTED_RACE_SELECTOR_TYPE, ADOPTIVE_PARENTAGE_CATEGORY};
use codex_ingest::pcgen_import::ingest_payload::{RaceTraitCacheData, RawBonusChain, RawToken};
...
fn ingest_book(book: &BookSource) {
    ...
    let out_root =
        codex_ingest::repo_root().join("data/corpus").join(corpus_book).join("race_trait");
...
mod tests {
    ...
    fn no_committed_trait_description_leaks_pcgen_syntax_in_any_declared_book() {
        ...
        for book in BOOK_SOURCES {
            let trait_root =
                codex_ingest::repo_root().join("data/corpus").join(book.corpus_book).join("race_trait");
