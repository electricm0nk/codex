...
//! `codex_ingest::pcgen_import::cache_gen::class_feature_grants`'s module doc
...
use codex_ingest::pcgen_import::cache_gen::class_feature_grants;
...
fn main() {
    ...
    let grants_root = PathBuf::from(manifest_dir.clone()).join("data/class_feature_grants");
    let repo_corpus_root = PathBuf::from(manifest_dir.clone()).join("data/corpus");
