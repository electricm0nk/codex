//! One-off generation entry point for the spell class-access `.MOD` row
//! JSON cache (SD-32 `decisions.md §20`). Run via
...
//! already-written row does NOT fail this binary; it exits non-zero only
//! if nothing at all resolved.

use std::path::PathBuf;
use std::process::Command;

use codex_ingest::pcgen_import::cache_gen::spell_mod_access;
...
fn main() {
    ...
    let manifest_dir = codex_ingest::repo_root().display().to_string();
    let out_root = PathBuf::from(manifest_dir).join("data/corpus");

    let ingested_at = real_now_iso8601();

    match spell_mod_access::generate(&corpus_root, &out_root, &ingested_at) {
        Ok(report) => {
            println!(
                "Spell mod-access cache generated: {} records; ingested_at={ingested_at}",
        ...
        Err(e) => {
            eprintln!("spell mod-access cache generation failed: {e:?}");
            std::process::exit(1);
        }
    }
}
