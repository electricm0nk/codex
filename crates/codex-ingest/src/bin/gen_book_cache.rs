...
fn wiring_class_for_source(
    ...
    lines: &mut codex_ingest::pcgen_import::wiring_class::CorpusLines,
    source: &CorpusSource,
) -> (String, Vec<String>) {
...
fn render_player_facing_description(record_key: &str, raw: &str) -> String {
    let rendered = codex_ingest::pcgen_import::pcgen_desc::render_pcgen_desc(raw);
    if let Some(leak) = codex_ingest::pcgen_import::pcgen_desc::leaked_pcgen_syntax(&rendered.text) {
        panic!(
...
fn gen_pathfinder_unchained() {
    ...
    let out_root = codex_ingest::repo_root().join("data/corpus/pathfinder_unchained");
    ...
    if out_root.join("feat").exists() {
        ...
        codex_ingest::pcgen_import::cache_gen::ultimate_equipment::remove_stale_owned_files(
            &out_root.join("feat"),
            &current_feat_keys,
            &|_path, _line| true,
        );
...
fn gen_advanced_race_guide() {
    ...
    let out_root = codex_ingest::repo_root().join("data/corpus/advanced_race_guide");
    let ingested_at = ingested_at_now();
...
fn gen_monster_book(spec: &MonsterBookSpec) {
    ...
    let out_root = codex_ingest::repo_root().join("data/corpus").join(book_id);
    let ingested_at = ingested_at_now();
