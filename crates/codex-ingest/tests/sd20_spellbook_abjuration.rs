//! SD-20 Epic 2 cycle 1 — Abjuration spellbook engine RED test
...
//! least one spell of the landed school (Abjuration) produces a
...
//! `cr_spells.lst`-shaped row for "Shield", independently confirmed
//! against `rules_tables::crb::spell_list::SPELL_LIST`'s own Abjuration
//! entry before this test was written — school Abjuration, level 1,
...
//! Abjuration spell selection to exercise its full round trip.
...
fn corpus_with_shield() -> SourcePackageContent<'static> {
    let raw_line = "Shield\tSCHOOL:Abjuration\tCLASSES:Sorcerer=1,Wizard=1\tDESCRIPTION:Invisible \
        shield of force.";
    let parsed = parse_lst_spell_row("sd20_spellbook_abjuration", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_shield_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        case_id: Some("sd20_spellbook_abjuration".to_string()),
        source_package_id: "sd20_spellbook_abjuration".to_string(),
...
#[test]
fn abjuration_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_shield();
    let input = wizard_input_with_shield_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Shield");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Abjuration);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Shield");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int = compute_spellbook_coverage(&wizard_input_with_shield_prepared(10), &corpus);
    let high_int = compute_spellbook_coverage(&wizard_input_with_shield_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_shield_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_shield();
    let input = wizard_input_with_shield_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
