...
//! `codex_ingest::pcgen_import::ir_converter`, which projects a parsed
...
use codex_ingest::pcgen_import::lst_parser::class::{parse_class_entries, ClassEntry, ClassToken};
...
use codex_ingest::pcgen_import::ir_content_payload::{IrContentPayload, IrContentRecord, IrPackageContent};
...
fn v3_malformed_record_produces_error_diagnostic_naming_the_kind() {
    ...
    // `MalformedRecord`.
    use codex_ingest::pcgen_import::lst_parser::metadata::{LstDiagnostic, LstDiagnosticKind};
    ...
    let parsed_doc = codex_ingest::pcgen_import::lst_parser::metadata::LstMetadataDocument {
        source_path: "bad_meta.lst".to_string(),
        records: vec![bad_record],
    };
    let (pkg, forwarded) = (
        codex_ingest::pcgen_import::ir_converter::convert_package_from_lst_metadata_document(
            &parsed_doc,
            "test_pkg",
    ...
    assert_eq!(
        d.severity,
        codex_ingest::pcgen_import::ir_converter::IRDiagnosticSeverity::Warning
    );
