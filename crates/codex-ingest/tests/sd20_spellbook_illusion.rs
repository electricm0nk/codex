//! SD-20 Epic 2 cycle 6 — Illusion spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Sixth PF1 spell school landed per §1.2 Step 2's cycle order (abjuration
...
//! `4bcfceb`; this cycle lands illusion; necromancy, transmutation,
//! universal remain).
//! Mirrors `tests/sd20_spellbook_evocation.rs` exactly, substituting a
//! real Illusion spell for the real Evocation spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Illusion) produces a non-empty
...
//! `cr_spells.lst`-shaped row for "Color Spray" -- independently
//! confirmed against `rules_tables::crb::spell_list::SPELL_LIST`'s own
//! Illusion entry before this test was written: school Illusion, level 1,
//! per the loop instruction's Step 4 corpus-existence check) rather than
...
//! Illusion spell selection to exercise its full round trip.
...
fn corpus_with_color_spray() -> SourcePackageContent<'static> {
    let raw_line = "Color Spray\tSCHOOL:Illusion\tCLASSES:Sorcerer=1,Wizard=1\t\
    ...
    let parsed = parse_lst_spell_row("sd20_spellbook_illusion", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_color_spray_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        ...
        chosen: ChosenCharacterState {
            ...
            spells_selected: vec![SpellSelection {
                spell_id: "Color Spray".to_string(),
...
#[test]
fn illusion_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_color_spray();
    let input = wizard_input_with_color_spray_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Color Spray");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Illusion);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Color Spray");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int =
        compute_spellbook_coverage(&wizard_input_with_color_spray_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_color_spray_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_color_spray_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_color_spray();
    let input = wizard_input_with_color_spray_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_illusion_spell_effect_returns_none_for_a_non_illusion_spell() {
    // "Charm Person" is a real SPELL_LIST record but Enchantment, not
    // Illusion -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::illusion::resolve_illusion_spell_effect("Charm Person")
            .is_none()
    );
}

#[test]
fn resolve_illusion_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
