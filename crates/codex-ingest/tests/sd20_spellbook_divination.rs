//! SD-20 Epic 2 cycle 3 — Divination spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Third PF1 spell school landed per §1.2 Step 2's cycle order
//! (abjuration landed in `3147b28`, conjuration landed in `4f53724`; this
//! cycle lands divination). Mirrors `tests/sd20_spellbook_conjuration.rs`
//! exactly, substituting a real Divination spell for the real Conjuration
//! spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Divination) produces a
...
//! `cr_spells.lst`-shaped row for "Comprehend Languages" — independently
//! confirmed against `rules_tables::crb::spell_list::SPELL_LIST`'s own
//! Divination entry before this test was written: school Divination,
...
//! resolvable Divination spell selection to exercise its full round trip.
...
fn corpus_with_comprehend_languages() -> SourcePackageContent<'static> {
    let raw_line = "Comprehend Languages\tSCHOOL:Divination\tCLASSES:Sorcerer=1,Wizard=1\t\
        DESCRIPTION:You can understand the spoken words of creatures or read otherwise \
        incomprehensible written messages.";
    let parsed = parse_lst_spell_row("sd20_spellbook_divination", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_comprehend_languages_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        case_id: Some("sd20_spellbook_divination".to_string()),
        source_package_id: "sd20_spellbook_divination".to_string(),
        chosen: ChosenCharacterState {
            ...
            spells_selected: vec![SpellSelection {
                spell_id: "Comprehend Languages".to_string(),
...
#[test]
fn divination_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_comprehend_languages();
    let input = wizard_input_with_comprehend_languages_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Comprehend Languages");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Divination);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Comprehend Languages");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int =
        compute_spellbook_coverage(&wizard_input_with_comprehend_languages_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_comprehend_languages_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_comprehend_languages_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_comprehend_languages();
    let input = wizard_input_with_comprehend_languages_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_divination_spell_effect_returns_none_for_a_non_divination_spell() {
    // "Mage Armor" is a real SPELL_LIST record but Conjuration, not
    // Divination -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::divination::resolve_divination_spell_effect("Mage Armor")
            .is_none()
    );
}

#[test]
fn resolve_divination_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
        codex::rules_core::spellbook::divination::resolve_divination_spell_effect(
            "Not A Real Spell"
        )
        .is_none()
    );
