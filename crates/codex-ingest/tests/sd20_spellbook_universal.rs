//! SD-20 Epic 2 cycle 9 — Universal spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Ninth and FINAL PF1 spell school landed per §1.2 Step 2's cycle order
...
//! landed in `396ebd4`, transmutation landed in `d1d0952`; this cycle
//! lands universal, CLOSING Epic 2 -- all nine PF1 spell schools done).
//! Mirrors `tests/sd20_spellbook_transmutation.rs` exactly, substituting a
//! real Universal spell for the real Transmutation spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Universal) produces a non-empty
...
//! resolvable Universal spell selection to exercise its full round trip.
...
fn corpus_with_permanency() -> SourcePackageContent<'static> {
    let raw_line = "Permanency\tSCHOOL:Universal\tCLASSES:Sorcerer=5,Wizard=5\t\
        DESCRIPTION:This spell makes the duration of certain other spells permanent.";
    let parsed = parse_lst_spell_row("sd20_spellbook_universal", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_permanency_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        case_id: Some("sd20_spellbook_universal".to_string()),
        source_package_id: "sd20_spellbook_universal".to_string(),
...
#[test]
fn universal_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_permanency();
    let input = wizard_input_with_permanency_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Permanency");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Universal);
    assert_eq!(prepared.effect.level, 5);
    assert!(
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Permanency");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    // 10 + spell level (5) + Intelligence modifier drives the DC; two
    // different Intelligence scores must produce two different DCs, which
    // a hardcoded constant could never do.
    let low_int = compute_spellbook_coverage(&wizard_input_with_permanency_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_permanency_prepared(18), &corpus);
    ...
    // INT 10 -> modifier 0 -> DC 10 + 5 + 0 = 15.
    assert_eq!(low_dc, 15, "DC must follow 10 + spell level + ability modifier, not a hardcoded value");
    // INT 18 -> modifier +4 -> DC 10 + 5 + 4 = 19.
    assert_eq!(high_dc, 19, "DC must follow 10 + spell level + ability modifier, not a hardcoded value");
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_permanency_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_permanency();
    let input = wizard_input_with_permanency_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_universal_spell_effect_returns_none_for_a_non_universal_spell() {
    // "Charm Person" is a real SPELL_LIST record but Enchantment, not
    // Universal -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::universal::resolve_universal_spell_effect("Charm Person")
            .is_none()
    );
}

#[test]
fn resolve_universal_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
        codex::rules_core::spellbook::universal::resolve_universal_spell_effect(
            "Not A Real Spell"
        )
        .is_none()
    );
