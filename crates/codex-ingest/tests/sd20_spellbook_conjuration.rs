//! SD-20 Epic 2 cycle 2 — Conjuration spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Second PF1 spell school landed per §1.2 Step 2's cycle order
//! (abjuration landed in `3147b28`; this cycle lands conjuration).
//! Mirrors `tests/sd20_spellbook_abjuration.rs` exactly, substituting a
//! real Conjuration spell for the real Abjuration spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Conjuration) produces a
...
//! `cr_spells.lst`-shaped row for "Mage Armor" — independently confirmed
//! against `rules_tables::crb::spell_list::SPELL_LIST`'s own Conjuration
//! entry before this test was written: school Conjuration, level 1, per
//! the loop instruction's Step 4 corpus-existence check) rather than
//! depending on `CORPUS_ROOT` or hand-copying the full corpus file --
//! `spellbook::compute_spellbook_coverage` only needs one resolvable
//! Conjuration spell selection to exercise its full round trip.
...
fn corpus_with_mage_armor() -> SourcePackageContent<'static> {
    let raw_line = "Mage Armor\tSCHOOL:Conjuration\tCLASSES:Sorcerer=1,Wizard=1\tDESCRIPTION:An \
        invisible but tangible field of force surrounds the subject, providing a +4 armor \
        bonus to AC.";
    let parsed = parse_lst_spell_row("sd20_spellbook_conjuration", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_mage_armor_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        case_id: Some("sd20_spellbook_conjuration".to_string()),
        source_package_id: "sd20_spellbook_conjuration".to_string(),
...
#[test]
fn conjuration_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_mage_armor();
    let input = wizard_input_with_mage_armor_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Mage Armor");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Conjuration);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Mage Armor");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int = compute_spellbook_coverage(&wizard_input_with_mage_armor_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_mage_armor_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_mage_armor_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_mage_armor();
    let input = wizard_input_with_mage_armor_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_conjuration_spell_effect_returns_none_for_a_non_conjuration_spell() {
    // "Shield" is a real SPELL_LIST record but Abjuration, not
    // Conjuration -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::conjuration::resolve_conjuration_spell_effect("Shield")
            .is_none()
    );
}

#[test]
fn resolve_conjuration_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
        codex::rules_core::spellbook::conjuration::resolve_conjuration_spell_effect(
            "Not A Real Spell"
        )
        .is_none()
    );
