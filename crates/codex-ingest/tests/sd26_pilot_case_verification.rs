...
use codex_ingest::oracle_validation::comparator::{compare, MismatchReason};
use codex_ingest::oracle_validation::golden_fixture::{load_golden_case_fixture, ClaimTier};
use codex_ingest::oracle_validation::normalization::default_normalization_rules;
use codex_ingest::oracle_validation::parity_report::{
    default_parity_report_dir, write_parity_report,
};
use codex_ingest::oracle_validation::pcgen_runner::{run_pcgen_character, PcgenRunOptions};
use codex_ingest::oracle_validation::selected_parity_dimensions::SelectedParityDimensions;
...
const GOLDEN_FIXTURE_TEXT: &str =
    include_str!("../../../tests/fixtures/oracle_validation/pf1_human_fighter_level1_golden_fixture.txt");
...
fn assert_pilot_pcg_fixture_is_pinned(path: &std::path::Path) {
    let actual = codex_ingest::pcgen_import::cache_gen::apg::sha256_file(path)
...
fn load_golden_fixture_or_panic() -> codex_ingest::oracle_validation::golden_fixture::GoldenCaseFixture {
...
fn full_pipeline_runs_end_to_end_and_finds_one_genuine_attack_bonus_mismatch() {
    ...
    if !pcgen_gradle_wrapper_is_runnable(&pcgen_repo_dir) {
        eprintln!(
            "[skip] sd26_pilot_case_verification: real PCGen headless export not available under {} \
