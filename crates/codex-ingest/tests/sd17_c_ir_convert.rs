...
//! `node.payload` directly via [`IrContentPayload`].
//!
//! ## Verification coverage
//!
//! - V1 round-trip per kind: every B-family entry type projects into
//!   the matching `IrContentPayload` variant and back.
...
use codex_ingest::pcgen_import::ir_converter::{
...
use codex_ingest::pcgen_import::lst_parser::class::{parse_class_entries, ClassEntry};
use codex_ingest::pcgen_import::lst_parser::equipment::{
    parse_equipment_entries, EquipmentRecord, EquipmentRecordKind,
};
use codex_ingest::pcgen_import::lst_parser::metadata::{parse_lst_metadata_text, LstRecord, MetadataKind};
use codex_ingest::pcgen_import::lst_parser::race_ability::{
...
use codex_ingest::pcgen_import::lst_parser::spellcasting_class::{
    parse_spellcasting_class_entries, CastingPosture, SpellcastingClassEntry,
};
use codex_ingest::pcgen_import::ir_content_payload::IrContentPayload;
use codex_ingest::pcgen_import::ir_converter::{convert_equipment_record_ir, convert_spell_record_ir};
...
";
    ...
    // canonical `IrContentPayload::Class(&ClassEntry)` borrow.
...
";
    ...
    for (i, inner) in parsed.records.iter().enumerate() {
        ...
        match record.payload {
            ...
            _ => panic!("record {} expected IrContentPayload::Metadata", i),
        }
...
fn v2_metadata_diagnostic_is_forwarded_as_ir_diagnostic_warning() {
    ...
    match record.payload {
        ...
        _ => panic!("expected IrContentPayload::Metadata"),
    }
}
...
fn v2_equipment_record_level_diagnostic_is_forwarded_with_b5_slice_tag() {
    ...
    use codex_ingest::pcgen_import::lst_parser::equipment::EquipmentParseResult;
...
fn v3_malformed_metadata_diagnostic_surfaces_in_ir_pipeline() {
    ...
    match malformed_record.payload {
        IrContentPayload::Metadata(p) => assert_eq!(p.name, ""),
        _ => panic!("expected IrContentPayload::Metadata"),
    }
}
...
#[test]
fn ir_content_payload_kind_token_is_canonical() {
...
fn equipment_record_carries_record_kind_and_token_payload() {
    ...
    let record = convert_equipment_record_ir(equip);
    match record.payload {
        IrContentPayload::Equipment(p) => {
        ...
        _ => panic!("expected IrContentPayload::Equipment"),
    }
}
...
fn ability_kind_enum_is_preserved_through_conversion() {
    ...
    match record.payload {
        ...
        _ => panic!("expected IrContentPayload::Ability"),
    }
}
...
fn metadata_kind_is_preserved_through_conversion() {
    ...
    for (text, expected_kind, expected_inner) in kinds.iter() {
        ...
        match record.payload {
            IrContentPayload::Metadata(p) => assert_eq!(p.kind, *expected_kind),
            _ => panic!("expected IrContentPayload::Metadata"),
        }
    }
}
...
fn spell_record_preserves_optional_field_set_through_conversion() {
    ...
    match record.payload {
        ...
        _ => panic!("expected IrContentPayload::Spell"),
    }
}
...
fn class_entry_carries_all_tab_delimited_tokens_through_conversion() {
    ...
    match record.payload {
        ...
        _ => panic!("expected IrContentPayload::Class"),
    }
}
...
fn spellcasting_class_entry_carries_casting_posture_through_conversion() {
    ...
    match record.payload {
        ...
        _ => panic!("expected IrContentPayload::SpellcastingClass"),
    }
}
...
fn equipment_record_kind_round_trips_through_conversion() {
    ...
    let record = convert_equipment_record_ir(&parsed.entries[0]);
