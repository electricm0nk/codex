//! SD-20 Epic 2 cycle 5 — Evocation spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Fifth PF1 spell school landed per §1.2 Step 2's cycle order (abjuration
//! landed in `3147b28`, conjuration landed in `4f53724`, divination landed
//! in `a7568a5`, enchantment landed in `9a9b359`; this cycle lands
//! evocation; illusion, necromancy, transmutation, universal remain).
//! Mirrors `tests/sd20_spellbook_enchantment.rs` exactly, substituting a
//! real Evocation spell for the real Enchantment spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Evocation) produces a non-empty
...
//! `cr_spells.lst`-shaped row for "Burning Hands" -- independently
//! confirmed against `rules_tables::crb::spell_list::SPELL_LIST`'s own
//! Evocation entry before this test was written: school Evocation, level
//! 1, per the loop instruction's Step 4 corpus-existence check) rather
//! than depending on `CORPUS_ROOT` or hand-copying the full corpus file --
//! `spellbook::compute_spellbook_coverage` only needs one resolvable
//! Evocation spell selection to exercise its full round trip.
...
fn corpus_with_burning_hands() -> SourcePackageContent<'static> {
    let raw_line = "Burning Hands\tSCHOOL:Evocation\tCLASSES:Sorcerer=1,Wizard=1\t\
        DESCRIPTION:A cone of searing flame shoots from your fingertips dealing fire damage to any creature in the effect area.";
    let parsed = parse_lst_spell_row("sd20_spellbook_evocation", 1, raw_line);
    let record = parsed
        .record
...
fn wizard_input_with_burning_hands_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        ...
        chosen: ChosenCharacterState {
            ...
            spells_selected: vec![SpellSelection {
                spell_id: "Burning Hands".to_string(),
...
#[test]
fn evocation_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_burning_hands();
    let input = wizard_input_with_burning_hands_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Burning Hands");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Evocation);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Burning Hands");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int =
        compute_spellbook_coverage(&wizard_input_with_burning_hands_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_burning_hands_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_burning_hands_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_burning_hands();
    let input = wizard_input_with_burning_hands_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_evocation_spell_effect_returns_none_for_a_non_evocation_spell() {
    // "Charm Person" is a real SPELL_LIST record but Enchantment, not
    // Evocation -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::evocation::resolve_evocation_spell_effect("Charm Person")
            .is_none()
    );
}

#[test]
fn resolve_evocation_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
        codex::rules_core::spellbook::evocation::resolve_evocation_spell_effect(
            "Not A Real Spell"
        )
        .is_none()
    );
