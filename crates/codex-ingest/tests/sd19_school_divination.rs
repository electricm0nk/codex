//! SD-19 §2.4 spell-school card: Divination.
//!
//! Proves every real-corpus Divination spell in `cr_spells.lst` (50
//! records, independently confirmed via `grep -c "SCHOOL:Divination"
//! cr_spells.lst`, and the "Comprehend Languages" record independently
//! confirmed via `grep -n "^Comprehend Languages\b" cr_spells.lst` before
//! this test was written, per the loop instruction's Step 4
//! corpus-existence check) is (a) resolvable via `spell_id_resolve`, and
//! (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Divination].spells`
...
//! hand-copying 50 fixture files. Skips with a documented `eprintln!` when
...
fn base_input() -> CharacterInput {
    CharacterInput {
        case_id: Some("sd19_school_divination".to_string()),
        source_package_id: "sd19_school_divination".to_string(),
...
#[test]
fn divination_school_spell_count_matches_real_corpus() {
    ...
    let divination: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Divination"))
        .collect();
    ...
    assert_eq!(
        divination.len(),
        50,
        "expected 50 SCHOOL:Divination records in cr_spells.lst"
    );
    assert!(
        divination.iter().any(|r| r.name == "Comprehend Languages"),
        "expected 'Comprehend Languages' among the resolved Divination records"
    );
}

#[test]
fn every_divination_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let divination_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Divination"))
        .collect();
    assert!(
        !divination_records.is_empty(),
        "corpus-existence check must find Divination records"
    );
    ...
    // (a) every Divination spell resolves via spell_id_resolve.
    for record in &divination_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Divination spell '{}'",
            record.name
        );
    }

    // (b) every Divination spell is present in
    // corpus_derived.school_coverage[Divination].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Divination)
        .expect("expected a Divination entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = divination_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Divination].spells to carry every real-corpus Divination spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Divination].table_cell to resolve through the foundation slice's table store"
    );
}
