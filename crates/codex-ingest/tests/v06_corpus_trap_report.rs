...
use codex_ingest::pcgen_import::corpus_trap_baseline::{load_baseline, reconcile_trap_count};
...
fn cache_dir() -> PathBuf {
    codex_ingest::repo_root().join("data/corpus")
}
...
fn load_trap_baseline() -> Vec<codex_ingest::pcgen_import::corpus_trap_baseline::TrapBaselineEntry> {
    let path = codex_ingest::repo_root().join("docs/governance/corpus-trap-baseline.tsv");
...
fn the_trap_baseline_reconciler_actually_fires_on_a_planted_regression_and_a_stale_row() {
    ...
    let baseline = codex_ingest::pcgen_import::corpus_trap_baseline::parse_baseline(synthetic_baseline_text)
...
fn scanner_runs_cold_against_an_uningested_book() {
    ...
    let scan = codex_ingest::pcgen_import::corpus_traps::scan_book(&book).expect("cold scan runs");
...
fn ingested_book_scans_match_pinned_counts() {
    ...
    let b1 = codex_ingest::pcgen_import::corpus_traps::scan_book(&dir.join("bestiary"))
        .expect("bestiary 1 scans");
    assert!(b1.declaring_lines() > 100);
}

fn scan_lst_file(path: &Path) -> codex_ingest::pcgen_import::corpus_traps::FileScan {
