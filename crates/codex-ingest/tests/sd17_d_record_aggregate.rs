...
use codex_ingest::pcgen_import::lst_parser::class::{ClassEntry, parse_class_entries};
use codex_ingest::pcgen_import::lst_parser::equipment::{
    EquipmentRecord, EquipmentRecordKind, parse_equipment_entries,
};
use codex_ingest::pcgen_import::lst_parser::metadata::{LstRecord, MetadataKind, parse_lst_metadata_text};
...
use codex_ingest::pcgen_import::lst_parser::ParsedLstRecord;
...
use codex_ingest::pcgen_import::ir_content_payload::IrContentRecord;
...
use codex_ingest::pcgen_import::ParsedLstRecord as ParsedLstRecordUmbrella;
...
fn d3_class_parser_malformed_emits_diagnostic_no_aggregate_regression() {
    ...
    let malformed = parsed.diagnostics.iter().find(|d| {
        matches!(
            d.kind,
            codex_ingest::pcgen_import::lst_parser::class::LstDiagnosticKind::MalformedSD17B1
        )
    });
    assert!(
        malformed.is_some(),
