//! SD-20 Epic 2 cycle 7 — Necromancy spellbook engine RED test
//! (`SD-20-rules-engine-completeness-scope-draft.md` §1.2,
//! `technical-design.md` §2.1).
//!
//! Seventh PF1 spell school landed per §1.2 Step 2's cycle order (abjuration
...
//! `4bcfceb`, illusion landed in `d5f1926`; this cycle lands necromancy;
//! transmutation, universal remain).
//! Mirrors `tests/sd20_spellbook_illusion.rs` exactly, substituting a real
//! Necromancy spell for the real Illusion spell.
//!
//! Per §1.2's per-cycle test description: a `CharacterInput` with at
//! least one spell of the landed school (Necromancy) produces a non-empty
//! `SpellbookCoverage`; the spell save DC is computed (not hardcoded --
//! asserted against the real PF1 formula, `10 + spell level + ability
//! modifier`, not a literal); bonus slots from a high ability score are
//! computed per `bonus_slots_from_ability`.
//!
//! Builds a minimal in-memory corpus via
//! `pcgen_import::lst_parser::spell::parse_lst_spell_row` (a single real
//! `cr_spells.lst`-shaped row for "Chill Touch" -- independently confirmed
//! against `rules_tables::crb::spell_list::SPELL_LIST`'s own Necromancy
//! entry before this test was written: school Necromancy, level 1, per the
//! loop instruction's Step 4 corpus-existence check) rather than depending
//! on `CORPUS_ROOT` or hand-copying the full corpus file --
//! `spellbook::compute_spellbook_coverage` only needs one resolvable
//! Necromancy spell selection to exercise its full round trip.

use codex_ingest::pcgen_import::ir_converter::convert_spell_record;
use codex_ingest::pcgen_import::lst_parser::spell::parse_lst_spell_row;
...
fn corpus_with_chill_touch() -> SourcePackageContent<'static> {
    let raw_line = "Chill Touch\tSCHOOL:Necromancy\tCLASSES:Sorcerer=1,Wizard=1\t\
        DESCRIPTION:A touch from your hand, which glows with blue energy, disrupts the life force of living creatures dealing 1d6 points of damage.";
    let parsed = parse_lst_spell_row("sd20_spellbook_necromancy", 1, raw_line);
    let record = parsed
        .record
        .expect("the raw row above must parse into a real LstSpellRecord");
    assert_eq!(record.school.as_deref(), Some("Necromancy"));

    let source_ref = SourceRef {
        source_path: "sd20_spellbook_necromancy".to_string(),
        line: 1,
    };
    let mut corpus = SourcePackageContent::empty("sd20_spellbook_necromancy", source_ref);
...
fn wizard_input_with_chill_touch_prepared(intelligence: i16) -> CharacterInput {
    CharacterInput {
        case_id: Some("sd20_spellbook_necromancy".to_string()),
        source_package_id: "sd20_spellbook_necromancy".to_string(),
        chosen: ChosenCharacterState {
            ...
            spells_selected: vec![SpellSelection {
                spell_id: "Chill Touch".to_string(),
...
#[test]
fn necromancy_spell_selection_produces_non_empty_spellbook_coverage() {
    let corpus = corpus_with_chill_touch();
    let input = wizard_input_with_chill_touch_prepared(17); // +3 Intelligence modifier
    ...
    assert_eq!(prepared.effect.spell_id, "Chill Touch");
    assert_eq!(prepared.effect.school, Pf1SchoolId::Necromancy);
    ...
    assert_eq!(prepared.effect.table_cell.row_key, "Chill Touch");
...
fn spell_save_dc_is_computed_from_the_real_pf1_formula_not_hardcoded() {
    ...
    let low_int =
        compute_spellbook_coverage(&wizard_input_with_chill_touch_prepared(10), &corpus);
    let high_int =
        compute_spellbook_coverage(&wizard_input_with_chill_touch_prepared(18), &corpus);
...
fn bonus_slots_from_ability_are_computed_for_a_high_casting_ability() {
    ...
    let input = wizard_input_with_chill_touch_prepared(18);
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
fn a_low_ability_modifier_grants_no_bonus_slots() {
    let corpus = corpus_with_chill_touch();
    let input = wizard_input_with_chill_touch_prepared(10); // +0 modifier
    let coverage = compute_spellbook_coverage(&input, &corpus);
...
#[test]
fn resolve_necromancy_spell_effect_returns_none_for_a_non_necromancy_spell() {
    // "Charm Person" is a real SPELL_LIST record but Enchantment, not
    // Necromancy -- this school's contribution function must not claim a
    // spell that belongs to a different school.
    assert!(
        codex::rules_core::spellbook::necromancy::resolve_necromancy_spell_effect(
            "Charm Person"
        )
        .is_none()
    );
...
#[test]
fn resolve_necromancy_spell_effect_returns_none_for_an_unknown_spell_id() {
    assert!(
        codex::rules_core::spellbook::necromancy::resolve_necromancy_spell_effect(
            "Not A Real Spell"
        )
