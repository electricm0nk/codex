...
use codex_ingest::pcgen_import::feat_prereq_tokens::joined_catalog_tokens;
use codex::rules_core::character_input::{
    AbilityScores, CharacterClassLevel, CharacterInput, ChosenCharacterState, SkillAllocation,
};
use codex_ingest::pcgen_import::pre_tokens::{
...
fn race_subtypes_match_the_corpus_race_templates() {
    ...
    for (race_token, directory, expected) in expectations {
        ...
        for subtype in *expected {
            ...
            let outcome =
                codex_ingest::pcgen_import::pre_tokens::evaluate_prerequisite_token(
                    &token, &facts,
                );
            assert!(
                matches!(
                    outcome,
                    codex_ingest::pcgen_import::pre_tokens::ClauseOutcome::Met { .. }
                ),
