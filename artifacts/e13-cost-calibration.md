...
- **A shared checkout makes "which commit did that verify.sh run actually verify?" unanswerable from launch time alone — worth naming explicitly for future cycles.** This cycle's own final `verify.sh` run took ~63 minutes for `root-full` alone under contention; a concurrent agent's unrelated clippy fix (`c7c9549f`) landed on the same shared tree partway through that run, so by the time the run reached its `clippy` stage it measured the tree *as it had evolved*, not as it stood at launch. The run's own `RESULT: PASS` is a true statement about the tree at completion time, not proof about the tree at launch time — a reviewer diffing "run started at commit X" against "run measured commit Y" on a long run against a shared checkout needs to check the commit log at completion, not assume launch-time HEAD held for the whole run. This also produced a same-day near-miss on the review side: an earlier verify run's exit-code file was misread against a differently-timestamped file of a similar name (`verify_exit_code.txt` vs `verify_exit_code2.txt`), briefly asserting a stale FAIL against a run that had actually PASSed — caught and corrected within the same exchange, not shipped as a wrong conclusion.
