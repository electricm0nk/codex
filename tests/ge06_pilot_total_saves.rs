...
fn wrong_fighter_level_blocks_total_saves() {
    ...
    // 1-15 (SD13-E5 widened levels 9-10, Weapon Training 2 and the level-10 bonus
    ...
    // seventh bonus-feat cadence slot and the Bravery magnitude rise, and
    // level 15, Armor Training 4, into the supported tranche). A Fighter
    // above that bounded tranche (level 16+, beyond the SD-13/SD-18 level-15
    ...
    let mutated = DETERMINISTIC_FIXTURE
        .replace("class_level=class:fighter:1", "class_level=class:fighter:16");
    assert!(
        mutated.contains("class_level=class:fighter:16"),
