...
fn paladin_level15_spell_level_access_stays_four() {
    ...
    assert!(
        ...
        "level-15 Paladin must still claim-block on the partial-caster spell burden: {:?}",
...
fn paladin_level_16_is_not_promoted_by_this_slice() {
    let level_16 = PALADIN_LEVEL15_FIXTURE.replace("class:paladin:15", "class:paladin:16");
