//! SD-19 §2.4 spell-school card: Transmutation.
//!
//! Proves every real-corpus Transmutation spell in `cr_spells.lst` (152
//! records, independently confirmed via `grep -c "SCHOOL:Transmutation"
//! cr_spells.lst`, and the "Air Walk" record independently confirmed
//! via `grep -m3 "SCHOOL:Transmutation" cr_spells.lst` before this test was
//! written, per the loop instruction's Step 4 corpus-existence check) is
//! (a) resolvable via `spell_id_resolve`, and (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Transmutation].spells`
...
//! hand-copying 152 fixture files. Skips with a documented `eprintln!` when
...
fn base_input() -> CharacterInput {
    CharacterInput {
        case_id: Some("sd19_school_transmutation".to_string()),
        source_package_id: "sd19_school_transmutation".to_string(),
...
#[test]
fn transmutation_school_spell_count_matches_real_corpus() {
    ...
    let transmutation: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Transmutation"))
        .collect();
    ...
    assert_eq!(
        transmutation.len(),
        152,
        "expected 152 SCHOOL:Transmutation records in cr_spells.lst"
    );
    assert!(
        transmutation.iter().any(|r| r.name == "Air Walk"),
        "expected 'Air Walk' among the resolved Transmutation records"
    );
}

#[test]
fn every_transmutation_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let transmutation_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Transmutation"))
        .collect();
    assert!(
        !transmutation_records.is_empty(),
        "corpus-existence check must find Transmutation records"
    );
    ...
    let mut corpus = SourcePackageContent::empty("sd19_school_transmutation", source_ref);
    for record in &transmutation_records {
        corpus.push(convert_spell_record(record));
    }

    // (a) every Transmutation spell resolves via spell_id_resolve.
    for record in &transmutation_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Transmutation spell '{}'",
            record.name
        );
    }

    // (b) every Transmutation spell is present in
    // corpus_derived.school_coverage[Transmutation].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Transmutation)
        .expect("expected a Transmutation entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = transmutation_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Transmutation].spells to carry every real-corpus Transmutation spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Transmutation].table_cell to resolve through the foundation slice's table store"
    );
}

#[test]
fn transmutation_matrix_row_reflects_the_grounded_reachability_proof() {
    let matrix = seeded_current_truth();
    let row = matrix
        .rows
        .iter()
        .find(|r| r.subject_type == MatrixSubjectType::School(Pf1SchoolId::Transmutation))
        .expect("expected a School(Transmutation) row in the seeded matrix");

    assert_eq!(row.support_state, SupportState::Supported);
    assert_eq!(row.evidence_tier, EvidenceTier::ProductVisible);
    assert!(
        row.grounding_ref.contains("sd19_school_transmutation"),
