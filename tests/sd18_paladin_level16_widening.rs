...
fn paladin_level16_spell_level_access_stays_four() {
    ...
    assert!(
        ...
        "level-16 Paladin must still claim-block on the partial-caster spell burden: {:?}",
...
fn paladin_level_17_is_not_promoted_by_this_slice() {
    let level_17 = PALADIN_LEVEL16_FIXTURE.replace("class:paladin:16", "class:paladin:17");
