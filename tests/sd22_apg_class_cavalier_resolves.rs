//! SD-22 Epic 3 acceptance test — APG Cavalier class chassis (criterion
//! 7-8's per-class shape for the second APG class: per-class chassis
//! resolution and the cross-book resolution invariant).
//!
//! Source grounding (per `decisions.md §5`, corrected 2026-07-19): the
//! real `apg_classes.lst:42` `CLASS:Cavalier` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")` (full BAB, no
//! fractional divisor), `BONUS:SAVE|BASE.Fortitude|classlevel(...)/2+2`
//! (good Fortitude), and `BONUS:SAVE|BASE.Will,BASE.Reflex|classlevel(...)/3`
//! (poor Will and Reflex), with `MAXLEVEL:20`. `real_cavalier_class_line`
//! below re-parses that exact line (via the SD-17/SD-22
//! `lst_parser::class` engine, widened this cycle to recognize
...
fn cavalier_level_1_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Cavalier, 1, RuleSetId::Apg)
        .expect("Cavalier level 1 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn cavalier_level_20_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Cavalier, 20, RuleSetId::Apg)
        .expect("Cavalier level 20 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn cavalier_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Cavalier, 21, RuleSetId::Apg),
        None,
        "the real CLASS:Cavalier record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn cavalier_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Cavalier, 1, RuleSetId::Crb),
        None,
...
fn real_cavalier_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Cavalier") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Cavalier BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_cavalier_class_line();
    assert!(
        line.contains("BASEAB|classlevel(\"APPLIEDAS=NONEPIC\")") && !line.contains("*3/4"),
    ...
    assert!(
        line.contains("SAVE|BASE.Will,BASE.Reflex|classlevel(\"APPLIEDAS=NONEPIC\")/3"),
        "real record's Will/Reflex save token should be poor (level/3): {line}"
    );
