...
fn barbarian_level18_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-18 Barbarian must still claim-block on the rage execution burden: {:?}",
...
fn barbarian_level_19_is_not_promoted_by_this_slice() {
    let level_19 = BARBARIAN_LEVEL18_FIXTURE.replace("class:barbarian:18", "class:barbarian:19");
