...
fn barbarian_level13_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-13 Barbarian must still claim-block on the rage execution burden: {:?}",
...
fn barbarian_level_19_is_not_promoted_by_this_slice() {
    let level_19 = BARBARIAN_LEVEL13_FIXTURE.replace("class:barbarian:13", "class:barbarian:19");
