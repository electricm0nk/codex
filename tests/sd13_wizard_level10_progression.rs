...
// (widened from level 10 all the way to level 19 by the SD18
// wizard-level11-widening through wizard-level19-widening cycles, which
// genuinely promote levels 11-19 — see tests/sd18_wizard_level11_widening.rs
...
// made for their own sibling level-10 progression test.)

#[test]
fn wizard_level_20_is_not_promoted_by_this_slice() {
    let level_20 = WIZARD_LEVEL10_FIXTURE.replace("class:wizard:10", "class:wizard:20");
    ...
    assert!(
        ...
        "level-20 Wizard must not gain any bounded wizard chassis explanation: {:?}",
        computation.explanations
    );
}
