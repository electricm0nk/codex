...
// (widened from level 13 to level 15 by the SD18 wizard-level13-widening,
// wizard-level14-widening, and wizard-level15-widening cycles, which
// genuinely promote levels 13, 14, and 15 — see
// tests/sd18_wizard_level13_widening.rs, tests/sd18_wizard_level14_widening.rs,
// and tests/sd18_wizard_level15_widening.rs — mirroring the exact same
// boundary move the Barbarian/Bard/Cleric/Druid/Fighter/Paladin/Rogue/Ranger/
// Sorcerer level-15 widening cycles each made for their own sibling
// level-12 widening test.)

#[test]
fn wizard_level_16_is_not_promoted_by_this_slice() {
    let level_16 = WIZARD_LEVEL12_FIXTURE.replace("class:wizard:12", "class:wizard:16");
...
fn multiclass_wizard_level12_is_not_promoted_by_this_slice() {
    ...
    assert!(
        !computation
            ...
            .any(|e| e.id.starts_with("class_chassis.wizard.")
                || e.id.starts_with("class_feature.wizard.")),
        "multiclass Wizard must not gain any bounded wizard explanation: {:?}",
        computation.explanations
    );
    assert!(
