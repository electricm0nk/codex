...
fn sorcerer_level7_still_claim_blocks_arcane_bond_and_spontaneous_spell_burdens() {
    ...
    assert!(
        ...
        "level-7 Sorcerer must still claim-block on the spontaneous spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn sorcerer_level_8_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-8 Sorcerer must not gain any bounded sorcerer chassis explanation: {:?}",
        computation.explanations
    );
}
