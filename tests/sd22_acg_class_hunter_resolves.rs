//! SD-22 Epic 4 acceptance test — ACG Hunter class chassis (criteria
//! 10-12: per-class chassis resolution and the cross-book resolution
//! invariant). Hunter is the fourth real ACG class ingested (class 4 of
...
//! `acg_classes.lst:108` `CLASS:Hunter` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")*3/4|TYPE=Base.REPLACE`
//! (three-quarter BAB — same posture as APG's
...
//! (poor Will), with `MAXLEVEL:20`, and (separate line) `SPELLSTAT:WIS
//! MEMORIZE:NO` (spontaneous divine casting — belongs in
//! `pcgen_import::lst_parser::spellcasting_class`'s `SPELLCASTING_CLASS_NAMES`
//! allowlist, not `class.rs`'s `MARTIAL_CLASS_NAMES`).
...
//! `rules_tables::acg::class_hunter` stay tied to the source record
...
#[test]
fn hunter_level_1_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Hunter, 1, RuleSetId::Acg)
        .expect("Hunter level 1 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn hunter_level_20_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Hunter, 20, RuleSetId::Acg)
        .expect("Hunter level 20 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn hunter_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Hunter, 21, RuleSetId::Acg),
        None,
        "the real CLASS:Hunter record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn hunter_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Hunter, 1, RuleSetId::Crb),
        None,
...
#[test]
fn hunter_chassis_returns_none_for_ruleset_apg() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Hunter, 1, RuleSetId::Apg),
        None,
        "ACG-only class chassis must not resolve under RuleSetId::Apg"
    );
}

// Cross-class regression: Arcanist, Bloodrager, and Brawler (the first
// three ACG classes landed) must still resolve after Hunter is added to
// the same enum/match arm.
#[test]
fn arcanist_bloodrager_and_brawler_chassis_still_resolve_after_hunter_lands() {
...
fn real_hunter_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Hunter") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Hunter BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_hunter_class_line();
    assert!(
