...
fn ge08_preview_bridge_valid_package_produces_bounded_armor_class() {
    ...
    assert_eq!(envelope.selected_slot_resolution.resolved_feat_id, PROOF_FEAT_ID);
    ...
    assert_eq!(envelope.baseline_armor_class, ArmorClassPreview::Computed(17));
    ...
    assert!(envelope.provenance_refs.iter().any(|p| p.stable_id == PROOF_FEAT_ID));
    assert!(envelope.provenance_refs.iter().any(|p| p.stable_id == PROOF_EFFECT_ID));
...
fn ge08_preview_bridge_blocked_package_returns_blocked_envelope_with_diagnostics() {
    ...
    match &envelope.baseline_armor_class {
        ArmorClassPreview::Blocked(marker) => {
            assert!(!marker.trim().is_empty(), "blocked marker must explain the refusal")
        }
