...
// ----- The animal companion is grounded; the prepared divine burden still fires at level 5 -----

#[test]
fn druid_level5_grounds_the_animal_companion_and_holds_the_prepared_divine_posture() {
    let input = load(DRUID_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    // at level 5. Commit `ae63aa4c` grounded the animal companion's whole
    // progression from the PCGen corpus at all twenty master levels, so at
    // level 5 the burden is genuinely closed and that blocker is correctly
...
#[test]
fn druid_level_6_was_later_widened_into_the_supported_tranche() {
    ...
    assert!(
        ...
        "level-6 Druid was later widened into the supported tranche and must now gain bounded \
         druid chassis explanations: {:?}",
        computation.explanations
    );
}
...
fn multiclass_druid_level5_is_not_promoted_by_this_slice() {
    ...
    // Druid 5/Fighter 1 really does have a companion, at effective druid
    // level 5. Rather than silently tolerate the whole family, pin the
    // thing that actually matters -- the companion is keyed to the DRUID class
    // level (5), not to the character's total level (6). If the engine
    // ever started advancing a multiclass character's companion off total
    // level, these values would be those of master level 6 and this fails.
    assert_wolf_companion_stat_block(
        &computation, 5, 5, 5, 4, 1, 14, 3, 38,
    );
