...
// widened it from 17 to 18, so this file's own negative-control boundary
// moves from 18 to 19, mirroring the exact same boundary-move idiom applied
// to tests/sd18_fighter_level14_widening.rs when MAX_SUPPORTED_FIGHTER_LEVEL
// widened from 14 to 15.)

#[test]
fn cleric_level_19_is_not_promoted_by_this_slice() {
    let level_19 = CLERIC_LEVEL14_FIXTURE.replace("class:cleric:14", "class:cleric:19");
