//! SD-22 Epic 4 acceptance test — ACG Bloodrager class chassis (criteria
//! 10-12: per-class chassis resolution and the cross-book resolution
//! invariant). Bloodrager is the second real ACG class ingested (class 2
//! of the corrected 10-class roster: Arcanist, Bloodrager, Brawler,
//! Hunter, Investigator, Shaman, Skald, Slayer, Swashbuckler, Warpriest —
//! see `progress.md`'s `## Open blockers` entry for the roster
//! correction that dropped the non-existent "Alchemist (ACG-side)" row
//! and added `Slayer`).
//!
//! Source grounding (per `decisions.md §5`): the real
//! `acg_classes.lst:40` `CLASS:Bloodrager` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")|TYPE=Base.REPLACE`
//! (full BAB — no fractional divisor, unlike Arcanist's poor/half BAB),
//! `BONUS:SAVE|BASE.Fortitude|classlevel(...)/2+2` (good Fortitude),
//! `BONUS:SAVE|BASE.Reflex,BASE.Will|classlevel(...)/3` (poor Reflex and
//! Will), with `MAXLEVEL:20`.
...
//! `rules_tables::acg::class_bloodrager` stay tied to the source record
...
#[test]
fn bloodrager_level_1_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Bloodrager, 1, RuleSetId::Acg)
        .expect("Bloodrager level 1 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn bloodrager_level_20_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Bloodrager, 20, RuleSetId::Acg)
        .expect("Bloodrager level 20 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn bloodrager_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Bloodrager, 21, RuleSetId::Acg),
        None,
        "the real CLASS:Bloodrager record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn bloodrager_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Bloodrager, 1, RuleSetId::Crb),
        None,
...
#[test]
fn bloodrager_chassis_returns_none_for_ruleset_apg() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Bloodrager, 1, RuleSetId::Apg),
        None,
        "ACG-only class chassis must not resolve under RuleSetId::Apg"
    );
}

// Cross-class regression: Arcanist (the first ACG class landed) must
// still resolve after Bloodrager is added to the same enum/match arm.
#[test]
fn arcanist_chassis_still_resolves_after_bloodrager_lands() {
...
// established pattern in tests/sd22_acg_class_arcanist_resolves.rs (the
// corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_bloodrager_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Bloodrager") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Bloodrager BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_bloodrager_class_line();
    assert!(
    ...
    assert!(
        line.contains("SAVE|BASE.Reflex,BASE.Will|classlevel(\"APPLIEDAS=NONEPIC\")/3"),
        "real record's Reflex/Will save token should be poor (level/3): {line}"
    );
