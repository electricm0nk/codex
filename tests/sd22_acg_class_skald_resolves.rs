//! SD-22 Epic 4 acceptance test — ACG Skald class chassis (criteria
//! 10-12: per-class chassis resolution and the cross-book resolution
//! invariant). Skald is the seventh real ACG class ingested (class 7 of
...
//! `acg_classes.lst:274` `CLASS:Skald` record carries
...
//! `BONUS:SAVE|BASE.Will,BASE.Fortitude|classlevel("APPLIEDAS=NONEPIC")/2+2`
//! (good Will and Fortitude, combined in one token),
//! `BONUS:SAVE|BASE.Reflex|classlevel("APPLIEDAS=NONEPIC")/3`
//! (poor Reflex, its own single-save token — the mirror-image pairing
//! from Shaman's good-Will/poor-Fortitude+Reflex shape), with
//! `MAXLEVEL:20`, and (a separate `CLASS:Skald` line further down the
//! block) `SPELLSTAT:CHA MEMORIZE:NO SPELLBOOK:YES` (spontaneous casting
//! — `MEMORIZE:NO` takes precedence over `SPELLBOOK:YES` in the parser's
//! posture derivation — belongs in
...
fn skald_level_1_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Skald, 1, RuleSetId::Acg)
        .expect("Skald level 1 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn skald_level_20_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Skald, 20, RuleSetId::Acg)
        .expect("Skald level 20 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn skald_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Skald, 21, RuleSetId::Acg),
        None,
        "the real CLASS:Skald record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn skald_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Skald, 1, RuleSetId::Crb),
        None,
...
#[test]
fn skald_chassis_returns_none_for_ruleset_apg() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Skald, 1, RuleSetId::Apg),
        None,
...
// Investigator, and Shaman (the first six ACG classes landed) must still
// resolve after Skald is added to the same enum/match arm.
#[test]
fn arcanist_bloodrager_brawler_hunter_investigator_and_shaman_chassis_still_resolve_after_skald_lands()
{
...
// established pattern in tests/sd22_acg_class_shaman_resolves.rs
// (the corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_skald_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Skald") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Skald BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_skald_class_line();
    assert!(
        line.contains("BASEAB|classlevel(\"APPLIEDAS=NONEPIC\")*3/4"),
        "real record's BASEAB token should be three-quarter BAB: {line}"
    );
    assert!(
        line.contains("SAVE|BASE.Will,BASE.Fortitude|classlevel(\"APPLIEDAS=NONEPIC\")/2+2"),
        "real record's Will/Fortitude save token should be good (level/2+2): {line}"
    );
    assert!(
