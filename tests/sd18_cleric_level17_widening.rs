...
fn cleric_level17_touch_of_good_stays_put_and_other_facets_carry_over() {
    ...
    assert_eq!(tog_uses.value, 6, "Touch of Good's uses per day must stay 6 at level 17");

    let rebuke_uses = explanation(
        &computation,
        "class_chassis.cleric.domain_power_healing_rebuke_death_uses_per_day",
    );
    assert_eq!(rebuke_uses.value, 6, "Rebuke Death's uses per day must stay 6 at level 17");
