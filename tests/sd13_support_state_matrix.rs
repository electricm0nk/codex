...
fn every_remaining_unproven_class_row_is_unverified_and_observed() {
    // After SD13-E3-F6, SD13-E4-F7, and SD13-E4-F8, Fighter (level 1 + levels 2-10), Rogue,
    // Paladin, Ranger, Sorcerer, and Druid all carry runtime evidence. The remaining five
    // core class rows must still be pure roster-scope placeholders with no runtime
    // evidence.
    let matrix = matrix();
    let proven_subjects = [
        "class:fighter",
...
fn only_pilot_grounded_rows_rise_above_observed() {
    ...
    assert_eq!(
        ...
        "only the pilot-grounded, hybrid-baseline, Sorcerer spell-baseline, and Druid divine-prepared baseline rows may rise above Observed, got {above_observed:?}"
    );
