...
fn every_remaining_unproven_class_row_is_unverified_and_observed() {
    // After SD13-E3-F6, SD13-E4-F7, and the SD13-E3 Barbarian martial-chassis slice,
    // Fighter (level 1 + levels 2-10), Rogue, Barbarian, Paladin, Ranger, and Sorcerer
    // all carry runtime evidence. The remaining five core class rows must still be
    ...
    assert_eq!(
        other_classes.len(),
        5,
        "there must be 5 remaining unproven core class rows"
    );
...
fn only_pilot_grounded_rows_rise_above_observed() {
    ...
    assert_eq!(
        ...
        "only the pilot-grounded, hybrid-baseline, Barbarian martial-baseline, and Sorcerer spell-baseline rows may rise above Observed, got {above_observed:?}"
    );
...
/// pilot-grounded, hybrid-baseline, Barbarian martial-baseline, and Sorcerer
/// spell-baseline rows that rise above `Observed` evidence.
const EXPECTED_REFRESHABLE_FROM_LIVE_PROOF: [&str; 9] = [
    "race.human.pilot_semantics",
    "class.fighter.level_1_pilot",
    "class.fighter.levels_2_10",
    "class.rogue.bounded_progression",
