...
fn sorcerer_level6_still_claim_blocks_arcane_bond_and_spontaneous_spell_burdens() {
    ...
    assert!(
        ...
        "level-6 Sorcerer must still claim-block on the spontaneous spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn sorcerer_level_7_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-7 Sorcerer must not gain any bounded sorcerer chassis explanation: {:?}",
        computation.explanations
    );
}
