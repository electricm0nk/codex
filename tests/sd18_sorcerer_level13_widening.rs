...
fn sorcerer_level13_still_recognizes_the_spell_bearing_baseline_and_claim_blocks_burdens() {
    ...
    assert!(
        ...
        "level-13 Sorcerer must still claim-block on the spontaneous which-spells-known / \
...
// so this boundary moved from 17 to 18, mirroring the exact same boundary move made
// to every other sorcerer sibling test file this cycle.

#[test]
fn sorcerer_level_18_is_not_promoted_by_this_slice() {
    let level_18 = SORCERER_LEVEL13_FIXTURE.replace("class:sorcerer:13", "class:sorcerer:18");
