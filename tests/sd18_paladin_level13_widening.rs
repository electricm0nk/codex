...
fn paladin_level13_spell_level_access_widens_to_four() {
    ...
    assert!(
        ...
        "level-13 Paladin must still claim-block on the partial-caster spell burden: {:?}",
...
// tests/sd18_paladin_level14_widening.rs for its own boundary.)

#[test]
fn paladin_level_16_is_not_promoted_by_this_slice() {
    let level_16 = PALADIN_LEVEL13_FIXTURE.replace("class:paladin:13", "class:paladin:16");
