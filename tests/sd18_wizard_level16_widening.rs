...
fn wizard_level16_grants_carry_over_unchanged() {
    ...
    assert_eq!(
        ...
        "Force Missile's uses per day (3 + Intelligence modifier 3) must stay 6 at level 16"
    );
...
fn wizard_level_17_is_not_promoted_by_this_slice() {
    let level_17 = WIZARD_LEVEL16_FIXTURE.replace("class:wizard:16", "class:wizard:17");
