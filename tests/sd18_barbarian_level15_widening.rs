...
fn barbarian_level15_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-15 Barbarian must still claim-block on the rage execution burden: {:?}",
...
#[test]
fn barbarian_level_18_is_not_promoted_by_this_slice() {
    let level_18 = BARBARIAN_LEVEL15_FIXTURE.replace("class:barbarian:15", "class:barbarian:18");
    ...
    assert!(
        ...
        "level-18 Barbarian must not gain any bounded barbarian explanation: {:?}",
        computation.explanations
    );
}
