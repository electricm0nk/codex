...
fn druid_level7_still_claim_blocks_animal_companion_and_prepared_divine_burdens() {
    ...
    assert!(
        ...
        "level-7 Druid must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn druid_level_8_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-8 Druid must not gain any bounded druid chassis explanation: {:?}",
        computation.explanations
    );
}
