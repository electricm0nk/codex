...
fn cleric_level3_touch_of_good_bonus_stays_one() {
    ...
    assert_eq!(
        uses.value, 6,
        "Cleric level 3 Touch of Good uses per day (3 + Wisdom modifier 3) must equal 6: {}",
        uses.detail
    );
}
...
fn cleric_level3_grounds_rebuke_death_uses_per_day() {
    ...
    assert_eq!(
        uses.value, 6,
        "Cleric level 3 Rebuke Death uses per day (3 + Wisdom modifier 3) must equal 6: {}",
        uses.detail
    );
}
...
fn cleric_level3_still_claim_blocks_domain_powers_and_prepared_divine_burdens() {
    ...
    assert!(
        ...
        "level-3 Cleric must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
