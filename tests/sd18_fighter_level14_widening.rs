...
// (an eighth bonus-feat cadence slot), and then to level 17 (Weapon
// Training 4, a fourth weapon-group choice slot), so this negative control
// now sits just above the current bound (level 18) rather than at level 15,
// level 16, or level 17.

#[test]
fn fighter_level_18_stays_claim_blocked() {
    let level_18 = FIGHTER_LEVEL14_FIXTURE.replace("class:fighter:14", "class:fighter:18");
...
fn multiclass_fighter_level14_is_not_promoted_by_this_slice() {
    ...
    assert!(
        !computation
            ...
            .iter()
            .any(|e| e.id == "class_chassis.base_attack_bonus"),
        "multiclass Fighter must not gain the bounded level-14 base-attack-bonus explanation: {:?}",
        computation.explanations
    );
    assert!(
        computation.diagnostics.iter().any(|d| d.claim_blocking),
