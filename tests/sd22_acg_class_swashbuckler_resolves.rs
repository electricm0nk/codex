//! SD-22 Epic 4 acceptance test — ACG Swashbuckler class chassis (criteria
//! 10-12: per-class chassis resolution and the cross-book resolution
//! invariant). Swashbuckler is the ninth real ACG class ingested (class 9 of
...
//! `acg_classes.lst:347` `CLASS:Swashbuckler` record carries
...
//! Slayer), `BONUS:SAVE|BASE.Fortitude|classlevel("APPLIEDAS=NONEPIC")/3`
//! (poor Fortitude), `BONUS:SAVE|BASE.Reflex|classlevel("APPLIEDAS=NONEPIC")/2+2`
//! (good Reflex — the class's only good save, unlike Slayer's combined
//! Fortitude+Reflex), `BONUS:SAVE|BASE.Will|classlevel("APPLIEDAS=NONEPIC")/3`
...
//! `rules_tables::acg::class_swashbuckler` stay tied to the source record
...
#[test]
fn swashbuckler_level_1_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Swashbuckler, 1, RuleSetId::Acg)
        .expect("Swashbuckler level 1 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn swashbuckler_level_20_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Swashbuckler, 20, RuleSetId::Acg)
        .expect("Swashbuckler level 20 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn swashbuckler_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Swashbuckler, 21, RuleSetId::Acg),
        None,
        "the real CLASS:Swashbuckler record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn swashbuckler_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Swashbuckler, 1, RuleSetId::Crb),
        None,
...
#[test]
fn swashbuckler_chassis_returns_none_for_ruleset_apg() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Swashbuckler, 1, RuleSetId::Apg),
        None,
...
// Investigator, Shaman, Skald, and Slayer (the first eight ACG classes
// landed) must still resolve after Swashbuckler is added to the same
// enum/match arm.
#[test]
fn prior_acg_classes_still_resolve_after_swashbuckler_lands() {
...
// established pattern in tests/sd22_acg_class_slayer_resolves.rs (the
// corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_swashbuckler_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Swashbuckler") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Swashbuckler BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_swashbuckler_class_line();
    assert!(
    ...
    assert!(
        line.contains("SAVE|BASE.Reflex|classlevel(\"APPLIEDAS=NONEPIC\")/2+2"),
        "real record's Reflex save token should be good (level/2+2): {line}"
    );
    assert!(
    ...
    assert!(
        ...
        "Swashbuckler should be a non-caster (no SPELLSTAT token): {line}"
    );
}
