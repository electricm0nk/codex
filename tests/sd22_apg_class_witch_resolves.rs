//! SD-22 Epic 3 acceptance test — APG Witch class chassis (criteria
//! 7-8's per-class shape for the sixth and last APG class in the
//! corrected 6-class ordering: per-class chassis resolution and the
//! cross-book resolution invariant).
//!
//! Source grounding (per `decisions.md §5`, corrected 2026-07-19): the
//! real `apg_classes.lst:172` `CLASS:Witch` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")/2` (half BAB,
//! poor — the first poor-BAB class in this APG roster),
//! `BONUS:SAVE|BASE.Will|classlevel(...)/2+2` (good Will), and
//! `BONUS:SAVE|BASE.Fortitude,BASE.Reflex|classlevel(...)/3` (poor
//! Fortitude and Reflex), with `MAXLEVEL:20`. `real_witch_class_line`
//! below re-parses that exact line (via the SD-17/SD-22
//! `lst_parser::spellcasting_class` engine, widened this cycle to
//! recognize `Witch`, real-corpus-gated on `PCGEN_CORPUS_ROOT`,
//! mirroring `tests/sd22_apg_class_summoner_resolves.rs`'s established
//! pattern) so the hand-transcribed chassis constants in
//! `rules_tables::apg::class_witch` stay tied to the source record
...
#[test]
fn witch_level_1_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Witch, 1, RuleSetId::Apg)
        .expect("Witch level 1 chassis should resolve via RuleSetId::Apg");
    assert_eq!(row.base_attack_bonus, 0, "level 1 half BAB is 1/2 = 0");
...
#[test]
fn witch_level_20_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Witch, 20, RuleSetId::Apg)
        .expect("Witch level 20 chassis should resolve via RuleSetId::Apg");
    assert_eq!(row.base_attack_bonus, 10, "level 20 half BAB is 20/2 = 10");
...
#[test]
fn witch_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Witch, 21, RuleSetId::Apg),
        None,
        "the real CLASS:Witch record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn witch_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Witch, 1, RuleSetId::Crb),
        None,
...
// established pattern in tests/sd22_apg_class_summoner_resolves.rs (the
// corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_witch_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Witch") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Witch BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_witch_class_line();
    assert!(
        line.contains("BASEAB|classlevel(\"APPLIEDAS=NONEPIC\")/2"),
        "real record's BASEAB token should be half BAB: {line}"
    );
    assert!(
