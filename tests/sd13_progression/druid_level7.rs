...
// ----- The animal companion is grounded; the prepared divine burden still fires at level 7 -----

#[test]
fn druid_level7_grounds_the_animal_companion_and_holds_the_prepared_divine_posture() {
    let input = load(DRUID_LEVEL7_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    // at level 7. Commit `ae63aa4c` grounded the animal companion's whole
    // progression from the PCGen corpus at all twenty master levels, so at
    // level 7 the burden is genuinely closed and that blocker is correctly
...
fn multiclass_druid_level7_is_not_promoted_by_this_slice() {
    ...
    // Druid 7/Fighter 1 really does have a companion, at effective druid
    // level 7. Rather than silently tolerate the whole family, pin the
    // thing that actually matters -- the companion is keyed to the DRUID class
    // level (7), not to the character's total level (8). If the engine
    // ever started advancing a multiclass character's companion off total
    // level, these values would be those of master level 8 and this fails.
    assert_wolf_companion_stat_block(
        &computation, 7, 6, 6, 5, 2, 16, 3, 45,
    );
...
recognition_negative_controls! {
    fighter_does_not_gain_druid_level7_recognition(FIGHTER_FIXTURE) {
