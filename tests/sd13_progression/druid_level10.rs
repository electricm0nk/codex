...
#[test]
fn druid_level10_grounds_the_animal_companion_and_holds_the_prepared_divine_posture() {
    let input = load(DRUID_LEVEL10_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    // at level 10. Commit `ae63aa4c` grounded the animal companion's whole
    // progression from the PCGen corpus at all twenty master levels, so at
    // level 10 the burden is genuinely closed and that blocker is correctly
    ...
    assert_wolf_companion_stat_block(
        &computation, 10, 9, 9, 6, 3, 18, 4, 66,
    );
...
fn druid_level_21_is_not_promoted_by_this_slice() {
    ...
    let level_21 = DRUID_LEVEL10_FIXTURE.replace("class:druid:10", "class:druid:21");
...
fn multiclass_druid_level10_is_not_promoted_by_this_slice() {
    ...
    // Druid 10/Fighter 1 really does have a companion, at effective druid
    // level 10. Rather than silently tolerate the whole family, pin the
    // thing that actually matters -- the companion is keyed to the DRUID class
    // level (10), not to the character's total level (11). If the engine
    // ever started advancing a multiclass character's companion off total
    // level, these values would be those of master level 11 and this fails.
    assert_wolf_companion_stat_block(
        &computation, 10, 9, 9, 6, 3, 18, 4, 66,
    );
...
recognition_negative_controls! {
    fighter_does_not_gain_druid_level10_recognition(FIGHTER_FIXTURE) {
