...
fn barbarian_level17_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-17 Barbarian must still claim-block on the rage execution burden: {:?}",
...
fn barbarian_level_18_is_not_promoted_by_this_slice() {
    let level_18 = BARBARIAN_LEVEL17_FIXTURE.replace("class:barbarian:17", "class:barbarian:18");
