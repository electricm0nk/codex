...
fn wizard_level17_grants_carry_over_unchanged() {
    ...
    assert_eq!(
        ...
        "Force Missile's uses per day (3 + Intelligence modifier 3) must stay 6 at level 17"
    );
...
fn wizard_level_18_is_not_promoted_by_this_slice() {
    let level_18 = WIZARD_LEVEL17_FIXTURE.replace("class:wizard:17", "class:wizard:18");
