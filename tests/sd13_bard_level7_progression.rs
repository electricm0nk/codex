...
fn bard_level7_still_claim_blocks_performance_execution_and_spontaneous_spell_burdens() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-7 Bard must still claim-block on the spontaneous spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn bard_level_8_is_not_promoted_by_this_slice() {
    ...
    assert!(
        !computation
            ...
            .any(|e| e.id.starts_with("class_chassis.bard.")
                ...
                || e.id == LORE_MASTER_ID),
        "level-8 Bard must not gain any bounded bard chassis explanation: {:?}",
        computation.explanations
    );
}
