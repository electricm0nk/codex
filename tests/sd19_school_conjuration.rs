//! SD-19 §2.4 spell-school card: Conjuration.
//!
//! Proves every real-corpus Conjuration spell in `cr_spells.lst` (116
//! records, independently confirmed via `grep -c "SCHOOL:Conjuration"
//! cr_spells.lst`, and the "Acid Arrow" record independently confirmed via
//! `grep -n "^Acid Arrow\b" cr_spells.lst` before this test was written, per
//! the loop instruction's Step 4 corpus-existence check) is (a) resolvable
//! via `spell_id_resolve`, and (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Conjuration].spells`
...
//! hand-copying 116 fixture files. Skips with a documented `eprintln!` when
...
fn base_input() -> CharacterInput {
    CharacterInput {
        case_id: Some("sd19_school_conjuration".to_string()),
        source_package_id: "sd19_school_conjuration".to_string(),
...
#[test]
fn conjuration_school_spell_count_matches_real_corpus() {
    ...
    let conjuration: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Conjuration"))
        .collect();
    ...
    assert_eq!(
        conjuration.len(),
        116,
        "expected 116 SCHOOL:Conjuration records in cr_spells.lst"
    );
    assert!(
        conjuration.iter().any(|r| r.name == "Acid Arrow"),
        "expected 'Acid Arrow' among the resolved Conjuration records"
    );
}

#[test]
fn every_conjuration_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let conjuration_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Conjuration"))
        .collect();
    assert!(
        !conjuration_records.is_empty(),
        "corpus-existence check must find Conjuration records"
    );
    ...
    let mut corpus = SourcePackageContent::empty("sd19_school_conjuration", source_ref);
    for record in &conjuration_records {
        corpus.push(convert_spell_record(record));
    }

    // (a) every Conjuration spell resolves via spell_id_resolve.
    for record in &conjuration_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Conjuration spell '{}'",
            record.name
        );
    }

    // (b) every Conjuration spell is present in
    // corpus_derived.school_coverage[Conjuration].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Conjuration)
        .expect("expected a Conjuration entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = conjuration_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Conjuration].spells to carry every real-corpus Conjuration spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Conjuration].table_cell to resolve through the foundation slice's table store"
    );
}

#[test]
fn conjuration_matrix_row_reflects_the_grounded_reachability_proof() {
    let matrix = seeded_current_truth();
    let row = matrix
        .rows
        .iter()
        .find(|r| r.subject_type == MatrixSubjectType::School(Pf1SchoolId::Conjuration))
        .expect("expected a School(Conjuration) row in the seeded matrix");

    assert_eq!(row.support_state, SupportState::Supported);
    assert_eq!(row.evidence_tier, EvidenceTier::ProductVisible);
    assert!(
        row.grounding_ref.contains("sd19_school_conjuration"),
