...
// ----- The animal companion is grounded; the prepared divine burden still fires at level 12 -----
...
fn druid_level12_grounds_the_animal_companion_and_holds_the_prepared_divine_posture() {
    ...
    // at level 12. Commit `ae63aa4c` grounded the animal companion's whole
    // progression from the PCGen corpus at all twenty master levels, so at
    // level 12 the burden is genuinely closed and that blocker is correctly
    ...
    assert_wolf_companion_stat_block(
        &computation, 12, 10, 10, 7, 3, 20, 4, 73,
    );
...
fn multiclass_druid_level12_is_not_promoted_by_this_slice() {
    ...
    // Druid 12/Fighter 1 really does have a companion, at effective druid
    // level 12. Rather than silently tolerate the whole family, pin the
    // thing that actually matters -- the companion is keyed to the DRUID class
    // level (12), not to the character's total level (13). If the engine
    // ever started advancing a multiclass character's companion off total
    // level, these values would be those of master level 13 and this fails.
    assert_wolf_companion_stat_block(
        &computation, 12, 10, 10, 7, 3, 20, 4, 73,
    );
