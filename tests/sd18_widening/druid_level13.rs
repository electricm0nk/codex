...
fn druid_level13_base_attack_and_saves_stay_at_level12_values() {
    let computation = crate::support::compute(DRUID_LEVEL13_FIXTURE);
...
// ----- The animal companion is grounded; the prepared divine burden still fires at level 13 -----

#[test]
fn druid_level13_grounds_the_animal_companion_and_holds_the_prepared_divine_posture() {
    let computation = crate::support::compute(DRUID_LEVEL13_FIXTURE);
    ...
    // at level 13. Commit `ae63aa4c` grounded the animal companion's whole
    // progression from the PCGen corpus at all twenty master levels, so at
    // level 13 the burden is genuinely closed and that blocker is correctly
    ...
    assert_wolf_companion_stat_block(
        &computation, 13, 11, 11, 7, 3, 20, 4, 80,
    );
...
fn druid_level_21_is_not_promoted_by_this_slice() {
    ...
    let level_21 = DRUID_LEVEL13_FIXTURE.replace("class:druid:13", "class:druid:21");
...
crate::sd18_fighter_neg_control_test!(fighter_does_not_gain_druid_level13_recognition, "druid");
...
fn multiclass_druid_level13_is_not_promoted_by_this_slice() {
    ...
    // Druid 13/Fighter 1 really does have a companion, at effective druid
    // level 13. Rather than silently tolerate the whole family, pin the
    // thing that actually matters -- the companion is keyed to the DRUID class
    // level (13), not to the character's total level (14). If the engine
    // ever started advancing a multiclass character's companion off total
    // level, these values would be those of master level 14 and this fails.
    assert_wolf_companion_stat_block(
        &computation, 13, 11, 11, 7, 3, 20, 4, 80,
    );
