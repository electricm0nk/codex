...
#[test]
fn rogue_level_7_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-7 Rogue must not gain any bounded rogue chassis explanation: {:?}",
        computation.explanations
    );
    assert!(
        !has_explanation(&computation, ROGUE_EVASION_ID),
        "level-7 Rogue must not gain the Evasion explanation from this bounded slice"
    );
    assert!(
        !has_explanation(&computation, ROGUE_TRAP_SENSE_ID),
        "level-7 Rogue must not gain the Trap Sense explanation from this bounded slice"
    );
    assert!(
        !has_explanation(&computation, ROGUE_UNCANNY_DODGE_ID),
        "level-7 Rogue must not gain the Uncanny Dodge explanation from this bounded slice"
    );
}
