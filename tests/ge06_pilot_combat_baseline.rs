...
fn wrong_fighter_level_blocks_combat_totals() {
    ...
    // milestone levels 1-14 (SD13-E5 widened levels 9-10, Weapon Training 2 and
    // the level-10 bonus feat; SD18 widened level 11, Armor Training 3, level
    // 12, a sixth bonus-feat cadence slot, level 13, Weapon Training 3, and
    // level 14, a seventh bonus-feat cadence slot and the Bravery magnitude
    // rise, into the supported tranche). A Fighter above that bounded tranche
    // (level 15+, beyond the SD-13/SD-18 level-14 progression matrix) must be
    // treated as an unsupported posture, not silently computed, even though
    // every other loadout/feat/choice condition still holds.
