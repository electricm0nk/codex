...
#[test]
fn monk_level8_does_not_fabricate_a_wholeness_of_body_record() {
    let input = load(MONK_LEVEL8_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert!(
        ...
        "Wholeness of Body must not be fabricated as a flat record at level 8: {:?}",
        computation.explanations
    );
}
...
#[test]
fn monk_level8_does_not_fabricate_a_high_jump_record() {
    let input = load(MONK_LEVEL8_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert!(
        ...
        "High Jump must not be fabricated as a flat record at level 8: {:?}",
        computation.explanations
    );
}
