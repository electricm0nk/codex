...
#[test]
fn barbarian_level_8_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-8 Barbarian must not gain any bounded barbarian chassis explanation: {:?}",
        computation.explanations
    );
    assert!(
        !has_explanation(&computation, BARBARIAN_UNCANNY_DODGE_ID),
        "level-8 Barbarian must not gain the Uncanny Dodge explanation from this bounded slice"
    );
    assert!(
        !has_explanation(&computation, BARBARIAN_TRAP_SENSE_ID),
        "level-8 Barbarian must not gain the Trap Sense explanation from this bounded slice"
    );
    assert!(
        !has_explanation(&computation, BARBARIAN_IMPROVED_UNCANNY_DODGE_ID),
        "level-8 Barbarian must not gain the Improved Uncanny Dodge explanation from this \
         bounded slice"
    );
    assert!(
        !has_explanation(&computation, BARBARIAN_DAMAGE_REDUCTION_ID),
        "level-8 Barbarian must not gain the Damage Reduction explanation from this bounded slice"
    );
}
