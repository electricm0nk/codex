//! SD-19 §2.4 spell-school card: Universal.
//!
//! Proves every real-corpus Universal spell in `cr_spells.lst` (5 records,
//! independently confirmed via `grep -c "SCHOOL:Universal" cr_spells.lst`,
//! and the "Wish" record independently confirmed via `grep -n "SCHOOL:Universal"
//! cr_spells.lst` before this test was written, per the loop instruction's
//! Step 4 corpus-existence check) is (a) resolvable via `spell_id_resolve`,
//! and (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Universal].spells`
...
//! hand-copying 5 fixture files. Skips with a documented `eprintln!` when
...
fn base_input() -> CharacterInput {
    CharacterInput {
        case_id: Some("sd19_school_universal".to_string()),
        source_package_id: "sd19_school_universal".to_string(),
...
#[test]
fn universal_school_spell_count_matches_real_corpus() {
    ...
    let universal: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Universal"))
        .collect();
    ...
    assert_eq!(
        universal.len(),
        5,
        "expected 5 SCHOOL:Universal records in cr_spells.lst"
    );
    assert!(
        universal.iter().any(|r| r.name == "Wish"),
        "expected 'Wish' among the resolved Universal records"
    );
}

#[test]
fn every_universal_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let universal_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Universal"))
        .collect();
    assert!(
        !universal_records.is_empty(),
        "corpus-existence check must find Universal records"
    );
    ...
    let mut corpus = SourcePackageContent::empty("sd19_school_universal", source_ref);
    for record in &universal_records {
        corpus.push(convert_spell_record(record));
    }

    // (a) every Universal spell resolves via spell_id_resolve.
    for record in &universal_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Universal spell '{}'",
            record.name
        );
    }

    // (b) every Universal spell is present in
    // corpus_derived.school_coverage[Universal].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Universal)
        .expect("expected a Universal entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = universal_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Universal].spells to carry every real-corpus Universal spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Universal].table_cell to resolve through the foundation slice's table store"
    );
}

#[test]
fn universal_matrix_row_reflects_the_grounded_reachability_proof() {
    let matrix = seeded_current_truth();
    let row = matrix
        .rows
        .iter()
        .find(|r| r.subject_type == MatrixSubjectType::School(Pf1SchoolId::Universal))
        .expect("expected a School(Universal) row in the seeded matrix");

    assert_eq!(row.support_state, SupportState::Supported);
    assert_eq!(row.evidence_tier, EvidenceTier::ProductVisible);
    assert!(
        row.grounding_ref.contains("sd19_school_universal"),
