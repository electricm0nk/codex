//! SD-22 Epic 3 acceptance test — APG Oracle class chassis (criteria
//! 7-8's per-class shape for the fourth APG class in the corrected
...
//! real `apg_classes.lst:107` `CLASS:Oracle` record carries
...
//! Fortitude and Reflex), with `MAXLEVEL:20`. `real_oracle_class_line`
...
fn oracle_level_1_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Oracle, 1, RuleSetId::Apg)
        .expect("Oracle level 1 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn oracle_level_20_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Oracle, 20, RuleSetId::Apg)
        .expect("Oracle level 20 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn oracle_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Oracle, 21, RuleSetId::Apg),
        None,
        "the real CLASS:Oracle record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn oracle_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Oracle, 1, RuleSetId::Crb),
        None,
...
// established pattern in tests/sd22_apg_class_inquisitor_resolves.rs (the
// corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_oracle_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Oracle") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Oracle BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_oracle_class_line();
    assert!(
