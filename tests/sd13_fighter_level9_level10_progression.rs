...
// ----- Negative control: level 19 stays blocked (beyond the bounded L2-18 row) -----
//
...
// tests/sd18_fighter_level18_widening.rs) widened the bounded tranche from
// level 10 to level 12 (Armor Training 3, then a sixth bonus-feat cadence
// slot), then to level 13 (Weapon Training 3), then to level 14 (a seventh
...
// then to level 17 (Weapon Training 4), and then to level 18 (a ninth
// bonus-feat cadence slot and a further Bravery magnitude rise), so this
// negative control now sits just above the current bound (level 19) rather
// than at level 12, level 13, level 14, level 15, level 16, level 17, or
// level 18.

#[test]
fn level_19_fighter_stays_claim_blocked() {
    let level_19 = LEVEL_10_FIXTURE.replace("class:fighter:10", "class:fighter:19");
