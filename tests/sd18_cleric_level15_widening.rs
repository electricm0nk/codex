...
fn cleric_level15_touch_of_good_stays_and_other_facets_carry_over() {
    ...
    assert_eq!(tog_uses.value, 6, "Touch of Good's uses per day must stay 6 at level 15");

    let rebuke_uses = explanation(
        &computation,
        "class_chassis.cleric.domain_power_healing_rebuke_death_uses_per_day",
    );
    assert_eq!(rebuke_uses.value, 6, "Rebuke Death's uses per day must stay 6 at level 15");
...
fn cleric_level15_still_claim_blocks_domain_powers_and_prepared_spell_burdens() {
    ...
    assert!(
        ...
        "level-15 Cleric must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
