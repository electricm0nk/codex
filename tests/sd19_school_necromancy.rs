//! SD-19 §2.4 spell-school card: Necromancy.
//!
//! Proves every real-corpus Necromancy spell in `cr_spells.lst` (62
//! records, independently confirmed via `grep -c "SCHOOL:Necromancy"
//! cr_spells.lst`, and the "Animate Dead" record independently confirmed
//! via `grep -n "SCHOOL:Necromancy" cr_spells.lst` before this test was
//! written, per the loop instruction's Step 4 corpus-existence check) is
//! (a) resolvable via `spell_id_resolve`, and (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Necromancy].spells`
...
//! hand-copying 62 fixture files. Skips with a documented `eprintln!` when
...
#[test]
fn necromancy_school_spell_count_matches_real_corpus() {
    ...
    let necromancy: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Necromancy"))
        .collect();
    ...
    assert_eq!(
        necromancy.len(),
        62,
        "expected 62 SCHOOL:Necromancy records in cr_spells.lst"
    );
    assert!(
        necromancy.iter().any(|r| r.name == "Animate Dead"),
        "expected 'Animate Dead' among the resolved Necromancy records"
    );
}

#[test]
fn every_necromancy_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let necromancy_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Necromancy"))
        .collect();
    assert!(
        !necromancy_records.is_empty(),
        "corpus-existence check must find Necromancy records"
    );
    ...
    let mut corpus = SourcePackageContent::empty("sd19_school_necromancy", source_ref);
    for record in &necromancy_records {
        corpus.push(convert_spell_record(record));
    }

    // (a) every Necromancy spell resolves via spell_id_resolve.
    for record in &necromancy_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Necromancy spell '{}'",
            record.name
        );
    }

    // (b) every Necromancy spell is present in
    // corpus_derived.school_coverage[Necromancy].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Necromancy)
        .expect("expected a Necromancy entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = necromancy_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Necromancy].spells to carry every real-corpus Necromancy spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Necromancy].table_cell to resolve through the foundation slice's table store"
    );
}
...
fn necromancy_matrix_row_reflects_the_grounded_reachability_proof() {
    ...
    let row = matrix
        .rows
        .iter()
        .find(|r| r.subject_type == MatrixSubjectType::School(Pf1SchoolId::Necromancy))
        .expect("expected a School(Necromancy) row in the seeded matrix");

    assert_eq!(row.support_state, SupportState::Supported);
    assert_eq!(row.evidence_tier, EvidenceTier::ProductVisible);
    assert!(
        row.grounding_ref.contains("sd19_school_necromancy"),
