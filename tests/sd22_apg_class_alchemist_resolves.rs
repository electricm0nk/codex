//! SD-22 Epic 3 acceptance test — APG Alchemist class chassis (criteria
//! 6-8: `rules_tables/apg/mod.rs` populated with the `RuleSetId::Apg`
...
//! real `apg_classes.lst:11` `CLASS:Alchemist` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")*3/4` (three-quarter
//! BAB), `BONUS:SAVE|BASE.Will|classlevel(...)/3` (poor Will), and
//! `BONUS:SAVE|BASE.Fortitude,BASE.Reflex|classlevel(...)/2+2` (good
//! Fortitude and Reflex), with `MAXLEVEL:20`. `real_alchemist_class_line`
//! below re-parses that exact line (via the SD-17/SD-22
//! `lst_parser::spellcasting_class` engine, real-corpus-gated on
//! `PCGEN_CORPUS_ROOT`, mirroring `tests/sd17_b_spellcasting_class.rs`'s
//! established pattern) so the hand-transcribed chassis constants in
//! `rules_tables::apg::class_alchemist` stay tied to the source record
...
#[test]
fn alchemist_level_1_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Alchemist, 1, RuleSetId::Apg)
        .expect("Alchemist level 1 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn alchemist_level_20_chassis_resolves_via_ruleset_apg() {
    let row = class_chassis_resolve(ApgClassId::Alchemist, 20, RuleSetId::Apg)
        .expect("Alchemist level 20 chassis should resolve via RuleSetId::Apg");
...
#[test]
fn alchemist_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Alchemist, 21, RuleSetId::Apg),
        None,
        "the real CLASS:Alchemist record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn alchemist_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(ApgClassId::Alchemist, 1, RuleSetId::Crb),
        None,
...
// established pattern in tests/sd17_b_spellcasting_class.rs (the corpus is
// a separate ~700MB checkout, not part of the codex repo).
fn real_alchemist_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Alchemist") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Alchemist BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_alchemist_class_line();
    assert!(
    ...
    assert!(
        ...
        "real record's Fort/Reflex save token should be good (level/2+2): {line}"
    );
