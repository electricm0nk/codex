//! SD-22 Epic 4 acceptance test — ACG Investigator class chassis (criteria
//! 10-12: per-class chassis resolution and the cross-book resolution
//! invariant). Investigator is the fifth real ACG class ingested (class 5
//! of the corrected 10-class roster: Arcanist, Bloodrager, Brawler, Hunter,
...
//! `acg_classes.lst:168` `CLASS:Investigator` record carries
//! `BONUS:COMBAT|BASEAB|classlevel("APPLIEDAS=NONEPIC")*3/4|TYPE=Base.REPLACE`
//! (three-quarter BAB — same posture as ACG's Hunter and APG's
//! Alchemist/Inquisitor/Oracle/Summoner),
//! `BONUS:SAVE|BASE.Fortitude|classlevel("APPLIEDAS=NONEPIC")/3`
//! (poor Fortitude, its own single-save token),
//! `BONUS:SAVE|BASE.Will,BASE.Reflex|classlevel("APPLIEDAS=NONEPIC")/2+2`
//! (good Will and Reflex, combined in one token), with `MAXLEVEL:20`, and
//! (a separate `CLASS:Investigator` line further down the block)
//! `SPELLSTAT:INT MEMORIZE:YES SPELLBOOK:YES` (spellbook-prepared
//! casting — belongs in
...
//! `rules_tables::acg::class_investigator` stay tied to the source record
...
#[test]
fn investigator_level_1_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Investigator, 1, RuleSetId::Acg)
        .expect("Investigator level 1 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn investigator_level_20_chassis_resolves_via_ruleset_acg() {
    let row = class_chassis_resolve(AcgClassId::Investigator, 20, RuleSetId::Acg)
        .expect("Investigator level 20 chassis should resolve via RuleSetId::Acg");
...
#[test]
fn investigator_chassis_is_none_for_level_beyond_maxlevel_20() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Investigator, 21, RuleSetId::Acg),
        None,
        "the real CLASS:Investigator record's MAXLEVEL:20 bounds the table"
    );
}
...
#[test]
fn investigator_chassis_returns_none_for_ruleset_crb() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Investigator, 1, RuleSetId::Crb),
        None,
...
#[test]
fn investigator_chassis_returns_none_for_ruleset_apg() {
    assert_eq!(
        class_chassis_resolve(AcgClassId::Investigator, 1, RuleSetId::Apg),
        None,
        "ACG-only class chassis must not resolve under RuleSetId::Apg"
    );
}

// Cross-class regression: Arcanist, Bloodrager, Brawler, and Hunter (the
// first four ACG classes landed) must still resolve after Investigator is
// added to the same enum/match arm.
#[test]
fn arcanist_bloodrager_brawler_and_hunter_chassis_still_resolve_after_investigator_lands() {
...
// established pattern in tests/sd22_acg_class_hunter_resolves.rs (the
// corpus is a separate ~700MB checkout, not part of the codex repo).
fn real_investigator_class_line() -> String {
    ...
    text.lines()
        .find(|line| line.starts_with("CLASS:Investigator") && line.contains("BASEAB"))
        .unwrap_or_else(|| panic!("expected a CLASS:Investigator BASEAB line in {}", source.display()))
        .to_string()
}
...
fn hand_transcribed_chassis_matches_the_real_lst_bonus_tokens() {
    let line = real_investigator_class_line();
    assert!(
        line.contains("BASEAB|classlevel(\"APPLIEDAS=NONEPIC\")*3/4"),
        "real record's BASEAB token should be three-quarter BAB: {line}"
    );
    assert!(
        line.contains("SAVE|BASE.Fortitude|classlevel(\"APPLIEDAS=NONEPIC\")/3"),
        "real record's Fortitude save token should be poor (level/3): {line}"
    );
    assert!(
        line.contains("SAVE|BASE.Will,BASE.Reflex|classlevel(\"APPLIEDAS=NONEPIC\")/2+2"),
        "real record's Will/Reflex save token should be good (level/2+2): {line}"
    );
