...
fn barbarian_level12_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-12 Barbarian must still claim-block on the rage execution burden: {:?}",
...
#[test]
fn barbarian_level_19_is_not_promoted_by_this_slice() {
    let level_19 = BARBARIAN_LEVEL12_FIXTURE.replace("class:barbarian:12", "class:barbarian:19");
    ...
    assert!(
        ...
        "level-19 Barbarian must not gain any bounded barbarian explanation: {:?}",
        computation.explanations
    );
}
