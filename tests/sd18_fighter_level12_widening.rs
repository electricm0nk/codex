...
// tests/sd18_fighter_level17_widening.rs) further widened the bounded
...
// Bravery magnitude rise), then to level 15 (Armor Training 4), then to
// level 16 (an eighth bonus-feat cadence slot), and then to level 17
// (Weapon Training 4, a fourth weapon-group choice slot), so this negative
// control now sits just above the current bound (level 18) rather than at
// level 13, level 14, level 15, level 16, or level 17.

#[test]
fn fighter_level_18_stays_claim_blocked() {
    let level_18 = FIGHTER_LEVEL12_FIXTURE.replace("class:fighter:12", "class:fighter:18");
