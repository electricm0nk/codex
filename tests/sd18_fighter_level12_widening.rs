...
// Training 3, a third weapon-group choice slot), then to level 14 (a
// seventh bonus-feat cadence slot and the Bravery magnitude rise), then to
// level 15 (Armor Training 4), then to level 16 (an eighth bonus-feat
// cadence slot), then to level 17 (Weapon Training 4, a fourth weapon-group
// choice slot), and then to level 18 (a ninth bonus-feat cadence slot and a
// further Bravery magnitude rise), so this negative control now sits just
// above the current bound (level 19) rather than at level 13, level 14,
// level 15, level 16, level 17, or level 18.

#[test]
fn fighter_level_19_stays_claim_blocked() {
    let level_19 = FIGHTER_LEVEL12_FIXTURE.replace("class:fighter:12", "class:fighter:19");
