//! SD-19 §2.4 spell-school card: Evocation.
//!
//! Proves every real-corpus Evocation spell in `cr_spells.lst` (87
//! records, independently confirmed via `grep -c "SCHOOL:Evocation"
//! cr_spells.lst`, and the "Burning Hands" record independently confirmed
//! via `grep -n "SCHOOL:Evocation" cr_spells.lst` before this test was
//! written, per the loop instruction's Step 4 corpus-existence check) is
//! (a) resolvable via `spell_id_resolve`, and (b) present in
//! `CorpusPilotReceipt.corpus_derived.school_coverage[Evocation].spells`
...
//! hand-copying 87 fixture files. Skips with a documented `eprintln!` when
...
fn base_input() -> CharacterInput {
    CharacterInput {
        case_id: Some("sd19_school_evocation".to_string()),
        source_package_id: "sd19_school_evocation".to_string(),
...
#[test]
fn evocation_school_spell_count_matches_real_corpus() {
    ...
    let evocation: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Evocation"))
        .collect();
    ...
    assert_eq!(
        evocation.len(),
        87,
        "expected 87 SCHOOL:Evocation records in cr_spells.lst"
    );
    assert!(
        evocation.iter().any(|r| r.name == "Burning Hands"),
        "expected 'Burning Hands' among the resolved Evocation records"
    );
}

#[test]
fn every_evocation_spell_resolves_and_reaches_school_coverage() {
    let Some(root) = corpus_root() else {
        eprintln!(
            "CORPUS_ROOT not set or not a directory; skipping (set \
             CORPUS_ROOT=$HOME/workspace/repos/pcgen/data to enable)"
    ...
    let spell_file = parse_lst_spell_file(&cr_spells).expect("cr_spells.lst must parse");
    let evocation_records: Vec<_> = spell_file
        .records
        .iter()
        .filter(|r| r.school.as_deref() == Some("Evocation"))
        .collect();
    assert!(
        !evocation_records.is_empty(),
        "corpus-existence check must find Evocation records"
    );
    ...
    let mut corpus = SourcePackageContent::empty("sd19_school_evocation", source_ref);
    for record in &evocation_records {
        corpus.push(convert_spell_record(record));
    }

    // (a) every Evocation spell resolves via spell_id_resolve.
    for record in &evocation_records {
        assert!(
            spell_id_resolve(&record.name, RuleSetId::Crb, &corpus).is_some(),
            "expected spell_id_resolve to resolve Evocation spell '{}'",
            record.name
        );
    }

    // (b) every Evocation spell is present in
    // corpus_derived.school_coverage[Evocation].spells after a call to
    ...
    let coverage = receipt
        .corpus_derived
        .school_coverage
        .get(&Pf1SchoolId::Evocation)
        .expect("expected an Evocation entry in corpus_derived.school_coverage");

    let mut expected: Vec<String> = evocation_records.iter().map(|r| r.name.clone()).collect();
    expected.sort();
    assert_eq!(
        coverage.spells, expected,
        "expected school_coverage[Evocation].spells to carry every real-corpus Evocation spell"
    );
    ...
    assert!(
        ...
        "expected school_coverage[Evocation].table_cell to resolve through the foundation slice's table store"
    );
}

#[test]
fn evocation_matrix_row_reflects_the_grounded_reachability_proof() {
    let matrix = seeded_current_truth();
    let row = matrix
        .rows
        .iter()
        .find(|r| r.subject_type == MatrixSubjectType::School(Pf1SchoolId::Evocation))
        .expect("expected a School(Evocation) row in the seeded matrix");

    assert_eq!(row.support_state, SupportState::Supported);
    assert_eq!(row.evidence_tier, EvidenceTier::ProductVisible);
    assert!(
        row.grounding_ref.contains("sd19_school_evocation"),
