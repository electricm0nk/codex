...
fn paladin_level4_lay_on_hands_and_divine_grace_scale_correctly() {
    ...
    assert_eq!(
        ...
        "lay on hands uses per day at level 4 (1/2 paladin level + Charisma modifier = 2 + 2) \
...
fn paladin_level4_effective_caster_level_genuinely_becomes_1() {
    ...
    let spell_blocker = computation
        ...
        .expect("paladin partial-caster spell blocker must still fire at level 4");
    assert!(
        spell_blocker.claim_blocking,
        "the partial-caster spell burden stays claim-blocking; no spell slots are fabricated \
         merely because the effective-caster-level gate is now nonzero"
    );
}
