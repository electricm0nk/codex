...
fn cleric_level16_touch_of_good_genuinely_rises_and_other_facets_carry_over() {
    ...
    assert_eq!(tog_uses.value, 6, "Touch of Good's uses per day must stay 6 at level 16");

    let rebuke_uses = explanation(
        &computation,
        "class_chassis.cleric.domain_power_healing_rebuke_death_uses_per_day",
    );
    assert_eq!(rebuke_uses.value, 6, "Rebuke Death's uses per day must stay 6 at level 16");
...
fn cleric_level16_still_recognizes_the_spell_bearing_baseline_and_claim_blocks_burdens() {
    ...
    assert!(
        ...
        "level-16 Cleric must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
fn cleric_level_17_is_not_promoted_by_this_slice() {
    let level_17 = CLERIC_LEVEL16_FIXTURE.replace("class:cleric:16", "class:cleric:17");
