...
fn barbarian_level11_still_claim_blocks_the_rage_execution_burden() {
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-11 Barbarian must still claim-block on the rage execution burden: {:?}",
...
fn barbarian_level_12_is_not_promoted_by_this_slice() {
    let level_12 = BARBARIAN_LEVEL11_FIXTURE.replace("class:barbarian:11", "class:barbarian:12");
