...
fn cleric_level8_touch_of_good_bonus_rises_to_four() {
    ...
    assert_eq!(
        uses.value, 6,
        "Cleric level 8 Touch of Good uses per day (3 + Wisdom modifier 3) must equal 6: {}",
        uses.detail
    );
}
...
fn cleric_level8_grounds_rebuke_death_uses_per_day() {
    ...
    assert_eq!(
        uses.value, 6,
        "Cleric level 8 Rebuke Death uses per day (3 + Wisdom modifier 3) must equal 6: {}",
        uses.detail
    );
}
...
fn cleric_level8_still_claim_blocks_domain_powers_and_prepared_divine_burdens() {
    ...
    assert!(
        ...
        "level-8 Cleric must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn cleric_level_9_is_not_promoted_by_this_slice() {
    ...
    assert!(
        ...
        "level-9 Cleric must not gain any bounded cleric chassis explanation: {:?}",
        computation.explanations
    );
}
