//! SD13-E5 Wizard level-5 progression grounding proof.
//!
//! Widens the accepted Wizard level-1/level-2/level-3/level-4 prepared-spell-burden
//! baseline (`tests/sd13_wizard_level1_prepared_spell_baseline.rs`,
//! `tests/sd13_wizard_base_attack_and_saves.rs`,
//! `tests/sd13_wizard_level2_progression.rs`,
//! `tests/sd13_wizard_level3_progression.rs`,
//! `tests/sd13_wizard_level4_progression.rs`) to Wizard level 5, mirroring the
//! Fighter/Paladin/Rogue/Barbarian/Monk/Cleric/Bard/Druid/Sorcerer/Ranger
//! level-range-gate idiom (`supported_wizard_level` is generalized from `1..=4` to
//! `1..=5` via `MAX_SUPPORTED_WIZARD_LEVEL = 5`). Both PF1 CRB primary sources
//! (d20pfsrd and a second independent Archives of Nethys mirror) were read directly
//! before writing any code or test:
//!
//! - level 5 base attack bonus is +2 (`5 / 2 = 2`, the Wizard's own 1/2-BAB
//!   progression, the SAME shape as Sorcerer) and base saves are +1 Fortitude (poor,
//!   `5 / 3 = 1`), +1 Reflex (poor, `5 / 3 = 1`), +4 Will (good, `5 / 2 + 2 = 4`) —
//!   confirmed by the same formulas already grounded at levels 1-4, not re-derived.
//!   Every one of these four values is numerically IDENTICAL to the level-4 values —
//!   an integer-division coincidence (`5 / 2` floors to the same `2` as `4 / 2`, `5 /
//!   3` floors to the same `1` as `4 / 3`), not a sign any formula stopped scaling.
//! - the school specialization choice recognition (Evocation chosen, Necromancy and
//!   Transmutation opposed) is not level-gated, so it still fires at level 5 for the
//!   same fixture selections.
//! - the specialist bonus slot flat count is the exact question this cycle was
//!   briefed to verify rather than assume: the raw Wizard spells-per-day table rows
//!   (verified independently against both primary sources) show a level-5 wizard's
//!   3rd-level spell column is no longer "—" (level 4 row: "4/3/2/—/—"; level 5 row:
//!   "4/3/2/1/—" — the first non-"—" 3rd-level column). So a level-5 specialist now
//!   casts 1st-, 2nd-, AND 3rd-level spells for the first time, and per the PF1 Core
//!   Rulebook arcane school class feature ("an additional spell slot of each spell
//!   level he can cast, from 1st on up"), the flat count genuinely becomes 3 (one
//!   1st-level bonus slot, one 2nd-level bonus slot, and one 3rd-level bonus slot),
//!   up from 2 at levels 3-4.
//! - Intense Spells' bonus-damage magnitude (half wizard level, minimum 1) STAYS at 2
//!   at level 5: `max(5 / 2, 1) = 2`, unchanged from level 4 — another integer-
//!   division coincidence (`5 / 2` floors to the same `2` as `4 / 2`), reached
...
//!   recognizes the grant identity at level 5, its body text still hardcoding "1st
//!   level" as the level the feat was actually granted.
//! - the Wizard class table's level-5 "Special" column reads "Bonus feat" (verified
//!   independently against both primary sources): a level-5 wizard gains a bonus
//!   feat, choosing a metamagic feat, an item creation feat, or Spell Mastery. This
//!   was checked and confirmed NOT flat: unlike Barbarian's Improved Uncanny Dodge or
//!   Monk's Purity of Body (both a single automatic grant), the Wizard's 5th-level
//!   bonus feat is a choice among a genuinely open-ended set of metamagic feats and
...
//!   (checked rather than assumed, and deliberately left named-but-unproven, not
//!   fabricated). No record or diagnostic for it is added by this slice.
//!
...
//! named-but-unproven, unchanged from levels 1-4), and it does not ground Wizard
//! level 6+. It also preserves the accepted Wizard level-1/level-2/level-3/level-4
//! truth (unchanged), the Fighter negative control, and the multiclass negative
//! control.
...
#[test]
fn wizard_level5_base_attack_bonus_is_grounded_by_the_same_formula() {
    ...
    assert_eq!(
        base_attack.value, 2,
        "Wizard level 5 1/2-BAB progression (5 / 2) must equal 2: {}",
        base_attack.detail
    );
}
...
#[test]
fn wizard_level5_base_saves_are_grounded_by_the_same_formulas() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);

    let fortitude = explanation(&computation, "class_chassis.wizard.base_save.fortitude");
    assert_eq!(fortitude.value, 1, "Wizard level 5 poor Fortitude (5/3) must equal 1");

    let reflex = explanation(&computation, "class_chassis.wizard.base_save.reflex");
    assert_eq!(reflex.value, 1, "Wizard level 5 poor Reflex (5/3) must equal 1");

    let will = explanation(&computation, "class_chassis.wizard.base_save.will");
    assert_eq!(will.value, 4, "Wizard level 5 good Will (5/2+2) must equal 4");
}

// ----- School specialization choice recognition still fires at level 5 -----

#[test]
fn wizard_level5_still_recognizes_the_specialization_choice() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert!(
        ...
        "specialization choice recognition must still name Evocation at level 5: {}",
        choice.detail
    );
}

// ----- The specialist bonus slot flat count becomes 3 at level 5 -----

#[test]
fn wizard_level5_specialist_bonus_slot_becomes_three() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert_eq!(
        slot.value, 3,
        "the specialist bonus slot must become 3 at level 5 (3rd-level wizard spells unlock \
         at level 5, verified against the raw spells-per-day table, so a level-5 specialist \
         now casts 1st-, 2nd-, and 3rd-level spells): {}",
        slot.detail
    );
}

// ----- Intense Spells bonus damage stays 2 at level 5 -----

#[test]
fn wizard_level5_intense_spells_bonus_damage_stays_two() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert_eq!(
        ...
        "Intense Spells bonus damage at level 5 (max(5/2, 1)) must stay 2, unchanged from \
         level 4 (an integer-division coincidence, not a formula that stopped scaling): {}",
        bonus_damage.detail
    );
}

// ----- Force Missile uses/day is level-independent, unchanged at level 5 -----

#[test]
fn wizard_level5_force_missile_uses_per_day_is_unchanged() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
...
// ----- Scribe Scroll grant is still recognized at level 5 -----

#[test]
fn wizard_level5_still_recognizes_the_scribe_scroll_grant() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert!(
        ...
        "Scribe Scroll detail must still name 1st level as when the feat was granted, not \
         re-derive a level-5 grant event: {}",
...
#[test]
fn wizard_level5_still_claim_blocks_school_powers_and_prepared_spellbook_burdens() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);
    ...
    assert!(
        computation.diagnostics.iter().any(|d| d.id
            ...
            && d.claim_blocking),
        "level-5 Wizard must still claim-block on the school-power / opposed-school-cost \
         burden: {:?}",
    ...
    assert!(
        computation
            ...
            .any(|d| d.id == "class_spell.wizard.prepared_spellbook.unsupported"
                && d.claim_blocking),
        "level-5 Wizard must still claim-block on the prepared spellbook posture burden: {:?}",
        computation.diagnostics
    );
}
...
#[test]
fn wizard_level5_still_recognizes_the_spell_bearing_baseline() {
    let input = load(WIZARD_LEVEL5_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);

    assert!(
        has_explanation(&computation, "class_chassis.spell_baseline.wizard"),
        "level-5 Wizard must still recognize the spell-bearing baseline identity: {:?}",
        computation.explanations
    );
}

// ----- The accepted Wizard level-4 truth is unaffected -----

#[test]
fn wizard_level4_truth_is_unchanged_by_this_widening() {
    let input = load(WIZARD_LEVEL4_FIXTURE);
    let computation = compute_pilot_base_chassis(&input);

    let base_attack = explanation(&computation, "class_chassis.wizard.base_attack_bonus");
    assert_eq!(base_attack.value, 2, "Wizard level 4 base attack bonus must stay 2");

    let will = explanation(&computation, "class_chassis.wizard.base_save.will");
    assert_eq!(will.value, 4, "Wizard level 4 good Will save must stay 4");

    let slot = explanation(&computation, "class_chassis.wizard.specialist_bonus_slot");
    assert_eq!(
        slot.value, 2,
        "Wizard level 4 specialist bonus slot must stay 2, unaffected by the level-5 widening"
    );

    let bonus_damage = explanation(&computation, "class_chassis.wizard.intense_bonus_damage");
    assert_eq!(
        bonus_damage.value, 2,
        "Wizard level 4 Intense Spells bonus damage must stay 2, unaffected by the level-5 \
         widening"
    );
}
...
#[test]
fn fighter_does_not_gain_wizard_level5_recognition() {
...
#[test]
fn multiclass_wizard_level5_is_not_promoted_by_this_slice() {
...
#[test]
fn matrix_wizard_row_names_level_5_widening() {
    ...
    assert!(
        wizard
            .grounding_ref
            .contains("sd13_wizard_level5_progression"),
        "wizard row must cite the live SD13-E5 level-5 proof surface: {}",
        wizard.grounding_ref
    );
    let note = wizard.blocker_or_lossiness_note;
    assert!(
        note.contains("level 5") || note.contains("level-5"),
        "wizard partial note must name the level-5 widening: {note}"
    );
}
