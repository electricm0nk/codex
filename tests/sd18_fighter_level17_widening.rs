...
// further widened the bounded tranche from level 17 to level 18 (a ninth
// bonus-feat cadence slot and a further Bravery magnitude rise), and then
// to level 19 (the Armor Mastery flat-magnitude damage reduction record),
// so this negative control now sits just above the current bound (level
// 20) rather than at level 18 or level 19.

#[test]
fn fighter_level_20_stays_claim_blocked() {
    let level_20 = FIGHTER_LEVEL17_FIXTURE.replace("class:fighter:17", "class:fighter:20");
