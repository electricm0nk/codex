...
//
// SD18 (tests/sd18_sorcerer_level13_widening.rs) further widened the bounded
// tranche from level 12 to level 13, so this negative control now sits just
// above the current bound (level 14) rather than at level 13.

#[test]
fn sorcerer_level_14_is_not_promoted_by_this_slice() {
    let level_14 = SORCERER_LEVEL12_FIXTURE.replace("class:sorcerer:12", "class:sorcerer:14");
