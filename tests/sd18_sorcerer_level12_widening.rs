...
//
// SD18 (tests/sd18_sorcerer_level14_widening.rs) further widened the bounded
// tranche from level 13 to level 14, so this negative control now sits just
// above the current bound (level 15) rather than at level 14.

#[test]
fn sorcerer_level_15_is_not_promoted_by_this_slice() {
    let level_15 = SORCERER_LEVEL12_FIXTURE.replace("class:sorcerer:12", "class:sorcerer:15");
