...
fn cleric_level18_touch_of_good_genuinely_rises_and_other_facets_carry_over() {
    ...
    assert_eq!(tog_uses.value, 6, "Touch of Good's uses per day must stay 6 at level 18");

    let rebuke_uses = explanation(
        &computation,
        "class_chassis.cleric.domain_power_healing_rebuke_death_uses_per_day",
    );
    assert_eq!(rebuke_uses.value, 6, "Rebuke Death's uses per day must stay 6 at level 18");
...
fn cleric_level18_still_recognizes_the_spell_bearing_baseline_and_claim_blocks_burdens() {
    ...
    assert!(
        ...
        "level-18 Cleric must still claim-block on the prepared divine spell posture burden: {:?}",
        computation.diagnostics
    );
}
...
fn cleric_level_19_is_not_promoted_by_this_slice() {
    let level_19 = CLERIC_LEVEL18_FIXTURE.replace("class:cleric:18", "class:cleric:19");
