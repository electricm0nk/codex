...
// (widened from level 15 to level 17 by the SD18 wizard-level15-widening
// and wizard-level16-widening cycles, which genuinely promote levels 15
// and 16 — see tests/sd18_wizard_level15_widening.rs and
// tests/sd18_wizard_level16_widening.rs — mirroring the exact same boundary
// move the Barbarian/Fighter level-16 widening cycles each made for their
// own sibling level-13 widening test.)

#[test]
fn wizard_level_18_is_not_promoted_by_this_slice() {
    let level_18 = WIZARD_LEVEL13_FIXTURE.replace("class:wizard:13", "class:wizard:18");
