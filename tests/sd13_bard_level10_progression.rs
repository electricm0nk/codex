...
#[test]
fn bard_level_11_is_not_promoted_by_this_slice() {
    let level_11 = BARD_LEVEL10_FIXTURE.replace("class:bard:10", "class:bard:11");
    ...
    assert!(
        ...
        "level-11 Bard must not gain any bounded bard explanation: {:?}",
        computation.explanations
    );
}
