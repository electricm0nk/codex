...
fn paladin_level12_effective_caster_level_rises_and_spell_access_stays() {
    ...
    assert!(
        ...
        "level-12 Paladin must still claim-block on the partial-caster spell burden: {:?}",
...
fn paladin_level_16_is_not_promoted_by_this_slice() {
    let level_16 = PALADIN_LEVEL12_FIXTURE.replace("class:paladin:12", "class:paladin:16");
