...
fn bard_level13_performance_rounds_and_fascinate_count_genuinely_rise() {
    ...
    assert_eq!(
        rounds.value, 30,
        "Bard level 13 Bardic Performance rounds (4 + Charisma modifier 2 + 2 x (13 - 1)) \
         must genuinely rise to 30, up from 28 at level 12: {}",
        rounds.detail
    );
    ...
    assert_eq!(
        dc.value, 18,
        "the Fascinate DC (10 + 13/2 + Charisma modifier 2) must stay 18, an \
...
fn bard_level13_still_claim_blocks_the_performance_execution_burden() {
    ...
    assert!(
        computation
            .diagnostics
            .iter()
            .any(|d| d.id == "class_feature.bard.bardic_performance_execution.unsupported"
                && d.claim_blocking),
        "level-13 Bard must still claim-block on the bardic performance-execution burden: {:?}",
...
fn bard_level12_truth_is_unchanged_by_this_slice() {
    ...
    assert_eq!(rounds.value, 28, "Bard level 12 Bardic Performance rounds must stay 28");
...
// (Bard levels 14 through 16 were widened into scope by later SD18 slices —
// up through tests/sd18_bard_level16_widening.rs — so this negative
// control's boundary moves from 14 to 17, mirroring the exact same
// boundary-move idiom applied to every prior sibling class's own level-16
// widening cycle.)

#[test]
fn bard_level_17_is_not_promoted_by_this_slice() {
    let level_17 = BARD_LEVEL13_FIXTURE.replace("class:bard:13", "class:bard:17");
    ...
    assert!(
        ...
        "level-17 Bard must not gain any bounded bard explanation: {:?}",
        computation.explanations
    );
}
