...
# Build the publish commit in a THROWAWAY WORKTREE, never by switching branches
# in the caller's checkout. Switching in place failed for a real reason: this
# script runs verify.sh stages in step 2, verify.sh appends to
# docs/retro/events/*.jsonl on every run, and `git checkout -b` then refuses
# with "local changes would be overwritten". Excluding that file from the
# precondition check was not enough -- git itself blocks the switch.
#
# A worktree also means the operator's checkout is never moved off its branch
# mid-publish, and a failure here cannot strand them on a publish branch.
PUBLISH_WT="$(mktemp -d -t site-publish-XXXXXX)"
cleanup() {
    git worktree remove --force "$PUBLISH_WT" 2>/dev/null || true
    rm -rf "$PUBLISH_WT" 2>/dev/null || true
}
trap cleanup EXIT
...
git -C "$PUBLISH_WT" commit --quiet -F "$MSG_FILE"
rm -f "$MSG_FILE"
echo "    committed $(git -C "$PUBLISH_WT" rev-parse --short HEAD) on $PUBLISH_BRANCH"
