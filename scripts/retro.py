#!/usr/bin/env python3
"""retro.py -- the emitter and reader for this repo's retrospective event log.

WHY THIS EXISTS
---------------
Git records what landed. It says nothing about what nearly landed wrong, who
caught it, or that the same person was wrong four times in one session. In one
observed session four different agents corrected four different figures the
lead had stated confidently -- a feat count, a spell total, a book count, and a
clippy baseline that was counting summary lines and had already propagated into
every brief written that day. Every correction was right. None of it survives
in git, and the pattern across the four is worth more to a retrospective than
any individual fix.

The event vocabulary and the field contract live in `docs/retro/schema.json`,
not in this file. This script reads that schema and builds its own command line
out of it, so adding a field is a data edit rather than a code change, and the
schema can never drift from what the tool accepts.
...
misled. So:
  ...
  * Emission is one shell line with only the fields that carry the finding.
    ...
    recording an event costs more than a sentence of prose, it will lose to the
    prose, and the prose scrolls away.
  * `scripts/verify.sh` emits its own events. Nobody decides to record a
    verification run, so the denominator is honest -- including the runs where
    a stage failed and the fix was quick enough that no one would have thought
    it worth writing down.
  * Nothing that git already holds is copied in. Commits and authors are joined
    at query time (`git_join` in the summary), so this log can never become a
    second, staler git.
  * The log is sharded one file per actor. Up to ~45 agents work concurrently in
    sibling worktrees; a single append-only file conflicts on nearly every merge,
    and a shared scratch artifact has already been clobbered once in this project
    by two writers. Distinct filenames remove the shared write target instead of
    coordinating access to it.

USAGE
-----
  scripts/retro.py types                       # the vocabulary, one line each
  scripts/retro.py help correction             # fields for one type

  scripts/retro.py correction --subject lead \\
      --claimed "186 feats" --actual "185 feats" \\
      --verified-by "rg -c '^FEAT' corpus | awk ..."

  scripts/retro.py near-miss --would-have "merge 34 failing tests" \\
      --caught-by "verify.sh:root-full"

  scripts/retro.py summary --since 7d          # the retrospective
  scripts/retro.py summary --since 7d --json   # same, for the dashboard
  scripts/retro.py query --type correction --subject lead
  scripts/retro.py validate                    # every line parses and conforms
  scripts/retro.py derive-git --since 30d      # revert-shaped commits -> rework

Stdlib only, no build step: an emission costs a process start, not a cargo
compile.
"""
...
SCHEMA_PATH = REPO_ROOT / "docs" / "retro" / "schema.json"
DEFAULT_EVENTS_DIR = REPO_ROOT / "docs" / "retro" / "events"
...
def first_sentence(text: str) -> str:
    ...
    Cut at a sentence boundary rather than a character count: a hard truncation
    of the `why` prose produced entries reading "the branch shipped
    un-compilabl", which is how a help screen teaches people not to read it.
    """
    for end in (". ", ".\n"):
        index = text.find(end)
        if index != -1:
...
def git_root() -> pathlib.Path:
    ...
    Anchored to the repo this script lives in, not to the caller's cwd, so
    `derive-git` and the summary's `git_join` cannot silently report on a
    different tree when run from a subdirectory or a sibling checkout.
    `RETRO_GIT_ROOT` overrides it, which is how the derivation path is tested
    against a real repository with real revert commits rather than a mock.
    """
    override = os.environ.get("RETRO_GIT_ROOT")
    return pathlib.Path(override) if override else REPO_ROOT
...
def resolve_actor(explicit: str | None) -> tuple[str, str]:
    ...
    `--actor`, then `$RETRO_ACTOR`, then the worktree directory name, then git
    `user.name`. The worktree fallback exists so an emission never needs setup
    to happen at all -- but it is the weakest of the four, and how the name was
    arrived at is recorded on the event rather than left for a reader to guess.
    `by_actor` counts in a retrospective mean something quite different when
    the names are self-declared than when they are scraped off a directory.

    No harness variable is consulted, because none exists: this box's agent
    processes carry `CLAUDE_CODE_SESSION_ID` and `CLAUDE_PID` but nothing
    naming the agent. A fallback keyed to a variable nothing sets would be one
    more shipped string asserting something untrue, which is the failure this
    whole artifact is a reaction to.
    """
    if explicit:
...
def warn_if_opaque_actor(actor: str, source: str) -> None:
    """A worktree named `agent-<hex>` identifies a checkout, not a role.
    ...
    and by then the worktrees are gone and the mapping is unrecoverable. Said
    once on stderr, at the only moment anyone can still fix it.
    """
    if source == "worktree" and OPAQUE_WORKTREE_RE.match(actor):
        print(
            f"retro: actor `{actor}` came from the worktree directory name and "
            f"identifies a checkout rather than a role; set RETRO_ACTOR to the "
...
def auto_summary(event_type: str, values: dict) -> str:
    ...
    if event_type == "verification":
        ...
        return f"verify.sh {get('mode')}: {get('result')}{tail}"
...
def build_summary(events: list[dict], problems, args: argparse.Namespace, directory) -> dict:
    ...
    return {
        ...
        "generated_by": "scripts/retro.py summary",
        "schema_version": 1,
        "window": {
...
def render_summary(doc: dict) -> str:
    ...
    add = lines.append
    window = doc["window"]
    add("codex retrospective — event log summary")
    ...
    if log["invalid_lines"]:
        add(f"  {log['invalid_lines']} INVALID LINES — run `scripts/retro.py validate`")
    return "\n".join(lines)
...
def cmd_derive_git(args: argparse.Namespace, schema: dict) -> int:
    ...
    unambiguously; everything else it might suggest (a `fix(` commit touching a
    file a recent commit touched, a force-push) is a guess, and a guessed event
    in this log is worth less than no event. As of this writing the branch has
    ZERO revert-shaped commits across 2905 -- which is the point, not a bug:
    rebase, amend and squash erase rework entirely, so almost all of it has to
    be emitted at the moment it happens or it is gone.
    """
