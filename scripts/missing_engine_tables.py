...
Reads the same `docs/work-inventory.json` (SD-36 D3: frozen,
`docs/work-inventory.FROZEN.md`) and selects bucket A the same way the
retired `completion_atlas.py` did (`status == engine-does-not-hold` and
...
For each of the 2 kinds this population actually contains, records:
  ...
  - `engine_surface` -- the `engine_does_not_hold(...)` call site that used
    to live in `src/bin/v06_work_inventory.rs` (SD-36 D3: the generator is
    retired and the file is gone; this is now a historical citation only,
    kept as documentation of where the frozen inventory's bucket-A
    classification came from -- it is no longer resolved against a live
    file, because there is no live generator left to drift against)
...
posture the retired `completion_atlas.py` established for the atlas itself
(`decisions.md §12` L1): a new kind reaching bucket A without an engine
surface citation must break the build, not disappear from the report. Since
`docs/work-inventory.json` is now frozen (never regenerated), a new kind can
only appear here if the frozen file itself is edited, which
`docs/work-inventory.FROZEN.md` already forbids.
"""
...
# The generator this cited (`src/bin/v06_work_inventory.rs`) was retired by
# SD-36 D3; this string is now a historical citation, not a live file path.
_ENGINE_SRC = "src/bin/v06_work_inventory.rs (retired, SD-36 D3)"
_A_MARKER = "has_no_engine_table"

# The `engine_does_not_hold(...)` call site that used to emit each kind's
# bucket-A marker in `_ENGINE_SRC`'s `classify` function, kept as a
# historical record of where the frozen inventory's bucket-A classification
# came from. No longer resolved against a live file (SD-36 D3: the
# generator is retired, so there is nothing left to drift-check against).
