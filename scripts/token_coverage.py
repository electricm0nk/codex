...
population the converter converts, so a second reading here would count a
different thing. The count of units resolving to no corpus record and no
source row went 829 -> 142 at SD-35 AT-35-E3-002 and 142 -> 0 at SD-35
AT-35-E7-CLOSURE-CLEANUP (the last 142 were reprints whose corpus record is
filed under the book that owns the `.lst`; `sheet_rule::load_population`'s
cross-book source-row fallback joins them). The REFUSED_SET and SHAPE_TOTALS
checks below are set equalities and hold at zero exactly as they held at 142 --
nothing here special-cases an empty refusal set.
