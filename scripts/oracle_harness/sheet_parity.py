...
#: SD-36 Epic E GATE-02 (SD-35 code review, fix cycle 2): the roster's first widening beyond
#: Core Rulebook-only. `(engine class token, PCGen CLASS name)` pairs for classes a non-CRB
#: book itself introduces, each generated as one Human level-1 member alongside its book's own
#: `CAMPAIGN:` line (`PRECAMPAIGN:1,INCLUDES=Core Rulebook` on the book's own `.pcc`, verified:
#: `grep PRECAMPAIGN data/pathfinder/paizo/roleplaying_game/advanced_players_guide/*.pcc`).
#: This is a real, run, cross-book start -- NOT the finding's full "stratified sample across
#: every book that ships a class or race" ask (31 books carry a `class` or `race_trait`
#: directory today, `ls data/sheet_rules/*/{class,race_trait} -d 2>/dev/null | ...`); the
#: remaining books, and equipment/feats exercising CONV-01..04 on a cross-book member, are
#: named as open follow-up in `risks-and-open-questions.md` §11, not silently left off this list.
EXTRA_BOOK_CLASSES = [
    ("Advanced Player's Guide", "advanced_players_guide", [("alchemist", "Alchemist"), ("witch", "Witch")]),
]
...
def roster_members():
    """`(name, race token, PCGen race, class token, PCGen class, level, [PCGen CAMPAIGN names],
    engine source_package_id)` for the generated members. The deterministic fighter is copied,
    not generated. Every original Core-Rulebook-only member keeps its original single-campaign
    shape; `EXTRA_BOOK_CLASSES` (SD-36 Epic E GATE-02) is additive."""
    out = []
    for cls, pcls in CLASSES:
        for level in LEVELS:
            out.append((f"human_{cls}_l{level}", "human", "Human", cls, pcls, level, ["Core Rulebook"], "pf1.core_rulebook"))
    for race, prace in RACES:
        if race == "human":
            continue
        out.append((f"{race.replace('-', '_')}_fighter_l1", race, prace, "fighter", "Fighter", 1, ["Core Rulebook"], "pf1.core_rulebook"))
    for campaign, book_key, classes in EXTRA_BOOK_CLASSES:
        for cls, pcls in classes:
            out.append((
                f"human_{cls}_l1",
                "human",
...
def engine_fixture_text(name, race, cls, level, adjustments=None, source_package_id="pf1.core_rulebook"):
    ...
    `source_package_id` is metadata only (`character_input.rs` requires it non-empty, does not
    validate its value) -- a non-CRB member (`EXTRA_BOOK_CLASSES`) names its own book so the
    fixture is honest about where the character actually comes from."""
...
def pcg_text(name, prace, pcls, level, campaigns=("Core Rulebook",)):
    extra = HUMAN_BONUS_PCG if prace == "Human" else ""
    campaign_lines = "\n".join(f"CAMPAIGN:{c}" for c in campaigns)
    return PCG_TEMPLATE.format(
        name=name, race=prace, pcgen_class=pcls, level=level, extra=extra, campaigns=campaign_lines, **SCORES
    )
...
def cmd_roster(args):
    ...
    for name, race, prace, cls, pcls, level, campaigns, source_package_id in roster_members():
        with open(os.path.join(args.out, f"{name}.txt"), "w", encoding="utf-8") as f:
            f.write(engine_fixture_text(name, race, cls, level, adjustments_by_race.get(prace), source_package_id))
        with open(os.path.join(args.out, f"{name}.pcg"), "w", encoding="utf-8") as f:
            f.write(pcg_text(name, prace, pcls, level, campaigns))
        n += 1
