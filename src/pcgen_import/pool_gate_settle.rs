...
//! [`crate::rules_core::record_vars::RecordVarPackage::pool_gates`].
//!
...
use crate::rules_core::class_feature_pool_catalog::{settled_pool_gate_key, SettledPoolGates};
...
mod tests {
    ...
    fn no_book_or_record_key_in_this_corpus_contains_the_key_separator() {
        use crate::rules_core::class_feature_pool_catalog::SETTLED_POOL_GATE_KEY_SEPARATOR;
        let root = repo_root();
        for (book, cf_dir) in class_feature_dirs(&root) {
            assert!(
