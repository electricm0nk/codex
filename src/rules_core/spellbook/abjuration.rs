...
pub fn resolve_abjuration_spell_effect(spell_id: &str) -> Option<AbjurationSpellEffect> {
    if let Some(entry) =
        SPELL_LIST.iter().find(|entry| entry.key == spell_id && entry.school == Pf1SchoolId::Abjuration)
    {
        return Some(AbjurationSpellEffect {
    ...
    if let Some(entry) = APG_SPELL_LIST
        .iter()
        .find(|entry| entry.key == spell_id && entry.school == Some(ApgPf1SchoolId::Abjuration))
    {
        return Some(AbjurationSpellEffect {
            spell_id: spell_id.to_string(),
            level: entry.level?,
    ...
    if let Some(entry) = ACG_SPELL_LIST
        .iter()
        .find(|entry| entry.key == spell_id && entry.school == AcgPf1SchoolId::Abjuration)
    {
        return Some(AbjurationSpellEffect {
            spell_id: spell_id.to_string(),
            level: entry.level,
...
mod tests {
    ...
    /// W21-SPELL-001: "Shield Companion" is a real
    /// `acg::spell_list::SPELL_LIST` Abjuration record, absent from CRB's
    /// own table (`advanced_class_guide:spell:shield_companion`,
    ...
    /// full explanation).
    #[test]
    fn resolves_a_real_acg_abjuration_spell_the_crb_table_does_not_carry() {
        let effect = resolve_abjuration_spell_effect("Shield Companion")
            .expect("Shield Companion is a real ACG Abjuration record, not in CRB's own table");
        assert_eq!(effect.level, 2);
        assert!(
            effect.effect_text.contains("mystical connection"),
    ...
    fn crb_resolution_takes_priority_over_the_wider_search() {
        let effect = resolve_abjuration_spell_effect("Shield")
            .expect("Shield is a real CRB Abjuration record");
        assert_eq!(effect.table_cell.rule_set, RuleSetId::Crb);
    }
}
