...
pub fn resolve_enchantment_spell_effect(spell_id: &str) -> Option<EnchantmentSpellEffect> {
    if let Some(entry) =
        SPELL_LIST.iter().find(|entry| entry.key == spell_id && entry.school == Pf1SchoolId::Enchantment)
    {
        return Some(EnchantmentSpellEffect {
    ...
    if let Some(entry) = APG_SPELL_LIST
        .iter()
        .find(|entry| entry.key == spell_id && entry.school == Some(ApgPf1SchoolId::Enchantment))
    {
        return Some(EnchantmentSpellEffect {
            spell_id: spell_id.to_string(),
            level: entry.level?,
    ...
    if let Some(entry) = ACG_SPELL_LIST
        .iter()
        .find(|entry| entry.key == spell_id && entry.school == AcgPf1SchoolId::Enchantment)
    {
        return Some(EnchantmentSpellEffect {
            spell_id: spell_id.to_string(),
            level: entry.level,
...
mod tests {
    ...
    /// W21-SPELL-001: "Mantle of Calm" is a real
    /// `acg::spell_list::SPELL_LIST` Enchantment record, absent from CRB's
    /// own table (`advanced_class_guide:spell:mantle_of_calm`,
    ...
    /// full explanation).
    #[test]
    fn resolves_a_real_acg_enchantment_spell_the_crb_table_does_not_carry() {
        let effect = resolve_enchantment_spell_effect("Mantle of Calm")
            .expect("Mantle of Calm is a real ACG Enchantment record, not in CRB's own table");
        assert_eq!(effect.level, 3);
        assert!(
            effect.effect_text.contains("mantle of calm serenity"),
    ...
    fn crb_resolution_takes_priority_over_the_wider_search() {
        let effect = resolve_enchantment_spell_effect("Charm Person")
            .expect("Charm Person is a real CRB Enchantment record");
        assert_eq!(effect.table_cell.rule_set, RuleSetId::Crb);
    }
}
