...
//! `magic_items`, `equipmods`). This cycle lands `arms_armor` only — see
//! `equipment_effects/arms_armor.rs`. The other three categories'
//! resolved items pass through this module with no stat contribution
//! (`EquipmentStatEffect::default()`, all `None`) until their own cycle
//! lands a sibling `equipment_effects/<category>.rs` file; that is an
...
fn resolve_category_effect(category: EquipmentCategory, record: &EquipmentRecord) -> EquipmentStatEffect {
    match category {
        ...
        // General / MagicItems / Equipmods land in future Epic-5 cycles
        // per scope-draft.md §1.5's work-unit order; until then their
        // items resolve with no stat contribution (honest absence, see
        // module doc comment).
