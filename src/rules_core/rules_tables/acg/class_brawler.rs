...
//! non-caster posture as Cavalier). The real record's chassis-bearing
//! tokens:
//!
...
//! - `BONUS:SAVE|BASE.Will|CL/3` — poor Will save.
//! - `MAXLEVEL:20`.
//!
//! `tests/sd22_acg_class_brawler_resolves.rs`'s
...
//!
//! Mirrors `rules_tables::acg::class_bloodrager`'s scope boundary: only
...
/// `MAXLEVEL:20` on the real `CLASS:Brawler` record.
