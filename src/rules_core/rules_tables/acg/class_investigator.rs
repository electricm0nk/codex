//! ACG Investigator class chassis table — one row per level.
//!
//! Source: PCGen `acg_classes.lst`, `CLASS:Investigator` record (line 168
...
//! line carries the same spellbook-prepared posture as
//! Alchemist/Arcanist). The real record's chassis-bearing tokens:
//!
...
//! - `BONUS:SAVE|BASE.Will,BASE.Reflex|classlevel("APPLIEDAS=NONEPIC")/2+2` — good Will and Reflex saves.
//! - `MAXLEVEL:20`.
//!
//! `tests/sd22_acg_class_investigator_resolves.rs`'s
...
//! (Inspiration, Trapfinding, Studied Combat, Investigator Talents, ...)
//! and the investigator extract/spell-per-day table are out of scope for
//! this cycle — transcribing them without going back through the LST's
//! per-level feature blocks (`acg_abilities_class.lst`) in a dedicated
//! ingest slice would be exactly the fabricated-data risk
//! `class_tables.rs`'s own doc comment and `AGENTS.md` rule out.

use super::ClassTableRow;

/// `MAXLEVEL:20` on the real `CLASS:Investigator` record.
...
/// Builds the Investigator class table: one row per level, from level 1
...
pub fn class_table() -> Vec<ClassTableRow> {
    (1..=MAX_SUPPORTED_LEVEL)
        .map(|level| ClassTableRow {
            ...
            ref_save: good_save(level),
            will_save: good_save(level),
        })
        .collect()
}

/// `HD:8` on the real `CLASS:Investigator` record (Investigator HD:8 in
