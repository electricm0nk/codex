...
//! is RE-VERIFIED this cycle, not merely inherited, and this comment's
//! evidence sentence was itself corrected by wave-8 integration review:
...
//! (`legacy_of_dragons`). None of those 5 books is in this package's
//! 38-book inventory scope (no `player_companion` book appears in
//! `docs/work-inventory.json`'s `book` set), so the CONCLUSION this
//! comment draws -- zero real archetype content in the IN-SCOPE 23-book
//! set, base chassis + class-feature mechanism only this cycle, no
//! supersession branch -- still holds. What was wrong was the evidence
//! sentence, not the decision: it reported a result scoped to
//! `data/pathfinder/paizo/roleplaying_game/` while claiming "the full
//! pinned oracle tree." Forward-scope note for whichever future cycle
//! ingests `dragon_empires_primer`, `armor_masters_handbook`,
//! `disciples_doctrine`, `martial_arts_handbook` or `legacy_of_dragons`:
//! Samurai's supersession branch becomes required at that point, and
