...
// ---------------------------------------------------------------------------
// The `PRE`-family tokens ARG's, PU's and UCA's own tables never gathered
// ---------------------------------------------------------------------------
// `ARG_FEAT_PREREQUISITES`, `PU_FEAT_PREREQUISITES` and
// `UCA_FEAT_PREREQUISITES` stood here, together with the
// `gathered_prerequisites` / `gathered` helpers that read them into
// `FeatCatalogRecord.prerequisites`. All three tables and both helpers moved
// to `pcgen_import::feat_prereq_tokens` — SD-35 `AT-35-E6-003-SWEEP`
// cycle 3, `decisions.md` §11.
//
// Nothing was lost in the move. The three tables listed **every** key in
// their book, including the rows whose corpus record carries no `PRE` token
// at all (an empty slice meaning "checked, and the corpus row has none", as
// distinct from a missing key meaning "not gathered"). That distinction
// survives: a checked-and-empty row is now listed by
// `feat_prereq_tokens::CHECKED_AND_CARRIES_NO_PRE_TOKEN`, and
// `feat_prereq_tokens::tests::every_backfilled_book_key_was_checked` is the
// successor to the
// `every_arg_and_pu_catalog_key_has_a_gathered_prerequisite_row` assertion
// that used to live in this file's own tests.
