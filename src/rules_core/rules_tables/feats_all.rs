...
mod tests {
    ...
    fn the_joined_catalog_is_the_hand_authored_one_plus_the_corpus_gap_rows() {
        ...
        assert_eq!(
            total, 2261,
            "1578 hand-authored + 683 corpus gap rows: the original 325 \
             ...
             32 MonsterCodex) + 358 more from Mythic Adventures' first-ever compiled \
    ...
    fn the_per_book_prerequisite_coverage_is_the_real_one() {
        ...
        // 1715 of the pre-Mythic 1903 (as before) + 195 of Mythic's 358 gap
        // rows (`SD31-E6-F2-007`) -- the gap rows carry their own `PRE`-
        // family tokens verbatim, gated by `feat_prereqs` like every other
        // record, never offered unconditionally.
        assert_eq!(total, 1910, "1910 of the joined catalog's 2261 records have a prerequisite");
    }
