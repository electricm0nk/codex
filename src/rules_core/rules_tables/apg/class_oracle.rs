//! APG Oracle class chassis table — one row per level.
//!
//! Source: PCGen `apg_classes.lst`, `CLASS:Oracle` record (line 107 of
...
//! the same spontaneous-divine posture as Sorcerer/Bard/Inquisitor). The
...
//! - `MAXLEVEL:20`.
//!
//! `tests/sd22_apg_class_oracle_resolves.rs`'s
...
//!
//! Mirrors `rules_tables::apg::class_inquisitor`'s scope boundary: only
//! the BAB/save chassis is transcribed here. Named per-level features
//! (Mystery, Curse, Revelation, ...) and the spell-per-day table are out
...
/// `MAXLEVEL:20` on the real `CLASS:Oracle` record.
pub const MAX_SUPPORTED_LEVEL: u8 = 20;

/// `HD:8` on the real `CLASS:Oracle` record (v0.6 alpha swarm, risks item
/// 8), verified directly against `apg_classes.lst` line 107.
...
/// Builds the Oracle class table: one row per level, from level 1
