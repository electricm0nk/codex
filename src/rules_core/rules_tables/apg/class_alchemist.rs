...
//! line carries the same posture-bearing shape as the CRB spellcasting
//! classes). The real record's chassis-bearing tokens:
//!
...
//! - `MAXLEVEL:20`.
//!
//! `tests/sd22_apg_class_alchemist_resolves.rs`'s
...
//!
//! Mirrors `rules_tables::crb::class_tables`'s scope boundary: only the
...
/// Builds the Alchemist class table: one row per level, from level 1
