//! Ingests Ultimate Wilderness's spell catalog into the engine's
//! spell-catalog capability -- SD-31 wave-19's `ultimate_wilderness` lane.
//!
//! **The finding this closes.** `spell_resolver::spell_catalog_rows()`
//! chained nine books before this cycle (CRB/APG/ACG/ARG/UI/UM/OA/UC/ISG);
//! Ultimate Wilderness's 61 `spell` units were structurally `not-ingested`
//! no matter how much ingest work ran against them, because `classify()`'s
//! `Kind::Spell` arm never consults anything but that one compiled table
//! (`v06_work_inventory.rs`, `Kind::Spell => ... facts.spell_levels.get(...)`).
//! This binary builds the missing tenth-book slice --
//! `rules_tables::ultimate_wilderness::spell_list::SPELL_LIST` -- from the
//! real `uw_spells.lst` corpus.
//!
...
//! binary (`ingest_ultimate_magic_spells.rs`, `ingest_ultimate_combat_spells.rs`,
//! ...) already established -- copied deliberately rather than re-derived,
//! per this program's own "fix the source, not the symptom" / no second
//! implementation discipline.
//!
//! `uw_spells.lst`: 120 non-header rows, 59 `.MOD`/`.COPY=` delta rows, 61
//! base declarations -- matching `docs/work-inventory.json`'s own 61
//! `ultimate_wilderness`/`spell` units exactly (re-derived at dispatch,
//! `SD-31 wave-19`).
//!
...
//! declared-PI reader), matching every prior spell-lane cycle. `uw_spells.lst`
//! carries zero `NAMEISPI:`/`DESCISPI:` tokens (re-derived below at run
//! time and reported), so this run is not expected to drop or redact
//! anything -- the declared-PI reader still runs on every record's own raw
//! tokens, not skipped on that assumption.
//!
//! Run with `cargo run --locked --bin ingest_ultimate_wilderness_spells`.
//! `PCGEN_CORPUS_ROOT` overrides the default `$HOME/workspace/repos/pcgen/data`.
//! Writes `src/rules_core/rules_tables/ultimate_wilderness/spell_list.rs`.
...
const UW_SPELLS_REL: &str =
    "pathfinder/paizo/roleplaying_game/ultimate_wilderness/uw_spells.lst";
const OUT_PATH: &str = "src/rules_core/rules_tables/ultimate_wilderness/spell_list.rs";
...
/// the level's own `=`** -- the same `ingest_ultimate_magic_spells.rs`
/// fix, carried forward: an unqualified `rsplit_once('=')` would grab the
/// LAST `=` in the whole group (inside a bracketed sub-condition), which
/// fails to parse as a `u8` and silently discards a real level as though
...
/// convention that parser applies to every other tag. Two `uw_spells.lst`
/// rows carry it (`Animal Growth (Reptiles Only)`, `Animal Shapes (Reptiles
/// Only)`, both `DOMAINS:Scalykind=N`).
...
/// `None`. Re-derived at dispatch: `uw_spells.lst`'s 61 base declarations
/// carry exactly {Abjuration, Conjuration, Divination, Enchantment,
/// Evocation, Illusion, Necromancy, Transmutation} -- no `Universal` row,
/// left in the match for parity with every sibling table regardless.
...
fn build_module_source(entries: &[SpellEntry]) -> String {
    ...
    out.push_str(
        "//! Ultimate Wilderness (UW) shared spell list.\n\
         //!\n\
         //! Generated by `src/bin/ingest_ultimate_wilderness_spells.rs` from the\n\
         //! real `uw_spells.lst` corpus (SD-31 wave-19, `ultimate_wilderness`\n\
         //! lane). Record coverage: every real, active (non-`.MOD`, non-`.COPY=`)\n\
         //! base spell declaration -- 61 of 120 raw rows, the remaining 59 being\n\
         //! `.MOD`/`.COPY=` delta rows this table deliberately excludes.\n\
         //!\n\
...
fn main() {
    ...
    let lst_path = data_root.join(UW_SPELLS_REL);
    ...
    eprintln!(
        "ingest_ultimate_wilderness_spells: {} base declarations, {} PI-dropped, {} no-level (real gap, not fabricated), {} school-unrecognized",
        entries.len(),
        dropped_pi.len(),
        no_level.len(),
...
mod tests {
    ...
    /// The real corpus row: `Animal Growth (Reptiles Only)`,
    /// `DOMAINS:Scalykind=5`.
    #[test]
    fn domains_field_reads_the_real_uw_shape() {
        let raw = "Animal Growth (Reptiles Only)\t\tTYPE:Arcane.Divine\t\tDOMAINS:Scalykind=5\t\tSCHOOL:Transmutation";
        assert_eq!(domains_field(raw), Some("Scalykind=5".to_string()));
    }
