...
mod tests {
    ...
    fn already_ingested_keys_reads_real_keys_off_disk_not_a_hand_list() {
        ...
        assert_eq!(
            keys.len(),
            332,
            "ARG's 332 ingested race-trait keys (156 -> 201 by SD-31 Epic 1-F2, 2026-08-15; \
             ...
             every one of those 131 new keys is `<NewRace> ~ ...` and shares no key with \
             any of APG's 7 CRB-race rows, so this discriminator's own behavior is \
             unaffected)"
        );
