...
//!    `codex::rules_core::class_seeds::canonical_seeds_for` (SD-36 Epic F,
//!    factored out so `src/bin/class_census.rs` can reuse it without
//!    duplicating the table); each seed cites the `pf1_adapter.rs` block
//!    it mirrors.
//!
