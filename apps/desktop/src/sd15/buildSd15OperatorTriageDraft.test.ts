...
import {
  ...
  SD15_PRIMARY_CLASSES,
  SD15_OUTCOME_STATES,
  SD15_REPRODUCTION_STATUSES,
  SD15_AUTHORITY_DISCLAIMERS,
  type Sd15OperatorInput,
} from './buildSd15OperatorTriageDraft';
import { assembleFeedbackEvidence } from '../sd11/feedback/evidence';
import type { FeedbackEvidencePayload } from '../sd11/feedback/evidence';
...
function completeBugPayload(surface: Sd11TesterWorkbenchSurface): FeedbackEvidencePayload {
...
function completeOperatorInput(overrides: Partial<Sd15OperatorInput> = {}): Sd15OperatorInput {
  return {
...
function exposesOnlyDictionaryVocabulary() {
  ...
  assertEqual(SD15_PRIMARY_CLASSES.length, 8, 'exactly the eight SD-15 primary classes are defined');
  const ids = SD15_PRIMARY_CLASSES.map((c) => c.id);
  ...
  for (const expected of ['defect', 'unsupported', 'partial', 'not-yet-verified', 'blocked', 'status-drift']) {
    assert(
      (SD15_OUTCOME_STATES as readonly string[]).includes(expected),
      `outcome state ${expected} is present`
    );
  }
  assertEqual(SD15_OUTCOME_STATES.length, 6, 'exactly the six outcome states are defined');
  ...
  for (const expected of [
    ...
    assert(
      (SD15_REPRODUCTION_STATUSES as readonly string[]).includes(expected),
...
function partitionsPreserveTesterAutoOperatorSplit() {
  ...
  const result = buildSd15OperatorTriageDraft({
  ...
  assert(!!build && build.value === 'codex-desktop-shell-scaffold@0.0.0-test', 'build label preserved from auto-captured evidence');
...
function completeOperatorInputProducesCompleteDraft() {
  ...
  assertEqual(result.draft.kind, 'sd15-operator-triage-receipt-draft', 'draft carries its receipt-draft kind marker');
}
...
function operatorJudgmentDrivesClassNotTesterNarrative() {
  ...
  const asRules = buildSd15OperatorTriageDraft({
    evidence: completeBugPayload(surface),
    operator: completeOperatorInput({ primaryClass: 'rules-engine-defect' }),
  });
  const asUnsupported = buildSd15OperatorTriageDraft({
    evidence: completeBugPayload(surface),
...
function authorityDisclaimersPresentAndNoSuccessClaims() {
  ...
  assert(SD15_AUTHORITY_DISCLAIMERS.length > 0, 'authority disclaimers are defined');
...
function textDraftIsCopyableAndPreservesEvidence() {
  ...
  assert(text.includes('codex-desktop-shell-scaffold@0.0.0-test'), 'text draft carries the build label');
