...
function partitionsPreserveTesterAutoOperatorSplit() {
  ...
  assert(!!build && build.value === 'Codex 0.5.95-test', 'build label preserved from auto-captured evidence');
...
function textDraftIsCopyableAndPreservesEvidence() {
  ...
  assert(text.includes('Codex 0.5.95-test'), 'text draft carries the build label');
