...
function partitionsPreserveTesterAutoOperatorSplit() {
  ...
  assert(!!build && build.value === 'Codex 0.16.0-test', 'build label preserved from auto-captured evidence');
...
function textDraftIsCopyableAndPreservesEvidence() {
  ...
  assert(text.includes('Codex 0.16.0-test'), 'text draft carries the build label');
