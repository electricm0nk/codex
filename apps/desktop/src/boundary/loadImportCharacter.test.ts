import { loadImportCharacter } from './loadImportCharacter';
...
async function testNoRuntimeThrowsDescriptiveError() {
  let thrown: unknown;
  try {
    await loadImportCharacter({ filePath: '/tmp/character-test.json', savedAt: '2026-07-20T00:00:00Z' });
  } catch (cause) {
    thrown = cause;
  }
  ...
  assertEqual(
    message,
    'Tauri runtime not available for importing a character',
