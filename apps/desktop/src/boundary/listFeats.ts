 ...
 * shape), which narrows the full 486-record catalog
 * (`list_feat_catalog`'s unfiltered source: 185 CRB + 172 APG + 129 ACG)
 * by `nameContains` (case-insensitive substring against `name`),
 * `category` (exact match against the `FeatCategory` variant name
 * verbatim, e.g. "Combat") and/or `source` (exact match against the
 * `RuleSetId` variant name, i.e. "Crb" / "Apg" / "Acg"). Every field is
 * optional and `null` matches everything.
 */

export interface FeatCatalogEntryDto {
  key: string;
  /**
   * The `FeatCategory` variant name verbatim, e.g. "Combat". `"Teamwork"`
   * and `"Panache"` only ever appear on APG/ACG records.
   */
  category: string;
  name: string;
  description: string | null;
  /** Which book this feat is from: `"Crb"`, `"Apg"` or `"Acg"`. */
  source: string;
  /**
...
export interface FeatCatalogFilter {
  ...
  /** `"Crb"` / `"Apg"` / `"Acg"`, or `null` to span every book. */
  source?: string | null;
}
