import { setSkillAllocations } from './setSkillAllocations';
...
async function testNoRuntimeThrowsDescriptiveError() {
  let thrown: unknown;
  try {
    await setSkillAllocations({
      characterId: 'char-test',
      skillAllocations: [{ skillId: 'skill:climb', ranks: 1 }],
  ...
  assertEqual(
    message,
    'Tauri runtime not available for setting skill allocations',
