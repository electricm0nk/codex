 ...
 * In node tsx that is false, so without this stub `loadUpdateAction`
...
function request(overrides: Partial<UpdateActionRequest> = {}): UpdateActionRequest {
  return {
    buildVersion: '0.0.0',
    buildLabel: 'codex@0.0.0',
    platformLabel: 'Linux',
    testerChannelLabel: 'alpha' as TesterChannelLabel,
    ...overrides,
  };
}
...
async function testHappyPathAlphaReturnsGovernedRelease() {
  ...
  try {
    const result = await loadUpdateAction(request());
...
async function testUpToDateBuildCollapsesLatestToCurrent() {
  ...
  try {
    const result = await loadUpdateAction(
      request({
        buildVersion: '0.1.0',
    ...
    // latest eligible build IS the current build (deriveWorkbenchUpdateAction
...
async function testBuildBelowVersionFloorIsManualOnly() {
  ...
  try {
    const result = await loadUpdateAction(request({ buildVersion: '0.0.1' }));
...
async function testUnsupportedChannelReturnsCheckFailed() {
  ...
  try {
    const result = await loadUpdateAction(
      request({ testerChannelLabel: 'omega-channel' as unknown as TesterChannelLabel })
    );
