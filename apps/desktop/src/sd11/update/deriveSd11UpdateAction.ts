 ...
 */
export function deriveSd11UpdateAction(
  context: Sd11UpdateCheckContext,
 ...
 */
function integrityGateSatisfied(manifest: Sd11UpdateManifestView, tier: Sd11SupportTier): boolean {
  if (tier !== 'first-class') {
    return false;
  }
...
function manualReason(
  tier: Sd11SupportTier,
  eligibility: Sd11UpdateManifestView['eligibilityState'],
...
function baseResult(
  context: Sd11UpdateCheckContext,
  partial: Pick<Sd11UpdateActionResult, 'state' | 'headline' | 'detail'> & Partial<Sd11UpdateActionResult>
): Sd11UpdateActionResult {
  return {
    state: partial.state,
