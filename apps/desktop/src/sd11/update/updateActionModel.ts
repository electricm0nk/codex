import type { Sd11SupportTier } from '../status/createSd11WorkbenchStatus';
 ...
 */
export type Sd11UpdateResultState =
  | 'up-to-date'
  | 'update-available'
  | 'manual-only'
  | 'blocked'
...
export type Sd11ManifestEligibilityState =
  | 'automatic'
  | 'manual-only'
  | 'unsupported'
  | 'withdrawn'
...
export type Sd11ReleaseLifecycleState = 'active' | 'superseded' | 'withdrawn' | 'blocked';

export type Sd11TesterChannelLabel = 'alpha' | 'beta' | 'stable';

/** Running-build identity used to classify the check honestly against the actual platform. */
export interface Sd11UpdateCheckContext {
  buildLabel: string;
  buildVersion: string;
...
export interface Sd11UpdateBuildIdentity {
  releaseId: string;
  version: string;
  buildLabel: string;
 ...
 */
export interface Sd11UpdateIntegrityMaterial {
...
export interface Sd11UpdateManifestView {
  manifestVersion: string;
  channel: string;
  operatorPromotionPathReference: string;
  platform: string;
  supportTier: string;
  currentBuild: Sd11UpdateBuildIdentity;
  latestEligibleBuild: Sd11UpdateBuildIdentity;
  lifecycleState: Sd11ReleaseLifecycleState;
  eligibilityState: Sd11ManifestEligibilityState;
  integrity: Sd11UpdateIntegrityMaterial;
 ...
 */
export type Sd11UpdateReleaseTruth =
  | {
  ...
  | {
      ...
      manifest: Sd11UpdateManifestView;
    };

export interface Sd11UpdateActionResult {
  state: Sd11UpdateResultState;
  headline: string;
  detail: string;
  platformLabel: string;
  platformTier: Sd11SupportTier;
