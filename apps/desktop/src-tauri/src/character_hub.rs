...
mod tests {
    ...
    fn list_feats_for_character_marks_every_record_and_removes_none() {
        ...
        // 1578 hand-authored + the 683 corpus gap rows (`SD31-E6-F8-001`'s
        // original 83 + `SD31-E6-F8-002`'s 242 + `SD31-E6-F2-007`'s 358
        // Mythic Adventures rows). This is the character sheet's own feat
        // list, so this number moving is the evidence that the gap lane's
        // rows reach a player and not only a table.
        assert_eq!(response.entries.len(), 2261, "no record may be filtered away");
    ...
    fn the_character_less_catalog_sends_no_eligibility_key() {
        ...
        assert_eq!(response.entries.len(), 2261);
