//! Character Hub Phase 1 — Tauri command adapter over the real rules-core
//! compute engine and saved-character persistence.
//!
//! Bridges the desktop shell and the headless `codex` crate: composes the
//! fixed "golden path" `CharacterInput` (the only combination the
//! deterministic pilot compute seam reaches `Computed` — zero
//! claim-blocking diagnostics — for today: single-class Fighter, level
//! 1-3), computes it via the real engine, and persists successful builds
//! through `SavedCharacterStore`. A `Blocked` result is never written to
//! disk — this module never saves an unproven build.
...
/// Build the fixed "golden path" `CharacterInput` for a single-class Fighter
/// at the requested level. Race and ability scores are the caller's real
/// choices; the feat/skill/equipment loadout is fixed to the only
/// combination the compute seam reaches `Computed` for. Human additionally
/// receives its own canonical choice-slot values — the ability-bonus target
/// is fixed to Strength for Phase 1 (not yet exposed as a form field); every
/// other race omits the Human-only slots and still reaches `Computed`.
pub fn compose_golden_path_character_input(request: &CreateCharacterRequest) -> CharacterInput {
...
pub fn create_character(
    ...
    let character_input = compose_golden_path_character_input(&request);
...
mod tests {
    ...
    #[test]
    fn compose_golden_path_character_input_for_human_includes_human_only_choice_slots() {
        let input = compose_golden_path_character_input(&request_for("race:human", 1));
    ...
    #[test]
    fn compose_golden_path_character_input_for_non_human_omits_human_only_choice_slots() {
        let input = compose_golden_path_character_input(&request_for("race:half-orc", 1));
    ...
    #[test]
    fn compose_golden_path_character_input_reaches_computed_status_for_supported_levels_1_to_3() {
        for race_id in CURATED_RACE_IDS {
            for level in 1..=3u8 {
                let input = compose_golden_path_character_input(&request_for(race_id, level));
